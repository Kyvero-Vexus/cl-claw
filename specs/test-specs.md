...
- passes CLI URL override context into shared gateway auth resolution
...
- auth profiles read-only external CLI sync
- syncs external CLI credentials in-memory without writing auth-profiles.json in read-only mode
...
- exec approvals
- reuses approval id as the node runId
- skips approval when node allowlist is satisfied
...
- denies node obfuscated command when approval request times out
...
- blocks obvious shell-as-js output before node execution
...
- caches Claude Code CLI credentials within the TTL window
- refreshes Claude Code CLI credentials after the TTL window
...
- runs CLI through supervisor and returns payload
...
- infers timeout from common node error codes
...
- downloads camera_snap url payloads when node remoteIp is available
- rejects camera_snap url payloads when node remoteIp is missing
- nodes camera_clip
- downloads camera_clip url payloads when node remoteIp is available
- rejects camera_clip url payloads when node remoteIp is missing
- nodes photos_latest
...
- allows forcing Codex transport to SSE
...
- stores and retrieves model from runtime (fallback for compact.ts workflow)
...
- uses runtime.model when ctx.model is undefined (compact.ts workflow)
...
- blocks common Docker socket directories
...
- drops unsafe npm package specs for node installers
...
- includes a CLI quick reference section
...
- routes to node proxy when target=node
- keeps sandbox bridge url when node proxy is available
- keeps chrome profile on host when node proxy is available
...
- falls back to most recently connected node when multiple non-Mac candidates exist
...
- listNodes
- falls back to node.pair.list only when node.list is unavailable
- rethrows unexpected node.list failures without fallback
...
- skips memory flush for CLI providers
...
- falls back to npm install hint for acpx when local extension is absent
...
- cdp.helpers
- preserves query params when appending CDP paths
...
- does not mutate NO_PROXY for non-loopback CDP URLs
...
- evaluates javascript via CDP
- fails when /json/version omits webSocketDebuggerUrl
- captures an aria snapshot via CDP
- normalizes loopback websocket URLs for remote CDP hosts
- propagates auth and query params onto normalized websocket URLs
- upgrades ws to wss when CDP uses https
...
- maps json response without CDP keys to wrong-port error
...
- stopOpenClawChrome sends SIGTERM and returns once CDP is down
- stopOpenClawChrome escalates to SIGKILL when CDP stays reachable
...
- supports overriding the local CDP auto-allocation range start
- rejects cdpPortRangeStart values that overflow the CDP range window
- normalizes hex colors
- supports custom remote CDP timeouts
- falls back to default color for invalid hex
- treats non-loopback cdpUrl as remote
- supports explicit CDP URLs for the default profile
...
- advertises CDP WS only when extension is connected
- uses relay-scoped token only for known relay ports
- rejects CDP access without relay auth token
...
- keeps CDP clients alive across a brief extension reconnect
...
- waits briefly for extension reconnect before failing CDP commands
- closes CDP clients after reconnect grace when extension stays disconnected
...
- falls back to derived CDP range when resolved CDP range is missing
...
## src/browser/pw-ai.e2e.test.ts

- pw-ai
- captures an ai snapshot via Playwright for a specific target
...
- reuses the CDP connection for repeated calls
...
- parses role refs
- preserves Playwright aria-ref ids in ai snapshots
...
- falls back to the only page when CDP session attachment is blocked (extension relays)
...
- restores refs for a different Page instance (same CDP targetId)
...
- waits for CDP readiness after launching to avoid follow-up PortInUseError races (#21149)
- stops launched chrome when CDP readiness never arrives
...
- stops local browser, closes playwright connection, and trashes profile dir
- forces playwright disconnect when loopback cdp is occupied by non-owned process
...
- browser CLI --browser-profile flag
...
- exec approvals CLI
- routes get command to local, gateway, and node modes
...
- wires --local-time through CLI parsing and emits local timestamps
...
- rejects camera clip url payloads without node remoteIp
...
- maps npm resolution metadata to install fields
- builds common npm install record fields
- logs pin warning/notice messages through provided writers
- resolves pinned install record and emits pin notice
- resolves pinned install record for CLI and formats warning output

## src/cli/outbound-send-mapping.test.ts

- createOutboundSendDepsFromCliSource
- maps CLI send deps to outbound send deps

## src/cli/pairing-cli.test.ts

- pairing cli
- evaluates pairing channels when registering the CLI (not at import)
...
- accepts channel as positional for approve (npm-run compatible)
...
- skips bundled pre-plan for scoped npm specs
- uses npm-spec bundled fallback only for package-not-found
- skips fallback for non-not-found npm failures
...
- runs nodes describe and calls node.describe
- runs nodes approve and calls node.pair.approve
- runs nodes invoke and calls node.invoke
...
- builds a node.list response with iOS node fixture
...
- command-registry
- includes both agent and agents in core CLI command names
...
- returns native optional dependency hint for node-gyp failures
- does not return npm hints for non-npm install modes
...
- prefers env parameter HOME over process.env.HOME for plist path
...
- clears stale Claude CLI legacy session IDs before retrying after session expiration
...
- skips lookup when runtime is not node
- emits warning when system node check returns one
...
- removes deprecated CLI auth profiles from store + config
...
- notes when gateway probe reports embeddings ready and CLI API key is missing
...
- warns when sandbox mode is enabled but Docker is not available
- warns when sandbox mode is 'all' but Docker is not available
- does not warn when sandbox mode is off
- does not warn when Docker is available
...
- installs from npm and enables the plugin
- uses local path when selected
- defaults to local on dev channel when local path exists
- defaults to npm on beta channel even when local path exists
- falls back to local path after npm install failure
...
- CLI session handoff (issue #29774)
...
- isNodeRuntime
- recognizes standard node binaries
- recognizes versioned node binaries with and without dashes
- handles quotes and casing
- rejects non-node runtimes
- isBunRuntime
...
- prefers execPath (version manager node) over system node
- falls back to system node when execPath version is unsupported
- ignores execPath when it is not node
- uses system node when it meets the minimum version
- skips system node when it is too old
- returns undefined when no system node is found
- resolveStableNodePath
- resolves Homebrew Cellar path to opt symlink
- falls back to bin symlink for default node formula
- resolves Intel Mac Cellar path to opt symlink
- resolves versioned node@22 formula to opt symlink
...
- returns undefined when system node is missing
- renders a warning when system node is too old
...
- passes through HOME for node services
- passes through OPENCLAW_GATEWAY_TOKEN for node services
- maps legacy CLAWDBOT_GATEWAY_TOKEN to OPENCLAW_GATEWAY_TOKEN for node services
...
- forwards proxy environment variables for node services
- forwards TMPDIR for node services
- falls back to os.tmpdir for node services when TMPDIR is not set
...
- passes CLI URL overrides to shared gateway auth resolver
...
- shares managers between ESM and alternate-loaded module instances
...
## src/discord/session-key-normalization.test.ts
...
- precreates config identity dir for CLI device auth writes
...
- keeps docker-compose CLI network namespace settings in sync
...
- installs optional browser dependencies after pnpm install
...
- does not apply remote tlsFingerprint for CLI url override
...
- GatewayClient
- uses a large maxPayload for node snapshots
...
- rejects non-node host requests
...
- streams OpenResponses SSE events
...
- authorizes roles against node vs operator methods
...
- broadcasts tool events to WS recipients even when verbose is off, but skips node send
- strips tool output when verbose is on
- keeps tool output when verbose is full
- broadcasts fallback events to agent subscribers and node session
- remaps chat-linked lifecycle runId to client runId
- suppresses chat and node session events for non-control-UI-visible runs
...
- chat.send inherits external delivery context for CLI clients on configured main sessions
...
- node.invoke APNs wake path
- keeps the existing not-connected response when wake path is unavailable
- wakes and retries invoke after the node reconnects
...
- returns invalid request when node has no APNs registration
...
- rejects host=node approval requests without nodeId
- rejects host=node approval requests without systemRunPlan
...
- authorizes canvas HTTP/WS via node-scoped capability and rejects misuse
...
- node.invoke approval bypass
- rejects malformed/forbidden node.invoke payloads before forwarding
- binds approvals to decision/device and blocks cross-device replay
- blocks cross-node replay on same device
...
- enforces operator and node permissions
- gateway update.run
- writes sentinel and schedules restart
- uses configured update channel
- gateway node command allowlist
- enforces command allowlists across node clients
- rejects reconnect metadata spoof for paired node devices
...
- rejects non-registry npm specs
- aborts when integrity drift callback rejects the fetched artifact
- rejects bare npm specs that resolve to prerelease versions
- gmail watcher
...
- hooks workspace
- ignores package.json hook paths that traverse outside package directory
- accepts package.json hook paths within package directory
- ignores package.json hook paths that escape via symlink
...
## src/imessage/monitor/loop-rate-limiter.test.ts
...
## src/imessage/monitor/reflection-guard.test.ts
...
## src/imessage/monitor/sanitize-outbound.test.ts
...
- falls back to package.json name matching when root resolution fails
...
- exec approvals node host allowlist check
...
- expandHomePrefix respects process.env.HOME changes
...
- returns true for dist/entry.js when launched via openclaw.mjs wrapper
...
- falls back to parsing final stdout line when npm json output is unavailable
- returns npm pack error details when command fails
- falls back to archive detected in cwd when npm pack stdout is empty
- falls back to archive detected in cwd when stdout does not contain a tgz
- returns friendly error for 404 (package not on npm)
- returns explicit error when npm pack produces no archive name
- parses scoped metadata from id-only json output even with npm notice prefix
...
- node pairing tokens
- reuses existing pending requests for the same node
- generates base64url node tokens with 256-bit entropy output length
- verifies token and rejects mismatches
- treats multibyte same-length token input as mismatch without throwing
...
- attaches npm metadata to successful install results

## src/infra/npm-registry-spec.test.ts

- npm registry spec validation
- accepts bare package names, exact versions, and dist-tags
- rejects semver ranges
- npm prerelease resolution policy
...
- ignores node task script hints for gateway restart detection on Windows
...
- stores and reloads node APNs registration
...
- supports idempotent remote node removal
...
- cleans stale npm rename dirs before global update
- retries global npm update with --omit=optional when initial install fails
- updates global bun installs when detected
- rejects git roots that are not a openclaw checkout
- fails with a clear reason when openclaw.mjs is missing
...
- watch-node script
- wires node watch to run-node with watched source/config paths
...
- uses a silent fast path in default Vitest mode without config reads
...
- falls back to CLI model when provider fails
...
- uses CLI image understanding and preserves caption for commands
...
- skips shared CLI entries without capabilities
...
- uses extension mapping for JavaScript assets
- extensionForMime
- isAudioFileName
...
- falls back to openai when node-llama-cpp is missing
...
## src/plugin-sdk/slack-message-actions.test.ts
...
- skips plugin CLI registrars when commands already exist
...
- before_model_resolve (run.ts pattern)
...
- before_prompt_build (attempt.ts pattern)
...
- uses --ignore-scripts for dependency install
- strips workspace devDependencies before npm install
...
- rejects non-registry npm specs
- aborts when integrity drift callback rejects the fetched artifact
- classifies npm package-not-found errors with a stable error code
- rejects bare npm specs that resolve to prerelease versions
- allows explicit prerelease npm tags

## src/plugins/installs.test.ts

- buildNpmResolutionInstallFields
- maps npm resolution metadata into install record fields
...
- preserves package.json metadata for bundled memory plugins
...
- skips integrity drift checks for unpinned npm specs during dry-run updates
- keeps integrity drift checks for exact-version npm specs during dry-run updates
...
- CLI latency summarizes turn metrics from JSONL
- CLI start prints JSON
...
- createPtyAdapter
- forwards explicit signals to node-pty kill on non-Windows
...
- keeps inherited env when no override env is provided
- passes explicit env overrides as strings
- does not pass a signal to node-pty on Windows
...
- enables node lane for node-relevant files
- keeps node lane off for native-only changes
- does not force macOS for generated protocol model-only changes
- enables node lane for non-native non-doc files by fallback
...
- warns when remote CDP uses HTTP
...
- warns on unpinned npm install specs and missing integrity metadata
- does not warn on pinned npm install specs with integrity metadata
...
- detects process.env access combined with network send (env harvesting)
...
- shared/node-list-parse
- parses node.list payloads
- parses node.pair.list payloads
...
- throws unknown node with known list
- throws ambiguous node with matches list
- prefers a unique connected node when names are duplicated
...
## src/telegram/bot-access.test.ts
...
- ignores unrelated nearby package.json files
...
- finds os.tmpdir calls imported from node:os
- finds tmpdir named import calls from node:os
- finds tmpdir calls imported from os
- ignores mentions in comments and strings
- ignores tmpdir symbols that are not imported from node:os
