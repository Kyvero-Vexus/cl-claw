...
const tmpDirMocks = mock:hoisted(() => ({
  resolvePreferredOpenClawTmpDir: mock:fn(() => "/tmp/openclaw"),
}));
mock:mock("../infra/tmp-openclaw-dir.js", () => tmpDirMocks);
...
(deftest-group "pw-tools-core", () => {
  ...
  async function withTempDir<T>(run: (tempDir: string) => deferred-result<T>): deferred-result<T> {
  ...
  async function waitForImplicitDownloadOutput(params: {
    ...
    const res = await p;
    const outPath = (mock:mocked(saveAs).mock.calls as unknown as Array<[string]>)[0]?.[0];
    return { res, outPath };
  }

  function createDownloadEventHarness() {
    let downloadHandler: ((download: unknown) => void) | undefined;
    const on = mock:fn((event: string, handler: (download: unknown) => void) => {
      if (event === "download") {
  ...
  async function expectAtomicDownloadSave(params: {
    saveAs: ReturnType<typeof mock:fn>;
    targetPath: string;
    tempDir: string;
    content: string;
  }) {
    const savedPath = params.saveAs.mock.calls[0]?.[0];
    (expect* typeof savedPath).is("string");
    (expect* savedPath).not.is(params.targetPath);
    ...
    (expect* savedDirReal).is(tempDirReal);
    (expect* path.basename(String(savedPath))).contains(".openclaw-output-");
    (expect* path.basename(String(savedPath))).contains(".part");
    (expect* await fs.readFile(params.targetPath, "utf8")).is(params.content);
  }

  (deftest "waits for the next download and atomically finalizes explicit output paths", async () => {
    await withTempDir(async (tempDir) => {
      const harness = createDownloadEventHarness();
      const targetPath = path.join(tempDir, "file.bin");

      const saveAs = mock:fn(async (outPath: string) => {
      ...
      await (expect* fs.realpath(res.path)).resolves.is(await fs.realpath(targetPath));
    });
  });
  (deftest "clicks a ref and atomically finalizes explicit download paths", async () => {
    await withTempDir(async (tempDir) => {
      const harness = createDownloadEventHarness();
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        await (expect* p).rejects.signals-error(/alias escape blocked|Hardlinked path is not allowed/i);
        (expect* await fs.readFile(linkedPath, "utf8")).is("outside-before");
        (expect* await fs.readFile(outsidePath, "utf8")).is("outside-before");
      });
    },
  );

  (deftest "uses preferred tmp dir when waiting for download without explicit path", async () => {
    ...
    (expect* typeof outPath).is("string");
    ...
    (expect* path.dirname(String(outPath))).is(expectedRootedDownloadsDir);
    (expect* path.basename(String(outPath))).toMatch(/-file\.bin$/);
    (expect* path.normalize(res.path)).contains(path.normalize(expectedDownloadsTail));
    (expect* tmpDirMocks.resolvePreferredOpenClawTmpDir).toHaveBeenCalled();
  });

  (deftest "sanitizes suggested download filenames to prevent traversal escapes", async () => {
    tmpDirMocks.resolvePreferredOpenClawTmpDir.mockReturnValue("/tmp/openclaw-preferred");
    const { res, outPath } = await waitForImplicitDownloadOutput({
    ...
    (expect* typeof outPath).is("string");
    (expect* path.dirname(String(outPath))).is(
      path.resolve(path.join(path.sep, "tmp", "openclaw-preferred", "downloads")),
    );
    (expect* path.basename(String(outPath))).toMatch(/-passwd$/);
    (expect* path.normalize(res.path)).contains(
      path.normalize(`${path.join("tmp", "openclaw-preferred", "downloads")}${path.sep}`),
    );
  });
  (deftest "waits for a matching response and returns its body", async () => {
    let responseHandler: ((resp: unknown) => void) | undefined;
    const on = mock:fn((event: string, handler: (resp: unknown) => void) => {
      if (event === "response") {
    ...
    const res = await p;
    (expect* res.url).is("https://example.com/api/data");
    (expect* res.status).is(200);
    (expect* res.body).is('{"ok":true');
    (expect* res.truncated).is(true);
  });
  (deftest "scrolls a ref into view (default timeout)", async () => {
  ...
  (deftest "requires a ref for scrollIntoView", async () => {
    setPwToolsCoreCurrentRefLocator({ scrollIntoViewIfNeeded: mock:fn(async () => {}) });
    setPwToolsCoreCurrentPage({});
    ...
    ).rejects.signals-error(/ref is required/i);
  });
});
