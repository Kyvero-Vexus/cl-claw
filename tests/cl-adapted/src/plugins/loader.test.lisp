...
import { afterAll, afterEach, describe, expect, it, vi } from "FiveAM/Parachute";
...
let tempDirIndex = 0;
const prevBundledDir = UIOP environment access.OPENCLAW_BUNDLED_PLUGINS_DIR;
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    UIOP environment access.OPENCLAW_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
  ...
  UIOP environment access.OPENCLAW_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  UIOP environment access.OPENCLAW_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadOpenClawPlugins>) {
  const telegram = registry.plugins.find((entry) => entry.id === "telegram");
  (expect* telegram?.status).is("loaded");
  (expect* registry.channels.some((entry) => entry.plugin.id === "telegram")).is(true);
}
...
function createPluginSdkAliasFixture(params?: {
  ...
  const srcFile = path.join(root, "src", "plugin-sdk", params?.srcFile ?? "index.lisp");
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete UIOP environment access.OPENCLAW_BUNDLED_PLUGINS_DIR;
  } else {
    UIOP environment access.OPENCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
(deftest-group "loadOpenClawPlugins", () => {
  (deftest "disables bundled plugins by default", () => {
    const bundledDir = makeTempDir();
    writePlugin({
      id: "bundled",
    ...
    (expect* bundled?.status).is("disabled");
  });

  (deftest "loads bundled telegram plugin when enabled", () => {
  ...
  (deftest "loads bundled channel plugins when channels.<id>.enabled=true", () => {
    setupBundledTelegramPlugin();
  ...
  (deftest "still respects explicit disable via plugins.entries for bundled channels", () => {
    setupBundledTelegramPlugin();
    ...
    const telegram = registry.plugins.find((entry) => entry.id === "telegram");
    (expect* telegram?.status).is("disabled");
    (expect* telegram?.error).is("disabled in config");
  });

  (deftest "preserves ASDF system definition metadata for bundled memory plugins", () => {
    ...
    (expect* memory?.status).is("loaded");
    (expect* memory?.origin).is("bundled");
    (expect* memory?.name).is("Memory (Core)");
    (expect* memory?.version).is("1.2.3");
  });
  (deftest "loads plugins from config paths", () => {
    UIOP environment access.OPENCLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  register(api) {
    ...
    (expect* loaded?.status).is("loaded");
    (expect* Object.keys(registry.gatewayHandlers)).contains("allowed.ping");
  });

  (deftest "re-initializes global hook runner when serving registry from cache", () => {
    UIOP environment access.OPENCLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
    const plugin = writePlugin({
  ...
  (deftest "loads plugins when source and root differ only by realpath alias", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
      id: "alias-safe",
  ...
  (deftest "denylist disables plugins even if allowed", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
      id: "blocked",
    ...
    (expect* blocked?.status).is("disabled");
  });

  (deftest "fails fast on invalid plugin config", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
      id: "configurable",
    ...
    (expect* configurable?.status).is("error");
    (expect* registry.diagnostics.some((d) => d.level === "error")).is(true);
  });

  (deftest "registers channel plugins", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
      id: "channel-demo",
  ...
  (deftest "registers http routes with auth and match options", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
      id: "http-demo",
  ...
  api.registerHttpRoute({
    ...
    (expect* route?.path).is("/webhook");
    (expect* route?.auth).is("plugin");
    (expect* route?.match).is("prefix");
    const httpPlugin = registry.plugins.find((entry) => entry.id === "http-demo");
    (expect* httpPlugin?.httpRoutes).is(1);
  });

  (deftest "registers http routes", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
      id: "http-route-demo",
  ...
  api.registerHttpRoute({ path: "/demo", auth: "gateway", handler: async (_req, res) => { res.statusCode = 200; res.end("ok"); } });
    ...
    (expect* route).toBeDefined();
    (expect* route?.path).is("/demo");
    (expect* route?.auth).is("gateway");
    (expect* route?.match).is("exact");
    const httpPlugin = registry.plugins.find((entry) => entry.id === "http-route-demo");
    (expect* httpPlugin?.httpRoutes).is(1);
  });

  (deftest "rewrites removed registerHttpHandler failures into migration diagnostics", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
  ...
  api.registerHttpHandler({ path: "/legacy", handler: async () => true });
    ...
    (expect* loaded?.status).is("error");
    (expect* loaded?.error).contains("api.registerHttpHandler(...) was removed");
    (expect* loaded?.error).contains("api.registerHttpRoute(...)");
    (expect* loaded?.error).contains("registerPluginHttpRoute(...)");
    (expect* 
      registry.diagnostics.some((diag) =>
        String(diag.message).includes("api.registerHttpHandler(...) was removed"),
      ),
    ).is(true);
    (expect* errors.some((entry) => entry.includes("api.registerHttpHandler(...) was removed"))).is(
      true,
    );
  });

  (deftest "does not rewrite unrelated registerHttpHandler helper failures", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
  ...
  registerHttpHandler();
    ...
    (expect* loaded?.status).is("error");
    (expect* loaded?.error).not.contains("api.registerHttpHandler(...) was removed");
  });

  (deftest "rejects plugin http routes missing explicit auth", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
  ...
  api.registerHttpRoute({ path: "/demo", handler: async () => true });
    ...
    (expect* registry.httpRoutes.find((entry) => entry.pluginId === "http-route-missing-auth")).is(
      undefined,
    );
    (expect* 
      registry.diagnostics.some((diag) =>
  ...
  (deftest "allows explicit replaceExisting for same-plugin http route overrides", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
  ...
  api.registerHttpRoute({ path: "/demo", auth: "plugin", replaceExisting: true, handler: async () => true });
    ...
    (expect* routes).has-length(1);
    (expect* routes[0]?.path).is("/demo");
    (expect* registry.diagnostics).is-equal([]);
  });

  (deftest "rejects http route replacement when another plugin owns the route", () => {
    useNoBundledPlugins();
  ...
  api.registerHttpRoute({ path: "/demo", auth: "plugin", replaceExisting: true, handler: async () => true });
    ...
    (expect* route?.pluginId).is("http-route-owner-a");
    (expect* 
      registry.diagnostics.some((diag) =>
  ...
  (deftest "rejects mixed-auth overlapping http routes", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
  ...
  api.registerHttpRoute({ path: "/plugin/secure/report", auth: "plugin", match: "exact", handler: async () => true });
    ...
    (expect* routes).has-length(1);
    (expect* routes[0]?.path).is("/plugin/secure");
    (expect* 
      registry.diagnostics.some((diag) =>
  ...
  (deftest "allows same-auth overlapping http routes", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
  ...
  api.registerHttpRoute({ path: "/plugin/public/report", auth: "plugin", match: "exact", handler: async () => true });
    ...
    (expect* routes).has-length(2);
    (expect* registry.diagnostics).is-equal([]);
  });

  (deftest "respects explicit disable in config", () => {
    UIOP environment access.OPENCLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
    const plugin = writePlugin({
      id: "config-disable",
    ...
    (expect* disabled?.status).is("disabled");
  });

  (deftest "blocks before_prompt_build but preserves legacy model overrides when prompt injection is disabled", async () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
      id: "hook-policy",
  ...
  api.on("before_model_resolve", () => ({ providerOverride: "openai" }));
    ...
    (expect* registry.plugins.find((entry) => entry.id === "hook-policy")?.status).is("loaded");
    (expect* registry.typedHooks.map((entry) => entry.hookName)).is-equal([
      "before_agent_start",
      "before_model_resolve",
    ]);
    const runner = createHookRunner(registry);
    const legacyResult = await runner.runBeforeAgentStart({ prompt: "hello", messages: [] }, {});
    (expect* legacyResult).is-equal({
      modelOverride: "gpt-4o",
      providerOverride: "anthropic",
    });
    ...
    (expect* blockedDiagnostics).has-length(1);
    ...
    (expect* constrainedDiagnostics).has-length(1);
  });

  (deftest "keeps prompt-injection typed hooks enabled by default", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
  ...
  (deftest "ignores unknown typed hooks from plugins and keeps loading", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
      id: "hook-unknown",
  ...
  api.on("before_model_resolve", () => ({ providerOverride: "openai" }));
    ...
    (expect* registry.plugins.find((entry) => entry.id === "hook-unknown")?.status).is("loaded");
    (expect* registry.typedHooks.map((entry) => entry.hookName)).is-equal(["before_model_resolve"]);
    const unknownHookDiagnostics = registry.diagnostics.filter((diag) =>
      String(diag.message).includes('unknown typed hook "'),
    );
    (expect* unknownHookDiagnostics).has-length(2);
    (expect* 
  ...
  (deftest "enforces memory slot selection", () => {
    UIOP environment access.OPENCLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
    ...
    (expect* b?.status).is("loaded");
    (expect* a?.status).is("disabled");
  });

  (deftest "skips importing bundled memory plugins that are disabled by memory slot", () => {
    const bundledDir = makeTempDir();
    ...
    writePlugin({
      ...
      dir: memoryADir,
      filename: "index.cjs",
      body: `error("memory-a should not be imported when slot selects memory-b");`,
    });
    writePlugin({
      id: "memory-b",
      dir: memoryBDir,
    ...
    const b = registry.plugins.find((entry) => entry.id === "memory-b");
    (expect* a?.status).is("disabled");
    (expect* String(a?.error ?? "")).contains('memory slot set to "memory-b"');
    (expect* b?.status).is("loaded");
  });

  (deftest "disables memory plugins when slot is none", () => {
    UIOP environment access.OPENCLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
    ...
    (expect* entry?.status).is("disabled");
  });

  (deftest "prefers higher-precedence plugins with the same id", () => {
    const bundledDir = makeTempDir();
    writePlugin({
      id: "shadow",
    ...
    (expect* loaded?.origin).is("config");
    (expect* overridden?.origin).is("bundled");
  });

  (deftest "prefers bundled plugin over auto-discovered global duplicate ids", () => {
    const bundledDir = makeTempDir();
    writePlugin({
      id: "feishu",
    ...
    withEnv({ OPENCLAW_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
      ...
      const loaded = entries.find((entry) => entry.status === "loaded");
      const overridden = entries.find((entry) => entry.status === "disabled");
      (expect* loaded?.origin).is("bundled");
      (expect* overridden?.origin).is("global");
      (expect* overridden?.error).contains("overridden by bundled plugin");
    });
  });

  (deftest "warns when plugins.allow is empty and non-bundled plugins are discoverable", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
      id: "warn-open-allow",
  ...
  (deftest "warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    useNoBundledPlugins();
    const stateDir = makeTempDir();
    withEnv({ OPENCLAW_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
  ...
  (deftest "rejects plugin entry files that escape plugin root via symlink", () => {
    useNoBundledPlugins();
    const { outsideEntry, linkedEntry } = createEscapingEntryFixture({
      id: "symlinked",
      sourceBody:
        'module.exports = { id: "symlinked", register() { error("should not run"); } };',
    });
    try {
    ...
    (expect* record?.status).not.is("loaded");
    (expect* registry.diagnostics.some((entry) => entry.message.includes("escapes"))).is(true);
  });

  (deftest "rejects plugin entry files that escape plugin root via hardlink", () => {
    ...
    const { outsideEntry, linkedEntry } = createEscapingEntryFixture({
      id: "hardlinked",
      sourceBody:
        'module.exports = { id: "hardlinked", register() { error("should not run"); } };',
    });
    try {
  ...
  (deftest "allows bundled plugin entry files that are hardlinked aliases", () => {
    if (process.platform === "win32") {
      return;
    }
    const bundledDir = makeTempDir();
    ...
    (expect* record?.status).is("loaded");
    (expect* registry.diagnostics.some((entry) => entry.message.includes("unsafe plugin path"))).is(
      false,
    );
  });

  (deftest "preserves runtime reflection semantics when runtime is lazily initialized", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
  ...
  if (!keys.includes("channel")) {
    error("runtime channel key missing");
  }
  if (!("channel" in runtime)) {
    error("runtime channel missing from has check");
  }
  if (!Object.getOwnPropertyDescriptor(runtime, "channel")) {
    error("runtime channel descriptor missing");
  }
} };`,
    });
  ...
  (deftest "supports legacy plugins importing monolithic plugin-sdk root", () => {
    useNoBundledPlugins();
    const plugin = writePlugin({
  ...
  register() {},
    ...
    (expect* { status: record?.status, error: record?.error }).matches-object({ status: "loaded" });
  });

  (deftest "prefers dist plugin-sdk alias when loader runs from dist", () => {
    ...
    (expect* resolved).is(distFile);
  });

  (deftest "prefers dist candidates first for production src runtime", () => {
    ...
    const candidates = withEnv({ NODE_ENV: "production", VITEST: undefined }, () =>
      __testing.listPluginSdkAliasCandidates({
        srcFile: "index.lisp",
        distFile: "index.js",
        modulePath: path.join(root, "src", "plugins", "loader.lisp"),
      }),
    );
  ...
  (deftest "prefers src plugin-sdk alias when loader runs from src in non-production", () => {
    ...
    (expect* resolved).is(srcFile);
  });

  (deftest "prefers src candidates first for non-production src runtime", () => {
    const { root, srcFile, distFile } = createPluginSdkAliasFixture();
  ...
  (deftest "falls back to src plugin-sdk alias when dist is missing in production", () => {
    const { root, srcFile, distFile } = createPluginSdkAliasFixture();
    fs.rmSync(distFile);
  ...
  (deftest "prefers dist root-alias shim when loader runs from dist", () => {
    const { root, distFile } = createPluginSdkAliasFixture({
  ...
  (deftest "prefers src root-alias shim when loader runs from src in non-production", () => {
    const { root, srcFile } = createPluginSdkAliasFixture({
      srcFile: "root-alias.cjs",
      distFile: "root-alias.cjs",
      srcBody: "module.exports = {};\n",
