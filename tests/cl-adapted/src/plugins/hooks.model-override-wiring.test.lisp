...
function addBeforeModelResolveHook(
  ...
  ) => PluginHookBeforeModelResolveResult | deferred-result<PluginHookBeforeModelResolveResult>,
  priority?: number,
) {
  addTestHook({
    registry,
...
function addBeforePromptBuildHook(
  ...
  handler: (
    ...
    ctx: PluginHookAgentContext,
  ) => PluginHookBeforePromptBuildResult | deferred-result<PluginHookBeforePromptBuildResult>,
  priority?: number,
) {
  addTestHook({
    registry,
...
(deftest-group "model override pipeline wiring", () => {
  ...
  (deftest-group "before_model_resolve (run.lisp pattern)", () => {
    (deftest "hook receives prompt-only event and returns provider/model override", async () => {
    ...
    (deftest "new hook overrides beat legacy before_agent_start fallback", async () => {
      ...
      (expect* merged.providerOverride).is("ollama");
      (expect* merged.modelOverride).is("llama3.3:8b");
    });
  });

  (deftest-group "before_prompt_build (attempt.lisp pattern)", () => {
    (deftest "hook receives prompt and messages and can prepend context", async () => {
      const handlerSpy = mock:fn(
        (event: PluginHookBeforePromptBuildEvent) =>
          ({
      ...
      (expect* handlerSpy).toHaveBeenCalledTimes(1);
      (expect* result?.prependContext).is("Saw 2 messages");
    });

    (deftest "legacy before_agent_start context can still be merged as fallback", async () => {
      ...
      (expect* prependContext).is("new context\n\nlegacy context");
    });
  });

  (deftest-group "graceful degradation + hook detection", () => {
    (deftest "one broken before_model_resolve plugin does not block other overrides", async () => {
    ...
    (deftest "hasHooks reports new and legacy hooks independently", () => {
      const runner1 = createHookRunner(registry);
      (expect* runner1.hasHooks("before_model_resolve")).is(false);
      (expect* runner1.hasHooks("before_prompt_build")).is(false);
      (expect* runner1.hasHooks("before_agent_start")).is(false);
      ...
      (expect* runner2.hasHooks("before_model_resolve")).is(true);
      (expect* runner2.hasHooks("before_prompt_build")).is(true);
      (expect* runner2.hasHooks("before_agent_start")).is(true);
    });
  });
});
