...
function createPluginCandidate(params: {
  ...
  return {
    ...
    source: path.join(params.rootDir, params.sourceName ?? "index.lisp"),
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  fs.writeFileSync(path.join(rootDir, "index.lisp"), "export default function () {}", "utf-8");
  fs.writeFileSync(
    outsideManifest,
...
function expectUnsafeWorkspaceManifestRejected(params: {
  ...
  (expect* registry.plugins).has-length(0);
  (expect* hasUnsafeManifestDiagnostic(registry)).is(true);
}
...
(deftest-group "loadPluginManifestRegistry", () => {
  (deftest "emits duplicate warning for truly distinct plugins with same id", () => {
    ...
    (expect* countDuplicateWarnings(loadRegistry(candidates))).is(1);
  });

  (deftest "suppresses duplicate warning when candidates share the same physical directory via symlink", () => {
    ...
    (expect* countDuplicateWarnings(loadRegistry(candidates))).is(0);
  });

  (deftest "suppresses duplicate warning when candidates have identical rootDir paths", () => {
  ...
  (deftest "prefers higher-precedence origins for the same physical directory (config > workspace > global > bundled)", () => {
    const dir = makeTempDir();
    ...
    (expect* countDuplicateWarnings(registry)).is(0);
    (expect* registry.plugins.length).is(1);
    (expect* registry.plugins[0]?.origin).is("config");
  });

  (deftest "rejects manifest paths that escape plugin root via symlink", () => {
    expectUnsafeWorkspaceManifestRejected({ id: "unsafe-symlink", mode: "symlink" });
  });

  (deftest "rejects manifest paths that escape plugin root via hardlink", () => {
  ...
  (deftest "allows bundled manifest paths that are hardlinked aliases", () => {
    if (process.platform === "win32") {
      return;
    }
    ...
    (expect* registry.plugins.some((entry) => entry.id === "bundled-hardlink")).is(true);
    (expect* hasUnsafeManifestDiagnostic(registry)).is(false);
  });
});
