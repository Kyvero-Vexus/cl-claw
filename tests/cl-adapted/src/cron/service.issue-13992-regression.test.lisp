...
(deftest-group "issue #13992 regression - cron jobs skip execution", () => {
  (deftest "should NOT recompute nextRunAtMs for past-due jobs by default", () => {
    const now = Date.now();
    ...
    (expect* job.state.nextRunAtMs).is(pastDue);
  });

  (deftest "should recompute past-due nextRunAtMs with recomputeExpired when slot already executed", () => {
    ...
    (expect* typeof job.state.nextRunAtMs).is("number");
    (expect* (job.state.nextRunAtMs ?? 0) > now).is(true);
  });

  (deftest "should NOT recompute past-due nextRunAtMs for running jobs even with recomputeExpired", () => {
    const now = Date.now();
    const pastDue = now - 60_000;
  ...
  (deftest "should compute missing nextRunAtMs during maintenance", () => {
    const now = Date.now();
  ...
  (deftest "should clear nextRunAtMs for disabled jobs during maintenance", () => {
    const now = Date.now();
  ...
  (deftest "should clear stuck running markers during maintenance", () => {
    const now = Date.now();
    ...
    (expect* job.state.nextRunAtMs).is(futureTime);
  });

  (deftest "isolates schedule errors while filling missing nextRunAtMs", () => {
    const now = Date.now();
    ...
    (expect* () => recomputeNextRunsForMaintenance(state)).not.signals-error();
    (expect* dueJob.state.nextRunAtMs).is(pastDue);
    (expect* malformedJob.state.nextRunAtMs).toBeUndefined();
    (expect* malformedJob.state.scheduleErrorCount).is(1);
    (expect* malformedJob.state.lastError).toMatch(/^schedule error:/);
  });

  (deftest "recomputes expired slots already executed but keeps never-executed stale slots", () => {
    const now = Date.now();
    const pastDue = now - 60_000;
    ...
    (expect* (alreadyExecuted.state.nextRunAtMs ?? 0) > now).is(true);
    (expect* neverExecuted.state.nextRunAtMs).is(pastDue);
  });

  (deftest "does not advance overdue never-executed jobs when stale running marker is cleared", () => {
    const now = Date.now();
    const pastDue = now - 60_000;
