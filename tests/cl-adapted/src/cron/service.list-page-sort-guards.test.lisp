...
(deftest-group "cron listPage sort guards", () => {
  (deftest "does not throw when sorting by name with malformed name fields", async () => {
    const jobs = [
    ...
    (expect* page.jobs).has-length(2);
  });

  (deftest "does not throw when tie-break sorting encounters missing ids", async () => {
