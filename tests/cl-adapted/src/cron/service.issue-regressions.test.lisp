...
(deftest-group "Cron issue regressions", () => {
  const { makeStorePath } = setupCronIssueRegressionFixtures();

  (deftest "covers schedule updates and payload patching", async () => {
    ...
    (expect* created.state.nextRunAtMs).is(Date.parse("2026-02-06T11:00:00.000Z") + offsetMs);

    const updated = await cron.update(created.id, {
      schedule: { kind: "cron", expr: "0 */2 * * *", tz: "UTC" },
    });

    (expect* updated.state.nextRunAtMs).is(Date.parse("2026-02-06T12:00:00.000Z") + offsetMs);
    ...
    (expect* patched.payload.kind).is("agentTurn");
    if (patched.payload.kind === "agentTurn") {
      (expect* patched.payload.allowUnsafeExternalContent).is(true);
      (expect* patched.payload.message).is("hi");
    }

    cron.stop();
  });

  (deftest "repairs isolated every jobs missing createdAtMs and sets nextWakeAtMs", async () => {
    const store = makeStorePath();
    ...
    const cron = new CronService({
      ...
      runIsolatedAgentJob: mock:fn().mockResolvedValue({ status: "ok", summary: "ok" }),
    });
    await cron.start();
    ...
    (expect* Number.isFinite(isolated?.state.nextRunAtMs)).is(true);
    (expect* Number.isFinite(status.nextWakeAtMs)).is(true);
    ...
    (expect* typeof persistedIsolated?.state?.nextRunAtMs).is("number");
    (expect* Number.isFinite(persistedIsolated?.state?.nextRunAtMs)).is(true);

    cron.stop();
  });

  (deftest "repairs missing nextRunAtMs on non-schedule updates without touching other jobs", async () => {
    const store = makeStorePath();
    ...
    (expect* typeof updated.state.nextRunAtMs).is("number");
    (expect* updated.state.nextRunAtMs).is(created.state.nextRunAtMs);

    cron.stop();
  });

  (deftest "does not advance unrelated due jobs when updating another job", async () => {
    const store = makeStorePath();
    ...
    (expect* typeof originalDueNextRunAtMs).is("number");

    // Make dueJob past-due without running timer callbacks.
    mock:setSystemTime(now + 5 * 60_000);
    ...
    (expect* persistedDueJob?.state?.nextRunAtMs).is(originalDueNextRunAtMs);

    cron.stop();
  });

  (deftest "treats persisted jobs with missing enabled as enabled during update()", async () => {
    const store = makeStorePath();
    const now = Date.parse("2026-02-06T10:05:00.000Z");
    await writeCronStoreSnapshot(store.storePath, [
      {
    ...
    (expect* listed.some((job) => job.id === "missing-enabled-update")).is(true);
  ...
  (deftest "treats persisted due jobs with missing enabled as runnable", async () => {
    const store = makeStorePath();
    const now = Date.parse("2026-02-06T10:05:00.000Z");
  ...
  (deftest "caps timer delay to 60s for far-future schedules", async () => {
    ...
    (expect* delaysAfterAdd.some((delay) => delay === 60_000)).is(true);

    cron.stop();
    timeoutSpy.mockRestore();
  });

  (deftest "re-arms timer without hot-looping when a run is already in progress", async () => {
    const timeoutSpy = mock:spyOn(globalThis, "setTimeout");
    const store = makeStorePath();
    const now = Date.parse("2026-02-06T10:05:00.000Z");
  ...
  (deftest "skips forced manual runs while a timer-triggered run is in progress", async () => {
    const store = makeStorePath();
    let resolveRun:
      | ((value: { status: "ok" | "error" | "skipped"; summary?: string; error?: string }) => void)
      | undefined;
    const runIsolatedAgentJob = mock:fn(
      async () =>
        await new deferred-result<{ status: "ok" | "error" | "skipped"; summary?: string; error?: string }>(
          (resolve) => {
            resolveRun = resolve;
          },
        ),
    ...
    targetJobId = job.id;
    await mock:advanceTimersByTimeAsync(2);
    await started.promise;
    (expect* runIsolatedAgentJob).toHaveBeenCalledTimes(1);

    const manualResult = await cron.run(job.id, "force");
    (expect* manualResult).is-equal({ ok: true, ran: false, reason: "already-running" });
    (expect* runIsolatedAgentJob).toHaveBeenCalledTimes(1);
  ...
  (deftest "does not double-run a job when cron.run overlaps a due timer tick", async () => {
    const store = makeStorePath();
    ...
    const runIsolatedAgentJob = mock:fn(async () => {
      ...
      return await new deferred-result<{ status: "ok" | "error" | "skipped"; summary?: string }>(
        (resolve) => {
    ...
    await mock:advanceTimersByTimeAsync(105);
  ...
  (deftest "manual cron.run preserves unrelated due jobs but advances already-executed stale slots", async () => {
    const store = makeStorePath();
    ...
    (expect* runResult).is-equal({ ok: true, ran: true });
    ...
    (expect* unrelated?.state.nextRunAtMs).is(dueNextRunAtMs);
    (expect* staleExecuted).toBeDefined();
    (expect* (staleExecuted?.state.nextRunAtMs ?? 0) > nowMs).is(true);

    cron.stop();
  });

  (deftest "keeps telegram delivery target writeback after manual cron.run", async () => {
    const store = makeStorePath();
    const originalTarget = "https://t.me/obviyus";
    const rewrittenTarget = "-10012345/6789";
    const runIsolatedAgentJob = mock:fn(async (params: { job: { id: string } }) => {
    ...
    (expect* persistedJob?.delivery?.to).is(rewrittenTarget);
    (expect* persistedJob?.state.lastStatus).is("ok");
    (expect* persistedJob?.state.lastDelivered).is(true);

    cron.stop();
  });

  (deftest "#13845: one-shot jobs with terminal statuses do not re-fire on restart", async () => {
    const store = makeStorePath();
    ...
    for (const { id, state } of terminalStates) {
      ...
      const cron = await startCronForStore({
        storePath: store.storePath,
        enqueueSystemEvent,
        runIsolatedAgentJob: mock:fn().mockResolvedValue({ status: "ok" }),
      });
  ...
  (deftest "#24355: one-shot retries then succeeds (with and without deleteAfterRun)", async () => {
    ...
    const runRetryScenario = async (params: {
      ...
      runIsolatedAgentJob: ReturnType<typeof mock:fn>;
      ...
      (expect* jobAfterRetry!.enabled).is(true);
      (expect* jobAfterRetry!.state.lastStatus).is("error");
    ...
    (expect* keepJob!.state.lastStatus).is("ok");
    ...
    (expect* overloadedJob!.state.lastStatus).is("ok");
    (expect* overloadedResult.runIsolatedAgentJob).toHaveBeenCalledTimes(2);
  });

  (deftest "#24355: one-shot job disabled after max transient retries", async () => {
    const store = makeStorePath();
    const scheduledAt = Date.parse("2026-02-06T10:00:00.000Z");
    ...
    let now = scheduledAt;
    const runIsolatedAgentJob = mock:fn().mockResolvedValue({
      status: "error",
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (i < 3) {
        (expect* job!.enabled).is(true);
        now = (job!.state.nextRunAtMs ?? now) + 1;
      } else {
        (expect* job!.enabled).is(false);
      }
    }
    (expect* runIsolatedAgentJob).toHaveBeenCalledTimes(4);
  });

  (deftest "#24355: one-shot job respects cron.retry config", async () => {
    const store = makeStorePath();
    const scheduledAt = Date.parse("2026-02-06T10:00:00.000Z");
  ...
  (deftest "#24355: one-shot job retries status-only 529 failures when retryOn only includes overloaded", async () => {
    const store = makeStorePath();
    const scheduledAt = Date.parse("2026-02-06T10:00:00.000Z");
    ...
    (expect* finishedJob!.state.lastStatus).is("ok");
    (expect* runIsolatedAgentJob).toHaveBeenCalledTimes(2);
  });

  (deftest "#24355: one-shot job disabled immediately on permanent error", async () => {
    const store = makeStorePath();
    const scheduledAt = Date.parse("2026-02-06T10:00:00.000Z");
    ...
    const state = createCronServiceState({
      ...
      enqueueSystemEvent: mock:fn(),
      requestHeartbeatNow: mock:fn(),
      runIsolatedAgentJob: mock:fn().mockResolvedValue({
        status: "error",
    ...
    (expect* job).toBeDefined();
    (expect* job!.enabled).is(false);
    (expect* job!.state.lastStatus).is("error");
    (expect* job!.state.nextRunAtMs).toBeUndefined();
  });

  (deftest "prevents spin loop when cron job completes within the scheduled second (#17821)", async () => {
    const store = makeStorePath();
  ...
  (deftest "enforces a minimum refire gap for second-granularity cron schedules (#17821)", async () => {
    const store = makeStorePath();
    const scheduledAt = Date.parse("2026-02-15T13:00:00.000Z");
  ...
  (deftest "treats timeoutSeconds=0 as no timeout for isolated agentTurn jobs", async () => {
    const store = makeStorePath();
    const scheduledAt = Date.parse("2026-02-15T13:00:00.000Z");
    ...
    (expect* job?.state.lastStatus).is("ok");
  });

  (deftest "does not time out agentTurn jobs at the default 10-minute safety window", async () => {
    const store = makeStorePath();
    const scheduledAt = Date.parse("2026-02-15T13:00:00.000Z");
    ...
    let now = scheduledAt;
    const deferredRun = createDeferred<{ status: "ok"; summary: string }>();
    const runIsolatedAgentJob = mock:fn(async ({ abortSignal }: { abortSignal?: AbortSignal }) => {
      const result = await deferredRun.promise;
    ...
    await mock:advanceTimersByTimeAsync(DEFAULT_JOB_TIMEOUT_MS + 1_000);
    await Promise.resolve();
    (expect* settled).is(false);
  ...
  (deftest "aborts isolated runs when cron timeout fires", async () => {
    mock:useRealTimers();
    const store = makeStorePath();
    const scheduledAt = Date.parse("2026-02-15T13:00:00.000Z");
    const cronJob = createIsolatedRegressionJob({
    ...
    const state = createCronServiceState({
      ...
      enqueueSystemEvent: mock:fn(),
      requestHeartbeatNow: mock:fn(),
      runIsolatedAgentJob: mock:fn(async (params) => {
    ...
    (expect* abortAwareRunner.getObservedAbortSignal()?.aborted).is(true);
    const job = state.store?.jobs.find((entry) => entry.id === "abort-on-timeout");
    (expect* job?.state.lastStatus).is("error");
    (expect* job?.state.lastError).contains("timed out");
  });

  (deftest "suppresses isolated follow-up side effects after timeout", async () => {
    mock:useRealTimers();
    const store = makeStorePath();
    const scheduledAt = Date.parse("2026-02-15T13:00:00.000Z");
    const enqueueSystemEvent = mock:fn();
    ...
    (expect* jobAfterTimeout?.state.lastStatus).is("error");
    (expect* jobAfterTimeout?.state.lastError).contains("timed out");
    (expect* enqueueSystemEvent).not.toHaveBeenCalled();
  });

  (deftest "applies timeoutSeconds to manual cron.run isolated executions", async () => {
    mock:useRealTimers();
    const store = makeStorePath();
    const abortAwareRunner = createAbortAwareIsolatedRunner();
    ...
    (expect* updated?.state.lastStatus).is("error");
    (expect* updated?.state.lastError).contains("timed out");
    (expect* updated?.state.runningAtMs).toBeUndefined();

    cron.stop();
  });

  (deftest "applies timeoutSeconds to startup catch-up isolated executions", async () => {
    mock:useRealTimers();
    const store = makeStorePath();
    const scheduledAt = Date.parse("2026-02-15T13:00:00.000Z");
    const cronJob = createIsolatedRegressionJob({
  ...
  (deftest "respects abort signals while retrying main-session wake-now heartbeat runs", async () => {
    const abortController = new AbortController();
    const runHeartbeatOnce = mock:fn(
      async (): deferred-result<HeartbeatRunResult> => ({
        status: "skipped",
  ...
  (deftest "retries cron schedule computation from the next second when the first attempt returns undefined (#17821)", () => {
    const scheduledAt = Date.parse("2026-02-15T13:00:00.000Z");
    const cronJob = createIsolatedRegressionJob({
    ...
    const spy = mock:spyOn(schedule, "computeNextRunAtMs");
    try {
      spy
  ...
  (deftest "records per-job start time and duration for batched due jobs", async () => {
    const store = makeStorePath();
    ...
    const state = createCronServiceState({
      ...
      runIsolatedAgentJob: mock:fn(async (params: { job: { id: string } }) => {
    ...
    (expect* firstDone?.state.lastRunAtMs).is(dueAt);
    (expect* firstDone?.state.lastDurationMs).is(50);
    (expect* secondDone?.state.lastRunAtMs).is(dueAt + 50);
    (expect* secondDone?.state.lastDurationMs).is(20);
    (expect* startedAtEvents).is-equal([dueAt, dueAt + 50]);
  });

  (deftest "#17554: run() clears stale runningAtMs and executes the job", async () => {
    const store = makeStorePath();
    const now = Date.parse("2026-02-06T10:05:00.000Z");
  ...
  (deftest "honors cron maxConcurrentRuns for due jobs", async () => {
    mock:useRealTimers();
    const store = makeStorePath();
    const dueAt = Date.parse("2026-02-06T10:05:01.000Z");
    ...
    const jobs = state.store?.jobs ?? [];
    (expect* jobs.find((job) => job.id === first.id)?.state.lastStatus).is("ok");
    (expect* jobs.find((job) => job.id === second.id)?.state.lastStatus).is("ok");
  });
  ...
  (deftest "outer cron timeout fires at configured timeoutSeconds, not at 1/3 (#29774)", async () => {
    mock:useRealTimers();
    const store = makeStorePath();
    const scheduledAt = Date.parse("2026-02-15T13:00:00.000Z");
    ...
    const state = createCronServiceState({
      ...
      enqueueSystemEvent: mock:fn(),
      requestHeartbeatNow: mock:fn(),
      runIsolatedAgentJob: mock:fn(async ({ abortSignal }: { abortSignal?: AbortSignal }) => {
        started = true;
  ...
  (deftest "keeps state updates when cron next-run computation throws after a successful run (#30905)", () => {
    ...
    (expect* shouldDelete).is(false);
    (expect* job.state.runningAtMs).toBeUndefined();
    (expect* job.state.lastRunAtMs).is(startedAt);
    (expect* job.state.lastStatus).is("ok");
    (expect* job.state.scheduleErrorCount).is(1);
    (expect* job.state.lastError).toMatch(/^schedule error:/);
    (expect* job.state.nextRunAtMs).is(endedAt + 2_000);
    (expect* job.enabled).is(true);
  });

  (deftest "falls back to backoff schedule when cron next-run computation throws on error path (#30905)", () => {
    ...
    (expect* job.state.lastRunAtMs).is(startedAt);
    (expect* job.state.lastStatus).is("error");
    (expect* job.state.consecutiveErrors).is(1);
    (expect* job.state.scheduleErrorCount).is(1);
    (expect* job.state.lastError).toMatch(/^schedule error:/);
    (expect* job.state.nextRunAtMs).is(endedAt + 30_000);
    (expect* job.enabled).is(true);
  });

  (deftest "force run preserves 'every' anchor while recording manual lastRunAtMs", () => {
    const nowMs = Date.now();
    ...
    (expect* job.state.lastRunAtMs).is(startedAt);
    (expect* job.state.nextRunAtMs).is(expectedNextMs);
  });
});
