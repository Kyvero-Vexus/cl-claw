...
(deftest-group "#16156: cron.list() must not silently advance past-due recurring jobs", () => {
  (deftest "does not skip a cron job when list() is called while the job is past-due", async () => {
    ...
    (expect* firstDueAt).is(Date.parse("2025-12-13T00:01:00.000Z"));
    ...
    (expect* jobBeforeTimer?.state.nextRunAtMs).is(firstDueAt);
  ...
  (deftest "does not skip a cron job when status() is called while the job is past-due", async () => {
    const store = await makeStorePath();
    const { cron, enqueueSystemEvent, finished } = createStartedCronServiceWithFinishedBarrier({
      storePath: store.storePath,
      logger: noopLogger,
    ...
    mock:setSystemTime(new Date(firstDueAt + 10));
  ...
  (deftest "still fills missing nextRunAtMs via list() for enabled jobs", async () => {
    const store = await makeStorePath();
