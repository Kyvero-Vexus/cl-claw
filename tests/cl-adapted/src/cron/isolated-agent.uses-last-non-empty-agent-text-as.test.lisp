...
function expectEmbeddedProviderModel(expected: { provider: string; model: string }) {
  const call = mock:mocked(runEmbeddedPiAgent).mock.calls.at(-1)?.[0] as {
    provider?: string;
    model?: string;
  };
  (expect* call?.provider).is(expected.provider);
  (expect* call?.model).is(expected.model);
}
...
async function runStoredOverrideAndExpectModel(params: {
  ...
  mock:mocked(runEmbeddedPiAgent).mockClear();
  mock:mocked(loadModelCatalog).mockResolvedValue(params.deterministicCatalog);
...
(deftest-group "runCronIsolatedAgentTurn", () => {
  ...
  (deftest "treats blank model overrides as unset", async () => {
  ...
  (deftest "uses last non-empty agent text as summary", async () => {
    await withTempHome(async (home) => {
      const { res } = await runCronTurn(home, {
        jobPayload: DEFAULT_AGENT_TURN_PAYLOAD,
      ...
      (expect* res.status).is("ok");
      (expect* res.summary).is("last");
    });
  });

  (deftest "returns error when embedded run payload is marked as error", async () => {
    await withTempHome(async (home) => {
      ...
      (expect* res.error).contains("command not found");
      (expect* res.summary).contains("Exec failed");
    });
  });

  (deftest "treats transient error payloads as non-fatal when a later success payload exists", async () => {
    await withTempHome(async (home) => {
      mockEmbeddedPayloads([
        {
      ...
      (expect* res.status).is("ok");
      (expect* res.summary).is("Write completed successfully.");
    });
  });

  (deftest "keeps error status when run-level error accompanies post-error text", async () => {
    await withTempHome(async (home) => {
      mock:mocked(runEmbeddedPiAgent).mockResolvedValue({
        payloads: [
  ...
  (deftest "passes resolved agentDir to runEmbeddedPiAgent", async () => {
    await withTempHome(async (home) => {
      const { res } = await runCronTurn(home, {
        jobPayload: DEFAULT_AGENT_TURN_PAYLOAD,
      });
      ...
      (expect* call?.agentDir).is(path.join(home, ".openclaw", "agents", "main", "agent"));
    });
  });

  (deftest "appends current time after the cron header line", async () => {
    await withTempHome(async (home) => {
      ...
      (expect* lines[0]).contains("[cron:job-1");
      (expect* lines[0]).contains("do it");
      (expect* lines[1]).toMatch(/^Current time: .+ \(.+\) \/ \d{4}-\d{2}-\d{2} \d{2}:\d{2} UTC$/);
    });
  });

  (deftest "uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      const deps = makeDeps();
      ...
      (expect* call?.sessionKey).is("agent:ops:cron:job-ops");
      (expect* call?.workspaceDir).is(opsWorkspace);
      (expect* call?.sessionFile).contains(path.join("agents", "ops"));
    });
  });

  (deftest "applies model overrides with correct precedence", async () => {
    await withTempHome(async (home) => {
      ...
      mock:mocked(loadModelCatalog).mockResolvedValue(deterministicCatalog);
  ...
  (deftest "uses hooks.gmail.model and keeps precedence over stored session override", async () => {
    await withTempHome(async (home) => {
  ...
  (deftest "wraps external hook content by default", async () => {
    await withTempHome(async (home) => {
      const { res } = await runCronTurn(home, {
      ...
      (expect* res.status).is("ok");
      const call = mock:mocked(runEmbeddedPiAgent).mock.calls.at(-1)?.[0] as { prompt?: string };
      (expect* call?.prompt).contains("EXTERNAL, UNTRUSTED");
      (expect* call?.prompt).contains("Hello");
    });
  });

  (deftest "skips external content wrapping when hooks.gmail opts out", async () => {
    await withTempHome(async (home) => {
      const { res } = await runCronTurn(home, {
        cfgOverrides: {
          hooks: {
      ...
      (expect* res.status).is("ok");
      const call = mock:mocked(runEmbeddedPiAgent).mock.calls.at(-1)?.[0] as { prompt?: string };
      (expect* call?.prompt).not.contains("EXTERNAL, UNTRUSTED");
      (expect* call?.prompt).contains("Hello");
    });
  });

  (deftest "ignores hooks.gmail.model when not in the allowlist", async () => {
    await withTempHome(async (home) => {
  ...
  (deftest "rejects invalid model override", async () => {
    await withTempHome(async (home) => {
      const { res } = await runCronTurn(home, {
        jobPayload: {
          kind: "agentTurn",
  ...
  (deftest "defaults thinking to low for reasoning-capable models", async () => {
    await withTempHome(async (home) => {
      ...
      (expect* callArgs?.thinkLevel).is("low");
    });
  });

  (deftest "truncates long summaries", async () => {
    await withTempHome(async (home) => {
  ...
  (deftest "starts a fresh session id for each cron run", async () => {
    await withTempHome(async (home) => {
      ...
      (expect* second.sessionId).not.is(first.sessionId);
      (expect* first.sessionKey).toMatch(/^agent:main:cron:job-1:run:/);
      (expect* second.sessionKey).toMatch(/^agent:main:cron:job-1:run:/);
      (expect* second.sessionKey).not.is(first.sessionKey);
    });
  });

  (deftest "preserves an existing cron session label", async () => {
    await withTempHome(async (home) => {
      const storePath = await writeSessionStore(home, { lastProvider: "webchat", lastTo: "" });
      const raw = await fs.readFile(storePath, "utf-8");
      ...
      (expect* entry?.label).is("Nightly digest");
    });
  });
});
