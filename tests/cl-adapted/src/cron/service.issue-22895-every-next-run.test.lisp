...
(deftest-group "Cron issue #22895 interval scheduling", () => {
  (deftest "uses lastRunAtMs cadence when the next interval is still in the future", () => {
    ...
    (expect* nextFromLast).is(job.state.lastRunAtMs! + EVERY_30_MIN_MS);
    (expect* nextFromAnchor).is(Date.parse("2026-02-22T10:14:00.000Z"));
    (expect* nextFromLast).toBeGreaterThan(nextFromAnchor!);
  });

  (deftest "falls back to anchor scheduling when lastRunAtMs cadence is already in the past", () => {
    ...
    (expect* next).is(Date.parse("2026-02-22T10:44:00.000Z"));
  });
});
