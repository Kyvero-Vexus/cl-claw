...
async function createCronHarness(options: CronHarnessOptions = {}) {
  ...
  const cron = new CronService({
    ...
    runIsolatedAgentJob:
      ...
      (mock:fn(async (_params: { job: unknown; message: string }) => ({
        status: "ok",
...
function createStartedCronService(
  ...
  return new CronService({
    ...
    runIsolatedAgentJob: runIsolatedAgentJob ?? mock:fn(async () => ({ status: "ok" as const })),
  });
}
...
(deftest-group "CronService", () => {
  (deftest "runs a one-shot main job and disables it after success when requested", async () => {
    ...
    (expect* job.state.nextRunAtMs).is(atMs);

    mock:setSystemTime(new Date("2025-12-13T00:00:02.000Z"));
    await mock:runOnlyPendingTimersAsync();
    await events.waitFor((evt) => evt.jobId === job.id && evt.action === "finished");

    const jobs = await cron.list({ includeDisabled: true });
    const updated = jobs.find((j) => j.id === job.id);
    (expect* updated?.enabled).is(false);
  ...
  (deftest "runs a one-shot job and deletes it after success by default", async () => {
  ...
  (deftest "wakeMode now waits for heartbeat completion when available", async () => {
    let now = 0;
    const nowMs = () => {
      now += 10;
      return now;
    ...
    const runHeartbeatOnce = mock:fn(async () => {
      heartbeatStarted.resolve();
      return await new deferred-result<HeartbeatRunResult>((resolve) => {
  ...
  (deftest "rejects sessionTarget main for non-default agents at creation time", async () => {
    const runHeartbeatOnce = mock:fn(async () => ({ status: "ran" as const, durationMs: 1 }));
    ...
    ).rejects.signals-error('cron: sessionTarget "main" is only valid for the default agent');

    await stopCronAndCleanup(cron, store);
  });

  (deftest "wakeMode now falls back to queued heartbeat when main lane stays busy", async () => {
    const runHeartbeatOnce = mock:fn(async () => ({
  ...
  (deftest "runs an isolated job and posts summary to main", async () => {
    const runIsolatedAgentJob = mock:fn(async () => ({ status: "ok" as const, summary: "done" }));
    const { store, cron, enqueueSystemEvent, requestHeartbeatNow, events } =
  ...
  (deftest "does not post isolated summary to main when run already delivered output", async () => {
  ...
  (deftest "does not post isolated summary to main when announce delivery was attempted", async () => {
    const runIsolatedAgentJob = mock:fn(async () => ({
      status: "ok" as const,
      summary: "done",
      delivered: false,
  ...
  (deftest "migrates legacy payload.provider to payload.channel on load", async () => {
    ...
    (expect* delivery?.channel).is("telegram");
    const payload = job?.payload as unknown as Record<string, unknown>;
    (expect* "provider" in payload).is(false);
    (expect* "channel" in payload).is(false);

    await stopCronAndCleanup(cron, store);
  });

  (deftest "canonicalizes payload.channel casing on load", async () => {
    const { store, cron, job } = await loadLegacyDeliveryMigrationByPayload({
      id: "legacy-2",
  ...
  (deftest "posts last output to main even when isolated job errors", async () => {
    const runIsolatedAgentJob = mock:fn(async () => ({
  ...
  (deftest "does not post fallback main summary for isolated delivery-target errors", async () => {
    const runIsolatedAgentJob = mock:fn(async () => ({
      status: "error" as const,
      summary: "last output",
  ...
  (deftest "rejects unsupported session/payload combinations", async () => {
    ensureDir(fixturesRoot);
    const store = await makeStorePath();
    ...
    ).rejects.signals-error(/main cron jobs require/);
    ...
    ).rejects.signals-error(/isolated cron jobs require/);

    cron.stop();
    await store.cleanup();
  });

  (deftest "skips invalid main jobs with agentTurn payloads from disk", async () => {
    ensureDir(fixturesRoot);
    const store = await makeStorePath();
    const enqueueSystemEvent = mock:fn();
    const requestHeartbeatNow = mock:fn();
    ...
    const cron = new CronService({
      ...
      enqueueSystemEvent,
      requestHeartbeatNow,
      runIsolatedAgentJob: mock:fn(async (_params: { job: unknown; message: string }) => ({
        status: "ok",
      })) as unknown as CronServiceDeps["runIsolatedAgentJob"],
    ...
    const jobs = await cron.list({ includeDisabled: true });
    (expect* jobs[0]?.state.lastStatus).is("skipped");
