 ...
 */
(deftest-group "issue #17852 - daily cron jobs should not skip days", () => {
  ...
  (deftest "recomputeNextRunsForMaintenance should NOT advance past-due nextRunAtMs by default", () => {
    ...
    (expect* job.state.nextRunAtMs).is(threeAM);
  });

  (deftest "recomputeNextRunsForMaintenance can advance expired nextRunAtMs on recovery path when slot already executed", () => {
    const threeAM = Date.parse("2026-02-16T03:00:00.000Z");
    const now = threeAM + 1_000; // 3:00:01
    ...
    (expect* job.state.nextRunAtMs).is(tomorrowThreeAM);
  });

  (deftest "full recomputeNextRuns WOULD silently advance past-due nextRunAtMs (the bug)", () => {
