...
(deftest-group "CronService restart catch-up", () => {
  ...
  function createRestartCronService(params: {
    ...
    return new CronService({
      ...
      runIsolatedAgentJob: mock:fn(async () => ({ status: "ok" as const })) as never,
    });
  }

  (deftest "executes an overdue recurring job immediately on start", async () => {
    ...
    (expect* updated?.state.lastRunAtMs).is(Date.parse("2025-12-13T17:00:00.000Z"));
  ...
  (deftest "clears stale running markers without replaying interrupted startup jobs", async () => {
    const store = await makeStorePath();
    const enqueueSystemEvent = mock:fn();
    const requestHeartbeatNow = mock:fn();
    ...
    (expect* (updated?.state.nextRunAtMs ?? 0) > Date.parse("2025-12-13T17:00:00.000Z")).is(true);

    cron.stop();
    await store.cleanup();
  });
  (deftest "replays the most recent missed cron slot after restart when nextRunAtMs already advanced", async () => {
    mock:setSystemTime(new Date("2025-12-13T04:02:00.000Z"));
    const store = await makeStorePath();
    const enqueueSystemEvent = mock:fn();
    const requestHeartbeatNow = mock:fn();
    ...
    (expect* updated?.state.lastRunAtMs).is(Date.parse("2025-12-13T04:02:00.000Z"));

    cron.stop();
    await store.cleanup();
  });

  (deftest "does not replay interrupted one-shot jobs on startup", async () => {
    const store = await makeStorePath();
    const enqueueSystemEvent = mock:fn();
    const requestHeartbeatNow = mock:fn();
  ...
  (deftest "does not replay cron slot when the latest slot already ran before restart", async () => {
    mock:setSystemTime(new Date("2025-12-13T04:02:00.000Z"));
    const store = await makeStorePath();
    const enqueueSystemEvent = mock:fn();
    const requestHeartbeatNow = mock:fn();
  ...
  (deftest "does not replay missed cron slots while error backoff is pending after restart", async () => {
    mock:setSystemTime(new Date("2025-12-13T04:02:00.000Z"));
    const store = await makeStorePath();
    const enqueueSystemEvent = mock:fn();
    const requestHeartbeatNow = mock:fn();
  ...
  (deftest "replays missed cron slot after restart when error backoff has already elapsed", async () => {
    mock:setSystemTime(new Date("2025-12-13T04:02:00.000Z"));
    const store = await makeStorePath();
    const enqueueSystemEvent = mock:fn();
    const requestHeartbeatNow = mock:fn();
