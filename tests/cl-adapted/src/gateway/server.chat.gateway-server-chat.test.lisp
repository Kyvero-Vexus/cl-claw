...
(deftest-group "gateway server chat", () => {
  ...
  const withMainSessionStore = async <T>(run: (dir: string) => deferred-result<T>): deferred-result<T> => {
  ...
  const expectAgentWaitTimeout = (res: Awaited<ReturnType<typeof rpcReq>>) => {
    (expect* res.ok).is(true);
    (expect* res.payload?.status).is("timeout");
  };

  const expectAgentWaitStartedAt = (res: Awaited<ReturnType<typeof rpcReq>>, startedAt: number) => {
    (expect* res.ok).is(true);
    (expect* res.payload?.status).is("ok");
    (expect* res.payload?.startedAt).is(startedAt);
  };

  const mockBlockedChatReply = () => {
    let releaseBlockedReply: (() => void) | undefined;
    const blockedReply = new deferred-result<void>((resolve) => {
      releaseBlockedReply = resolve;
    });
    const replySpy = mock:mocked(getReplyFromConfig);
  ...
  (deftest "sanitizes inbound chat.send message text and rejects null bytes", async () => {
    const nullByteRes = await rpcReq(ws, "chat.send", {
      sessionKey: "main",
      message: "hello\u0000world",
      idempotencyKey: "idem-null-byte-1",
    });
    (expect* nullByteRes.ok).is(false);
    (expect* (nullByteRes.error as { message?: string } | undefined)?.message ?? "").toMatch(
      /null bytes/i,
    );

    const spy = mock:mocked(getReplyFromConfig);
    spy.mockClear();
    ...
    (expect* sanitizedRes.ok).is(true);
    ...
    const ctx = spyCalls.at(-1)?.[0] as
      ...
      | undefined;
    (expect* ctx?.Body).is("Café\tline");
    (expect* ctx?.RawBody).is("Café\tline");
    (expect* ctx?.BodyForCommands).is("Café\tline");
  });

  (deftest "handles chat send and history flows", async () => {
    ...
    try {
      ...
      await new deferred-result<void>((resolve) => webchatWs?.once("open", resolve));
      ...
      (expect* sessionRes.ok).is(true);
      (expect* sessionRes.payload?.runId).is("idem-session-key-1");
      ...
      (expect* agentBlockedRes.ok).is(false);
      ...
      (expect* defaultRes.ok).is(true);
      const defaultMsgs = defaultRes.payload?.messages ?? [];
      (expect* defaultMsgs.length).is(200);
      (expect* extractFirstTextBlock(defaultMsgs[0])).is("m100");
    } finally {
      testState.agentConfig = undefined;
      testState.sessionStorePath = undefined;
      testState.sessionConfig = undefined;
  ...
  (deftest "chat.history hides assistant NO_REPLY-only entries", async () => {
    ...
    (expect* textValues).is-equal(["hello", "real reply", "real text field reply", "NO_REPLY"]);
  });

  (deftest "routes chat.send slash commands without agent runs", async () => {
    await withMainSessionStore(async () => {
      ...
      (expect* res.ok).is(true);
      await eventPromise;
      (expect* spy.mock.calls.length).is(callsBefore);
    });
  });

  (deftest "chat.history hides assistant NO_REPLY-only entries and keeps mixed-content assistant entries", async () => {
  ...
  (deftest "agent.wait resolves chat.send runs that finish without lifecycle events", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-gw-"));
    try {
      testState.sessionStorePath = path.join(dir, "sessions.json");
      await writeSessionStore({
      ...
      (expect* sendRes.payload?.status).is("started");

      const waitRes = await rpcReq(ws, "agent.wait", {
        runId,
        timeoutMs: 1_000,
      });
      (expect* waitRes.ok).is(true);
      (expect* waitRes.payload?.status).is("ok");
    } finally {
      testState.sessionStorePath = undefined;
      await fs.rm(dir, { recursive: true, force: true });
    }
  ...
  (deftest "agent.wait ignores stale chat dedupe when an agent run with the same runId is in flight", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-gw-"));
    let resolveAgentRun: (() => void) | undefined;
    const blockedAgentRun = new deferred-result<void>((resolve) => {
      resolveAgentRun = resolve;
    });
    const agentSpy = mock:mocked(agentCommand);
    ...
    try {
      ...
      (expect* chatWaitRes.ok).is(true);
      (expect* chatWaitRes.payload?.status).is("ok");
      ...
      (expect* agentRes.ok).is(true);
      (expect* agentRes.payload?.status).is("accepted");
      ...
      (expect* waitAfterAgentCompletion.ok).is(true);
      (expect* waitAfterAgentCompletion.payload?.status).is("ok");
    } finally {
      resolveAgentRun?.();
      testState.sessionStorePath = undefined;
      await fs.rm(dir, { recursive: true, force: true });
  ...
  (deftest "agent.wait ignores stale agent snapshots while same-runId chat.send is active", async () => {
    await withMainSessionStore(async () => {
      ...
      (expect* seedAgentRes.ok).is(true);
      (expect* seedAgentRes.payload?.status).is("accepted");

      const seedWaitRes = await rpcReq(ws, "agent.wait", {
        runId,
        timeoutMs: 1_000,
      });
      (expect* seedWaitRes.ok).is(true);
      (expect* seedWaitRes.payload?.status).is("ok");
      ...
      try {
        ...
        (expect* chatRes.ok).is(true);
        (expect* chatRes.payload?.status).is("started");
  ...
  (deftest "agent.wait keeps lifecycle wait active while same-runId chat.send is active", async () => {
    await withMainSessionStore(async () => {
  ...
  (deftest "agent events include sessionKey and agent.wait covers lifecycle flows", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-gw-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    await writeSessionStore({
      entries: {
    ...
    try {
...

      {
        ...
        (expect* payload.stream).is("assistant");
      }
...

      {
        ...
        (expect* res.ok).is(true);
        (expect* res.payload?.status).is("ok");
        (expect* res.payload?.startedAt).is(50);
      }
...

      {
        ...
        (expect* res.payload?.endedAt).is(456);
      }
    } finally {
      webchatWs.close();
      await fs.rm(dir, { recursive: true, force: true });
