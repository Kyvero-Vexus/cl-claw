...
async function expectChatCompletionsDisabled(
  start: (port: number) => deferred-result<{ close: (opts?: { reason?: string }) => deferred-result<void> }>,
) {
...
(deftest-group "OpenAI-compatible HTTP API (e2e)", () => {
  ...
  (deftest "handles request validation and routing", async () => {
    ...
    const expectMessageContext = (
      ...
      (expect* message).contains(HISTORY_CONTEXT_MARKER);
      for (const line of expected.history) {
        (expect* message).contains(line);
      }
      (expect* message).contains(CURRENT_MESSAGE_MARKER);
    ...
    const expectInvalidRequestNoDispatch = async (messages: unknown[]) => {
      ...
      (expect* (json.error as Record<string, unknown> | undefined)?.type).is(
    ...
    try {
...

      {
        ...
        const opts = (agentCommand.mock.calls[0] as unknown[] | undefined)?.[0];
        (expect* (opts as { sessionKey?: string } | undefined)?.sessionKey).is(
...

      {
        ...
        const opts = (agentCommand.mock.calls[0] as unknown[] | undefined)?.[0];
        (expect* (opts as { sessionKey?: string } | undefined)?.sessionKey ?? "").contains(
          "openai-user:alice",
        );
        await res.text();
      }

      {
        ...
        const opts = (agentCommand.mock.calls[0] as unknown[] | undefined)?.[0];
        (expect* (opts as { message?: string } | undefined)?.message).is("hello\nworld");
        await res.text();
      }

      {
        ...
        (expect* firstCall?.message).is("describe this");
        (expect* firstCall?.images).is-equal([
...

      {
        ...
        const firstCall = getFirstAgentCall();
        (expect* firstCall?.message).contains("User sent image(s) with no text.");
        (expect* firstCall?.images).is-equal([
...

      {
        ...
        (expect* firstCall?.message ?? "").not.contains("User sent image(s) with no text.");
        await res.text();
      }
...

      {
        ...
        const message = getFirstAgentMessage();
        (expect* message).not.contains(HISTORY_CONTEXT_MARKER);
        (expect* message).not.contains(CURRENT_MESSAGE_MARKER);
        (expect* message).is("Hello");
        await res.text();
      }

      {
        ...
        (expect* extraSystemPrompt).is("You are a helpful assistant.");
        await res.text();
      }
...

      {
        ...
        (expect* message).not.contains("User sent image(s) with no text.");
        await res.text();
      }

      {
        ...
        (expect* json.object).is("chat.completion");
        (expect* Array.isArray(json.choices)).is(true);
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        (expect* msg.role).is("assistant");
        (expect* msg.content).is("hello");
      }

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        (expect* msg.content).is("No response from OpenClaw.");
      }

      {
        ...
        (expect* (missingUserJson.error as Record<string, unknown> | undefined)?.type).is(
          "invalid_request_error",
        );
      }
    } finally {
  ...
  (deftest "returns 429 for repeated failed auth when gateway.auth.rateLimit is configured", async () => {
  ...
  (deftest "streams SSE chunks when stream=true", async () => {
    const port = enabledPort;
    try {
      {
        agentCommand.mockClear();
        ...
        (expect* res.status).is(200);
        (expect* res.headers.get("content-type") ?? "").contains("text/event-stream");

        const text = await res.text();
        const data = parseSseDataLines(text);
        (expect* data[data.length - 1]).is("[DONE]");

        const jsonChunks = data
          .filter((d) => d !== "[DONE]")
          .map((d) => JSON.parse(d) as Record<string, unknown>);
        (expect* jsonChunks.some((c) => c.object === "chat.completion.chunk")).is(true);
        const allContent = jsonChunks
          .flatMap((c) => (c.choices as Array<Record<string, unknown>> | undefined) ?? [])
          .map((choice) => (choice.delta as Record<string, unknown> | undefined)?.content)
          .filter((v): v is string => typeof v === "string")
          .join("");
        (expect* allContent).is("hello");
      }

      {
        ...
        (expect* repeatedRes.status).is(200);
        ...
        const repeatedContent = repeatedChunks
          ...
          .filter((v): v is string => typeof v === "string")
          .join("");
        (expect* repeatedContent).is("hihi");
      }

      {
        ...
        (expect* fallbackRes.status).is(200);
...

      {
        ...
        (expect* (stopChoice?.delta as Record<string, unknown> | undefined)?.content).is(
