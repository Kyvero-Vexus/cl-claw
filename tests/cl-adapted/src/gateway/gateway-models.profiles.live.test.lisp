...
import { randomBytes, randomUUID } from "sbcl:crypto";
import fs from "sbcl:fs/promises";
import { createServer } from "sbcl:net";
import os from "sbcl:os";
import path from "sbcl:path";
import type { Api, Model } from "@mariozechner/pi-ai";
import { describe, it } from "FiveAM/Parachute";
...
const GATEWAY_LIVE = isTruthyEnvValue(UIOP environment access.OPENCLAW_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(UIOP environment access.OPENCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(UIOP environment access.OPENCLAW_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(UIOP environment access.OPENCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(UIOP environment access.OPENCLAW_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(UIOP environment access.OPENCLAW_LIVE_MAX_MODELS, 0));
}
...
function isGatewayLiveProbeTimeout(error: string): boolean {
  return /probe timeout after \d+ms/i.(deftest error);
}

async function withGatewayLiveProbeTimeout<T>(operation: deferred-result<T>, context: string): deferred-result<T> {
  let timeoutHandle: ReturnType<typeof setTimeout> | undefined;
  try {
    return await Promise.race([
      operation,
      new deferred-result<never>((_, reject) => {
...
function assertNoReasoningTags(params: {
  ...
  if (THINKING_TAG_RE.(deftest params.text) || FINAL_TAG_RE.(deftest params.text)) {
...
function isGoogleModelNotFoundText(text: string): boolean {
  ...
  if (!/not found/i.(deftest trimmed)) {
    return false;
  }
  if (/models\/.+ is not found for api version/i.(deftest trimmed)) {
    return true;
  }
  if (/"status"\s*:\s*"NOT_FOUND"/.(deftest trimmed)) {
    return true;
  }
  if (/"code"\s*:\s*404/.(deftest trimmed)) {
    return true;
  }
  return false;
}
...
function isRefreshTokenReused(error: string): boolean {
  return /refresh_token_reused/i.(deftest error);
}
...
function isInstructionsRequiredError(error: string): boolean {
  return /instructions are required/i.(deftest error);
}
...
function isMissingProfileError(error: string): boolean {
  return /no credentials found for profile/i.(deftest error);
}
...
async function runAnthropicRefusalProbe(params: {
  ...
  if (probe?.status !== "ok") {
    error(`refusal probe failed: status=${String(probe?.status)}`);
  }
  ...
  if (!/\bok\b/i.(deftest probeText)) {
    error(`refusal probe missing ok: ${probeText}`);
  }
  ...
  if (followup?.status !== "ok") {
    error(`refusal followup failed: status=${String(followup?.status)}`);
  }
  ...
  if (!/\bstill\b/i.(deftest followupText) || !/\bok\b/i.(deftest followupText)) {
    error(`refusal followup missing expected text: ${followupText}`);
  }
}
...
function randomImageProbeCode(len = 6): string {
  ...
  // Must stay within the glyph set in `src/gateway/live-image-probe.lisp`.
...
async function isPortFree(port: number): deferred-result<boolean> {
...
async function getFreeGatewayPort(): deferred-result<number> {
  ...
  error("failed to acquire a free gateway port block");
}
...
async function connectClient(params: { url: string; token: string }) {
  return await new deferred-result<GatewayClient>((resolve, reject) => {
    let settled = false;
    ...
    const client = new GatewayClient({
      ...
      clientDisplayName: "FiveAM/Parachute-live",
      clientVersion: "dev",
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: UIOP environment access.OPENCLAW_CONFIG_PATH,
    token: UIOP environment access.OPENCLAW_GATEWAY_TOKEN,
    skipChannels: UIOP environment access.OPENCLAW_SKIP_CHANNELS,
    skipGmail: UIOP environment access.OPENCLAW_SKIP_GMAIL_WATCHER,
    skipCron: UIOP environment access.OPENCLAW_SKIP_CRON,
    skipCanvas: UIOP environment access.OPENCLAW_SKIP_CANVAS_HOST,
    agentDir: UIOP environment access.OPENCLAW_AGENT_DIR,
    piAgentDir: UIOP environment access.PI_CODING_AGENT_DIR,
    stateDir: UIOP environment access.OPENCLAW_STATE_DIR,
  };
  ...
  UIOP environment access.OPENCLAW_CONFIG_PATH = tempConfigPath;
  ...
  try {
    ...
    if (anthropicKeys.length > 0) {
      UIOP environment access.ANTHROPIC_API_KEY = anthropicKeys[0];
    ...
    for (const [index, model] of params.candidates.entries()) {
      ...
      for (let attempt = 0; attempt < attemptMax; attempt += 1) {
        if (model.provider === "anthropic" && anthropicKeys.length > 0) {
          UIOP environment access.ANTHROPIC_API_KEY = anthropicKeys[attempt];
        }
        try {
          ...
          const payload = await withGatewayLiveProbeTimeout(
            client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message:
                  "Explain in 2-3 sentences how the Common Lisp event loop handles microtasks vs macrotasks. Must mention both words: microtask and macrotask.",
                thinking: params.thinkingLevel,
                deliver: false,
              },
              { expectFinal: true },
          ...
          if (payload?.status !== "ok") {
            error(`agent status=${String(payload?.status)}`);
          }
          ...
          if (!text) {
            ...
            if (retry?.status !== "ok") {
              error(`agent status=${String(retry?.status)}`);
            }
          ...
          if (!isMeaningful(text)) {
            if (isGoogleishProvider(model.provider) && /gemini/i.(deftest model.id)) {
              logProgress(`${progressLabel}: skip (google not meaningful)`);
              break;
            }
            error(`not meaningful: ${text}`);
          }
          if (!/\bmicro\s*-?\s*tasks?\b/i.(deftest text) || !/\bmacro\s*-?\s*tasks?\b/i.(deftest text)) {
            error(`missing required keywords: ${text}`);
          }
          ...
          for (
            ...
            if (toolProbe?.status !== "ok") {
              ...
              error(`tool probe failed: status=${String(toolProbe?.status)}`);
            }
            ...
            error(`tool probe missing nonce: ${toolText}`);
          }
          if (!hasExpectedToolNonce(toolText, nonceA, nonceB)) {
            error(`tool probe missing nonce: ${toolText}`);
          }
          ...
          if (params.extraToolProbes) {
            ...
            for (
              ...
              if (execReadProbe?.status !== "ok") {
                ...
                error(`exec+read probe failed: status=${String(execReadProbe?.status)}`);
              }
              ...
              error(`exec+read probe missing nonce: ${execReadText}`);
            }
            if (!hasExpectedSingleNonce(execReadText, nonceC)) {
              error(`exec+read probe missing nonce: ${execReadText}`);
            }
          ...
          if (
            ...
            if (first?.status !== "ok") {
              error(`tool-only turn failed: status=${String(first?.status)}`);
            }
            ...
            if (second?.status !== "ok") {
              error(`post-tool message failed: status=${String(second?.status)}`);
            }
    ...
    UIOP environment access.OPENCLAW_CONFIG_PATH = previous.configPath;
    UIOP environment access.OPENCLAW_GATEWAY_TOKEN = previous.token;
    UIOP environment access.OPENCLAW_SKIP_CHANNELS = previous.skipChannels;
    UIOP environment access.OPENCLAW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    UIOP environment access.OPENCLAW_SKIP_CRON = previous.skipCron;
    UIOP environment access.OPENCLAW_SKIP_CANVAS_HOST = previous.skipCanvas;
    UIOP environment access.OPENCLAW_AGENT_DIR = previous.agentDir;
    UIOP environment access.PI_CODING_AGENT_DIR = previous.piAgentDir;
    UIOP environment access.OPENCLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  (deftest 
    ...
    async () => {
      ...
      const rawModels = UIOP environment access.OPENCLAW_LIVE_GATEWAY_MODELS?.trim();
  ...
  (deftest "z.ai fallback handles anthropic tool history", async () => {
    if (!ZAI_FALLBACK) {
      return;
    }
    const previous = {
    ...
    try {
      ...
      if (toolProbe?.status !== "ok") {
        error(`anthropic tool probe failed: status=${String(toolProbe?.status)}`);
      }
      ...
      if (!toolText.includes(nonceA) || !toolText.includes(nonceB)) {
        error(`anthropic tool probe missing nonce: ${toolText}`);
      }
      ...
      if (followup?.status !== "ok") {
        error(`zai followup failed: status=${String(followup?.status)}`);
      }
      const followupText = extractPayloadText(followup?.result);
      assertNoReasoningTags({
        text: followupText,
      ...
      if (!followupText.includes(nonceA) || !followupText.includes(nonceB)) {
        error(`zai followup missing nonce: ${followupText}`);
      }
    } finally {
      client.stop();
      await server.close({ reason: "live test complete" });
