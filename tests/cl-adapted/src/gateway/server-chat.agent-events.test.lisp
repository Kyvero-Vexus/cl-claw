...
mock:mock("../infra/heartbeat-visibility.js", () => ({
  resolveHeartbeatVisibility: mock:fn(() => ({
    showOk: false,
    showAlerts: true,
    useIndicator: true,
  })),
...
(deftest-group "agent event handler", () => {
  beforeEach(() => {
    mock:mocked(loadConfig).mockReturnValue({});
    mock:mocked(resolveHeartbeatVisibility).mockReturnValue({
      showOk: false,
      showAlerts: true,
      useIndicator: true,
    });
  ...
  function createHarness(params?: {
    ...
    const nowSpy =
      params?.now === undefined ? undefined : mock:spyOn(Date, "now").mockReturnValue(params.now);
    const broadcast = mock:fn();
    const broadcastToConnIds = mock:fn();
  ...
  function chatBroadcastCalls(broadcast: ReturnType<typeof mock:fn>) {
    return broadcast.mock.calls.filter(([event]) => event === "chat");
  }

  function sessionChatCalls(nodeSendToSession: ReturnType<typeof mock:fn>) {
  ...
  function expectSingleAgentBroadcastPayload(broadcast: ReturnType<typeof mock:fn>) {
    const broadcastAgentCalls = broadcast.mock.calls.filter(([event]) => event === "agent");
    (expect* broadcastAgentCalls).has-length(1);
  ...
  function expectSingleFinalChatPayload(broadcast: ReturnType<typeof mock:fn>) {
    const chatCalls = chatBroadcastCalls(broadcast);
    (expect* chatCalls).has-length(1);
    const payload = chatCalls[0]?.[1] as {
      state?: string;
      message?: unknown;
    };
    (expect* payload.state).is("final");
    return payload;
  }

  (deftest "emits chat delta for assistant text-only events", () => {
    ...
    (expect* payload.state).is("delta");
    (expect* payload.message?.content?.[0]?.text).is("Hello world");
    (expect* sessionChatCalls(nodeSendToSession)).has-length(1);
    nowSpy?.mockRestore();
  });

  (deftest "strips inline directives from assistant chat events", () => {
    const { broadcast, nodeSendToSession, nowSpy } = emitRun1AssistantText(
      createHarness({ now: 1_000 }),
  ...
  (deftest "does not emit chat delta for NO_REPLY streaming text", () => {
    const { broadcast, nodeSendToSession, nowSpy } = emitRun1AssistantText(
      createHarness({ now: 1_000 }),
      " NO_REPLY  ",
    );
    (expect* chatBroadcastCalls(broadcast)).has-length(0);
    (expect* sessionChatCalls(nodeSendToSession)).has-length(0);
    nowSpy?.mockRestore();
  });

  (deftest "does not include NO_REPLY text in chat final message", () => {
  ...
  (deftest "suppresses NO_REPLY lead fragments and does not leak NO in final chat message", () => {
    const { broadcast, nodeSendToSession, chatRunState, handler, nowSpy } = createHarness({
      now: 2_100,
    });
  ...
  (deftest "keeps final short replies like 'No' even when lead-fragment deltas are suppressed", () => {
    const { broadcast, nodeSendToSession, chatRunState, handler, nowSpy } = createHarness({
      now: 2_200,
    });
    ...
    (expect* payload.message?.content?.[0]?.text).is("No");
    (expect* sessionChatCalls(nodeSendToSession)).has-length(1);
    nowSpy?.mockRestore();
  });

  (deftest "flushes buffered text as delta before final when throttle suppresses the latest chunk", () => {
    let now = 10_000;
    const nowSpy = mock:spyOn(Date, "now").mockImplementation(() => now);
    ...
    (expect* firstPayload.state).is("delta");
    (expect* secondPayload.state).is("delta");
    (expect* secondPayload.message?.content?.[0]?.text).is("Hello world");
    (expect* thirdPayload.state).is("final");
    (expect* sessionChatCalls(nodeSendToSession)).has-length(3);
    nowSpy.mockRestore();
  });

  (deftest "preserves pre-tool assistant text when later segments stream as non-prefix snapshots", () => {
    let now = 10_500;
    const nowSpy = mock:spyOn(Date, "now").mockImplementation(() => now);
    const { broadcast, nodeSendToSession, chatRunState, handler } = createHarness();
    ...
    (expect* secondPayload.state).is("delta");
    (expect* secondPayload.message?.content?.[0]?.text).is("Before tool call\nAfter tool call");
    (expect* finalPayload.state).is("final");
    (expect* finalPayload.message?.content?.[0]?.text).is("Before tool call\nAfter tool call");
    (expect* sessionChatCalls(nodeSendToSession)).has-length(3);
    nowSpy.mockRestore();
  });

  (deftest "flushes merged segmented text before final when latest segment is throttled", () => {
    let now = 10_800;
    const nowSpy = mock:spyOn(Date, "now").mockImplementation(() => now);
    const { broadcast, nodeSendToSession, chatRunState, handler } = createHarness();
    ...
    (expect* flushPayload.state).is("delta");
    (expect* flushPayload.message?.content?.[0]?.text).is("Before tool call\nAfter tool call");
    (expect* finalPayload.state).is("final");
    (expect* finalPayload.message?.content?.[0]?.text).is("Before tool call\nAfter tool call");
    (expect* sessionChatCalls(nodeSendToSession)).has-length(3);
    nowSpy.mockRestore();
  ...
  (deftest "does not flush an extra delta when the latest text already broadcast", () => {
    let now = 11_000;
    const nowSpy = mock:spyOn(Date, "now").mockImplementation(() => now);
    const { broadcast, nodeSendToSession, chatRunState, handler } = createHarness();
    ...
    const chatCalls = chatBroadcastCalls(broadcast);
    (expect* chatCalls).has-length(3);
    (expect* chatCalls.map(([, payload]) => (payload as { state?: string }).state)).is-equal([
      "delta",
      "delta",
      "final",
    ]);
  ...
  (deftest "cleans up agent run sequence tracking when lifecycle completes", () => {
    ...
    (expect* agentRunSeq.get("run-cleanup")).is(1);
    ...
    (expect* agentRunSeq.has("run-cleanup")).is(false);
    (expect* agentRunSeq.has("client-cleanup")).is(false);
    nowSpy?.mockRestore();
  });

  (deftest "flushes buffered chat delta before tool start events", () => {
    let now = 12_000;
    const nowSpy = mock:spyOn(Date, "now").mockImplementation(() => now);
    const {
      broadcast,
    ...
    const chatCalls = chatBroadcastCalls(broadcast);
    (expect* chatCalls).has-length(2);
    const flushedPayload = chatCalls[1]?.[1] as {
      state?: string;
      message?: { content?: Array<{ text?: string }> };
    };
    (expect* flushedPayload.state).is("delta");
    (expect* flushedPayload.message?.content?.[0]?.text).is("Before tool expanded");
    (expect* sessionChatCalls(nodeSendToSession)).has-length(2);
  ...
  (deftest "routes tool events only to registered recipients when verbose is enabled", () => {
  ...
  (deftest "broadcasts tool events to WS recipients even when verbose is off, but skips sbcl send", () => {
    ...
    (expect* broadcastToConnIds).toHaveBeenCalledTimes(1);
    // But sbcl/channel subscribers should NOT receive when verbose is off
    const nodeToolCalls = nodeSendToSession.mock.calls.filter(([, event]) => event === "agent");
    (expect* nodeToolCalls).has-length(0);
    resetAgentRunContextForTest();
  });

  (deftest "strips tool output when verbose is on", () => {
  ...
  (deftest "keeps tool output when verbose is full", () => {
    const { broadcastToConnIds, toolEventRecipients, handler } = createHarness({
      resolveSessionKeyForRun: () => "session-1",
    });
    ...
    (expect* broadcastToConnIds).toHaveBeenCalledTimes(1);
    const payload = broadcastToConnIds.mock.calls[0]?.[1] as { data?: Record<string, unknown> };
    (expect* payload.data?.result).is-equal(result);
    resetAgentRunContextForTest();
  });

  (deftest "broadcasts fallback events to agent subscribers and sbcl session", () => {
    ...
    (expect* payload.stream).is("lifecycle");
    (expect* payload.data?.phase).is("fallback");
    (expect* payload.sessionKey).is("session-fallback");
    (expect* payload.data?.activeProvider).is("deepinfra");

    const nodeCalls = nodeSendToSession.mock.calls.filter(([, event]) => event === "agent");
    (expect* nodeCalls).has-length(1);
  });

  (deftest "remaps chat-linked lifecycle runId to client runId", () => {
    const { broadcast, nodeSendToSession, chatRunState, handler } = createHarness({
      resolveSessionKeyForRun: () => "session-fallback",
    });
    ...
    const payload = expectSingleAgentBroadcastPayload(broadcast);
    (expect* payload.runId).is("run-fallback-client");
    (expect* payload.stream).is("lifecycle");
    (expect* payload.data?.phase).is("fallback");
    ...
    (expect* nodePayload.runId).is("run-fallback-client");
  });

  (deftest "suppresses chat and sbcl session events for non-control-UI-visible runs", () => {
  ...
  (deftest "uses agent event sessionKey when run-context lookup cannot resolve", () => {
    ...
    const payload = expectSingleAgentBroadcastPayload(broadcast);
    (expect* payload.sessionKey).is("session-from-event");
  });

  (deftest "remaps chat-linked tool runId for non-full verbose payloads", () => {
    ...
    (expect* payload.runId).is("run-tool-client");
    resetAgentRunContextForTest();
  });

  (deftest "suppresses heartbeat ack-like chat output when showOk is false", () => {
    const { broadcast, nodeSendToSession, chatRunState, handler } = createHarness({
      now: 2_000,
    });
  ...
  (deftest "keeps heartbeat alert text in final chat output when remainder exceeds ackMaxChars", () => {
    mock:mocked(loadConfig).mockReturnValue({
    ...
    (expect* payload.message?.content?.[0]?.text).is(
