...
(deftest-group "gateway server hooks", () => {
  (deftest "handles auth, wake, and agent flows", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      (expect* resAgentWithId.status).is(200);
      await waitForSystemEvent();
      const routedCall = (cronIsolatedRun.mock.calls[0] as unknown[] | undefined)?.[0] as {
        job?: { agentId?: string };
      };
      (expect* routedCall?.job?.agentId).is("hooks");
      drainSystemEvents(resolveMainKey());
      ...
      (expect* resAgentUnknown.status).is(200);
      await waitForSystemEvent();
      const fallbackCall = (cronIsolatedRun.mock.calls[0] as unknown[] | undefined)?.[0] as {
        job?: { agentId?: string };
      };
      (expect* fallbackCall?.job?.agentId).is("main");
      drainSystemEvents(resolveMainKey());
      ...
      (expect* resQuery.status).is(400);

      const resBadChannel = await postHook(port, "/hooks/agent", {
        message: "Nope",
        channel: "sms",
      });
      (expect* resBadChannel.status).is(400);
      (expect* peekSystemEvents(resolveMainKey()).length).is(0);
      ...
      (expect* headerEvents.some((e) => e.includes("Header auth"))).is(true);
      drainSystemEvents(resolveMainKey());
      ...
      (expect* resGet.status).is(405);

      const resBlankText = await postHook(port, "/hooks/wake", { text: " " });
      (expect* resBlankText.status).is(400);

      const resBlankMessage = await postHook(port, "/hooks/agent", { message: " " });
      (expect* resBlankMessage.status).is(400);

      const resBadJson = await postHook(port, "/hooks/wake", "{");
      (expect* resBadJson.status).is(400);
    });
  });

  (deftest "rejects request sessionKey unless hooks.allowRequestSessionKey is enabled", async () => {
    testState.hooksConfig = { enabled: true, token: HOOK_TOKEN };
    await withGatewayServer(async ({ port }) => {
      ...
      (expect* denied.status).is(400);
      const deniedBody = (await denied.json()) as { error?: string };
      (expect* deniedBody.error).contains("hooks.allowRequestSessionKey");
    });
  });

  (deftest "respects hooks session policy for request + mapping session keys", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      (expect* defaultRoute.status).is(200);
      await waitForSystemEvent();
      const defaultCall = (cronIsolatedRun.mock.calls[0] as unknown[] | undefined)?.[0] as
        | { sessionKey?: string }
        | undefined;
      (expect* defaultCall?.sessionKey).is("hook:ingress");
      drainSystemEvents(resolveMainKey());
      ...
      (expect* mappedOk.status).is(200);
      await waitForSystemEvent();
      const mappedCall = (cronIsolatedRun.mock.calls[0] as unknown[] | undefined)?.[0] as
        | { sessionKey?: string }
        | undefined;
      (expect* mappedCall?.sessionKey).is("hook:mapped:42");
      drainSystemEvents(resolveMainKey());
      ...
      (expect* requestBadPrefix.status).is(400);

      const mappedBadPrefix = await postHook(port, "/hooks/mapped-bad", { subject: "hello" });
      (expect* mappedBadPrefix.status).is(400);
    });
  });

  (deftest "normalizes duplicate target-agent prefixes before isolated dispatch", async () => {
    testState.hooksConfig = {
      enabled: true,
      token: HOOK_TOKEN,
      allowRequestSessionKey: true,
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      const routedCall = (cronIsolatedRun.mock.calls[0] as unknown[] | undefined)?.[0] as
        ...
        | undefined;
      (expect* routedCall?.job?.agentId).is("hooks");
      (expect* routedCall?.sessionKey).is("slack:channel:c123");
      drainSystemEvents(resolveMainKey());
    });
  });

  (deftest "enforces hooks.allowedAgentIds for explicit agent routing", async () => {
    testState.hooksConfig = {
      enabled: true,
      token: HOOK_TOKEN,
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      (expect* resNoAgent.status).is(200);
      await waitForSystemEvent();
      ...
      (expect* resDenied.status).is(400);
      const deniedBody = (await resDenied.json()) as { error?: string };
      (expect* deniedBody.error).contains("hooks.allowedAgentIds");

      const resMappedDenied = await postHook(port, "/hooks/mapped", { subject: "hello" });
      (expect* resMappedDenied.status).is(400);
      const mappedDeniedBody = (await resMappedDenied.json()) as { error?: string };
      (expect* mappedDeniedBody.error).contains("hooks.allowedAgentIds");
      (expect* peekSystemEvents(resolveMainKey()).length).is(0);
    });
  });

  (deftest "denies explicit agentId when hooks.allowedAgentIds is empty", async () => {
    testState.hooksConfig = {
      enabled: true,
      token: HOOK_TOKEN,
      allowedAgentIds: [],
  ...
  (deftest "throttles repeated hook auth failures and resets after success", async () => {
    testState.hooksConfig = { enabled: true, token: HOOK_TOKEN };
    await withGatewayServer(async ({ port }) => {
      ...
      (expect* firstFail.status).is(401);

      let throttled: Response | null = null;
      for (let i = 0; i < 20; i++) {
        throttled = await postHook(port, "/hooks/wake", { text: "blocked" }, { token: "wrong" });
      }
      (expect* throttled?.status).is(429);
      (expect* throttled?.headers.get("retry-after")).is-truthy();

      const allowed = await postHook(port, "/hooks/wake", { text: "auth reset" });
      (expect* allowed.status).is(200);
      await waitForSystemEvent();
      drainSystemEvents(resolveMainKey());
      ...
      (expect* failAfterSuccess.status).is(401);
    });
  });

  (deftest "rejects non-POST hook requests without consuming auth failure budget", async () => {
    testState.hooksConfig = { enabled: true, token: HOOK_TOKEN };
    await withGatewayServer(async ({ port }) => {
      ...
      (expect* lastGet?.status).is(405);
      (expect* lastGet?.headers.get("allow")).is("POST");
