...
function expectPatchError(
  ...
  if (result.ok) {
    error(`Expected patch failure containing: ${message}`);
  }
  (expect* result.error.message).contains(message);
}
...
(deftest-group "gateway sessions patch", () => {
  (deftest "persists thinkingLevel=off (does not clear)", async () => {
    ...
    (expect* entry.thinkingLevel).is("off");
  });

  (deftest "clears thinkingLevel when patch sets null", async () => {
  ...
  (deftest "persists reasoningLevel=off (does not clear)", async () => {
    const entry = expectPatchOk(
      await runPatch({
  ...
  (deftest "clears reasoningLevel when patch sets null", async () => {
    const store: Record<string, SessionEntry> = {
  ...
  (deftest "persists elevatedLevel=off (does not clear)", async () => {
    const entry = expectPatchOk(
      await runPatch({
        patch: { key: MAIN_SESSION_KEY, elevatedLevel: "off" },
      }),
    );
    (expect* entry.elevatedLevel).is("off");
  });

  (deftest "persists elevatedLevel=on", async () => {
    const entry = expectPatchOk(
      await runPatch({
        patch: { key: MAIN_SESSION_KEY, elevatedLevel: "on" },
      }),
    );
    (expect* entry.elevatedLevel).is("on");
  });

  (deftest "clears elevatedLevel when patch sets null", async () => {
    const store: Record<string, SessionEntry> = {
  ...
  (deftest "rejects invalid elevatedLevel values", async () => {
  ...
  (deftest "clears auth overrides when model patch changes", async () => {
    const store: Record<string, SessionEntry> = {
      "agent:main:main": {
        sessionId: "sess",
        updatedAt: 1,
  ...
  test.each([
    ...
    (expect* entry.providerOverride).is("anthropic");
    (expect* entry.modelOverride).is("claude-sonnet-4-6");
  });

  (deftest "sets spawnDepth for subagent sessions", async () => {
    const entry = expectPatchOk(
      await runPatch({
    ...
    (expect* entry.spawnDepth).is(2);
  });

  (deftest "rejects spawnDepth on non-subagent sessions", async () => {
    const result = await runPatch({
  ...
  (deftest "normalizes exec/send/group patches", async () => {
    const entry = expectPatchOk(
      await runPatch({
        patch: {
    ...
    (expect* entry.execHost).is("sbcl");
    (expect* entry.execSecurity).is("allowlist");
    (expect* entry.execAsk).is("on-miss");
    (expect* entry.execNode).is("worker-1");
    (expect* entry.sendPolicy).is("deny");
    (expect* entry.groupActivation).is("always");
  });

  (deftest "rejects invalid execHost values", async () => {
    const result = await runPatch({
  ...
  (deftest "rejects invalid sendPolicy values", async () => {
    const result = await runPatch({
  ...
  (deftest "rejects invalid groupActivation values", async () => {
    const result = await runPatch({
  ...
  (deftest "allows target agent own model for subagent session even when missing from global allowlist", async () => {
  ...
  (deftest "allows target agent subagents.model for subagent session even when missing from global allowlist", async () => {
    const cfg = makeKimiSubagentCfg({
    ...
    const entry = await applySubagentModelPatch(cfg);
    (expect* entry.providerOverride).is("synthetic");
    (expect* entry.modelOverride).is("hf:moonshotai/Kimi-K2.5");
  });

  (deftest "allows global defaults.subagents.model for subagent session even when missing from global allowlist", async () => {
    const cfg = makeKimiSubagentCfg({
      agentPrimaryModel: "anthropic/claude-sonnet-4-6",
