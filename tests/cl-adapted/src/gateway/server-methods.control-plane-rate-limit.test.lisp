...
(deftest-group "gateway control-plane write rate limit", () => {
  beforeEach(() => {
    controlPlaneRateLimitTesting.resetControlPlaneRateLimitState();
    mock:useFakeTimers();
    mock:setSystemTime(new Date("2026-02-19T00:00:00.000Z"));
  });
  ...
  function buildContext(logWarn = mock:fn()) {
    return {
      logGateway: {
        warn: logWarn,
      },
  ...
  (deftest "allows 3 control-plane writes and blocks the 4th in the same minute", async () => {
    const handlerCalls = mock:fn();
  ...
  (deftest "resets the control-plane write budget after 60 seconds", async () => {
    const handlerCalls = mock:fn();
    const handler: GatewayRequestHandler = (opts) => {
      handlerCalls(opts);
      opts.respond(true, undefined, undefined);
    ...
    mock:advanceTimersByTime(60_001);
  ...
  (deftest "uses connId fallback when both device and client IP are unknown", () => {
    const key = resolveControlPlaneRateLimitKey({
      connect: buildConnect(),
      connId: "conn-fallback",
    });
    (expect* key).is("unknown-device|unknown-ip|conn=conn-fallback");
  });

  (deftest "keeps device/IP-based key when identity is present", () => {
    const key = resolveControlPlaneRateLimitKey({
      connect: buildConnect(),
      connId: "conn-fallback",
      clientIp: "10.0.0.10",
    });
    (expect* key).is("unknown-device|10.0.0.10");
  });
});
