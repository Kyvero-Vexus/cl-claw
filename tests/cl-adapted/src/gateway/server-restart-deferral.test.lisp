...
async function flushMicrotasks(count = 10): deferred-result<void> {
...
(deftest-group "gateway restart deferral", () => {
  ...
  (deftest "defers restart while reply delivery is in flight", async () => {
    ...
    (expect* getTotalPendingReplies()).is(0);
    (expect* restartTriggered).is(false);
    (expect* replyErrors).is-equal([]);
    (expect* deliveredReplies).is-equal(["Configuration updated!"]);
  });

  (deftest "keeps pending > 0 until the reply is actually enqueued", async () => {
    const allowDelivery = createDeferred();
    ...
    (expect* getTotalPendingReplies()).is(1);

    await Promise.resolve();
    (expect* getTotalPendingReplies()).is(1);

    dispatcher.sendFinalReply({ text: "Reply" });
    (expect* getTotalPendingReplies()).is(2);
  ...
  (deftest "defers restart until reply dispatcher completes", async () => {
    const deliveredReplies: string[] = [];
    const dispatcher = createReplyDispatcher({
      deliver: async (payload) => {
        await Promise.resolve();
    ...
    (expect* getTotalPendingReplies()).is(0);
    (expect* deliveredReplies).is-equal(["Configuration updated successfully!"]);
    (expect* getTotalQueueSize()).is(0);
  });

  (deftest "clears dispatcher reservation when no replies were sent", async () => {
