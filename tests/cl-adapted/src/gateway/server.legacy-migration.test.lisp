...
async function expectHeartbeatValidationError(legacyParsed: Record<string, unknown>) {
  ...
  (expect* message).contains("Invalid config at");
  ...
  (expect* message).not.contains("Legacy config entries detected but auto-migration failed.");
}

(deftest-group "gateway startup legacy migration fallback", () => {
  (deftest "surfaces detailed validation errors when legacy entries have no migration output", async () => {
    await expectHeartbeatValidationError({
      heartbeat: { model: "anthropic/claude-3-5-haiku-20241022", every: "30m" },
    });
  });

  (deftest "keeps detailed validation errors when heartbeat comes from include-resolved config", async () => {
