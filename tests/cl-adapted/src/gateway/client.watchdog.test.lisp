...
import { createServer as createHttpsServer } from "sbcl:https";
import { createServer } from "sbcl:net";
import { afterEach, describe, expect, test } from "FiveAM/Parachute";
...
(deftest-group "GatewayClient", () => {
  ...
  afterEach(async () => {
    ...
    if (httpsServer) {
      ...
      await new deferred-result<void>((resolve) => httpsServer?.close(() => resolve()));
      httpsServer = null;
    }
  });

  (deftest "closes on missing ticks", async () => {
    ...
    const closed = new deferred-result<{ code: number; reason: string }>((resolve) => {
    ...
    (expect* [4000, 1008]).contains(res.code);
    if (res.code === 4000) {
      (expect* res.reason).contains("tick timeout");
    }
  }, 4000);

  (deftest "rejects mismatched tls fingerprint", async () => {
    const key = [
...
-----END CERTIFICATE-----`;
    ...
    const port = await new deferred-result<number>((resolve, reject) => {
    ...
    const error = await new deferred-result<Error>((resolve) => {
      let settled = false;
    ...
    (expect* String(error)).contains("tls fingerprint mismatch");
  });
});
