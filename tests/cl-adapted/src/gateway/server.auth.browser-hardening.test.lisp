...
(deftest-group "gateway auth browser hardening", () => {
  (deftest "rejects non-local browser origins for non-control-ui clients", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      try {
        ...
        (expect* res.error?.message ?? "").contains("origin not allowed");
      } finally {
        ws.close();
      }
    });
  ...
  (deftest "rate-limits browser-origin auth failures on loopback even when loopback exemption is enabled", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      try {
        ...
        (expect* first.error?.message ?? "").not.contains("retry later");
      } finally {
        firstWs.close();
      }
      ...
      try {
        ...
        (expect* second.error?.message ?? "").contains("retry later");
      } finally {
        secondWs.close();
      }
    });
  ...
  (deftest "does not silently auto-pair non-control-ui browser clients on loopback", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      ...
      try {
        ...
        (expect* res.ok).is(false);
        (expect* res.error?.message ?? "").contains("pairing required");

        const pairing = await listDevicePairing();
        const pending = pairing.pending.find((entry) => entry.deviceId === identity.deviceId);
        (expect* pending).is-truthy();
        (expect* pending?.silent).is(false);
      } finally {
        browserWs.close();
      }
    });
  ...
  (deftest "rejects forged loopback origin for control-ui when proxy headers make client non-local", async () => {
    testState.gatewayAuth = { mode: "token", token: "secret" };
    await withGatewayServer(async ({ port }) => {
