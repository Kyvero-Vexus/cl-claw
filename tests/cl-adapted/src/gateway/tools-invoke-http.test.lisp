...
let pluginHttpHandlers: Array<(req: IncomingMessage, res: ServerResponse) => deferred-result<boolean>> = [];
...
(deftest-group "POST /tools/invoke", () => {
  (deftest "invokes a tool and returns {ok:true,result}", async () => {
    ...
    (expect* lastCreateOpenClawToolsContext?.allowMediaInvokeCommands).is(true);
  });

  (deftest "supports tools.alsoAllow in profile and implicit modes", async () => {
    cfg = {
      ...cfg,
    ...
    (expect* resProfile.status).is(200);
    const profileBody = await resProfile.json();
    (expect* profileBody.ok).is(true);
    ...
    (expect* resImplicit.status).is(200);
    const implicitBody = await resImplicit.json();
    (expect* implicitBody.ok).is(true);
  });

  (deftest "routes tools invoke before plugin HTTP handlers", async () => {
    const pluginHandler = mock:fn(async (_req: IncomingMessage, res: ServerResponse) => {
      res.statusCode = 418;
      res.end("plugin");
      return true;
    });
  ...
  (deftest "returns 404 when denylisted or blocked by tools.profile", async () => {
    cfg = {
      ...cfg,
      agents: {
        list: [
    ...
    (expect* profileRes.status).is(404);
  });

  (deftest "denies sessions_spawn via HTTP even when agent policy allows", async () => {
    cfg = {
      ...cfg,
      agents: {
        list: [
    ...
    const body = await res.json();
    (expect* body.ok).is(false);
    (expect* body.error.type).is("not_found");
  });

  (deftest "propagates message target/thread headers into tools context for sessions_spawn", async () => {
    cfg = {
      ...cfg,
      agents: {
    ...
    (expect* body.result?.route).is-equal({
      agentTo: "channel:24514",
      agentThreadId: "thread-24514",
    });
  });

  (deftest "denies sessions_send via HTTP gateway", async () => {
  ...
  (deftest "denies gateway tool via HTTP", async () => {
  ...
  (deftest "allows gateway tool via HTTP when explicitly enabled in gateway.tools.allow", async () => {
    ...
    const body = await res.json();
    (expect* body.ok).is(false);
    (expect* body.error?.type).is("tool_error");
  });

  (deftest "treats gateway.tools.deny as higher priority than gateway.tools.allow", async () => {
    setMainAllowedTools({
      allow: ["gateway"],
  ...
  (deftest "uses the configured main session key when sessionKey is missing or main", async () => {
    cfg = {
      ...cfg,
      agents: {
        list: [
    ...
    (expect* resDefault.status).is(200);

    const resMain = await invokeAgentsListAuthed({ sessionKey: "main" });
    (expect* resMain.status).is(200);
  });

  (deftest "maps tool input/auth errors to 400/403 and unexpected execution errors to 500", async () => {
    cfg = {
      ...cfg,
      agents: {
    ...
    (expect* inputRes.status).is(400);
    const inputBody = await inputRes.json();
    (expect* inputBody.ok).is(false);
    (expect* inputBody.error?.type).is("tool_error");
    (expect* inputBody.error?.message).is("mode invalid");
    ...
    (expect* authRes.status).is(403);
    const authBody = await authRes.json();
    (expect* authBody.ok).is(false);
    (expect* authBody.error?.type).is("tool_error");
    (expect* authBody.error?.message).is("mode forbidden");
    ...
    (expect* crashRes.status).is(500);
    const crashBody = await crashRes.json();
    (expect* crashBody.ok).is(false);
    (expect* crashBody.error?.type).is("tool_error");
    (expect* crashBody.error?.message).is("tool execution failed");
  });

  (deftest "passes deprecated format alias through invoke payloads even when schema omits it", async () => {
    ...
    const body = await expectOkInvokeResponse(res);
    (expect* body.result?.observedFormat).is("pdf");
