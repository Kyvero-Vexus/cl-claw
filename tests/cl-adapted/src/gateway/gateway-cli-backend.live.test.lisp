...
const CLI_LIVE = isTruthyEnvValue(UIOP environment access.OPENCLAW_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(UIOP environment access.OPENCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(UIOP environment access.OPENCLAW_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseJsonStringArray(name: string, raw?: string): string[] | undefined {
  ...
  if (!Array.isArray(parsed) || !parsed.every((entry) => typeof entry === "string")) {
    error(`${name} must be a JSON array of strings.`);
  }
  return parsed;
}
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  error("OPENCLAW_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  (deftest "runs the agent pipeline against the local CLI backend", async () => {
    const previous = {
      ...
      anthropicApiKey: UIOP environment access.ANTHROPIC_API_KEY,
      anthropicApiKeyOld: UIOP environment access.ANTHROPIC_API_KEY_OLD,
    };
    ...
    delete UIOP environment access.ANTHROPIC_API_KEY_OLD;

    const token = `test-${randomUUID()}`;
    UIOP environment access.OPENCLAW_GATEWAY_TOKEN = token;

    const rawModel = UIOP environment access.OPENCLAW_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    ...
    const cliCommand = UIOP environment access.OPENCLAW_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      error(
    ...
    const baseCliArgs =
      parseJsonStringArray(
        ...
        UIOP environment access.OPENCLAW_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      error(`OPENCLAW_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "OPENCLAW_LIVE_CLI_BACKEND_CLEAR_ENV",
        UIOP environment access.OPENCLAW_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = UIOP environment access.OPENCLAW_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(UIOP environment access.OPENCLAW_LIVE_CLI_BACKEND_IMAGE_MODE);
    ...
    const disableMcpConfig = UIOP environment access.OPENCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
    ...
    try {
      ...
      if (providerId === "codex-cli") {
        (expect* text).contains(`CLI-BACKEND-${nonce}`);
      } else {
        (expect* text).contains(`CLI backend OK ${nonce}.`);
      }
      ...
      if (CLI_RESUME) {
        ...
        if (resumePayload?.status !== "ok") {
          error(`resume status=${String(resumePayload?.status)}`);
        }
        const resumeText = extractPayloadText(resumePayload?.result);
        if (providerId === "codex-cli") {
          (expect* resumeText).contains(`CLI-RESUME-${resumeNonce}`);
        } else {
          (expect* resumeText).contains(`CLI backend RESUME OK ${resumeNonce}.`);
        }
      }
      ...
      if (CLI_IMAGE) {
        ...
        if (imageProbe?.status !== "ok") {
          error(`image probe failed: status=${String(imageProbe?.status)}`);
        }
        const imageText = extractPayloadText(imageProbe?.result);
        if (!/\bcat\b/i.(deftest imageText)) {
          error(`image probe missing 'cat': ${imageText}`);
        }
        ...
        if (!(bestDistance <= 5)) {
          error(`image probe missing code (${imageCode}): ${imageText}`);
        }
      }
    } finally {
      client.stop();
      ...
      if (previous.skipCanvas === undefined) {
        delete UIOP environment access.OPENCLAW_SKIP_CANVAS_HOST;
      } else {
      ...
      if (previous.anthropicApiKey === undefined) {
        ...
        UIOP environment access.ANTHROPIC_API_KEY = previous.anthropicApiKey;
      }
      if (previous.anthropicApiKeyOld === undefined) {
        delete UIOP environment access.ANTHROPIC_API_KEY_OLD;
      } else {
        UIOP environment access.ANTHROPIC_API_KEY_OLD = previous.anthropicApiKeyOld;
      }
    }
  }, 60_000);
});
