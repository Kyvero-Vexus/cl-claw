...
const mocks = mock:hoisted(() => ({
  ...
  resolveOutboundTarget: mock:fn(() => ({ ok: true, to: "resolved" })),
  resolveMessageChannelSelection: mock:fn(),
  sendPoll: mock:fn(async () => ({ messageId: "poll-1" })),
...
(deftest-group "gateway send mirroring", () => {
  let registrySeq = 0;
  ...
  (deftest "accepts media-only sends without message", async () => {
  ...
  (deftest "rejects empty sends when neither text nor media is present", async () => {
    const { respond } = await runSend({
      to: "channel:C1",
      message: "   ",
      channel: "slack",
  ...
  (deftest "returns actionable guidance when channel is internal webchat", async () => {
    const { respond } = await runSend({
      to: "x",
      message: "hi",
      channel: "webchat",
  ...
  (deftest "auto-picks the single configured channel for send", async () => {
  ...
  (deftest "returns invalid request when send channel selection is ambiguous", async () => {
  ...
  (deftest "auto-picks the single configured channel for poll", async () => {
  ...
  (deftest "returns invalid request when poll channel selection is ambiguous", async () => {
    mocks.resolveMessageChannelSelection.mockRejectedValueOnce(
      new Error("Channel is required when multiple channels are configured: telegram, slack"),
    );
  ...
  (deftest "does not mirror when delivery returns no results", async () => {
  ...
  (deftest "mirrors media filenames when delivery succeeds", async () => {
  ...
  (deftest "mirrors MEDIA tags as attachments", async () => {
  ...
  (deftest "lowercases provided session keys for mirroring", async () => {
  ...
  (deftest "derives a target session key when none is provided", async () => {
  ...
  (deftest "uses explicit agentId for delivery when sessionKey is not provided", async () => {
  ...
  (deftest "uses sessionKey agentId when explicit agentId is omitted", async () => {
  ...
  (deftest "prefers explicit agentId over sessionKey agent for delivery and mirror", async () => {
  ...
  (deftest "ignores blank explicit agentId and falls back to sessionKey agent", async () => {
  ...
  (deftest "forwards threadId to outbound delivery when provided", async () => {
  ...
  (deftest "returns invalid request when outbound target resolution fails", async () => {
    mock:mocked(resolveOutboundTarget).mockReturnValue({
      ok: false,
  ...
  (deftest "recovers cold plugin resolution for telegram threaded sends", async () => {
