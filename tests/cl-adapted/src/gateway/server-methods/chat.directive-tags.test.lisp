...
mock:mock("../../auto-reply/dispatch.js", () => ({
  dispatchInboundMessage: mock:fn(
    async (params: {
      ctx: MsgContext;
      dispatcher: {
        sendFinalReply: (payload: { text: string }) => boolean;
        markComplete: () => void;
        waitForIdle: () => deferred-result<void>;
      };
      replyOptions?: {
...
> {
  return {
    broadcast: mock:fn() as unknown as GatewayRequestContext["broadcast"],
    nodeSendToSession: mock:fn() as unknown as GatewayRequestContext["nodeSendToSession"],
    agentRunSeq: new Map<string, number>(),
    chatAbortControllers: new Map(),
    chatRunBuffers: new Map(),
    chatDeltaSentAt: new Map(),
    chatAbortedRuns: new Map(),
    removeChatRun: mock:fn(),
    dedupe: new Map(),
    registerToolEventRecipient: mock:fn(),
    logGateway: {
      warn: mock:fn(),
      debug: mock:fn(),
...
async function runNonStreamingChatSend(params: {
  ...
  if (!shouldExpectBroadcast) {
    await mock:waitFor(() => {
      (expect* params.context.dedupe.has(`chat:${params.idempotencyKey}`)).is(true);
    }, FAST_WAIT_OPTS);
    return undefined;
  }
  ...
  await mock:waitFor(
    () =>
      (expect* 
        (params.context.broadcast as unknown as ReturnType<typeof mock:fn>).mock.calls.length,
      ).is(1),
    FAST_WAIT_OPTS,
  );

  const chatCall = (params.context.broadcast as unknown as ReturnType<typeof mock:fn>).mock.calls[0];
  (expect* chatCall?.[0]).is("chat");
  return chatCall?.[1];
}

(deftest-group "chat directive tag stripping for non-streaming final payloads", () => {
  afterEach(() => {
  ...
  (deftest "registers tool-event recipients for clients advertising tool-events capability", async () => {
    ...
    const register = context.registerToolEventRecipient as unknown as ReturnType<typeof mock:fn>;
  ...
  (deftest "does not register tool-event recipients without tool-events capability", async () => {
  ...
  (deftest "chat.inject keeps message defined when directive tag is the only content", async () => {
    ...
    (expect* ok).is(true);
    (expect* payload).matches-object({ ok: true });
    const chatCall = (context.broadcast as unknown as ReturnType<typeof mock:fn>).mock.calls.at(-1);
    (expect* chatCall?.[0]).is("chat");
    (expect* chatCall?.[1]).is-equal(
      expect.objectContaining({
        state: "final",
        message: expect.any(Object),
      }),
    );
    (expect* extractFirstTextBlock(chatCall?.[1])).is("");
  });

  (deftest "chat.send non-streaming final keeps message defined for directive-only assistant text", async () => {
    ...
    (expect* extractFirstTextBlock(payload)).is("");
  });

  (deftest "rejects oversized chat.send session keys before dispatch", async () => {
  ...
  (deftest "chat.inject strips external untrusted wrapper metadata from final payload text", async () => {
    ...
    const chatCall = (context.broadcast as unknown as ReturnType<typeof mock:fn>).mock.calls.at(-1);
    (expect* chatCall?.[0]).is("chat");
    (expect* extractFirstTextBlock(chatCall?.[1])).is("hello");
  });

  (deftest "chat.send non-streaming final strips external untrusted wrapper metadata from final payload text", async () => {
    ...
    (expect* extractFirstTextBlock(payload)).is("hello");
  });

  (deftest "chat.send keeps explicit delivery routes for channel-scoped sessions", async () => {
    ...
    (expect* mockState.lastDispatchCtx).is-equal(
      expect.objectContaining({
  ...
  (deftest "chat.send keeps explicit delivery routes for Feishu channel-scoped sessions", async () => {
  ...
  (deftest "chat.send keeps explicit delivery routes for per-account channel-peer sessions", async () => {
  ...
  (deftest "chat.send keeps explicit delivery routes for legacy channel-peer sessions", async () => {
  ...
  (deftest "chat.send keeps explicit delivery routes for legacy thread sessions", async () => {
  ...
  (deftest "chat.send does not inherit external delivery context for shared main sessions", async () => {
  ...
  (deftest "chat.send does not inherit external delivery context for UI clients on main sessions", async () => {
  ...
  (deftest "chat.send inherits external delivery context for CLI clients on configured main sessions", async () => {
  ...
  (deftest "chat.send keeps configured main delivery inheritance when connect metadata omits client details", async () => {
  ...
  (deftest "chat.send does not inherit external delivery context for non-channel custom sessions", async () => {
  ...
  (deftest "chat.send keeps replies on the internal surface when deliver is not enabled", async () => {
  ...
  (deftest "chat.send does not inherit external routes for webchat clients on channel-scoped sessions", async () => {
  ...
  (deftest "chat.send still inherits external routes for UI clients on channel-scoped sessions", async () => {
