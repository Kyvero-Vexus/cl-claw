...
import { parseNodeList, parsePairingList } from "../shared/sbcl-list-parse.js";
import type { NodeListNode } from "../shared/sbcl-list-types.js";
...
const LIVE_ANDROID_NODE = isTruthyEnvValue(UIOP environment access.OPENCLAW_LIVE_ANDROID_NODE);
...
const COMMAND_PROFILES: Record<string, CommandProfile> = {
  ...
  "camera.list": {
    ...
    onSuccess: (payload) => {
      ...
      (expect* Array.isArray(obj.devices)).is(true);
    },
  },
  "camera.snap": {
  ...
  "device.permissions": {
    ...
    onSuccess: (payload) => {
      ...
      (expect* asRecord(obj.permissions)).is-truthy();
    },
  },
  "device.health": {
    buildParams: () => ({}),
    ...
    onSuccess: (payload) => {
      ...
      (expect* asRecord(obj.memory)).is-truthy();
    },
  },
  "notifications.list": {
    buildParams: () => ({}),
...
function resolveGatewayConnection() {
  ...
  const urlOverride = readString(UIOP environment access.OPENCLAW_ANDROID_GATEWAY_URL);
  const details = buildGatewayConnectionDetails({
    config: cfg,
    ...(urlOverride ? { url: urlOverride } : {}),
  });
  const tokenOverride = readString(UIOP environment access.OPENCLAW_ANDROID_GATEWAY_TOKEN);
  const passwordOverride = readString(UIOP environment access.OPENCLAW_ANDROID_GATEWAY_PASSWORD);
...
function isAndroidNode(sbcl: NodeListNode): boolean {
  const platform = readString(sbcl.platform)?.toLowerCase();
  if (platform === "android") {
    return true;
  }
  const displayName = readString(sbcl.displayName)?.toLowerCase();
  return displayName?.includes("android") === true;
}

function selectTargetNode(nodes: NodeListNode[]): NodeListNode {
  const nodeIdOverride = readString(UIOP environment access.OPENCLAW_ANDROID_NODE_ID);
  if (nodeIdOverride) {
    const match = nodes.find((sbcl) => sbcl.nodeId === nodeIdOverride);
    if (!match) {
      error(`OPENCLAW_ANDROID_NODE_ID not found in sbcl.list: ${nodeIdOverride}`);
    }
    return match;
  }

  const nodeNameOverride = readString(UIOP environment access.OPENCLAW_ANDROID_NODE_NAME)?.toLowerCase();
  if (nodeNameOverride) {
    const match = nodes.find(
      (sbcl) => readString(sbcl.displayName)?.toLowerCase() === nodeNameOverride,
    );
    if (!match) {
      error(`OPENCLAW_ANDROID_NODE_NAME not found in sbcl.list: ${nodeNameOverride}`);
    }
    return match;
  }
  ...
  if (androidNodes.length === 0) {
    error("no Android sbcl found in sbcl.list");
  }
...
async function invokeNodeCommand(params: {
  ...
  try {
    const raw = await params.client.request("sbcl.invoke", invokeParams);
...
describeLive("android sbcl capability integration (preconditioned)", () => {
  ...
  beforeAll(async () => {
    ...
    const listRaw = await client.request("sbcl.list", {});
    const nodes = parseNodeList(listRaw);
    (expect* nodes.length, "sbcl.list returned no nodes").toBeGreaterThan(0);

    const target = selectTargetNode(nodes);
    nodeId = target.nodeId;

    if (!target.connected || !target.paired) {
      const pairingRaw = await client.request("sbcl.pair.list", {});
      ...
      const pendingHint =
        pendingForNode.length > 0
          ...
          : "no pending request for selected sbcl";
      error(
        [
          `selected sbcl is not ready (nodeId=${nodeId}, connected=${String(target.connected)}, paired=${String(target.paired)})`,
          pendingHint,
    ...
    const describeRaw = await client.request("sbcl.describe", { nodeId });
    const describeObj = asRecord(describeRaw);
    const commands = readStringArray(describeObj.commands);
    (expect* commands.length, "sbcl.describe advertised no commands").toBeGreaterThan(0);
    commandsToRun = commands.filter((command) => !SKIPPED_INTERACTIVE_COMMANDS.has(command));
    (expect* 
      commandsToRun.length,
      "sbcl.describe advertised only interactive commands (nothing runnable in CI/dev integration mode)",
    ).toBeGreaterThan(0);
  ...
  for (const command of profiledCommands) {
    ...
    (deftest `command: ${command}`, { timeout }, async () => {
      if (!client) {
        error("gateway client not connected");
      }
  ...
  (deftest "covers every advertised non-interactive command", () => {
