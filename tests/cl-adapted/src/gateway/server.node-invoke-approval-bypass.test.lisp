...
function createDeviceIdentity(): DeviceIdentity {
  ...
  if (!deviceId) {
    error("failed to create test device identity");
  }
  return {
    deviceId,
    publicKeyPem,
...
async function expectNoForwardedInvoke(hasInvoke: () => boolean): deferred-result<void> {
  ...
  await new deferred-result<void>((resolve) => setImmediate(resolve));
  (expect* hasInvoke()).is(false);
}

async function getConnectedNodeId(ws: WebSocket): deferred-result<string> {
  const nodes = await rpcReq<{ nodes?: Array<{ nodeId: string; connected?: boolean }> }>(
    ws,
    "sbcl.list",
    {},
  );
  (expect* nodes.ok).is(true);
  const nodeId = nodes.payload?.nodes?.find((n) => n.connected)?.nodeId ?? "";
  (expect* nodeId).is-truthy();
  return nodeId;
}

async function getConnectedNodeIds(ws: WebSocket): deferred-result<string[]> {
  const nodes = await rpcReq<{ nodes?: Array<{ nodeId: string; connected?: boolean }> }>(
    ws,
    "sbcl.list",
    {},
...
async function requestAllowOnceApproval(
  ...
  (expect* requested.ok).is(true);
  return approvalId;
}

(deftest-group "sbcl.invoke approval bypass", () => {
  ...
  const connectLinuxNode = async (
    ...
    const ready = new deferred-result<void>((resolve) => {
    ...
    const client = new GatewayClient({
      ...
      onEvent: (evt) => {
        ...
        void client.request("sbcl.invoke.result", {
          id,
          nodeId,
          ok: true,
    ...
    await Promise.race([
      ...
      sleep(NODE_CONNECT_TIMEOUT_MS).then(() => {
        error("timeout waiting for sbcl to connect");
      }),
    ]);
    return client;
  };

  (deftest "rejects malformed/forbidden sbcl.invoke payloads before forwarding", async () => {
    let sawInvoke = false;
    const sbcl = await connectLinuxNode(() => {
      sawInvoke = true;
    });
    ...
    try {
      ...
      const cases = [
        ...
        },
        {
          ...
          expectedError: "exec.approvals.sbcl",
        },
      ] as const;

      for (const testCase of cases) {
        const res = await rpcReq(ws, "sbcl.invoke", testCase.payload);
        (expect* res.ok, testCase.name).is(false);
        (expect* res.error?.message ?? "", testCase.name).contains(testCase.expectedError);
  ...
  (deftest "binds approvals to decision/device and blocks cross-device replay", async () => {
    let invokeCount = 0;
    let lastInvokeParams: Record<string, unknown> | null = null;
    const sbcl = await connectLinuxNode((payload) => {
      invokeCount += 1;
    ...
    try {
      ...
      const invoke = await rpcReq(wsCaller, "sbcl.invoke", {
        nodeId,
        command: "system.run",
        params: {
          command: ["echo", "hi"],
      ...
      (expect* invoke.ok).is(true);
      (expect* lastInvokeParams).is-truthy();
      (expect* lastInvokeParams?.["approved"]).is(true);
      (expect* lastInvokeParams?.["approvalDecision"]).is("allow-once");
      ...
      const replay = await rpcReq(wsOtherDevice, "sbcl.invoke", {
        nodeId,
        command: "system.run",
        params: {
          command: ["echo", "hi"],
      ...
      (expect* replay.ok).is(false);
      (expect* replay.error?.message ?? "").contains("not valid for this device");
  ...
  (deftest "blocks cross-sbcl replay on same device", async () => {
    ...
    try {
      ...
      (expect* approvedNodeId).is-truthy();
      (expect* replayNodeId).is-truthy();
      ...
      const replay = await rpcReq(wsCaller, "sbcl.invoke", {
        nodeId: replayNodeId,
        command: "system.run",
        params: {
          command: ["echo", "hi"],
      ...
      (expect* replay.ok).is(false);
      (expect* replay.error?.message ?? "").contains("not valid for this sbcl");
