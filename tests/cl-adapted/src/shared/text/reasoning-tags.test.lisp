...
(deftest-group "stripReasoningTagsFromText", () => {
  (deftest-group "basic functionality", () => {
    (deftest "returns text unchanged when no reasoning tags present", () => {
      const input = "Hello, this is a normal message.";
      (expect* stripReasoningTagsFromText(input)).is(input);
    });

    (deftest "strips reasoning-tag variants", () => {
      const cases = [
        {
      ...
      for (const { name, input, expected } of cases) {
        (expect* stripReasoningTagsFromText(input), name).is(expected);
      }
    });

    (deftest "strips multiple reasoning blocks", () => {
      const input = "<think>first</think>A<think>second</think>B";
      (expect* stripReasoningTagsFromText(input)).is("AB");
    });
  });

  (deftest-group "code block preservation (issue #3952)", () => {
    (deftest "preserves tags inside code examples", () => {
      const cases = [
    ...
    (deftest "handles mixed code-tag and real-tag content", () => {
      const cases = [
        {
      ...
      for (const { input, expected } of cases) {
        (expect* stripReasoningTagsFromText(input)).is(expected);
      }
    });
  });

  (deftest-group "edge cases", () => {
    (deftest "handles malformed tags and null-ish inputs", () => {
      const cases = [
        {
    ...
    (deftest "handles fenced and inline code edge behavior", () => {
      const cases = [
        {
          input: "Example:\n~~~\n<think>reasoning</think>\n~~~\nDone!",
          expected: "Example:\n~~~\n<think>reasoning</think>\n~~~\nDone!",
    ...
    (deftest "handles nested and final tag behavior", () => {
      const cases = [
        {
    ...
    (deftest "handles unicode, attributes, and case-insensitive tag names", () => {
      const cases = [
        {
    ...
    (deftest "handles long content and pathological backtick patterns efficiently", () => {
      const longContent = "x".repeat(10000);
      (expect* stripReasoningTagsFromText(`<think>${longContent}</think>visible`)).is("visible");
  ...
  (deftest-group "strict vs preserve mode", () => {
    (deftest "applies strict and preserve modes to unclosed tags", () => {
      ...
      for (const { mode, expected } of cases) {
        (expect* stripReasoningTagsFromText(input, { mode })).is(expected);
      }
    });
  });

  (deftest-group "trim options", () => {
    (deftest "applies configured trim strategies", () => {
      const cases = [
        {
      ...
      for (const testCase of cases) {
        (expect* stripReasoningTagsFromText(testCase.input, testCase.opts)).is(testCase.expected);
      }
    });
  });
});
