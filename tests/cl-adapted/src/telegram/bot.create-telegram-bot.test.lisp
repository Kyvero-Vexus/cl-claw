...
const ORIGINAL_TZ = UIOP environment access.TZ;
const TELEGRAM_TEST_TIMINGS = {
  mediaGroupFlushMs: 20,
  textFragmentGapMs: 30,
} as const;

(deftest-group "createTelegramBot", () => {
  beforeAll(() => {
    UIOP environment access.TZ = "UTC";
  });
  afterAll(() => {
    UIOP environment access.TZ = ORIGINAL_TZ;
  });

  // groupPolicy tests

  (deftest "installs grammY throttler", () => {
    createTelegramBot({ token: "tok" });
    (expect* throttlerSpy).toHaveBeenCalledTimes(1);
    (expect* useSpy).toHaveBeenCalledWith("throttler");
  });
  (deftest "uses wrapped fetch when global fetch is available", () => {
    const originalFetch = globalThis.fetch;
    const fetchSpy = mock:fn() as unknown as typeof fetch;
    globalThis.fetch = fetchSpy;
    try {
      createTelegramBot({ token: "tok" });
      const fetchImpl = resolveTelegramFetch();
      (expect* fetchImpl).toBeTypeOf("function");
      (expect* fetchImpl).not.is(fetchSpy);
      const clientFetch = (botCtorSpy.mock.calls[0]?.[1] as { client?: { fetch?: unknown } })
        ?.client?.fetch;
      (expect* clientFetch).toBeTypeOf("function");
      (expect* clientFetch).not.is(fetchSpy);
    } finally {
      globalThis.fetch = originalFetch;
    }
  });
  (deftest "applies global and per-account timeoutSeconds", () => {
  ...
  (deftest "sequentializes updates by chat and thread", () => {
    createTelegramBot({ token: "tok" });
    (expect* sequentializeSpy).toHaveBeenCalledTimes(1);
    (expect* middlewareUseSpy).toHaveBeenCalledWith(sequentializeSpy.mock.results[0]?.value);
    (expect* sequentializeKey).is(getTelegramSequentialKey);
  });
  (deftest "routes callback_query payloads as messages and answers callbacks", async () => {
    createTelegramBot({ token: "tok" });
    ...
    (expect* payload.Body).contains("cmd:option_a");
    (expect* answerCallbackQuerySpy).toHaveBeenCalledWith("cbq-1");
  });
  (deftest "wraps inbound message with Telegram envelope", async () => {
    await withEnvAsync({ TZ: "Europe/Vienna" }, async () => {
      createTelegramBot({ token: "tok" });
      (expect* onSpy).toHaveBeenCalledWith("message", expect.any(Function));
      const handler = getOnHandler("message") as (ctx: Record<string, unknown>) => deferred-result<void>;
      ...
      (expect* payload.Body).contains("hello world");
    });
  });
  (deftest "handles pairing DM flows for new and already-pending requests", async () => {
    const cases = [
      {
    ...
    for (const testCase of cases) {
      ...
      if (testCase.expectPairingText) {
        (expect* sendMessageSpy.mock.calls[0]?.[0], testCase.name).is(1234);
        const pairingText = String(sendMessageSpy.mock.calls[0]?.[1]);
        (expect* pairingText, testCase.name).contains("Your Telegram user id: 999");
        (expect* pairingText, testCase.name).contains("Pairing code:");
        (expect* pairingText, testCase.name).contains("PAIRME12");
        (expect* pairingText, testCase.name).contains("openclaw pairing approve telegram PAIRME12");
        (expect* pairingText, testCase.name).not.contains("<code>");
      }
    }
  });
  (deftest "blocks unauthorized DM media before download and sends pairing reply", async () => {
    loadConfig.mockReturnValue({
      channels: { telegram: { dmPolicy: "pairing" } },
    });
    readChannelAllowFromStore.mockResolvedValue([]);
    ...
    const fetchSpy = mock:spyOn(globalThis, "fetch").mockImplementation(
      async () =>
    ...
    const getFileSpy = mock:fn(async () => ({ file_path: "photos/p1.jpg" }));
    ...
    try {
      ...
      (expect* String(sendMessageSpy.mock.calls[0]?.[1])).contains("Pairing code:");
      (expect* replySpy).not.toHaveBeenCalled();
    } finally {
      fetchSpy.mockRestore();
    }
  });
  (deftest "blocks DM media downloads completely when dmPolicy is disabled", async () => {
    loadConfig.mockReturnValue({
  ...
  (deftest "blocks unauthorized DM media groups before any photo download", async () => {
    loadConfig.mockReturnValue({
      channels: { telegram: { dmPolicy: "pairing" } },
    });
    readChannelAllowFromStore.mockResolvedValue([]);
  ...
  (deftest "triggers typing cue via onReplyStart", async () => {
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message") as (ctx: Record<string, unknown>) => deferred-result<void>;
    await handler({
  ...
  (deftest "dedupes duplicate updates for callback_query, message, and channel_post", async () => {
    loadConfig.mockReturnValue({
      channels: {
        telegram: {
          dmPolicy: "open",
  ...
  (deftest "does not persist update offset past pending updates", async () => {
    // For this test we need sequentialize(...) to behave like a normal middleware and call next().
    sequentializeSpy.mockImplementationOnce(
      () => async (_ctx: unknown, next: () => deferred-result<void>) => {
        await next();
      },
    );

    const onUpdateId = mock:fn();
    loadConfig.mockReturnValue({
    ...
    const runMiddlewareChain = async (
      ctx: Record<string, unknown>,
      finalNext: () => deferred-result<void>,
    ) => {
      let idx = -1;
      const dispatch = async (i: number): deferred-result<void> => {
        if (i <= idx) {
          error("middleware dispatch called multiple times");
        }
        idx = i;
    ...
    const update101Gate = new deferred-result<void>((resolve) => {
    ...
    (expect* maxPersistedAfterDrain).is(102);
  });
  (deftest "allows distinct callback_query ids without update_id", async () => {
    loadConfig.mockReturnValue({
      channels: {
  ...
  (deftest "applies groupPolicy cases", async () => {
    for (const [index, testCase] of groupPolicyCases.entries()) {
      ...
      (expect* replySpy.mock.calls.length, testCase.name).is(testCase.expectedReplyCount);
    }
  });

  (deftest "routes DMs by telegram accountId binding", async () => {
    loadConfig.mockReturnValue({
      channels: {
        telegram: {
          accounts: {
    ...
    (expect* replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
    (expect* payload.AccountId).is("opie");
    (expect* payload.SessionKey).is("agent:opie:main");
  });

  (deftest "drops non-default account DMs without explicit bindings", async () => {
    loadConfig.mockReturnValue({
      channels: {
        telegram: {
          accounts: {
  ...
  (deftest "applies group mention overrides and fallback behavior", async () => {
    const cases: Array<{
      config: Record<string, unknown>;
      message: Record<string, unknown>;
  ...
  (deftest "routes forum topics to parent or topic-specific bindings", async () => {
    const cases: Array<{
      config: Record<string, unknown>;
    ...
    for (const testCase of cases) {
      ...
      (expect* replySpy).toHaveBeenCalledTimes(1);
      const payload = replySpy.mock.calls[0][0];
      (expect* payload.SessionKey).contains(testCase.expectedSessionKeyFragment);
    }
  });

  (deftest "sends GIF replies as animations", async () => {
  ...
  function getMessageHandler() {
    createTelegramBot({ token: "tok" });
    return getOnHandler("message") as (ctx: Record<string, unknown>) => deferred-result<void>;
  }
  ...
  (deftest "accepts mentionPatterns matches with and without unrelated mentions", async () => {
    const cases = [
      {
    ...
    for (const testCase of cases) {
      ...
      (expect* replySpy.mock.calls.length, testCase.name).is(1);
      const payload = replySpy.mock.calls[0][0];
      (expect* payload.WasMentioned, testCase.name).is(true);
      if (testCase.assertEnvelope) {
        (expect* payload.SenderName).is("Ada");
        (expect* payload.SenderId).is("9");
        const expectedTimestamp = formatEnvelopeTimestamp(new Date("2025-01-09T00:00:00Z"));
        const timestampPattern = escapeRegExp(expectedTimestamp);
        (expect* payload.Body).toMatch(
  ...
  (deftest "keeps group envelope headers stable (sender identity is separate)", async () => {
    resetHarnessSpies();
    ...
    (expect* replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
    (expect* payload.SenderName).is("Ada Lovelace");
    (expect* payload.SenderId).is("99");
    (expect* payload.SenderUsername).is("ada");
    const expectedTimestamp = formatEnvelopeTimestamp(new Date("2025-01-09T00:00:00Z"));
    const timestampPattern = escapeRegExp(expectedTimestamp);
    (expect* payload.Body).toMatch(
  ...
  (deftest "reacts to mention-gated group messages when ackReaction is enabled", async () => {
    resetHarnessSpies();
  ...
  (deftest "clears native commands when disabled", () => {
    resetHarnessSpies();
    loadConfig.mockReturnValue({
  ...
  (deftest "handles requireMention when mentions do and do not resolve", async () => {
    const cases = [
      {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      if (testCase.expectedWasMentioned != null) {
        const payload = replySpy.mock.calls[0][0];
        (expect* payload.WasMentioned, testCase.name).is(testCase.expectedWasMentioned);
      }
    }
  });
  (deftest "includes reply-to context when a Telegram reply is received", async () => {
    resetHarnessSpies();
    ...
    (expect* replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
    (expect* payload.Body).contains("[Replying to Ada id:9001]");
    (expect* payload.Body).contains("Can you summarize this?");
    (expect* payload.ReplyToId).is("9001");
    (expect* payload.ReplyToBody).is("Can you summarize this?");
    (expect* payload.ReplyToSender).is("Ada");
  });

  (deftest "blocks group messages for restrictive group config edge cases", async () => {
    const blockedCases = [
      {
    ...
    for (const testCase of blockedCases) {
      ...
      (expect* replySpy.mock.calls.length, testCase.name).is(0);
    }
  });
  ...
  (deftest "allows control commands with TG-prefixed groupAllowFrom entries", async () => {
    loadConfig.mockReturnValue({
      channels: {
        telegram: {
          groupPolicy: "allowlist",
  ...
  (deftest "handles forum topic metadata and typing thread fallbacks", async () => {
    const forumCases = [
      {
    ...
    for (const testCase of forumCases) {
      ...
      if (testCase.assertTopicMetadata) {
        (expect* payload.SessionKey).contains("telegram:group:-1001234567890:topic:99");
        (expect* payload.From).is("telegram:group:-1001234567890:topic:99");
        (expect* payload.MessageThreadId).is(99);
        (expect* payload.IsForum).is(true);
      }
  ...
  (deftest "threads forum replies only when a topic id exists", async () => {
    const threadCases = [
    ...
    for (const testCase of threadCases) {
      ...
      (expect* sendMessageSpy.mock.calls.length, testCase.name).is(1);
      ...
      if (testCase.expectedMessageThreadId == null) {
        ...
        (expect* sendParams?.message_thread_id, testCase.name).is(testCase.expectedMessageThreadId);
      }
    }
  });
  ...
  (deftest "applies allowFrom edge cases", async () => {
  ...
  (deftest "sends replies without native reply threading", async () => {
  ...
  (deftest "prefixes final replies with responsePrefix", async () => {
    ...
    (expect* sendMessageSpy).toHaveBeenCalledTimes(1);
    (expect* sendMessageSpy.mock.calls[0][1]).is("PFX final reply");
  });
  (deftest "honors threaded replies for replyToMode=first/all", async () => {
  ...
  (deftest "honors routed group activation from session store", async () => {
  ...
  (deftest "applies topic skill filters and system prompts", async () => {
    loadConfig.mockReturnValue({
      channels: {
        telegram: {
          groupPolicy: "open",
    ...
    (expect* replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
    (expect* payload.GroupSystemPrompt).is("Group prompt\n\nTopic prompt");
    const opts = replySpy.mock.calls[0][1] as { skillFilter?: unknown };
    (expect* opts?.skillFilter).is-equal([]);
  });
  (deftest "threads native command replies inside topics", async () => {
    ...
    const handler = commandSpy.mock.calls[0][1] as (ctx: Record<string, unknown>) => deferred-result<void>;
  ...
  (deftest "skips tool summaries for native slash commands", async () => {
    commandSpy.mockClear();
    ...
    const verboseHandler = commandSpy.mock.calls.find((call) => call[0] === "verbose")?.[1] as
      | ((ctx: Record<string, unknown>) => deferred-result<void>)
      | undefined;
    if (!verboseHandler) {
      error("verbose command handler missing");
    }
    ...
    (expect* sendMessageSpy).toHaveBeenCalledTimes(1);
    (expect* sendMessageSpy.mock.calls[0]?.[1]).contains("final reply");
  });
  (deftest "buffers channel_post media groups and processes them together", async () => {
    loadConfig.mockReturnValue({
      channels: {
        telegram: {
          groupPolicy: "open",
    ...
    const setTimeoutSpy = mock:spyOn(globalThis, "setTimeout");
    try {
      createTelegramBot({ token: "tok", testTimings: TELEGRAM_TEST_TIMINGS });
      ...
      (expect* payload.Body).contains("album caption");
  ...
  (deftest "coalesces channel_post near-limit text fragments into one message", async () => {
    loadConfig.mockReturnValue({
      channels: {
        telegram: {
          groupPolicy: "open",
    ...
    try {
      ...
      (expect* replySpy).not.toHaveBeenCalled();
      await mock:advanceTimersByTimeAsync(TELEGRAM_TEST_TIMINGS.textFragmentGapMs + 100);

      (expect* replySpy).toHaveBeenCalledTimes(1);
      const payload = replySpy.mock.calls[0]?.[0] as { RawBody?: string };
      (expect* payload.RawBody).contains(part1.slice(0, 32));
      (expect* payload.RawBody).contains(part2.slice(0, 32));
    } finally {
      useRealTime();
    }
  });
  (deftest "drops oversized channel_post media instead of dispatching a placeholder message", async () => {
    loadConfig.mockReturnValue({
      channels: {
        telegram: {
          groupPolicy: "open",
    ...
    const handler = getOnHandler("channel_post") as (ctx: Record<string, unknown>) => deferred-result<void>;
  ...
  (deftest "notifies users when media download fails for direct messages", async () => {
    loadConfig.mockReturnValue({
      channels: {
        telegram: { dmPolicy: "open", allowFrom: ["*"] },
      },
  ...
  (deftest "processes remaining media group photos when one photo download fails", async () => {
    onSpy.mockReset();
    replySpy.mockReset();
    ...
    const fetchSpy = mock:spyOn(globalThis, "fetch").mockImplementation(async () => {
      fetchCallIndex++;
      if (fetchCallIndex === 2) {
        error("MediaFetchError: Failed to fetch media");
      }
    ...
    try {
      ...
      (expect* replySpy).toHaveBeenCalledTimes(1);
      const payload = replySpy.mock.calls[0]?.[0] as { Body?: string; MediaPaths?: string[] };
      (expect* payload.Body).contains("partial album");
  ...
  (deftest "drops the media group when a non-recoverable media error occurs", async () => {
    onSpy.mockReset();
    replySpy.mockReset();
  ...
  (deftest "dedupes duplicate message updates by update_id", async () => {
    onSpy.mockReset();
    replySpy.mockReset();
