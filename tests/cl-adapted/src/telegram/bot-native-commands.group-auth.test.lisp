...
const matchPluginCommand = mock:hoisted(() => mock:fn(() => null));
const executePluginCommand = mock:hoisted(() => mock:fn(async () => ({ text: "ok" })));
...
(deftest-group "native command auth in groups", () => {
  ...
  (deftest "authorizes native commands in groups when sender is in groupAllowFrom", async () => {
    ...
    (expect* notAuthCalls).has-length(0);
  });

  (deftest "authorizes native commands in groups from commands.allowFrom.telegram", async () => {
    const { handlers, sendMessage } = setup({
      cfg: {
        commands: {
          allowFrom: {
  ...
  (deftest "uses commands.allowFrom.telegram as the sole auth source when configured", async () => {
    const { handlers, sendMessage } = setup({
      cfg: {
        commands: {
          allowFrom: {
  ...
  (deftest "keeps groupPolicy disabled enforced when commands.allowFrom is configured", async () => {
    const { handlers, sendMessage } = setup({
      cfg: {
        commands: {
          allowFrom: {
  ...
  (deftest "keeps group chat allowlists enforced when commands.allowFrom is configured", async () => {
    const { handlers, sendMessage } = setup({
      cfg: {
        commands: {
          allowFrom: {
  ...
  (deftest "rejects native commands in groups when sender is in neither allowlist", async () => {
    const { handlers, sendMessage } = setup({
      allowFrom: ["99999"],
      groupAllowFrom: ["99999"],
      useAccessGroups: true,
  ...
  (deftest "replies in the originating forum topic when auth is rejected", async () => {
    const { handlers, sendMessage } = setup({
      allowFrom: ["99999"],
      groupAllowFrom: ["99999"],
      useAccessGroups: true,
