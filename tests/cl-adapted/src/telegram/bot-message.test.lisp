...
const buildTelegramMessageContext = mock:hoisted(() => mock:fn());
const dispatchTelegramMessage = mock:hoisted(() => mock:fn());

mock:mock("./bot-message-context.js", () => ({
  buildTelegramMessageContext,
}));

mock:mock("./bot-message-dispatch.js", () => ({
  dispatchTelegramMessage,
}));

import { createTelegramMessageProcessor } from "./bot-message.js";

(deftest-group "telegram bot message processor", () => {
  beforeEach(() => {
  ...
  (deftest "dispatches when context is available", async () => {
  ...
  (deftest "skips dispatch when no context is produced", async () => {
  ...
  (deftest "sends user-visible fallback when dispatch throws", async () => {
  ...
  (deftest "swallows fallback delivery failures after dispatch throws", async () => {
    const sendMessage = mock:fn().mockRejectedValue(new Error("blocked by user"));
    const runtimeError = mock:fn();
    buildTelegramMessageContext.mockResolvedValue({
      chatId: 123,
      route: { sessionKey: "agent:main:main" },
