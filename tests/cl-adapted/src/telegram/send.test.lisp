...
async function expectChatNotFoundWithChatId(
  action: deferred-result<unknown>,
  expectedChatId: string,
): deferred-result<void> {
  try {
    await action;
    error("Expected action to reject with chat-not-found context");
  } catch (error) {
    if (
...
(deftest-group "sent-message-cache", () => {
  afterEach(() => {
    clearSentMessageCache();
  });

  (deftest "records and retrieves sent messages", () => {
    recordSentMessage(123, 1);
    recordSentMessage(123, 2);
    recordSentMessage(456, 10);

    (expect* wasSentByBot(123, 1)).is(true);
    (expect* wasSentByBot(123, 2)).is(true);
    (expect* wasSentByBot(456, 10)).is(true);
    (expect* wasSentByBot(123, 3)).is(false);
    (expect* wasSentByBot(789, 1)).is(false);
  });

  (deftest "handles string chat IDs", () => {
    recordSentMessage("123", 1);
    (expect* wasSentByBot("123", 1)).is(true);
    (expect* wasSentByBot(123, 1)).is(true);
  });

  (deftest "clears cache", () => {
    recordSentMessage(123, 1);
    (expect* wasSentByBot(123, 1)).is(true);

    clearSentMessageCache();
    (expect* wasSentByBot(123, 1)).is(false);
  });
});

(deftest-group "buildInlineKeyboard", () => {
  (deftest "normalizes keyboard inputs", () => {
    const cases: Array<{
      name: string;
    ...
    for (const testCase of cases) {
      ...
      (expect* buildInlineKeyboard(input), testCase.name).is-equal(testCase.expected);
    }
  });
});

(deftest-group "sendMessageTelegram", () => {
  (deftest "applies timeoutSeconds config precedence", async () => {
    const cases = [
      {
  ...
  (deftest "falls back to plain text when Telegram rejects HTML and preserves send params", async () => {
    ...
    for (const testCase of cases) {
      ...
      (expect* res.chatId, testCase.name).is(testCase.chatId);
      (expect* res.messageId, testCase.name).is(String(testCase.messageId));
    }
  });

  (deftest "keeps link_preview_options disabled for both html and plain-text fallback", async () => {
    const parseErr = new Error(
      "400: Bad Request: can't parse entities: Can't find end of the entity starting at byte offset 9",
    );
    const cases = [
      {
        ...
        text: "hi",
        sendMessage: mock:fn().mockResolvedValue({ message_id: 7, chat: { id: "123" } }),
        expectedCalls: [
    ...
    for (const testCase of cases) {
      ...
      (expect* testCase.sendMessage.mock.calls, testCase.name).is-equal(testCase.expectedCalls);
    }
  });

  (deftest "fails when Telegram text send returns no message_id", async () => {
    ...
    ).rejects.signals-error(/returned no message_id/i);
  });

  (deftest "fails when Telegram media send returns no message_id", async () => {
  ...
  (deftest "uses native fetch for BAN compatibility when api is omitted", async () => {
  ...
  (deftest "normalizes chat ids with internal prefixes", async () => {
    const sendMessage = mock:fn().mockResolvedValue({
      message_id: 1,
      chat: { id: "123" },
    });
  ...
  (deftest "resolves t.me targets to numeric chat ids via getChat", async () => {
    const sendMessage = mock:fn().mockResolvedValue({
      message_id: 1,
      chat: { id: "-100123" },
    });
    const getChat = mock:fn().mockResolvedValue({ id: -100123 });
  ...
  (deftest "fails clearly when a legacy target cannot be resolved", async () => {
    const getChat = mock:fn().mockRejectedValue(new Error("400: Bad Request: chat not found"));
    ...
    ).rejects.signals-error(/could not be resolved to a numeric chat ID/i);
  });

  (deftest "includes thread params in media messages", async () => {
  ...
  (deftest "splits long captions into media + text messages when text exceeds 1024 chars", async () => {
    const chatId = "123";
    ...
    (expect* res.messageId).is("71");
  });

  (deftest "uses caption when text is within 1024 char limit", async () => {
    const chatId = "123";
  ...
  (deftest "sends video notes when requested and regular videos otherwise", async () => {
    const chatId = "123";

    {
      const text = "ignored caption context";
      const sendVideoNote = mock:fn().mockResolvedValue({
        message_id: 101,
        chat: { id: chatId },
      });
      const sendMessage = mock:fn().mockResolvedValue({
      ...
      (expect* res.messageId).is("102");
    }

    {
      const text = "my caption";
      const sendVideo = mock:fn().mockResolvedValue({
        message_id: 201,
        chat: { id: chatId },
      });
      ...
      (expect* res.messageId).is("201");
    }
  });

  (deftest "applies reply markup and thread options to split video-note sends", async () => {
    const chatId = "123";
    const cases: Array<{
      text: string;
  ...
  (deftest "retries on transient errors with retry_after", async () => {
    mock:useFakeTimers();
    const chatId = "123";
    ...
    await (expect* promise).resolves.is-equal({ messageId: "1", chatId });
    (expect* setTimeoutSpy.mock.calls[0]?.[1]).is(500);
    setTimeoutSpy.mockRestore();
    mock:useRealTimers();
  });

  (deftest "does not retry on non-transient errors", async () => {
    const chatId = "123";
    const sendMessage = mock:fn().mockRejectedValue(new Error("400: Bad Request"));
    const api = { sendMessage } as unknown as {
      sendMessage: typeof sendMessage;
    };
    ...
    ).rejects.signals-error(/Bad Request/);
    (expect* sendMessage).toHaveBeenCalledTimes(1);
  });

  (deftest "retries when grammY network envelope message includes failed-after wording", async () => {
    const chatId = "123";
    const sendMessage = vi
      .fn()
    ...
    (expect* result).is-equal({ messageId: "7", chatId });
  });

  (deftest "sends GIF media as animation", async () => {
    const chatId = "123";
    const sendAnimation = mock:fn().mockResolvedValue({
      message_id: 9,
      chat: { id: chatId },
    });
    ...
    (expect* res.messageId).is("9");
  });

  (deftest "routes audio media to sendAudio/sendVoice based on voice compatibility", async () => {
    const cases: Array<{
      name: string;
      chatId: string;
      text: string;
    ...
    for (const testCase of cases) {
      const sendAudio = mock:fn().mockResolvedValue({
        message_id: 10,
        chat: { id: testCase.chatId },
      });
      const sendVoice = mock:fn().mockResolvedValue({
        message_id: 11,
        chat: { id: testCase.chatId },
      });
  ...
  (deftest "keeps message_thread_id for forum/private/group sends", async () => {
    const cases = [
      {
        name: "forum topic",
        chatId: "-1001234567890",
  ...
  (deftest "retries sends without message_thread_id on thread-not-found", async () => {
    const cases = [
  ...
  (deftest "does not retry on non-retriable thread/chat errors", async () => {
    const cases: Array<{
      chatId: string;
      text: string;
      error: Error;
    ...
    for (const testCase of cases) {
      const sendMessage = mock:fn().mockRejectedValueOnce(testCase.error);
      const api = { sendMessage } as unknown as {
        sendMessage: typeof sendMessage;
      };
      ...
      ).rejects.signals-error(testCase.expectedError);
  ...
  (deftest "sets disable_notification when silent is true", async () => {
    const chatId = "123";
    const sendMessage = mock:fn().mockResolvedValue({
      message_id: 1,
      chat: { id: chatId },
  ...
  (deftest "parses message_thread_id from recipient string (telegram:group:...:topic:...)", async () => {
    const chatId = "-1001234567890";
    const sendMessage = mock:fn().mockResolvedValue({
      message_id: 55,
      chat: { id: chatId },
  ...
  (deftest "retries media sends without message_thread_id when thread is missing", async () => {
    const chatId = "-100123";
    const threadErr = new Error("400: Bad Request: message thread not found");
    const sendPhoto = vi
      .fn()
    ...
    (expect* res.messageId).is("59");
  });

  (deftest "defaults outbound media uploads to 100MB", async () => {
    const chatId = "123";
    const sendPhoto = mock:fn().mockResolvedValue({
      message_id: 60,
      chat: { id: chatId },
  ...
  (deftest "uses configured telegram mediaMaxMb for outbound uploads", async () => {
    const chatId = "123";
    const sendPhoto = mock:fn().mockResolvedValue({
      message_id: 61,
      chat: { id: chatId },
...
(deftest-group "reactMessageTelegram", () => {
  it.each([
    {
    ...
    const setMessageReaction = mock:fn().mockResolvedValue(undefined);
  ...
  (deftest "resolves legacy telegram targets before reacting", async () => {
    const setMessageReaction = mock:fn().mockResolvedValue(undefined);
    const getChat = mock:fn().mockResolvedValue({ id: -100123 });
...
(deftest-group "sendStickerTelegram", () => {
  ...
  for (const testCase of positiveSendCases) {
    (deftest testCase.name, async () => {
      const chatId = "123";
      const sendSticker = mock:fn().mockResolvedValue({
      ...
      (expect* res.messageId).is(String(testCase.expectedMessageId));
      (expect* res.chatId).is(chatId);
    });
  }

  (deftest "throws error when fileId is blank", async () => {
    for (const fileId of ["", "   "]) {
      await (expect* sendStickerTelegram("123", fileId, { token: "tok" })).rejects.signals-error(
        /file_id is required/i,
      );
    }
  });

  (deftest "retries sticker sends without message_thread_id when thread is missing", async () => {
    const chatId = "-100123";
    const threadErr = new Error("400: Bad Request: message thread not found");
    ...
    (expect* res.messageId).is("109");
  });

  (deftest "fails when sticker send returns no message_id", async () => {
    const chatId = "123";
    const sendSticker = mock:fn().mockResolvedValue({
      chat: { id: chatId },
    });
...
(deftest-group "shared send behaviors", () => {
  (deftest "includes reply_to_message_id for threaded replies", async () => {
    const cases = [
      {
        name: "message send",
        run: async () => {
  ...
  (deftest "wraps chat-not-found with actionable context", async () => {
    const cases = [
      {
        name: "message send",
        run: async () => {
          ...
          const sendMessage = mock:fn().mockRejectedValue(err);
          const api = { sendMessage } as unknown as {
            sendMessage: typeof sendMessage;
          };
      ...
      },
      {
        ...
        run: async () => {
          const chatId = "123";
          const err = new Error("400: Bad Request: chat not found");
          const sendSticker = mock:fn().mockRejectedValue(err);
          const api = { sendSticker } as unknown as {
            sendSticker: typeof sendSticker;
          };
          await expectChatNotFoundWithChatId(
...
(deftest-group "editMessageTelegram", () => {
  it.each([
    {
    ...
    (expect* botCtorSpy.mock.calls[0]?.[0], testCase.name).is("tok");
    ...
    (expect* firstParams, testCase.name).is-equal(expect.objectContaining({ parse_mode: "HTML" }));
    ...
    if ("firstExpectReplyMarkup" in testCase && testCase.firstExpectReplyMarkup) {
      (expect* firstParams, testCase.name).is-equal(
    ...
    if ("secondExpectReplyMarkup" in testCase && testCase.secondExpectReplyMarkup) {
      const secondParams = (botApi.editMessageText.mock.calls[1] ?? [])[3] as Record<
        ...
        unknown
      >;
      (expect* secondParams, testCase.name).is-equal(
        expect.objectContaining({ reply_markup: testCase.secondExpectReplyMarkup }),
      );
    }
  });

  (deftest "treats 'message is not modified' as success", async () => {
    ...
    ).resolves.is-equal({ ok: true, messageId: "1", chatId: "123" });
    (expect* botApi.editMessageText).toHaveBeenCalledTimes(1);
  });

  (deftest "disables link previews when linkPreview is false", async () => {
    botApi.editMessageText.mockResolvedValue({ message_id: 1, chat: { id: "123" } });
...
(deftest-group "sendPollTelegram", () => {
  (deftest "maps durationSeconds to open_period", async () => {
    const api = {
      sendPoll: mock:fn(async () => ({ message_id: 123, chat: { id: 555 }, poll: { id: "p1" } })),
    };
    ...
    (expect* res).is-equal({ messageId: "123", chatId: "555", pollId: "p1" });
    (expect* api.sendPoll).toHaveBeenCalledTimes(1);
    const sendPollMock = api.sendPoll as ReturnType<typeof mock:fn>;
    (expect* sendPollMock.mock.calls[0]?.[0]).is("123");
    (expect* sendPollMock.mock.calls[0]?.[1]).is("Q");
    (expect* sendPollMock.mock.calls[0]?.[2]).is-equal(["A", "B"]);
    (expect* sendPollMock.mock.calls[0]?.[3]).matches-object({ open_period: 60 });
  });

  (deftest "retries without message_thread_id on thread-not-found", async () => {
    const api = {
      sendPoll: mock:fn(
    ...
    (expect* res).is-equal({ messageId: "1", chatId: "2", pollId: "p2" });
    (expect* api.sendPoll).toHaveBeenCalledTimes(2);
    (expect* api.sendPoll.mock.calls[0]?.[3]).matches-object({ message_thread_id: 99 });
    (expect* 
  ...
  (deftest "rejects durationHours for Telegram polls", async () => {
    const api = { sendPoll: mock:fn() };
    ...
    ).rejects.signals-error(/durationHours is not supported/i);

    (expect* api.sendPoll).not.toHaveBeenCalled();
  });

  (deftest "fails when poll send returns no message_id", async () => {
    const api = {
      sendPoll: mock:fn(async () => ({ chat: { id: 555 }, poll: { id: "p1" } })),
    };
...
(deftest-group "createForumTopicTelegram", () => {
  const cases = [
    {
  ...
  for (const testCase of cases) {
    (deftest testCase.name, async () => {
      const createForumTopic = mock:fn().mockResolvedValue(testCase.response);
      ...
      (expect* result).is-equal(testCase.expectedResult);
    });
  }
});
