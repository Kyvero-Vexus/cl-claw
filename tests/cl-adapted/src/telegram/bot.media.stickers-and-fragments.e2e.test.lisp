...
(deftest-group "telegram stickers", () => {
  ...
  (deftest 
    ...
    async () => {
      ...
      (expect* payload.Body).contains("<media:sticker>");
      (expect* payload.Sticker?.emoji).is("🎉");
      (expect* payload.Sticker?.setName).is("TestStickerPack");
      (expect* payload.Sticker?.fileId).is("sticker_file_id_123");
  ...
  (deftest 
    ...
    async () => {
      ...
      const fetchSpy = mock:spyOn(globalThis, "fetch").mockResolvedValueOnce({
        ok: true,
        status: 200,
        statusText: "OK",
      ...
      const payload = replySpy.mock.calls[0][0];
      (expect* payload.Sticker?.fileId).is("new_file_id");
      (expect* payload.Sticker?.cachedDescription).is("Cached description");
...
(deftest-group "telegram text fragments", () => {
  afterEach(() => {
    mock:clearAllTimers();
  });
  ...
  (deftest 
    ...
    async () => {
      ...
      try {
        ...
        (expect* replySpy).not.toHaveBeenCalled();
        await mock:advanceTimersByTimeAsync(TEXT_FRAGMENT_FLUSH_MS * 2);
        (expect* replySpy).toHaveBeenCalledTimes(1);
