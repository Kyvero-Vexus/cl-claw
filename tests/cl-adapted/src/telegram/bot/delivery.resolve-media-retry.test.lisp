...
(deftest-group "resolveMedia getFile retry", () => {
  beforeEach(() => {
    mock:useFakeTimers();
  ...
  (deftest "retries getFile on transient failure and succeeds on second attempt", async () => {
  ...
  it.each(["voice", "photo", "video"] as const)(
    ...
    async (mediaField) => {
      const getFile = mock:fn().mockRejectedValue(new Error("Network request for 'getFile' failed!"));
  ...
  (deftest "does not catch errors from fetchRemoteMedia (only getFile is retried)", async () => {
    const getFile = mock:fn().mockResolvedValue({ file_path: "voice/file_0.oga" });
    ...
    ).rejects.signals-error("download failed");

    (expect* getFile).toHaveBeenCalledTimes(1);
  });

  (deftest "does not retry 'file is too big' error (400 Bad Request) and returns null", async () => {
    // Simulate Telegram Bot API error when file exceeds 20MB limit.
    const fileTooBigError = createFileTooBigError();
    const getFile = mock:fn().mockRejectedValue(fileTooBigError);
  ...
  (deftest "does not retry 'file is too big' GrammyError instances and returns null", async () => {
  ...
  it.each(["audio", "voice"] as const)(
    ...
    async (mediaField) => {
      const getFile = mock:fn().mockRejectedValue(createFileTooBigError());
  ...
  (deftest "throws when getFile returns no file_path", async () => {
    const getFile = mock:fn().mockResolvedValue({});
    await (expect* 
      resolveMedia(makeCtx("voice", getFile), MAX_MEDIA_BYTES, BOT_TOKEN),
    ).rejects.signals-error("Telegram getFile returned no file_path");
    (expect* getFile).toHaveBeenCalledTimes(1);
  });

  (deftest "still retries transient errors even after encountering file too big in different call", async () => {
    const result = await expectTransientGetFileRetrySuccess();
    // Should retry transient errors.
    (expect* result).not.toBeNull();
  });

  (deftest "retries getFile for stickers on transient failure", async () => {
    const getFile = vi
      .fn()
      .mockRejectedValueOnce(new Error("Network request for 'getFile' failed!"))
  ...
  (deftest "returns null for sticker when getFile exhausts retries", async () => {
    const getFile = mock:fn().mockRejectedValue(new Error("Network request for 'getFile' failed!"));
...
(deftest-group "resolveMedia original filename preservation", () => {
  beforeEach(() => {
    mock:useFakeTimers();
    fetchRemoteMedia.mockClear();
    saveMediaBuffer.mockClear();
  ...
  (deftest "passes document.file_name to saveMediaBuffer instead of server-side path", async () => {
    const getFile = mock:fn().mockResolvedValue({ file_path: "documents/file_42.pdf" });
    fetchRemoteMedia.mockResolvedValueOnce({
      buffer: Buffer.from("pdf-data"),
      contentType: "application/pdf",
    ...
    (expect* result).is-equal(expect.objectContaining({ path: "/tmp/business-plan---uuid.pdf" }));
  });

  (deftest "passes audio.file_name to saveMediaBuffer", async () => {
    const getFile = mock:fn().mockResolvedValue({ file_path: "music/file_99.mp3" });
    fetchRemoteMedia.mockResolvedValueOnce({
  ...
  (deftest "passes video.file_name to saveMediaBuffer", async () => {
    const getFile = mock:fn().mockResolvedValue({ file_path: "videos/file_55.mp4" });
    fetchRemoteMedia.mockResolvedValueOnce({
  ...
  (deftest "falls back to fetched.fileName when telegram file_name is absent", async () => {
    const getFile = mock:fn().mockResolvedValue({ file_path: "documents/file_42.pdf" });
  ...
  (deftest "falls back to filePath when neither telegram nor fetched fileName is available", async () => {
    const getFile = mock:fn().mockResolvedValue({ file_path: "documents/file_42.pdf" });
