...
(deftest-group "buildTelegramMessageContext implicitMention forum service messages", () => {
  /**
  ...
  (deftest "does NOT trigger implicitMention for forum_topic_created service message", async () => {
  ...
  (deftest "does NOT trigger implicitMention for forum_topic_closed service message", async () => {
    const ctx = await buildGroupReplyCtx({
      replyToMessageText: undefined,
      replyFromIsBot: true,
  ...
  (deftest "does NOT trigger implicitMention for general_forum_topic_hidden service message", async () => {
    const ctx = await buildGroupReplyCtx({
      replyToMessageText: undefined,
      replyFromIsBot: true,
  ...
  (deftest "DOES trigger implicitMention for real bot replies (non-empty text)", async () => {
    const ctx = await buildGroupReplyCtx({
    ...
    (expect* ctx?.ctxPayload?.WasMentioned).is(true);
  });

  (deftest "DOES trigger implicitMention for bot media messages with caption", async () => {
  ...
  (deftest "DOES trigger implicitMention for bot sticker/voice (no text, no caption, no service field)", async () => {
  ...
  (deftest "does NOT trigger implicitMention when reply is from a different user", async () => {
    const ctx = await buildGroupReplyCtx({
