...
(deftest-group "createTelegramBot", () => {
  ...
  (deftest "merges custom commands with native commands", async () => {
    const config = {
      channels: {
        telegram: {
          customCommands: [
    ...
    (expect* registered.slice(0, native.length)).is-equal(native);
    (expect* registered.slice(native.length)).is-equal([
      { command: "custom_backup", description: "Git backup" },
      { command: "custom_generate", description: "Create an image" },
    ]);
  });

  (deftest "ignores custom commands that collide with native commands", async () => {
    ...
    (expect* registered.filter((command) => command.command === "status")).is-equal([nativeStatus]);
    (expect* errorSpy).toHaveBeenCalled();
  });

  (deftest "registers custom commands when native commands are disabled", async () => {
    const config = {
    ...
    (expect* registered).is-equal([
      { command: "custom_backup", description: "Git backup" },
      { command: "custom_generate", description: "Create an image" },
    ]);
    const reserved = new Set(listNativeCommandSpecs().map((command) => command.name));
    (expect* registered.some((command) => reserved.has(command.command))).is(false);
  });

  (deftest "blocks callback_query when inline buttons are allowlist-only and sender not authorized", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
  ...
  (deftest "allows callback_query in groups when group policy authorizes the sender", async () => {
    onSpy.mockClear();
  ...
  (deftest "edits commands list for pagination callbacks", async () => {
    onSpy.mockClear();
    listSkillCommandsForAgents.mockClear();
    ...
    (expect* chatId).is(1234);
    (expect* messageId).is(12);
    (expect* String(text)).contains("ℹ️ Commands");
  ...
  (deftest "falls back to default agent for pagination callbacks without agent suffix", async () => {
    onSpy.mockClear();
    listSkillCommandsForAgents.mockClear();
  ...
  (deftest "blocks pagination callbacks when allowlist rejects sender", async () => {
    onSpy.mockClear();
    editMessageTextSpy.mockClear();
  ...
  (deftest "routes compact model callbacks by inferring provider", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    ...
    (expect* payload?.Body).contains(`/model amazon-bedrock/${modelId}`);
    (expect* answerCallbackQuerySpy).toHaveBeenCalledWith("cbq-model-compact-1");
  });

  (deftest "rejects ambiguous compact model callbacks and returns provider list", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    editMessageTextSpy.mockClear();
    ...
    (expect* replySpy).not.toHaveBeenCalled();
    (expect* editMessageTextSpy).toHaveBeenCalledTimes(1);
    (expect* editMessageTextSpy.mock.calls[0]?.[2]).contains(
      'Could not resolve model "shared-model".',
    );
    (expect* answerCallbackQuerySpy).toHaveBeenCalledWith("cbq-model-compact-2");
  });

  (deftest "includes sender identity in group envelope headers", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
  ...
  (deftest "uses quote text when a Telegram partial reply is received", async () => {
    onSpy.mockClear();
    ...
    (expect* replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
    (expect* payload.Body).contains("[Quoting Ada id:9001]");
    (expect* payload.Body).contains('"summarize this"');
    (expect* payload.ReplyToId).is("9001");
    (expect* payload.ReplyToBody).is("summarize this");
    (expect* payload.ReplyToSender).is("Ada");
  });

  (deftest "includes replied image media in inbound context for text replies", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    ...
    try {
      ...
      (expect* payload.ReplyToBody).is("<media:image>");
      (expect* payload.MediaPaths).has-length(1);
      (expect* payload.MediaPath).is(payload.MediaPaths?.[0]);
  ...
  (deftest "does not fetch reply media for unauthorized DM replies", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    getFileSpy.mockClear();
    sendMessageSpy.mockClear();
  ...
  (deftest "defers reply media download until debounce flush", async () => {
  ...
  (deftest "isolates inbound debounce by DM topic thread id", async () => {
    const DEBOUNCE_MS = 4321;
    onSpy.mockClear();
    replySpy.mockClear();
    loadConfig.mockReturnValue({
    ...
    try {
      ...
      (expect* threadIds).is-equal([100, 200]);
    } finally {
      setTimeoutSpy.mockRestore();
    }
  });

  (deftest "handles quote-only replies without reply metadata", async () => {
    onSpy.mockClear();
    sendMessageSpy.mockClear();
    replySpy.mockClear();
    ...
    (expect* replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
    (expect* payload.Body).contains("[Quoting unknown sender]");
    (expect* payload.Body).contains('"summarize this"');
    (expect* payload.ReplyToId).toBeUndefined();
    (expect* payload.ReplyToBody).is("summarize this");
    (expect* payload.ReplyToSender).is("unknown sender");
  });

  (deftest "uses external_reply quote text for partial replies", async () => {
    onSpy.mockClear();
    sendMessageSpy.mockClear();
    replySpy.mockClear();
    ...
    (expect* replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
    (expect* payload.Body).contains("[Quoting Ada id:9002]");
    (expect* payload.Body).contains('"summarize this"');
    (expect* payload.ReplyToId).is("9002");
    (expect* payload.ReplyToBody).is("summarize this");
    (expect* payload.ReplyToSender).is("Ada");
  });

  (deftest "propagates forwarded origin from external_reply targets", async () => {
    onSpy.mockReset();
    ...
    (expect* replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
    (expect* payload.ReplyToForwardedFrom).is("Bob Smith (@bobsmith)");
    (expect* payload.ReplyToForwardedFromType).is("user");
    (expect* payload.ReplyToForwardedFromId).is("999");
    (expect* payload.ReplyToForwardedFromUsername).is("bobsmith");
    (expect* payload.ReplyToForwardedFromTitle).is("Bob Smith");
    (expect* payload.ReplyToForwardedDate).is(500000);
    (expect* payload.Body).contains(
      "[Forwarded from Bob Smith (@bobsmith) at 1970-01-01T00:08:20.000Z]",
    );
  });

  (deftest "accepts group replies to the bot without explicit mention when requireMention is enabled", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    loadConfig.mockReturnValue({
      channels: {
    ...
    (expect* replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
    (expect* payload.WasMentioned).is(true);
  });

  (deftest "inherits group allowlist + requireMention in topics", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    loadConfig.mockReturnValue({
      channels: {
  ...
  (deftest "prefers topic allowFrom over group allowFrom", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    loadConfig.mockReturnValue({
      channels: {
  ...
  (deftest "allows group messages for per-group groupPolicy open override (global groupPolicy allowlist)", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    loadConfig.mockReturnValue({
      channels: {
  ...
  (deftest "blocks control commands from unauthorized senders in per-group open groups", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    loadConfig.mockReturnValue({
      channels: {
  ...
  (deftest "sets command target session key for dm topic commands", async () => {
    onSpy.mockClear();
    sendMessageSpy.mockClear();
    ...
    const handler = commandSpy.mock.calls.find((call) => call[0] === "status")?.[1] as
      ...
      | undefined;
    if (!handler) {
      error("status command handler missing");
    }
    ...
    (expect* replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
    (expect* payload.CommandTargetSessionKey).is("agent:main:main:thread:12345:99");
  });

  (deftest "allows native DM commands for paired users", async () => {
    onSpy.mockClear();
    sendMessageSpy.mockClear();
    commandSpy.mockClear();
    replySpy.mockClear();
  ...
  (deftest "blocks native DM commands for unpaired users", async () => {
    onSpy.mockClear();
    sendMessageSpy.mockClear();
    commandSpy.mockClear();
    replySpy.mockClear();
  ...
  (deftest "registers message_reaction handler", () => {
    onSpy.mockClear();
    createTelegramBot({ token: "tok" });
  ...
  (deftest "enqueues system event for reaction", async () => {
    onSpy.mockClear();
  ...
  (deftest "skips reaction when reactionNotifications is off", async () => {
    onSpy.mockClear();
    enqueueSystemEventSpy.mockClear();
  ...
  (deftest "defaults reactionNotifications to own", async () => {
    onSpy.mockClear();
    enqueueSystemEventSpy.mockClear();
    wasSentByBot.mockReturnValue(true);
  ...
  (deftest "allows reaction in all mode regardless of message sender", async () => {
    onSpy.mockClear();
    enqueueSystemEventSpy.mockClear();
  ...
  (deftest "skips reaction in own mode when message is not sent by bot", async () => {
    onSpy.mockClear();
    enqueueSystemEventSpy.mockClear();
    wasSentByBot.mockReturnValue(false);
  ...
  (deftest "allows reaction in own mode when message is sent by bot", async () => {
    onSpy.mockClear();
    enqueueSystemEventSpy.mockClear();
    wasSentByBot.mockReturnValue(true);
  ...
  (deftest "skips reaction from bot users", async () => {
    onSpy.mockClear();
    enqueueSystemEventSpy.mockClear();
    wasSentByBot.mockReturnValue(true);
  ...
  (deftest "skips reaction removal (only processes added reactions)", async () => {
    onSpy.mockClear();
    enqueueSystemEventSpy.mockClear();
  ...
  (deftest "enqueues one event per added emoji reaction", async () => {
    onSpy.mockClear();
    enqueueSystemEventSpy.mockClear();
    ...
    (expect* enqueueSystemEventSpy.mock.calls.map((call) => call[0])).is-equal([
      "Telegram reaction added: 🔥 by Ada on msg 42",
      "Telegram reaction added: 🎉 by Ada on msg 42",
    ]);
  });

  (deftest "routes forum group reactions to the general topic (thread id not available on reactions)", async () => {
    onSpy.mockClear();
    enqueueSystemEventSpy.mockClear();
  ...
  (deftest "uses correct session key for forum group reactions in general topic", async () => {
    onSpy.mockClear();
    enqueueSystemEventSpy.mockClear();
  ...
  (deftest "uses correct session key for regular group reactions without topic", async () => {
    onSpy.mockClear();
    enqueueSystemEventSpy.mockClear();
    ...
    (expect* sessionKey).not.contains(":topic:");
  });
});
