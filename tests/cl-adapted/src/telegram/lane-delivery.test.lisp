...
function createHarness(params?: {
  ...
  const sendPayload = mock:fn().mockResolvedValue(true);
  const flushDraftLane = mock:fn().mockImplementation(async (lane: DraftLaneState) => {
    await lane.stream?.flush();
  });
  const stopDraftLane = mock:fn().mockImplementation(async (lane: DraftLaneState) => {
  ...
  const editPreview = mock:fn().mockResolvedValue(undefined);
  const deletePreviewMessage = mock:fn().mockResolvedValue(undefined);
  const log = mock:fn();
  const markDelivered = mock:fn();
...
(deftest-group "createLaneTextDeliverer", () => {
  (deftest "finalizes text-only replies by editing an existing preview message", async () => {
    ...
    (expect* result).is("preview-finalized");
  ...
  (deftest "primes stop-created previews with final text before editing", async () => {
  ...
  (deftest "treats stop-created preview edit failures as delivered", async () => {
    const harness = createHarness({ answerMessageIdAfterStop: 777 });
  ...
  (deftest "treats 'message is not modified' preview edit errors as delivered", async () => {
    const harness = createHarness({ answerMessageId: 999 });
  ...
  (deftest "falls back to normal delivery when editing an existing preview fails", async () => {
    const harness = createHarness({ answerMessageId: 999 });
  ...
  (deftest "falls back to normal delivery when stop-created preview has no message id", async () => {
  ...
  (deftest "keeps existing preview when final text regresses", async () => {
    const harness = createHarness({ answerMessageId: 999 });
  ...
  (deftest "falls back to normal delivery when final text exceeds preview edit limit", async () => {
  ...
  (deftest "materializes DM draft streaming final even when text is unchanged", async () => {
  ...
  (deftest "materializes DM draft streaming final when revision changes", async () => {
  ...
  (deftest "falls back to normal send when draft materialize returns no message id", async () => {
  ...
  (deftest "does not use DM draft final shortcut for media payloads", async () => {
    const answerStream = createTestDraftStream({ previewMode: "draft" });
    const harness = createHarness({
      answerStream: answerStream as DraftLaneState["stream"],
      answerHasStreamedMessage: true,
  ...
  (deftest "does not use DM draft final shortcut when inline buttons are present", async () => {
    const answerStream = createTestDraftStream({ previewMode: "draft" });
    const harness = createHarness({
      answerStream: answerStream as DraftLaneState["stream"],
      answerHasStreamedMessage: true,
  ...
  (deftest "deletes consumed boundary previews after fallback final send", async () => {
    const harness = createHarness();
