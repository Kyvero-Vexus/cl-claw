...
async function openInboxMonitor(onMessage = mock:fn()) {
...
(deftest-group "web monitor inbox", () => {
  ...
  (deftest "allows messages from senders in allowFrom list", async () => {
  ...
  (deftest "allows same-phone messages even if not in allowFrom", async () => {
  ...
  (deftest "locks down when no config is present (pairing for unknown senders)", async () => {
  ...
  (deftest "skips pairing replies for outbound DMs in same-phone mode", async () => {
  ...
  (deftest "skips pairing replies for outbound DMs when same-phone mode is disabled", async () => {
    await expectOutboundDmSkipsPairing({
      selfChatMode: false,
      messageId: "fromme-2",
      body: "hello again",
  ...
  (deftest "handles append messages by marking them read but skipping auto-reply", async () => {
    const { onMessage, listener, sock } = await openInboxMonitor();
  ...
  (deftest "normalizes participant phone numbers to JIDs in sendReaction", async () => {
    const listener = await monitorWebInbox({
      verbose: false,
      onMessage: mock:fn(),
      accountId: DEFAULT_ACCOUNT_ID,
