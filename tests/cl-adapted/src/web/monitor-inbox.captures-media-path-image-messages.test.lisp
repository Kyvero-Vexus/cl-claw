...
(deftest-group "web monitor inbox", () => {
  ...
  async function openMonitor(onMessage = mock:fn()) {
  ...
  (deftest "captures media path for image messages", async () => {
  ...
  (deftest "sets gifPlayback on outbound video payloads when requested", async () => {
    const onMessage = mock:fn();
    const listener = await openMonitor(onMessage);
    const sock = getSock();
  ...
  (deftest "resolves onClose when the socket closes", async () => {
    const listener = await openMonitor(mock:fn());
    const sock = getSock();
    ...
    await (expect* reasonPromise).resolves.is-equal(
      expect.objectContaining({ status: 500, isLoggedOut: false }),
    );
    await listener.close();
  });

  (deftest "logs inbound bodies to file", async () => {
  ...
  (deftest "includes participant when marking group messages read", async () => {
  ...
  (deftest "passes through group messages with participant metadata", async () => {
  ...
  (deftest "unwraps ephemeral messages, preserves mentions, and still delivers group pings", async () => {
    const { onMessage, listener } = await runSingleUpsertAndCapture({
      type: "notify",
      messages: [
        {
  ...
  (deftest "still forwards group messages (with sender info) even when allowFrom is restrictive", async () => {
