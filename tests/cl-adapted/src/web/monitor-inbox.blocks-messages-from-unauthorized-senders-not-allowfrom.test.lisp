...
(deftest-group "web monitor inbox", () => {
  ...
  (deftest "blocks messages from unauthorized senders not in allowFrom", async () => {
  ...
  (deftest "skips read receipts in self-chat mode", async () => {
    const config = {
      channels: {
        whatsapp: {
  ...
  (deftest "skips read receipts when disabled", async () => {
    const { onMessage, listener, sock } = await startWebInboxMonitor({
  ...
  (deftest "lets group messages through even when sender not in allowFrom", async () => {
    const { onMessage, listener, sock } = await startWebInboxMonitor({
      config: {
    ...
    (expect* payload.chatType).is("group");
    (expect* payload.senderE164).is("+999");

    await listener.close();
  });

  (deftest "blocks all group messages when groupPolicy is 'disabled'", async () => {
    const { onMessage, listener, sock } = await startWebInboxMonitor({
      config: {
  ...
  (deftest "blocks group messages from senders not in groupAllowFrom when groupPolicy is 'allowlist'", async () => {
    const { onMessage, listener, sock } = await startWebInboxMonitor({
      config: {
        channels: {
          whatsapp: {
  ...
  (deftest "allows group messages from senders in groupAllowFrom when groupPolicy is 'allowlist'", async () => {
    const { onMessage, listener, sock } = await startWebInboxMonitor({
      config: {
        channels: {
          whatsapp: {
    ...
    const payload = onMessage.mock.calls[0][0];
    (expect* payload.chatType).is("group");
    (expect* payload.senderE164).is("+15551234567");

    await listener.close();
  });

  (deftest "allows all group senders with wildcard in groupPolicy allowlist", async () => {
    const { onMessage, listener, sock } = await startWebInboxMonitor({
      config: {
        channels: {
          whatsapp: {
  ...
  (deftest "blocks group messages when groupPolicy allowlist has no groupAllowFrom", async () => {
    const { onMessage, listener, sock } = await startWebInboxMonitor({
      config: {
        channels: {
          whatsapp: {
