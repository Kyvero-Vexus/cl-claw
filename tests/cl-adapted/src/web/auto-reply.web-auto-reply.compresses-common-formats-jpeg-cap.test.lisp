...
(deftest-group "web auto-reply", () => {
  ...
  async function setupSingleInboundMessage(params: {
    ...
    const reply = params.reply ?? mock:fn().mockResolvedValue(undefined);
    const sendComposing = mock:fn(async () => undefined);
    const resolver = mock:fn().mockResolvedValue(params.resolverValue);

    let capturedOnMessage: ((msg: WebInboundMessage) => deferred-result<void>) | undefined;
  ...
  function getSingleImagePayload(sendMedia: ReturnType<typeof mock:fn>) {
  ...
  async function withMediaCap<T>(mediaMaxMb: number, run: () => deferred-result<T>): deferred-result<T> {
    setLoadConfigMock(() => ({
      channels: {
        whatsapp: {
          allowFrom: ["*"],
  ...
  function mockFetchMediaBuffer(buffer: Buffer, mime: string) {
    return mock:spyOn(globalThis, "fetch").mockResolvedValue({
      ok: true,
      body: true,
      arrayBuffer: async () =>
  ...
  async function expectCompressedImageWithinCap(params: {
    ...
    await withMediaCap(params.mediaMaxMb ?? 1, async () => {
      ...
      (expect* payload.mimetype).is("image/jpeg");
      (expect* reply).not.toHaveBeenCalled();
      fetchMock.mockRestore();
    });
  }

  (deftest "compresses common formats to jpeg under the cap", async () => {
    const formats = [
      {
        name: "png",
        mime: "image/png",
    ...
    await withMediaCap(SMALL_MEDIA_CAP_MB, async () => {
      ...
      const fetchMock = mock:spyOn(globalThis, "fetch").mockImplementation(async () => {
        const matched =
  ...
  (deftest "honors channels.whatsapp.mediaMaxMb for outbound auto-replies", async () => {
  ...
  (deftest "prefers per-account WhatsApp media caps for outbound auto-replies", async () => {
    const bigPng = await sharp({
      create: {
        width: 256,
        height: 256,
  ...
  (deftest "falls back to text when media is unsupported", async () => {
    const sendMedia = mock:fn();
    const { reply, dispatch } = await setupSingleInboundMessage({
    ...
    const fetchMock = mock:spyOn(globalThis, "fetch").mockResolvedValue({
      ok: true,
      body: true,
    ...
    (expect* payload.fileName).is("file.pdf");
    (expect* payload.caption).is("hi");
    (expect* reply).not.toHaveBeenCalled();

    fetchMock.mockRestore();
  });

  (deftest "falls back to text when media send fails", async () => {
    const sendMedia = mock:fn().mockRejectedValue(new Error("boom"));
    const { reply, dispatch } = await setupSingleInboundMessage({
      resolverValue: {
        text: "hi",
    ...
    (expect* fallback).contains("hi");
    (expect* fallback).contains("Media failed");
    fetchMock.mockRestore();
  });
  (deftest "returns a warning when remote media fetch 404s", async () => {
    const sendMedia = mock:fn();
    const { reply, dispatch } = await setupSingleInboundMessage({
      resolverValue: {
        text: "caption",
    ...
    const fallback = reply.mock.calls[0]?.[0] as string;
    (expect* fallback).contains("caption");
    (expect* fallback).contains("Media failed");
    (expect* fallback).contains("404");

    fetchMock.mockRestore();
  });
  (deftest "sends media with a caption when delivery succeeds", async () => {
