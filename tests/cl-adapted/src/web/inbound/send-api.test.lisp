...
const recordChannelActivity = mock:fn();
...
(deftest-group "createWebSendApi", () => {
  const sendMessage = mock:fn(async () => ({ key: { id: "msg-1" } }));
  const sendPresenceUpdate = mock:fn(async () => {});
  ...
  (deftest "uses sendOptions fileName for outbound documents", async () => {
  ...
  (deftest "falls back to default document filename when fileName is absent", async () => {
    const payload = Buffer.from("pdf");
  ...
  (deftest "sends plain text messages", async () => {
  ...
  (deftest "supports image media with caption", async () => {
  ...
  (deftest "supports audio as push-to-talk voice note", async () => {
  ...
  (deftest "supports video media and gifPlayback option", async () => {
  ...
  (deftest "falls back to unknown messageId if Baileys result does not expose key.id", async () => {
    sendMessage.mockResolvedValueOnce({ key: {} as { id: string } });
    const res = await api.sendMessage("+1555", "hello");
    (expect* res.messageId).is("unknown");
  });

  (deftest "sends polls and records outbound activity", async () => {
  ...
  (deftest "sends reactions with participant JID normalization", async () => {
  ...
  (deftest "sends composing presence updates to the recipient JID", async () => {
