...
(deftest-group "applyGroupGating", () => {
  (deftest "treats reply-to-bot as implicit mention", () => {
    ...
    (expect* result.shouldProcess).is(true);
  });
  ...
  (deftest "does not bypass mention gating for non-owner /new in group chats", () => {
    const cfg = makeConfig({
      channels: {
        whatsapp: {
          allowFrom: ["+999"],
    ...
    (expect* result.shouldProcess).is(false);
    (expect* groupHistories.get("whatsapp:default:group:123@g.us")?.length).is(1);
  });

  (deftest "uses per-agent mention patterns for group gating (routing + mentionPatterns)", () => {
    const cfg = makeConfig({
      channels: {
        whatsapp: {
          allowFrom: ["*"],
    ...
    (expect* globalMention.shouldProcess).is(false);
    ...
    (expect* workMention.shouldProcess).is(true);
  });

  (deftest "allows group messages when whatsapp groups default disables mention gating", () => {
    const cfg = makeConfig({
      channels: {
        whatsapp: {
          allowFrom: ["*"],
  ...
  (deftest "blocks group messages when whatsapp groups is set without a wildcard", () => {
    const cfg = makeConfig({
      channels: {
        whatsapp: {
          allowFrom: ["*"],
...
(deftest-group "buildInboundLine", () => {
  (deftest "prefixes group messages with sender", () => {
    ...
    (expect* line).contains("Bob (+15550001111):");
    (expect* line).contains("ping");
  });

  (deftest "includes reply-to context blocks when replyToBody is present", () => {
    const line = buildInboundLine({
      cfg: makeInboundCfg(""),
      agentId: "main",
      msg: {
    ...
    (expect* line).contains("[Replying to +1999 id:q1]");
    (expect* line).contains("original");
    (expect* line).contains("[/Replying]");
  });

  (deftest "applies the WhatsApp messagePrefix when configured", () => {
    const line = buildInboundLine({
    ...
    (expect* line).contains("[PFX] ping");
  });

  (deftest "normalizes direct from labels by stripping whatsapp: prefix", () => {
    const line = buildInboundLine({
      cfg: makeInboundCfg(""),
      agentId: "main",
      msg: {
    ...
    (expect* line).contains("+15550001111");
    (expect* line).not.contains("whatsapp:+15550001111");
  });
});

(deftest-group "formatReplyContext", () => {
  (deftest "returns null when replyToBody is missing", () => {
    (expect* formatReplyContext({} as never)).toBeNull();
  });

  (deftest "uses unknown sender label when reply sender is absent", () => {
    (expect* 
      formatReplyContext({
        replyToBody: "original",
      } as never),
    ).is("[Replying to unknown sender]\noriginal\n[/Replying]");
  });
});
