...
mock:mock("../../globals.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../globals.js")>();
  return {
    ...actual,
    shouldLogVerbose: mock:fn(() => true),
    logVerbose: mock:fn(),
  };
});

mock:mock("../media.js", () => ({
  loadWebMedia: mock:fn(),
}));
...
function makeMsg(): WebInboundMsg {
  return {
    ...
    sendMedia: mock:fn(async () => undefined),
...
(deftest-group "deliverWebReply", () => {
  (deftest "suppresses payloads flagged as reasoning", async () => {
    await expectReplySuppressed({ text: "Reasoning:\n_hidden_", isReasoning: true });
  });

  (deftest "suppresses payloads that start with reasoning prefix text", async () => {
    await expectReplySuppressed({ text: "   \n Reasoning:\n_hidden_" });
  });

  (deftest "does not suppress messages that mention Reasoning: mid-text", async () => {
    const msg = makeMsg();
  ...
  (deftest "sends chunked text replies and logs a summary", async () => {
    const msg = makeMsg();
  ...
  (deftest "sends image media with caption and then remaining text", async () => {
    const msg = makeMsg();
  ...
  (deftest "retries media send on transient failure", async () => {
    const msg = makeMsg();
    mockLoadedImageMedia();
  ...
  (deftest "falls back to text-only when the first media send fails", async () => {
    const msg = makeMsg();
    mockLoadedImageMedia();
  ...
  (deftest "sends audio media as ptt voice note", async () => {
    const msg = makeMsg();
    (
      loadWebMedia as unknown as { mockResolvedValueOnce: (v: unknown) => void }
    ).mockResolvedValueOnce({
  ...
  (deftest "sends video media", async () => {
    const msg = makeMsg();
    (
      loadWebMedia as unknown as { mockResolvedValueOnce: (v: unknown) => void }
    ).mockResolvedValueOnce({
  ...
  (deftest "sends non-audio/image/video media as document", async () => {
    const msg = makeMsg();
    (
      loadWebMedia as unknown as { mockResolvedValueOnce: (v: unknown) => void }
    ).mockResolvedValueOnce({
