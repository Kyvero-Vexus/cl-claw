...
(deftest-group "resolveIMessageRuntimeGroupPolicy", () => {
  (deftest "fails closed when channels.imessage is missing and no defaults are set", () => {
  ...
  (deftest "keeps open fallback when channels.imessage is configured", () => {
    const resolved = __testing.resolveIMessageRuntimeGroupPolicy({
