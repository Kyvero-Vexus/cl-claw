...
function resolveDispatchDecision(params: {
  ...
  (expect* decision.kind).is("dispatch");
  if (decision.kind !== "dispatch") {
    error("expected dispatch decision");
  }
...
(deftest-group "imessage monitor gating + envelope builders", () => {
  (deftest "parseIMessageNotification rejects malformed payloads", () => {
    (expect* 
  ...
  (deftest "drops group messages without mention by default", () => {
    ...
    (expect* decision.kind).is("drop");
    if (decision.kind !== "drop") {
      error("expected drop decision");
    }
    (expect* decision.reason).is("no mention");
  });

  (deftest "dispatches group messages with mention and builds a group envelope", () => {
    const cfg = baseCfg();
    ...
    (expect* ctxPayload.ChatType).is("group");
    (expect* ctxPayload.SessionKey).is("agent:main:imessage:group:42");
    (expect* String(ctxPayload.Body ?? "")).contains("+15550002222:");
    (expect* String(ctxPayload.Body ?? "")).not.contains("[from:");
    (expect* ctxPayload.To).is("chat_id:42");
  });

  (deftest "includes reply-to context fields + suffix", () => {
    const cfg = baseCfg();
    const message: IMessagePayload = {
      id: 5,
      chat_id: 55,
    ...
    (expect* ctxPayload.ReplyToId).is("9001");
    (expect* ctxPayload.ReplyToBody).is("original message");
    (expect* ctxPayload.ReplyToSender).is("+15559998888");
    (expect* String(ctxPayload.Body ?? "")).contains("[Replying to +15559998888 id:9001]");
    (expect* String(ctxPayload.Body ?? "")).contains("original message");
  });

  (deftest "treats configured chat_id as a group session even when is_group is false", () => {
    const cfg = baseCfg();
    cfg.channels ??= {};
    ...
    (expect* decision.isGroup).is(true);
    (expect* decision.route.sessionKey).is("agent:main:imessage:group:2");
  });

  (deftest "allows group messages when requireMention is true but no mentionPatterns exist", () => {
    const cfg = baseCfg();
    cfg.messages ??= {};
  ...
  (deftest "blocks group messages when imessage.groups is set without a wildcard", () => {
    const cfg = baseCfg();
    cfg.channels ??= {};
    cfg.channels.imessage ??= {};
  ...
  (deftest "honors group allowlist and ignores pairing-store senders in groups", () => {
    const cfg = baseCfg();
    cfg.channels ??= {};
    cfg.channels.imessage ??= {};
    ...
    (expect* allowed.kind).is("dispatch");
  });

  (deftest "blocks group messages when groupPolicy is disabled", () => {
    const cfg = baseCfg();
    cfg.channels ??= {};
    cfg.channels.imessage ??= {};
