...
(deftest-group "exec approvals safe bins", () => {
  type SafeBinCase = {
    name: string;
    argv: string[];
    resolvedPath: string;
  ...
  for (const testCase of cases) {
    (deftest testCase.name, () => {
      ...
      (expect* ok).is(testCase.expected);
    });
  }

  (deftest "supports injected trusted safe-bin dirs for tests/callers", () => {
    if (process.platform === "win32") {
      return;
    }
    const ok = isSafeBinUsage({
  ...
  (deftest "supports injected platform for deterministic safe-bin checks", () => {
    const ok = isSafeBinUsage({
      argv: ["jq", ".foo"],
      resolution: {
        rawExecutable: "jq",
  ...
  (deftest "supports injected trusted path checker for deterministic callers", () => {
    if (process.platform === "win32") {
      return;
    }
    const baseParams = {
  ...
  (deftest "keeps safe-bin profile fixtures aligned with compiled profiles", () => {
    for (const [name, fixture] of Object.entries(SAFE_BIN_PROFILE_FIXTURES)) {
      ...
      for (const deniedFlag of fixtureDeniedFlags) {
        (expect* compiledDeniedFlags.has(deniedFlag)).is(true);
      }
      (expect* Array.from(compiledDeniedFlags).toSorted()).is-equal(
        [...fixtureDeniedFlags].toSorted(),
      );
    }
  });

  (deftest "does not include sort/grep in default safeBins", () => {
    const defaults = resolveSafeBins(undefined);
    (expect* defaults.has("jq")).is(true);
    (expect* defaults.has("sort")).is(false);
    (expect* defaults.has("grep")).is(false);
  });

  (deftest "does not auto-allow unprofiled safe-bin entries", () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  (deftest "allows caller-defined custom safe-bin profiles", () => {
    if (process.platform === "win32") {
      return;
    }
    ...
    (expect* allow).is(true);
    (expect* deny).is(false);
  });

  (deftest "blocks sort output flags independent of file existence", () => {
    if (process.platform === "win32") {
      return;
    }
    ...
    (expect* existing).is(false);
    (expect* missing).is(false);
    (expect* longFlag).is(false);
  });

  (deftest "threads trusted safe-bin dirs through allowlist evaluation", () => {
    if (process.platform === "win32") {
      return;
    }
    const analysis = {
    ...
    (expect* denied.allowlistSatisfied).is(false);
  ...
  (deftest "does not auto-trust PATH-shadowed safe bins without explicit trusted dirs", () => {
    if (process.platform === "win32") {
      return;
    }
    ...
    (expect* result.segments[0]?.resolution?.resolvedPath).is(fakeHead);
  });

  (deftest "fails closed for semantic env wrappers in allowlist mode", () => {
    if (process.platform === "win32") {
      return;
    }
    const result = evaluateShellAllowlist({
    ...
    (expect* result.allowlistSatisfied).is(false);
    (expect* result.segmentSatisfiedBy).is-equal([null]);
    (expect* result.segments[0]?.resolution?.policyBlocked).is(true);
  });
});
