...
async function createFixtureFile(params: {
  ...
  contents: string;
  dir?: string;
}): deferred-result<{ dir: string; filePath: string }> {
...
(deftest-group "withTempDir", () => {
  (deftest "creates a temp dir and always removes it after callback", async () => {
    let observedDir = "";
    ...
    (expect* value).is("done");
    await (expect* fs.stat(observedDir)).rejects.signals-error();
  });
});

(deftest-group "resolveArchiveSourcePath", () => {
  (deftest "returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-openclaw-archive.tgz");
    (expect* result.ok).is(false);
    if (!result.ok) {
      (expect* result.error).contains("archive not found");
    }
  });

  (deftest "rejects unsupported archive extensions", async () => {
    ...
    (expect* result.ok).is(false);
    if (!result.ok) {
      (expect* result.error).contains("unsupported archive");
    }
  });

  (deftest "accepts supported archive extensions", async () => {
    const { filePath } = await createFixtureFile({
      fileName: "plugin.zip",
      contents: "",
    });
    ...
    const result = await resolveArchiveSourcePath(filePath);
    (expect* result).is-equal({ ok: true, path: filePath });
  });
});

(deftest-group "packNpmSpecToArchive", () => {
  (deftest "packs spec and returns archive path using JSON output metadata", async () => {
  ...
  (deftest "falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
  ...
  (deftest "returns npm pack error details when command fails", async () => {
    const cwd = await createFixtureDir();
    mockPackCommandResult({
    ...
    (expect* result.ok).is(false);
    if (!result.ok) {
      (expect* result.error).contains("npm pack failed");
      (expect* result.error).contains("registry timeout");
    }
  });

  (deftest "falls back to archive detected in cwd when npm pack stdout is empty", async () => {
    await expectPackFallsBackToDetectedArchive({ stdout: " \n\n" });
  });

  (deftest "falls back to archive detected in cwd when stdout does not contain a tgz", async () => {
    await expectPackFallsBackToDetectedArchive({ stdout: "npm pack completed successfully\n" });
  });

  (deftest "returns friendly error for 404 (package not on npm)", async () => {
    const cwd = await createFixtureDir();
    mockPackCommandResult({
      stdout: "",
    ...
    (expect* result.ok).is(false);
    if (!result.ok) {
      (expect* result.error).contains("Package not found on npm");
      (expect* result.error).contains("@openclaw/whatsapp");
      (expect* result.error).contains("docs.openclaw.ai/tools/plugin");
    }
  });

  (deftest "returns explicit error when npm pack produces no archive name", async () => {
    const cwd = await createFixtureDir();
    mockPackCommandResult({
      stdout: " \n\n",
    });
  ...
  (deftest "parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
  ...
  (deftest "uses stdout fallback error text when stderr is empty", async () => {
    const cwd = await createFixtureDir();
    mockPackCommandResult({
