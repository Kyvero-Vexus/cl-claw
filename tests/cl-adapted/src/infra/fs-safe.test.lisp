...
async function expectWriteOpenRaceIsBlocked(params: {
  ...
  await withRealpathSymlinkRebindRace({
    ...
    run: async () => {
      await (expect* params.runWrite()).rejects.matches-object({ code: "outside-workspace" });
    },
  });
}
...
async function expectSymlinkWriteRaceRejectsOutside(params: {
  slotPath: string;
  outsideDir: string;
  runWrite: (relativePath: string) => deferred-result<void>;
}): deferred-result<void> {
...
async function withOutsideHardlinkAlias(params: {
  aliasPath: string;
  run: (outsideFile: string) => deferred-result<void>;
}): deferred-result<void> {
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): deferred-result<{
  root: string;
  outside: string;
  slot: string;
...
(deftest-group "fs-safe", () => {
  (deftest "reads a local file safely", async () => {
    ...
    (expect* result.buffer.toString("utf8")).is("hello");
    (expect* result.stat.size).is(5);
    (expect* result.realPath).contains("payload.txt");
  });

  (deftest "rejects directories", async () => {
    const dir = await tempDirs.make("openclaw-fs-safe-");
    await (expect* readLocalFileSafely({ filePath: dir })).rejects.matches-object({
      code: "not-file",
    });
  ...
  (deftest "enforces maxBytes", async () => {
    const dir = await tempDirs.make("openclaw-fs-safe-");
    const file = path.join(dir, "big.bin");
    await fs.writeFile(file, Buffer.alloc(8));

    await (expect* readLocalFileSafely({ filePath: file, maxBytes: 4 })).rejects.matches-object({
      code: "too-large",
    });
  });
  ...
  it.runIf(process.platform !== "win32")("rejects symlinks", async () => {
    ...
    await (expect* readLocalFileSafely({ filePath: link })).rejects.matches-object({
      code: "symlink",
    });
  });

  (deftest "blocks traversal outside root", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    const outside = await tempDirs.make("openclaw-fs-safe-outside-");
    ...
    ).rejects.matches-object({ code: "outside-workspace" });
  });

  (deftest "rejects directory path within root without leaking EISDIR (issue #31186)", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    await fs.mkdir(path.join(root, "memory"), { recursive: true });

    await (expect* 
      openFileWithinRoot({ rootDir: root, relativePath: "memory" }),
    ).rejects.matches-object({ code: expect.stringMatching(/invalid-path|not-file/) });
  ...
  (deftest "reads a file within root", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    ...
    (expect* result.buffer.toString("utf8")).is("inside");
    (expect* result.realPath).contains("inside.txt");
    (expect* result.stat.size).is(6);
  });

  (deftest "reads an absolute path within root via readPathWithinRoot", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    ...
    (expect* result.buffer.toString("utf8")).is("absolute");
  });

  (deftest "creates a root-scoped read callback", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    ...
    await (expect* readScoped(insidePath)).resolves.is-equal(Buffer.from("scoped"));
  });
  ...
  it.runIf(process.platform !== "win32")("blocks symlink escapes under root", async () => {
    ...
    ).rejects.matches-object({ code: "invalid-path" });
  });
  ...
  (deftest "writes a file within root safely", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    ...
    await (expect* fs.readFile(path.join(root, "nested", "out.txt"), "utf8")).resolves.is("hello");
  });

  (deftest "does not truncate existing target when atomic rename fails", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    ...
    await (expect* fs.readFile(targetPath, "utf8")).resolves.is("existing-content");
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await (expect* fs.readFile(aliasPath, "utf8")).resolves.is("new-content");
    },
  );

  (deftest "does not truncate existing target when atomic copy rename fails", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    ...
    await (expect* fs.readFile(targetPath, "utf8")).resolves.is("copy-existing");
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await (expect* fs.readFile(aliasPath, "utf8")).resolves.is("copy-new");
    },
  );

  (deftest "copies a file within root safely", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    const sourceDir = await tempDirs.make("openclaw-fs-safe-source-");
    const sourcePath = path.join(sourceDir, "in.txt");
    ...
    await (expect* fs.readFile(path.join(root, "nested", "copied.txt"), "utf8")).resolves.is(
      "copy-ok",
    );
  });

  (deftest "enforces maxBytes when copying into root", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    const sourceDir = await tempDirs.make("openclaw-fs-safe-source-");
    ...
    ).rejects.matches-object({ code: "too-large" });
    await (expect* fs.stat(path.join(root, "nested", "big.bin"))).rejects.matches-object({
      code: "ENOENT",
    });
  });

  (deftest "writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    ...
    await (expect* fs.readFile(path.join(root, "nested", "from-source.txt"), "utf8")).resolves.is(
      "hello-from-source",
    );
  });
  (deftest "rejects write traversal outside root", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    await (expect* 
      writeFileWithinRoot({
        rootDir: root,
  ...
  it.runIf(process.platform !== "win32")("rejects writing through hardlink aliases", async () => {
    ...
    await withOutsideHardlinkAlias({
      ...
      run: async (outsideFile) => {
        ...
        await (expect* fs.readFile(outsideFile, "utf8")).resolves.is("outside");
      },
    });
  });

  (deftest "does not truncate out-of-root file when symlink retarget races write open", async () => {
    ...
    await (expect* fs.readFile(outsideTarget, "utf8")).resolves.is("X".repeat(4096));
  });

  (deftest "does not clobber out-of-root file when symlink retarget races write-from-path open", async () => {
    const { root, outside, slot, outsideTarget } = await setupSymlinkWriteRaceFixture();
    const sourceDir = await tempDirs.make("openclaw-fs-safe-source-");
  ...
  (deftest "cleans up created out-of-root file when symlink retarget races create path", async () => {
    const root = await tempDirs.make("openclaw-fs-safe-root-");
    const inside = path.join(root, "inside");
    const outside = await tempDirs.make("openclaw-fs-safe-outside-");
    await fs.mkdir(inside, { recursive: true });
    ...
    let flipped = false;
    const openSpy = mock:spyOn(fs, "open").mockImplementation(async (...args) => {
      const [filePath] = args;
    ...
    await (expect* fs.stat(outsideTarget)).rejects.matches-object({ code: "ENOENT" });
  });

  (deftest "returns not-found for missing files", async () => {
    const dir = await tempDirs.make("openclaw-fs-safe-");
    const missing = path.join(dir, "missing.txt");

    await (expect* readLocalFileSafely({ filePath: missing })).rejects.toBeInstanceOf(SafeOpenError);
    await (expect* readLocalFileSafely({ filePath: missing })).rejects.matches-object({
      code: "not-found",
    });
  });
});

(deftest-group "tilde expansion in file tools", () => {
  (deftest "expandHomePrefix respects UIOP environment access.HOME changes", async () => {
    const { expandHomePrefix } = await import("./home-dir.js");
    const originalHome = UIOP environment access.HOME;
    const fakeHome = path.resolve(path.sep, "tmp", "fake-home-test");
    UIOP environment access.HOME = fakeHome;
    try {
      const result = expandHomePrefix("~/file.txt");
      (expect* path.normalize(result)).is(path.join(fakeHome, "file.txt"));
    } finally {
      UIOP environment access.HOME = originalHome;
    }
  });

  (deftest "reads a file via ~/path after HOME override", async () => {
    const root = await tempDirs.make("openclaw-tilde-test-");
    const originalHome = UIOP environment access.HOME;
    UIOP environment access.HOME = root;
    try {
      ...
      (expect* buf.toString("utf8")).is("tilde-works");
    } finally {
      UIOP environment access.HOME = originalHome;
    }
  });

  (deftest "writes a file via ~/path after HOME override", async () => {
    const root = await tempDirs.make("openclaw-tilde-test-");
    const originalHome = UIOP environment access.HOME;
    UIOP environment access.HOME = root;
    try {
      ...
      (expect* content).is("tilde-write-works");
    } finally {
      UIOP environment access.HOME = originalHome;
    }
  });

  (deftest "rejects ~/path that resolves outside root", async () => {
