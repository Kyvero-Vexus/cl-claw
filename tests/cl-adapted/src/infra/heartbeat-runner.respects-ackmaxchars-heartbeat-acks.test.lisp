...
(deftest-group "runHeartbeatOnce ack handling", () => {
  ...
  function makeWhatsAppDeps(
    params: {
      sendWhatsApp?: ReturnType<typeof mock:fn>;
      getQueueSize?: () => number;
      nowMs?: () => number;
      webAuthExists?: () => deferred-result<boolean>;
  ...
  async function runTelegramHeartbeatWithDefaults(params: {
    tmpDir: string;
    storePath: string;
    replySpy: ReturnType<typeof mock:spyOn>;
    replyText: string;
    messages?: Record<string, unknown>;
  ...
  (deftest "respects ackMaxChars for heartbeat acks", async () => {
  ...
  (deftest "sends HEARTBEAT_OK when visibility.showOk is true", async () => {
    await withTempHeartbeatSandbox(async ({ tmpDir, storePath, replySpy }) => {
      const cfg = createWhatsAppHeartbeatConfig({
        tmpDir,
        storePath,
  ...
  (deftest "skips heartbeat LLM calls when visibility disables all output", async () => {
    await withTempHeartbeatSandbox(async ({ tmpDir, storePath, replySpy }) => {
      const cfg = createWhatsAppHeartbeatConfig({
        tmpDir,
        storePath,
      ...
      (expect* result).is-equal({ status: "skipped", reason: "alerts-disabled" });
    });
  });

  (deftest "skips delivery for markup-wrapped HEARTBEAT_OK", async () => {
    await withTempHeartbeatSandbox(async ({ tmpDir, storePath, replySpy }) => {
  ...
  (deftest "does not regress updatedAt when restoring heartbeat sessions", async () => {
    await withTempHeartbeatSandbox(async ({ tmpDir, storePath, replySpy }) => {
      ...
      >;
      (expect* finalStore[sessionKey]?.updatedAt).is(bumpedUpdatedAt);
    });
  });

  (deftest "skips WhatsApp delivery when not linked or running", async () => {
    await withTempHeartbeatSandbox(async ({ tmpDir, storePath, replySpy }) => {
      const cfg = await createSeededWhatsAppHeartbeatConfig({
        tmpDir,
        storePath,
      ...
      (expect* res).matches-object({ reason: "whatsapp-not-linked" });
      (expect* sendWhatsApp).not.toHaveBeenCalled();
    });
  });
