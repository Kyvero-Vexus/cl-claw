...
(deftest-group "resolveHeartbeatIntervalMs", () => {
  (deftest "returns default when unset", () => {
    (expect* resolveHeartbeatIntervalMs({})).is(30 * 60_000);
  });

  (deftest "returns null when invalid or zero", () => {
    (expect* 
      resolveHeartbeatIntervalMs({
  ...
  (deftest "parses duration strings with minute defaults", () => {
    (expect* 
      resolveHeartbeatIntervalMs({
  ...
  (deftest "uses explicit heartbeat overrides when provided", () => {
    (expect* 
      resolveHeartbeatIntervalMs(
...
(deftest-group "resolveHeartbeatPrompt", () => {
  (deftest "uses default or trimmed override prompts", () => {
    const cases = [
    ...
    for (const testCase of cases) {
      (expect* resolveHeartbeatPrompt(testCase.cfg)).is(testCase.expected);
    }
  });
});

(deftest-group "isHeartbeatEnabledForAgent", () => {
  (deftest "enables only explicit heartbeat agents when configured", () => {
    ...
    (expect* isHeartbeatEnabledForAgent(cfg, "main")).is(false);
    (expect* isHeartbeatEnabledForAgent(cfg, "ops")).is(true);
  });

  (deftest "falls back to default agent when no explicit heartbeat entries", () => {
    const cfg: OpenClawConfig = {
      agents: {
        defaults: { heartbeat: { every: "30m" } },
        list: [{ id: "main" }, { id: "ops" }],
    ...
    (expect* isHeartbeatEnabledForAgent(cfg, "main")).is(true);
    (expect* isHeartbeatEnabledForAgent(cfg, "ops")).is(false);
  });
});

(deftest-group "resolveHeartbeatDeliveryTarget", () => {
  const baseEntry = {
    sessionId: "sid",
    updatedAt: Date.now(),
  };

  (deftest "resolves target variants across route and allowlist rules", () => {
    const cases: Array<{
      name: string;
      cfg: OpenClawConfig;
  ...
  (deftest "parses optional telegram :topic: threadId suffix", () => {
    const cases = [
    ...
    for (const testCase of cases) {
      ...
      (expect* result.to).is(testCase.expectedTo);
      (expect* result.threadId).is(testCase.expectedThreadId);
    }
  });

  (deftest "handles explicit heartbeat accountId allow/deny", () => {
    const cases = [
      {
        accountId: "work",
        expected: {
    ...
    for (const testCase of cases) {
      ...
      (expect* resolveHeartbeatDeliveryTarget({ cfg, entry: baseEntry })).is-equal(testCase.expected);
    }
  });

  (deftest "prefers per-agent heartbeat overrides when provided", () => {
    const cfg: OpenClawConfig = {
...
(deftest-group "resolveHeartbeatSenderContext", () => {
  (deftest "prefers delivery accountId for allowFrom resolution", () => {
    const cfg: OpenClawConfig = {
      channels: {
        telegram: {
          allowFrom: ["111"],
    ...
    (expect* ctx.allowFrom).is-equal(["222"]);
  });
});
...
(deftest-group "runHeartbeatOnce", () => {
  ...
  (deftest "skips when agent heartbeat is not enabled", async () => {
    const cfg: OpenClawConfig = {
      agents: {
        defaults: { heartbeat: { every: "30m" } },
        list: [{ id: "main" }, { id: "ops", heartbeat: { every: "1h" } }],
    ...
    (expect* res.status).is("skipped");
    if (res.status === "skipped") {
      (expect* res.reason).is("disabled");
    }
  });

  (deftest "skips outside active hours", async () => {
    const cfg: OpenClawConfig = {
      agents: {
        defaults: {
          userTimezone: "UTC",
    ...
    (expect* res.status).is("skipped");
    if (res.status === "skipped") {
      (expect* res.reason).is("quiet-hours");
    }
  });

  (deftest "uses the last non-empty payload for delivery", async () => {
    ...
    try {
      ...
      const sendWhatsApp = mock:fn<NonNullable<HeartbeatDeps["sendWhatsApp"]>>().mockResolvedValue({
        messageId: "m1",
        toJid: "jid",
      });
  ...
  (deftest "uses per-agent heartbeat overrides and session keys", async () => {
  ...
  (deftest "reuses non-default agent sessionFile from templated stores", async () => {
  ...
  (deftest "resolves configured and forced session key overrides", async () => {
    const replySpy = mock:spyOn(replyModule, "getReplyFromConfig");
    try {
  ...
  (deftest "suppresses duplicate heartbeat payloads within 24h", async () => {
  ...
  (deftest "handles reasoning payload delivery variants", async () => {
    const replySpy = mock:spyOn(replyModule, "getReplyFromConfig");
    try {
      const cases = typedCases<{
        name: string;
  ...
  (deftest "loads the default agent session from templated stores", async () => {
  ...
  (deftest "adds explicit workspace HEARTBEAT.md path guidance to heartbeat prompts", async () => {
    ...
    try {
      ...
      (expect* calledCtx.Body).contains(`use workspace file ${expectedPath} (exact case)`);
      (expect* calledCtx.Body).contains("Do not read docs/heartbeat.md.");
    } finally {
      replySpy.mockRestore();
    }
  });

  (deftest "applies HEARTBEAT.md gating rules across file states and triggers", async () => {
    const cases: Array<{
      name: string;
      fileState: HeartbeatFileState;
      reason?: "interval" | "wake";
    ...
    for (const testCase of cases) {
      ...
      try {
        (expect* res.status, testCase.name).is(testCase.expectedStatus);
        if (res.status === "skipped") {
          (expect* res.reason, testCase.name).is(testCase.expectedSkipReason);
        }
        ...
        if (testCase.expectCronContext) {
          ...
          (expect* calledCtx.Provider, testCase.name).is("cron-event");
          (expect* calledCtx.Body, testCase.name).contains("scheduled reminder has been triggered");
        }
      } finally {
        replySpy.mockRestore();
      }
  ...
  (deftest "uses an internal-only cron prompt when heartbeat delivery target is none", async () => {
    ...
    try {
      ...
      (expect* calledCtx.Body).not.contains("Please relay this reminder to the user");
    } finally {
      replySpy.mockRestore();
    }
  });

  (deftest "uses an internal-only exec prompt when heartbeat delivery target is none", async () => {
    ...
    try {
      ...
      (expect* calledCtx.Body).not.contains("Please relay the command output to the user");
    } finally {
      replySpy.mockRestore();
    }
  });
