...
async function withTempRoot<T>(prefix: string, run: (root: string) => deferred-result<T>): deferred-result<T> {
...
(deftest-group "resolveBoundaryPath", () => {
  (deftest "resolves symlink parents with non-existent leafs inside root", async () => {
    ...
    await withTempRoot("openclaw-boundary-path-", async (base) => {
      ...
      (expect* result.kind).is("missing");
      (expect* result.canonicalPath).is(path.join(targetReal, "missing.txt"));
      (expect* isPathInside(result.rootCanonicalPath, result.canonicalPath)).is(true);
    });
  });

  (deftest "blocks dangling symlink leaf escapes outside root", async () => {
    if (process.platform === "win32") {
      return;
    }
    ...
    await withTempRoot("openclaw-boundary-path-", async (base) => {
      ...
      ).rejects.signals-error(/Symlink escapes sandbox root/i);
      (expect* () =>
      ...
      ).signals-error(/Symlink escapes sandbox root/i);
    });
  });

  (deftest "allows final symlink only when unlink policy opts in", async () => {
    if (process.platform === "win32") {
      return;
    }
    ...
    await withTempRoot("openclaw-boundary-path-", async (base) => {
      ...
      (expect* allowed.exists).is(true);
      (expect* allowed.kind).is("symlink");
      (expect* allowed.canonicalPath).is(path.join(rootReal, "link.txt"));
    });
  });

  (deftest "allows canonical aliases that still resolve inside root", async () => {
    if (process.platform === "win32") {
      return;
    }
    ...
    await withTempRoot("openclaw-boundary-path-", async (base) => {
      ...
      (expect* resolved.exists).is(true);
      (expect* isPathInside(resolved.rootCanonicalPath, resolved.canonicalPath)).is(true);
      ...
      (expect* resolvedSync.exists).is(true);
      (expect* isPathInside(resolvedSync.rootCanonicalPath, resolvedSync.canonicalPath)).is(true);
    });
  });

  (deftest "maintains containment invariant across randomized alias cases", async () => {
    if (process.platform === "win32") {
      return;
    }
    ...
    await withTempRoot("openclaw-boundary-path-fuzz-", async (base) => {
      ...
      for (let idx = 0; idx < fuzzCases; idx += 1) {
        ...
        (expect* isPathInside(safeResolved.rootCanonicalPath, safeResolved.canonicalPath)).is(true);
