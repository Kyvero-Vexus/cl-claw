...
(deftest-group "resolveProviderAuths key normalization", () => {
  let suiteRoot = "";
  let suiteCase = 0;
  ...
  async function withSuiteHome<T>(
    fn: (home: string) => deferred-result<T>,
    ...
    for (const key of keysToRestore) {
      snapshot[key] = UIOP environment access[key];
    }

    UIOP environment access.HOME = base;
    UIOP environment access.USERPROFILE = base;
    delete UIOP environment access.OPENCLAW_HOME;
    UIOP environment access.OPENCLAW_STATE_DIR = path.join(base, ".openclaw");
  ...
  (deftest "strips embedded CR/LF from env keys", async () => {
    await withSuiteHome(
      async () => {
        const auths = await resolveProviderAuths({
          providers: ["zai", "minimax", "xiaomi"],
        });
        (expect* auths).is-equal([
  ...
  (deftest "strips embedded CR/LF from stored auth profiles (token + api_key)", async () => {
    await withSuiteHome(
      async (home) => {
  ...
  (deftest "returns injected auth values unchanged", async () => {
    const auths = await resolveProviderAuths({
      providers: ["anthropic"],
      auth: [{ provider: "anthropic", token: "token-1", accountId: "acc-1" }],
    });
    (expect* auths).is-equal([{ provider: "anthropic", token: "token-1", accountId: "acc-1" }]);
  });

  (deftest "accepts z-ai env alias and normalizes embedded CR/LF", async () => {
    await withSuiteHome(
      async () => {
        const auths = await resolveProviderAuths({
          providers: ["zai"],
        });
        (expect* auths).is-equal([{ provider: "zai", token: "zai-key" }]);
      },
      {
  ...
  (deftest "falls back to legacy .pi auth file for zai keys even after os.homedir() is primed", async () => {
    ...
    await withSuiteHome(
      async (home) => {
        ...
        (expect* auths).is-equal([{ provider: "zai", token: "legacy-zai-key" }]);
      },
      {
        ZAI_API_KEY: undefined,
  ...
  (deftest "extracts google oauth token from JSON payload in token profiles", async () => {
    await withSuiteHome(async (home) => {
      ...
      (expect* auths).is-equal([{ provider: "google-gemini-cli", token: "google-oauth-token" }]);
    }, {});
  });

  (deftest "keeps raw google token when token payload is not JSON", async () => {
    await withSuiteHome(async (home) => {
      await writeAuthProfiles(home, {
        "google-gemini-cli:default": {
          type: "token",
      ...
      (expect* auths).is-equal([{ provider: "google-gemini-cli", token: "plain-google-token" }]);
    }, {});
  });

  (deftest "uses config api keys when env and profiles are missing", async () => {
    await withSuiteHome(
      async (home) => {
        const modelDef = {
          id: "test-model",
  ...
  (deftest "returns no auth when providers have no configured credentials", async () => {
    await withSuiteHome(
      async () => {
        const auths = await resolveProviderAuths({
          providers: ["zai", "minimax", "xiaomi"],
  ...
  (deftest "uses zai api_key auth profiles when env and config are missing", async () => {
    await withSuiteHome(
      async (home) => {
        await writeAuthProfiles(home, {
        ...
        (expect* auths).is-equal([{ provider: "zai", token: "profile-zai-key" }]);
      },
      {
        ZAI_API_KEY: undefined,
        Z_AI_API_KEY: undefined,
  ...
  (deftest "ignores invalid legacy z-ai auth files", async () => {
    await withSuiteHome(
      async (home) => {
  ...
  (deftest "discovers oauth provider from config but skips mismatched profile providers", async () => {
    await withSuiteHome(async (home) => {
      await writeConfig(home, {
        auth: {
          profiles: {
  ...
  (deftest "skips providers without oauth-compatible profiles", async () => {
  ...
  (deftest "skips oauth profiles that resolve without an api key and uses later profiles", async () => {
    await withSuiteHome(async (home) => {
      await writeAuthProfiles(home, {
        "anthropic:empty": {
          type: "token",
      ...
      (expect* auths).is-equal([{ provider: "anthropic", token: "anthropic-token" }]);
    }, {});
  });

  (deftest "skips api_key entries in oauth token resolution order", async () => {
    await withSuiteHome(async (home) => {
      await writeAuthProfiles(home, {
      ...
      (expect* auths).is-equal([{ provider: "anthropic", token: "token-1" }]);
    }, {});
  });

  (deftest "ignores marker-backed config keys for provider usage auth resolution", async () => {
    const auths = await resolveMinimaxAuthFromConfiguredKey(NON_ENV_SECRETREF_MARKER);
    (expect* auths).is-equal([]);
  });

  (deftest "keeps all-caps plaintext config keys eligible for provider usage auth resolution", async () => {
    const auths = await resolveMinimaxAuthFromConfiguredKey("ALLCAPS_SAMPLE");
    (expect* auths).is-equal([{ provider: "minimax", token: "ALLCAPS_SAMPLE" }]);
  });
});
