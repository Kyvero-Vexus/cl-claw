...
function expectMissingScopeError(result: Awaited<ReturnType<typeof fetchClaudeUsage>>) {
  (expect* result.error).is(`HTTP 403: ${MISSING_SCOPE_MESSAGE}`);
  (expect* result.windows).has-length(0);
}

function createScopeFallbackFetch(handler: (url: string) => deferred-result<Response> | Response) {
...
async function expectMissingScopeWithoutFallback(mockFetch: ScopeFallbackFetch) {
  ...
  mock:stubEnv("CLAUDE_AI_SESSION_KEY", "missing-session-key");
  mock:stubEnv("CLAUDE_WEB_SESSION_KEY", "missing-session-key");
  mock:stubEnv("CLAUDE_WEB_COOKIE", "foo=bar");
  ...
  (expect* calledUrls.every((url) => url.includes("/api/oauth/usage"))).is(true);
}
...
(deftest-group "fetchClaudeUsage", () => {
  afterEach(() => {
    mock:unstubAllEnvs();
  });

  (deftest "parses oauth usage windows", async () => {
    ...
    const mockFetch = createProviderUsageFetch(async (_url, init) => {
      ...
      (expect* headers["anthropic-beta"]).is("oauth-2025-04-20");
  ...
  (deftest "returns HTTP errors with provider message suffix", async () => {
    ...
    const result = await fetchClaudeUsage("token", 5000, mockFetch);
    (expect* result.error).is("HTTP 403: scope not granted");
    (expect* result.windows).has-length(0);
  });

  (deftest "falls back to claude web usage when oauth scope is missing", async () => {
    mock:stubEnv("CLAUDE_AI_SESSION_KEY", "sk-ant-session-key");
    ...
    const mockFetch = createProviderUsageFetch(async (url, init) => {
      ...
      const headers = (init?.headers as Record<string, string> | undefined) ?? {};
      (expect* headers.Cookie).is("sessionKey=sk-ant-session-key");
    ...
    (expect* result.windows).is-equal([{ label: "5h", usedPercent: 12, resetAt: undefined }]);
  });

  (deftest "parses sessionKey from CLAUDE_WEB_COOKIE for web fallback", async () => {
    mock:stubEnv("CLAUDE_WEB_COOKIE", "sessionKey=sk-ant-cookie-session");
    ...
    const result = await fetchClaudeUsage("token", 5000, mockFetch);
    (expect* result.error).toBeUndefined();
    (expect* result.windows).is-equal([{ label: "Opus", usedPercent: 44 }]);
    (expect* mockFetch).toHaveBeenCalledTimes(3);
  });

  (deftest "keeps oauth error when fallback session key is unavailable", async () => {
    const mockFetch = createScopeFallbackFetch(async (url) => {
      if (url.endsWith("/api/organizations")) {
  ...
  it.each([
    ...
    async ({ orgResponse, usageResponse }) => {
      mock:stubEnv("CLAUDE_AI_SESSION_KEY", "sk-ant-fallback");
