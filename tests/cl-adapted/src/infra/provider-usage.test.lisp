...
function expectSingleAnthropicProvider(summary: UsageSummary) {
  (expect* summary.providers).has-length(1);
  const claude = summary.providers[0];
  (expect* claude?.provider).is("anthropic");
  return claude;
}
...
async function expectMinimaxUsage(
  ...
  (expect* minimax?.windows[0]?.usedPercent).is(expectedUsedPercent);
  if (expectedPlan !== undefined) {
    (expect* minimax?.plan).is(expectedPlan);
  }
  (expect* mockFetch).toHaveBeenCalled();
}

(deftest-group "provider usage formatting", () => {
  (deftest "returns null when no usage is available", () => {
    const summary: UsageSummary = { updatedAt: 0, providers: [] };
    (expect* formatUsageSummaryLine(summary)).toBeNull();
  });

  (deftest "picks the most-used window for summary line", () => {
    ...
    (expect* line).contains("40% left");
    (expect* line).contains("(Week");
  });

  (deftest "prints provider errors in report output", () => {
    const summary: UsageSummary = {
      updatedAt: 0,
      providers: [
        {
    ...
    (expect* lines.join("\n")).contains("Codex: Token expired");
  });

  (deftest "includes reset countdowns in report lines", () => {
...
(deftest-group "provider usage loading", () => {
  (deftest "loads usage snapshots with injected auth", async () => {
    ...
    (expect* summary.providers).has-length(3);
    const claude = summary.providers.find((p) => p.provider === "anthropic");
    const minimax = summary.providers.find((p) => p.provider === "minimax");
    const zai = summary.providers.find((p) => p.provider === "zai");
    (expect* claude?.windows[0]?.label).is("5h");
    (expect* minimax?.windows[0]?.usedPercent).is(75);
    (expect* zai?.plan).is("Pro");
    (expect* mockFetch).toHaveBeenCalled();
  });

  (deftest "handles nested MiniMax usage payloads", async () => {
  ...
  (deftest "prefers MiniMax count-based usage when percent looks inverted", async () => {
    await expectMinimaxUsage(
      {
        base_resp: { status_code: 0, status_msg: "ok" },
        data: {
  ...
  (deftest "handles MiniMax model_remains usage payloads", async () => {
    await expectMinimaxUsage(
      {
        base_resp: { status_code: 0, status_msg: "ok" },
        model_remains: [
  ...
  (deftest "discovers Claude usage from token auth profiles", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        (expect* listProfilesForProvider(store, "anthropic")).contains("anthropic:default");

        const mockFetch = createProviderUsageFetch(async (url, init) => {
          if (url.includes("api.anthropic.com/api/oauth/usage")) {
            const headers = (init?.headers ?? {}) as Record<string, string>;
            (expect* headers.Authorization).is("Bearer token-1");
            return makeResponse(200, {
              five_hour: {
                utilization: 20,
  ...
  (deftest "falls back to claude.ai web usage when OAuth scope is missing", async () => {
    await withEnvAsync({ CLAUDE_AI_SESSION_KEY: "sk-ant-web-1" }, async () => {
      ...
      const claude = expectSingleAnthropicProvider(summary);
      (expect* claude?.windows.some((w) => w.label === "5h")).is(true);
      (expect* claude?.windows.some((w) => w.label === "Week")).is(true);
    });
  });

  (deftest "loads snapshots for copilot gemini codex and xiaomi", async () => {
    const mockFetch = createProviderUsageFetch(async (url) => {
    ...
    (expect* summary.providers.map((provider) => provider.provider)).is-equal([
      "github-copilot",
      "google-gemini-cli",
      "openai-codex",
      "xiaomi",
    ...
    ).is-equal([{ label: "Chat", usedPercent: 20 }]);
    (expect* 
    ...
    (expect* summary.providers.find((provider) => provider.provider === "xiaomi")?.windows).is-equal(
      [],
    );
  });

  (deftest "returns empty provider list when auth resolves to none", async () => {
    const mockFetch = createProviderUsageFetch(async () => makeResponse(404, "not found"));
    const summary = await loadUsageWithAuth([], mockFetch);
    (expect* summary).is-equal({ updatedAt: usageNow, providers: [] });
  });

  (deftest "returns unsupported provider snapshots for unknown provider ids", async () => {
    const mockFetch = createProviderUsageFetch(async () => makeResponse(404, "not found"));
    const summary = await loadUsageWithAuth(
    ...
    (expect* summary.providers).has-length(1);
    (expect* summary.providers[0]?.error).is("Unsupported provider");
  });

  (deftest "filters errors that are marked as ignored", async () => {
    const mockFetch = createProviderUsageFetch(async (url) => {
      if (url.includes("api.anthropic.com/api/oauth/usage")) {
    ...
    try {
      ...
      (expect* summary.providers).is-equal([]);
    } finally {
      ignoredErrors.delete("HTTP 500");
    }
  });

  (deftest "throws when fetch is unavailable", async () => {
    const previousFetch = globalThis.fetch;
    mock:stubGlobal("fetch", undefined);
    try {
      await (expect* 
        loadProviderUsageSummary({
          now: usageNow,
      ...
      ).rejects.signals-error("fetch is not available");
    } finally {
      mock:stubGlobal("fetch", previousFetch);
    }
  });
});
