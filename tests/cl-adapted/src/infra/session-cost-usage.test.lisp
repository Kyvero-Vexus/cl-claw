...
(deftest-group "session cost usage", () => {
  const withStateDir = async <T>(stateDir: string, fn: () => deferred-result<T>): deferred-result<T> =>
    await withEnvAsync({ OPENCLAW_STATE_DIR: stateDir }, fn);

  (deftest "aggregates daily totals with log cost and pricing fallback", async () => {
    ...
    await withStateDir(root, async () => {
      ...
      (expect* summary.daily.length).is(1);
      (expect* summary.totals.totalTokens).is(50);
      (expect* summary.totals.totalCost).toBeCloseTo(0.03003, 5);
    });
  });

  (deftest "summarizes a single session file", async () => {
    ...
    (expect* summary?.totalTokens).is(30);
    (expect* summary?.lastActivity).toBeGreaterThan(0);
  });

  (deftest "captures message counts, tool usage, and model usage", async () => {
    ...
    (expect* summary?.messageCounts).is-equal({
      total: 2,
      user: 1,
      assistant: 1,
      toolCalls: 1,
    ...
    (expect* summary?.toolUsage?.totalCalls).is(1);
    (expect* summary?.toolUsage?.uniqueTools).is(1);
    (expect* summary?.toolUsage?.tools[0]?.name).is("weather");
    (expect* summary?.modelUsage?.[0]?.provider).is("openai");
    (expect* summary?.modelUsage?.[0]?.model).is("gpt-5.2");
    (expect* summary?.durationMs).is(5 * 60 * 1000);
    (expect* summary?.latency?.count).is(1);
    (expect* summary?.latency?.avgMs).is(5 * 60 * 1000);
    (expect* summary?.latency?.p95Ms).is(5 * 60 * 1000);
    (expect* summary?.dailyLatency?.[0]?.date).is("2026-02-01");
    (expect* summary?.dailyLatency?.[0]?.count).is(1);
    (expect* summary?.dailyModelUsage?.[0]?.date).is("2026-02-01");
    (expect* summary?.dailyModelUsage?.[0]?.model).is("gpt-5.2");
  });

  (deftest "does not exclude sessions with mtime after endMs during discovery", async () => {
    ...
    await withStateDir(root, async () => {
      ...
      (expect* sessions.length).is(1);
      (expect* sessions[0]?.sessionId).is("sess-late");
    });
  });

  (deftest "resolves non-main absolute sessionFile using explicit agentId for cost summary", async () => {
    ...
    await withStateDir(root, async () => {
      ...
      (expect* summary?.totalTokens).is(18);
      (expect* summary?.totalCost).toBeCloseTo(0.01, 5);
    });
  });

  (deftest "resolves non-main absolute sessionFile using explicit agentId for timeseries", async () => {
    ...
    await withStateDir(root, async () => {
      ...
      (expect* timeseries?.points.length).is(1);
      (expect* timeseries?.points[0]?.totalTokens).is(8);
    });
  });

  (deftest "resolves non-main absolute sessionFile using explicit agentId for logs", async () => {
    ...
    await withStateDir(root, async () => {
      ...
      (expect* logs?.[0]?.content).contains("hello worker");
      (expect* logs?.[0]?.role).is("user");
    });
  });

  (deftest "strips inbound and untrusted metadata blocks from session usage logs", async () => {
...
<<<END_EXTERNAL_UNTRUSTED_CONTENT id="deadbeefdeadbeef">>>`,
    ...
    (expect* logs).has-length(1);
    (expect* logs?.[0]?.role).is("user");
    (expect* logs?.[0]?.content).is("hello there");
  });

  (deftest "preserves totals and cumulative values when downsampling timeseries", async () => {
    ...
    (expect* timeseries).is-truthy();
    (expect* timeseries?.points.length).is(3);
    ...
    (expect* totalTokens).is(165);
    (expect* totalCost).toBeCloseTo(0.055, 8);
    (expect* lastPoint?.cumulativeTokens).is(165);
