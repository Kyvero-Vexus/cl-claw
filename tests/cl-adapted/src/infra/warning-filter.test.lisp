...
(deftest-group "warning filter", () => {
  ...
  (deftest "suppresses known deprecation and experimental warning signatures", () => {
    (expect* 
      shouldIgnoreWarning({
  ...
  (deftest "keeps unknown warnings visible", () => {
    (expect* 
      shouldIgnoreWarning({
        name: "DeprecationWarning",
        code: "DEP9999",
  ...
  (deftest "installs once and suppresses known warnings at emit time", async () => {
