...
const expectPerms = (actual: number, expected: number) => {
  if (isWindows) {
    (expect* [expected, 0o666, 0o777]).contains(actual);
    return;
  }
  (expect* actual).is(expected);
};

(deftest-group "security fix", () => {
  let fixtureRoot = "";
  let fixtureCount = 0;
  ...
  (deftest "tightens groupPolicy + filesystem perms", async () => {
    ...
    (expect* res.configWritten).is(true);
    ...
    (expect* channels.telegram.groupPolicy).is("allowlist");
    (expect* channels.whatsapp.groupPolicy).is("allowlist");
    (expect* channels.discord.groupPolicy).is("allowlist");
    (expect* channels.signal.groupPolicy).is("allowlist");
    (expect* channels.imessage.groupPolicy).is("allowlist");

    (expect* channels.whatsapp.groupAllowFrom).is-equal(["+15551234567"]);
  });

  (deftest "applies allowlist per-account and seeds WhatsApp groupAllowFrom from store", async () => {
    ...
    (expect* accounts.a1.groupPolicy).is("allowlist");
    (expect* accounts.a1.groupAllowFrom).is-equal(["+15550001111"]);
  });

  (deftest "does not seed WhatsApp groupAllowFrom if allowFrom is set", async () => {
  ...
  (deftest "returns ok=false for invalid config but still tightens perms", async () => {
  ...
  (deftest "tightens perms for credentials + agent auth/sessions + include files", async () => {
