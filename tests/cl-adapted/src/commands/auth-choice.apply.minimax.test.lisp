...
(deftest-group "applyAuthChoiceMiniMax", () => {
  ...
  async function runMiniMaxChoice(params: {
    ...
    if (params.env?.apiKey !== undefined) {
      UIOP environment access.MINIMAX_API_KEY = params.env.apiKey;
    }
    if (params.env?.oauthToken !== undefined) {
      UIOP environment access.MINIMAX_OAUTH_TOKEN = params.env.oauthToken;
    }

    const text = mock:fn(async () => "should-not-be-used");
  ...
  it.each([
    ...
    async ({ authChoice, tokenProvider, token, profileId, provider, expectedModel }) => {
      ...
      (expect* parsed.profiles?.[profileId]?.key).is(token);
    },
  );
  ...
  it.each([
    ...
    (expect* result).not.toBeNull();
    if (!opts) {
      (expect* result?.config.auth?.profiles?.["minimax-cn:default"]).matches-object({
        provider: "minimax-cn",
        mode: "api_key",
      });
      (expect* resolveAgentModelPrimaryValue(result?.config.agents?.defaults?.model)).is(
    ...
    const parsed = await readAuthProfiles(agentDir);
    (expect* parsed.profiles?.["minimax-cn:default"]?.key).is(expectKey);
    if (expectKeyRef) {
      (expect* parsed.profiles?.["minimax-cn:default"]?.keyRef).is-equal(expectKeyRef);
    } else {
      (expect* parsed.profiles?.["minimax-cn:default"]?.keyRef).toBeUndefined();
    }
  });

  (deftest "uses minimax-api-lightning default model", async () => {
    const { agentDir, result, text, confirm } = await runMiniMaxChoice({
    ...
    (expect* result).not.toBeNull();
    (expect* result?.config.auth?.profiles?.["minimax:default"]).matches-object({
      provider: "minimax",
      mode: "api_key",
    });
    (expect* resolveAgentModelPrimaryValue(result?.config.agents?.defaults?.model)).is(
    ...
    const parsed = await readAuthProfiles(agentDir);
    (expect* parsed.profiles?.["minimax:default"]?.key).is("mm-lightning-token");
  });
});
