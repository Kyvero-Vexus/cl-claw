...
const loadModelCatalog = mock:hoisted(() => mock:fn());
mock:mock("../agents/model-catalog.js", () => ({
  loadModelCatalog,
}));

const ensureAuthProfileStore = mock:hoisted(() =>
  mock:fn(() => ({
    version: 1,
    profiles: {},
  })),
);
const listProfilesForProvider = mock:hoisted(() => mock:fn(() => []));
const upsertAuthProfile = mock:hoisted(() => mock:fn());
const upsertAuthProfileWithLock = mock:hoisted(() => mock:fn(async () => {}));
...
const resolveEnvApiKey = mock:hoisted(() => mock:fn(() => undefined));
const getCustomProviderApiKey = mock:hoisted(() => mock:fn(() => undefined));
...
function expectRouterModelFiltering(options: Array<{ value: string }>) {
  (expect* options.some((opt) => opt.value === "openrouter/auto")).is(false);
  (expect* options.some((opt) => opt.value === "openrouter/meta-llama/llama-3.3-70b:free")).is(
    true,
  );
}

function createSelectAllMultiselect() {
  return mock:fn(async (params) => params.options.map((option: { value: string }) => option.value));
}

(deftest-group "promptDefaultModel", () => {
  (deftest "supports configuring vLLM during onboarding", async () => {
    ...
    const select = mock:fn(async (params) => {
    ...
    (expect* result.model).is("vllm/meta-llama/Meta-Llama-3-8B-Instruct");
    (expect* result.config?.models?.providers?.vllm).matches-object({
...
(deftest-group "promptModelAllowlist", () => {
  (deftest "filters to allowed keys when provided", async () => {
    loadModelCatalog.mockResolvedValue([
      {
        provider: "anthropic",
        id: "claude-opus-4-5",
    ...
    (expect* options.map((opt: { value: string }) => opt.value)).is-equal([
      "anthropic/claude-opus-4-5",
    ]);
  });
});

(deftest-group "router model filtering", () => {
  (deftest "filters internal router models in both default and allowlist prompts", async () => {
    ...
    (expect* allowlistCall?.searchable).is(true);
  });
});

(deftest-group "applyModelAllowlist", () => {
  (deftest "preserves existing entries for selected models", () => {
    const config = {
      agents: {
        defaults: {
          models: {
    ...
    (expect* next.agents?.defaults?.models).is-equal({
      "openai/gpt-5.2": { alias: "gpt" },
    });
  });

  (deftest "clears the allowlist when no models remain", () => {
    const config = {
      agents: {
        defaults: {
          models: {
...
(deftest-group "applyModelFallbacksFromSelection", () => {
  (deftest "sets fallbacks from selection when the primary is included", () => {
    const config = {
      agents: {
        defaults: {
    ...
    (expect* next.agents?.defaults?.model).is-equal({
      primary: "anthropic/claude-opus-4-5",
      fallbacks: ["anthropic/claude-sonnet-4-5"],
    });
  });

  (deftest "keeps existing fallbacks when the primary is not selected", () => {
    const config = {
      agents: {
        defaults: {
