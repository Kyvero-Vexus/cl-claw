...
(deftest-group "healthCommand", () => {
  beforeEach(() => {
    mock:clearAllMocks();
  });

  (deftest "outputs JSON from gateway", async () => {
    const agentSessions = {
      path: "/tmp/sessions.json",
      count: 1,
    ...
    (expect* parsed.channels.whatsapp?.linked).is(true);
    (expect* parsed.channels.telegram?.configured).is(true);
    (expect* parsed.sessions.count).is(1);
  });

  (deftest "prints text summary when not json", async () => {
  ...
  (deftest "formats per-account probe timings", () => {
...
(deftest-group "formatHealthCheckFailure", () => {
  (deftest "keeps non-rich output stable", () => {
    const err = new Error("gateway closed (1006 abnormal closure): no close reason");
    (expect* formatHealthCheckFailure(err, { rich: false })).is(
      `Health check failed: ${String(err)}`,
    );
  });

  (deftest "formats gateway connection details as indented key/value lines", () => {
    const err = new Error(
      [
    ...
    (expect* stripAnsi(formatHealthCheckFailure(err, { rich: true }))).is(
      [
