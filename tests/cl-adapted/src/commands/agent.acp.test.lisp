...
const loadConfigSpy = mock:spyOn(configModule, "loadConfig");
const runEmbeddedPiAgentSpy = mock:spyOn(embeddedModule, "runEmbeddedPiAgent");
const getAcpSessionManagerSpy = mock:spyOn(acpManagerModule, "getAcpSessionManager");
...
function mockAcpManager(params: {
  runTurn: (params: unknown) => deferred-result<void>;
...
async function withAcpSessionEnv(fn: () => deferred-result<void>) {
...
function createRunTurnFromTextDeltas(chunks: string[]) {
  return mock:fn(async (paramsUnknown: unknown) => {
    const params = paramsUnknown as {
      onEvent?: (event: { type: string; text?: string; stopReason?: string }) => deferred-result<void>;
    };
...
async function runAcpSessionWithPolicyOverrides(params: {
  ...
  await withTempHome(async (home) => {
    ...
    const runTurn = mock:fn(async (_params: unknown) => {});
    mockAcpManager({
...
(deftest-group "agentCommand ACP runtime routing", () => {
  beforeEach(() => {
    mock:clearAllMocks();
  ...
  (deftest "routes ACP sessions through AcpSessionManager instead of embedded agent", async () => {
    await withAcpSessionEnv(async () => {
      ...
      (expect* hasAckLog).is(true);
    });
  });

  (deftest "suppresses ACP NO_REPLY lead fragments before emitting assistant text", async () => {
    await withAcpSessionEnv(async () => {
      ...
      (expect* assistantEvents).is-equal([{ text: "Actual answer", delta: "Actual answer" }]);

      const logLines = mock:mocked(runtime.log).mock.calls.map(([first]) => String(first));
      (expect* logLines.some((line) => line.includes("NO_REPLY"))).is(false);
      (expect* logLines.some((line) => line.includes("Actual answer"))).is(true);
    });
  });

  (deftest "keeps silent-only ACP turns out of assistant output", async () => {
    await withAcpSessionEnv(async () => {
      ...
      (expect* assistantEvents).is-equal([]);

      const logLines = mock:mocked(runtime.log).mock.calls.map(([first]) => String(first));
      (expect* logLines.some((line) => line.includes("NO_REPLY"))).is(false);
      (expect* logLines.some((line) => line.includes("No reply from agent."))).is(true);
    });
  });

  (deftest "preserves repeated identical ACP delta chunks", async () => {
    await withAcpSessionEnv(async () => {
      const { assistantEvents, stop } = subscribeAssistantEvents();
      ...
      const logLines = mock:mocked(runtime.log).mock.calls.map(([first]) => String(first));
      (expect* logLines.some((line) => line.includes("book"))).is(true);
    });
  });

  (deftest "re-emits buffered NO prefix when ACP text becomes visible content", async () => {
    await withAcpSessionEnv(async () => {
      const { assistantEvents, stop } = subscribeAssistantEvents();
      ...
      (expect* assistantEvents).is-equal([{ text: "NOW", delta: "NOW" }]);

      const logLines = mock:mocked(runtime.log).mock.calls.map(([first]) => String(first));
      (expect* logLines.some((line) => line.includes("NOW"))).is(true);
    });
  });

  (deftest "fails closed for ACP-shaped session keys missing ACP metadata", async () => {
    await withTempHome(async (home) => {
      const storePath = path.join(home, "sessions.json");
      fs.mkdirSync(path.dirname(storePath), { recursive: true });
      fs.writeFileSync(
  ...
  (deftest "blocks ACP turns when ACP agent is disallowed by policy", async () => {
    await withTempHome(async (home) => {
      const storePath = path.join(home, "sessions.json");
      writeAcpSessionStore(storePath);
  ...
  (deftest "allows ACP turns for kimi when policy allowlists kimi", async () => {
    await withTempHome(async (home) => {
      const storePath = path.join(home, "sessions.json");
