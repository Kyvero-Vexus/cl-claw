...
const mocks = mock:hoisted(() => {
  ...
  return {
    ...
    sourceConfig,
    resolvedConfig,
    loadModelsConfigWithSource: mock:fn().mockResolvedValue({
      sourceConfig,
      resolvedConfig,
      diagnostics: [],
    }),
    ensureAuthProfileStore: mock:fn().mockReturnValue({ version: 1, profiles: {}, order: {} }),
    loadModelRegistry: vi
      .fn()
      .mockResolvedValue({ models: [], availableKeys: new Set(), registry: {} }),
    resolveConfiguredEntries: mock:fn().mockReturnValue({
      entries: [
        {
    ...
    printModelTable,
    listProfilesForProvider: mock:fn().mockReturnValue([]),
    resolveModelWithRegistry: mock:fn().mockReturnValue({
...
mock:mock("./list.registry.js", async (importOriginal) => {
...
(deftest-group "modelsListCommand forward-compat", () => {
  (deftest "does not mark configured codex model as missing when forward-compat can build a fallback", async () => {
    const runtime = { log: mock:fn(), error: mock:fn() };
    ...
    (expect* codex).is-truthy();
    (expect* codex?.missing).is(false);
    (expect* codex?.tags).not.contains("missing");
  });

  (deftest "passes source config to model registry loading for persistence safety", async () => {
    const runtime = { log: mock:fn(), error: mock:fn() };
  ...
  (deftest "keeps configured local openai gpt-5.4 entries visible in --local output", async () => {
  ...
  (deftest "marks synthetic codex gpt-5.4 rows as available when provider auth exists", async () => {
  ...
  (deftest "exits with an error when configured-mode listing has no model registry", async () => {
    mock:clearAllMocks();
