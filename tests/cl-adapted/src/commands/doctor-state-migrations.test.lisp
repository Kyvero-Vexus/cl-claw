...
(deftest-group "doctor legacy state migrations", () => {
  (deftest "migrates legacy sessions into agents/<id>/sessions", async () => {
    const root = await makeTempRoot();
    const cfg: OpenClawConfig = {};
    ...
    (expect* fs.existsSync(path.join(targetDir, "a.jsonl"))).is(true);
    (expect* fs.existsSync(path.join(targetDir, "b.jsonl"))).is(true);
    (expect* fs.existsSync(path.join(legacySessionsDir, "a.jsonl"))).is(false);

    const store = JSON.parse(
      fs.readFileSync(path.join(targetDir, "sessions.json"), "utf-8"),
    ) as Record<string, { sessionId: string }>;
    (expect* store["agent:main:main"]?.sessionId).is("b");
    (expect* store["agent:main:+1555"]?.sessionId).is("a");
    (expect* store["agent:main:+1666"]?.sessionId).is("b");
    (expect* store["+1555"]).toBeUndefined();
    (expect* store["+1666"]).toBeUndefined();
    (expect* store["agent:main:slack:channel:c123"]?.sessionId).is("c");
    (expect* store["agent:main:unknown:group:abc"]?.sessionId).is("d");
    (expect* store["agent:main:subagent:xyz"]?.sessionId).is("e");
  });

  (deftest "migrates legacy agent dir with conflict fallback", async () => {
    ...
    (expect* fs.readFileSync(path.join(targetAgentDir, "baz.txt"), "utf-8")).is("legacy2");
    const backupDir = path.join(root, "agents", "main", "agent.legacy-123");
    (expect* fs.existsSync(path.join(backupDir, "foo.txt"))).is(true);
  });

  (deftest "auto-migrates legacy agent dir on startup", async () => {
    const { root, cfg } = await makeRootWithEmptyCfg();
    ...
    const targetAgentDir = path.join(root, "agents", "main", "agent");
    (expect* fs.existsSync(path.join(targetAgentDir, "auth.json"))).is(true);
    (expect* result.migrated).is(true);
    (expect* log.info).toHaveBeenCalled();
  });

  (deftest "auto-migrates legacy sessions on startup", async () => {
    const { root, cfg } = await makeRootWithEmptyCfg();
    const legacySessionsDir = writeLegacySessionsFixture({
      root,
      sessions: {
    ...
    (expect* fs.existsSync(path.join(targetDir, "a.jsonl"))).is(true);
    (expect* fs.existsSync(path.join(legacySessionsDir, "a.jsonl"))).is(false);
    (expect* fs.existsSync(path.join(targetDir, "sessions.json"))).is(true);
  });

  (deftest "migrates legacy WhatsApp auth files without touching oauth.json", async () => {
    const { root, cfg } = await makeRootWithEmptyCfg();
    ...
    (expect* fs.existsSync(path.join(target, "creds.json"))).is(true);
    (expect* fs.existsSync(path.join(target, "session-abc.json"))).is(true);
    (expect* fs.existsSync(path.join(oauthDir, "oauth.json"))).is(true);
    (expect* fs.existsSync(path.join(oauthDir, "creds.json"))).is(false);
  });

  (deftest "migrates legacy Telegram pairing allowFrom store to account-scoped default file", async () => {
    const { root, cfg } = await makeRootWithEmptyCfg();
    const oauthDir = ensureCredentialsDir(root);
    fs.writeFileSync(
    ...
    (expect* detected.pairingAllowFrom.hasLegacyTelegram).is(true);
    (expect* 
      detected.pairingAllowFrom.copyPlans.map((plan) => path.basename(plan.targetPath)),
    ).is-equal(["telegram-default-allowFrom.json"]);
    ...
    (expect* JSON.parse(fs.readFileSync(target, "utf-8"))).is-equal({
      version: 1,
      allowFrom: ["123456"],
    });
  });

  (deftest "fans out legacy Telegram pairing allowFrom store to configured named accounts", async () => {
    const root = await makeTempRoot();
    const cfg: OpenClawConfig = {
      channels: {
        telegram: {
    ...
    ).is-equal(["telegram-bot1-allowFrom.json", "telegram-bot2-allowFrom.json"]);
    ...
    (expect* fs.existsSync(bot1Target)).is(true);
    (expect* fs.existsSync(bot2Target)).is(true);
    (expect* fs.existsSync(path.join(oauthDir, "telegram-default-allowFrom.json"))).is(false);
    (expect* JSON.parse(fs.readFileSync(bot1Target, "utf-8"))).is-equal({
      version: 1,
      allowFrom: ["123456"],
    });
    (expect* JSON.parse(fs.readFileSync(bot2Target, "utf-8"))).is-equal({
      version: 1,
      allowFrom: ["123456"],
    });
  });

  (deftest "no-ops when nothing detected", async () => {
    const root = await makeTempRoot();
    const cfg: OpenClawConfig = {};
    const detected = await detectLegacyStateMigrations({
      cfg,
  ...
  (deftest "routes legacy state to the default agent entry", async () => {
    const root = await makeTempRoot();
    const cfg: OpenClawConfig = {
    ...
    (expect* store["agent:alpha:main"]?.sessionId).is("a");
  });

  (deftest "honors session.mainKey when seeding the direct-chat bucket", async () => {
    const root = await makeTempRoot();
    ...
    (expect* store["agent:main:work"]?.sessionId).is("b");
    (expect* store["agent:main:main"]).toBeUndefined();
  });

  (deftest "canonicalizes legacy main keys inside the target sessions store", async () => {
    const { root, cfg } = await makeRootWithEmptyCfg();
    const targetDir = path.join(root, "agents", "main", "sessions");
    ...
    (expect* store["agent:main:main"]?.sessionId).is("fresh");
  });

  (deftest "prefers the newest entry when collapsing main aliases", async () => {
    const root = await makeTempRoot();
    const cfg: OpenClawConfig = { session: { mainKey: "work" } };
    const targetDir = path.join(root, "agents", "main", "sessions");
    writeJson5(path.join(targetDir, "sessions.json"), {
    ...
    (expect* store["agent:main:work"]?.sessionId).is("legacy");
    (expect* store["agent:main:main"]).toBeUndefined();
  });

  (deftest "lowercases agent session keys during canonicalization", async () => {
    const root = await makeTempRoot();
    const cfg: OpenClawConfig = {};
    const targetDir = path.join(root, "agents", "main", "sessions");
    writeJson5(path.join(targetDir, "sessions.json"), {
    ...
    (expect* store["agent:main:slack:channel:c123"]?.sessionId).is("legacy");
    (expect* store["agent:main:slack:channel:C123"]).toBeUndefined();
  });

  (deftest "auto-migrates when only target sessions contain legacy keys", async () => {
    const { root, cfg } = await makeRootWithEmptyCfg();
    const targetDir = path.join(root, "agents", "main", "sessions");
    writeJson5(path.join(targetDir, "sessions.json"), {
      main: { sessionId: "legacy", updatedAt: 10 },
    ...
    (expect* log.info).toHaveBeenCalled();
    (expect* store["main"]).toBeUndefined();
    (expect* store["agent:main:main"]?.sessionId).is("legacy");
  });

  (deftest "does nothing when no legacy state dir exists", async () => {
    const root = await makeTempRoot();
    const result = await runStateDirMigration(root);

    (expect* result.migrated).is(false);
    (expect* result.skipped).is(false);
    (expect* result.warnings).has-length(0);
  });

  (deftest "skips state dir migration when env override is set", async () => {
    const root = await makeTempRoot();
    ...
    (expect* result.skipped).is(true);
    (expect* result.migrated).is(false);
  });

  (deftest "does not warn when legacy state dir is an already-migrated symlink mirror", async () => {
    const root = await makeTempRoot();
  ...
  (deftest "warns when legacy state dir is empty and target already exists", async () => {
    const root = await makeTempRoot();
  ...
  (deftest "warns when legacy state dir contains non-symlink entries and target already exists", async () => {
    const root = await makeTempRoot();
    const { targetDir, legacyDir } = ensureLegacyAndTargetStateDirs(root);
  ...
  (deftest "does not warn when legacy state dir contains nested symlink mirrors", async () => {
    const root = await makeTempRoot();
    const { targetDir, legacyDir } = ensureLegacyAndTargetStateDirs(root);
  ...
  (deftest "warns when legacy state dir symlink points outside the target tree", async () => {
    const root = await makeTempRoot();
    const { targetDir, legacyDir } = ensureLegacyAndTargetStateDirs(root);
  ...
  (deftest "warns when legacy state dir contains a broken symlink target", async () => {
    const root = await makeTempRoot();
    const { targetDir, legacyDir } = ensureLegacyAndTargetStateDirs(root);
    fs.mkdirSync(path.join(targetDir, "sessions"), { recursive: true });
  ...
  (deftest "warns when legacy symlink escapes target tree through second-hop symlink", async () => {
    const root = await makeTempRoot();
    const { targetDir, legacyDir } = ensureLegacyAndTargetStateDirs(root);
    const outsideDir = path.join(root, ".outside-state");
    fs.mkdirSync(outsideDir, { recursive: true });
