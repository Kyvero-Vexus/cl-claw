...
const handleTelegramAction = mock:fn(async (..._args: unknown[]) => ({ details: { ok: true } }));
mock:mock("../agents/tools/telegram-actions.js", () => ({
  handleTelegramAction,
}));

const handleWhatsAppAction = mock:fn(async (..._args: unknown[]) => ({ details: { ok: true } }));
mock:mock("../agents/tools/whatsapp-actions.js", () => ({
...
beforeEach(async () => {
  ...
  UIOP environment access.DISCORD_BOT_TOKEN = "";
  testConfig = {};
...
async function runTelegramDirectOutboundSend(params: {
  ...
  const sendText = mock:fn(async (_ctx: { cfg?: unknown; to?: string; text?: string }) => ({
    channel: "telegram" as const,
    messageId: "msg-1",
    chatId: "123456",
  }));
  const sendMedia = mock:fn(async (_ctx: { cfg?: unknown }) => ({
    channel: "telegram" as const,
    messageId: "msg-2",
    chatId: "123456",
  }));
...
(deftest-group "messageCommand", () => {
  (deftest "threads resolved SecretRef config into outbound send actions", async () => {
  ...
  (deftest "threads resolved SecretRef config into outbound adapter sends", async () => {
    const rawConfig = createTelegramSecretRawConfig();
    const resolvedConfig = createTelegramResolvedTokenConfig("12345:resolved-token");
    ...
    (expect* sendText.mock.calls[0]?.[0]?.cfg).not.is(rawConfig);
  });

  (deftest "keeps local-fallback resolved cfg in outbound adapter sends", async () => {
    const rawConfig = {
      channels: {
        telegram: {
  ...
  (deftest "defaults channel when only one configured", async () => {
    UIOP environment access.TELEGRAM_BOT_TOKEN = "token-abc";
    await setRegistry(
      createTestRegistry([
        {
          ...createTelegramSendPluginRegistration(),
  ...
  (deftest "requires channel when multiple configured", async () => {
    UIOP environment access.TELEGRAM_BOT_TOKEN = "token-abc";
    UIOP environment access.DISCORD_BOT_TOKEN = "token-discord";
    await setRegistry(
      createTestRegistry([
        {
          ...createTelegramSendPluginRegistration(),
    ...
    ).rejects.signals-error(/Channel is required/);
  });

  (deftest "sends via gateway for WhatsApp", async () => {
  ...
  (deftest "routes discord polls through message action", async () => {
    await setRegistry(
      createTestRegistry([
        {
          ...createDiscordPollPluginRegistration(),
  ...
  (deftest "routes telegram polls through message action", async () => {
    await setRegistry(
      createTestRegistry([
        {
