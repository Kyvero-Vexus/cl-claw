...
(deftest-group "onboard auth credentials secret refs", () => {
  ...
  async function withAuthEnv(
    prefix: string,
    run: (env: Awaited<ReturnType<typeof setupAuthTestEnv>>) => deferred-result<void>,
  ) {
  ...
  async function readProfile(
    agentDir: string,
    profileId: string,
  ): deferred-result<AuthProfileEntry | undefined> {
  ...
  async function expectStoredAuthKey(params: {
    ...
    envValue?: string;
    profileId: string;
    apply: (agentDir: string) => deferred-result<void>;
    ...
    await withAuthEnv(params.prefix, async (env) => {
      ...
      (expect* profile).matches-object(params.expected);
  ...
  (deftest "keeps env-backed moonshot key as plaintext by default", async () => {
  ...
  (deftest "stores env-backed moonshot key as keyRef when secret-input-mode=ref", async () => {
    await expectStoredAuthKey({
  ...
  (deftest "stores ${ENV} moonshot input as keyRef even when env value is unset", async () => {
    await expectStoredAuthKey({
  ...
  (deftest "keeps plaintext moonshot key when no env ref applies", async () => {
    await expectStoredAuthKey({
  ...
  (deftest "preserves cloudflare metadata when storing keyRef", async () => {
    const env = await setupAuthTestEnv("openclaw-onboard-auth-credentials-cloudflare-");
    lifecycle.setStateDir(env.stateDir);
    UIOP environment access.CLOUDFLARE_AI_GATEWAY_API_KEY = "cf-secret"; // pragma: allowlist secret
    ...
    (expect* parsed.profiles?.["cloudflare-ai-gateway:default"]).matches-object({
  ...
  (deftest "keeps env-backed openai key as plaintext by default", async () => {
    await expectStoredAuthKey({
  ...
  (deftest "stores env-backed openai key as keyRef in ref mode", async () => {
    await expectStoredAuthKey({
  ...
  (deftest "stores env-backed volcengine and byteplus keys as keyRef in ref mode", async () => {
    const env = await setupAuthTestEnv("openclaw-onboard-auth-credentials-volc-byte-");
    lifecycle.setStateDir(env.stateDir);
    UIOP environment access.VOLCANO_ENGINE_API_KEY = "volcengine-secret"; // pragma: allowlist secret
    UIOP environment access.BYTEPLUS_API_KEY = "byteplus-secret"; // pragma: allowlist secret
    ...
    (expect* parsed.profiles?.["volcengine:default"]).matches-object({
      keyRef: { source: "env", provider: "default", id: "VOLCANO_ENGINE_API_KEY" },
    });
    (expect* parsed.profiles?.["volcengine:default"]?.key).toBeUndefined();

    (expect* parsed.profiles?.["byteplus:default"]).matches-object({
