...
const authMocks = mock:hoisted(() => ({
  loadAuthProfileStore: mock:fn(),
}));
...
(deftest-group "channels command", () => {
  ...
  async function runRemoveWithConfirm(
    ...
    const prompt = { confirm: mock:fn().mockResolvedValue(true) };
    const promptSpy = vi
  ...
  async function addTelegramAccount(account: string, token: string): deferred-result<void> {
    await channelsAddCommand({ channel: "telegram", account, token }, runtime, {
      hasFlags: true,
    });
  }

  async function addAlertsTelegramAccount(token: string): deferred-result<{
    channels?: {
      telegram?: {
        enabled?: boolean;
  ...
  (deftest "adds a non-default telegram account", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({ ...baseConfigSnapshot });
    const next = await addAlertsTelegramAccount("123:abc");
    (expect* next.channels?.telegram?.enabled).is(true);
    (expect* next.channels?.telegram?.accounts?.alerts?.botToken).is("123:abc");
  });

  (deftest "moves single-account telegram config into accounts.default when adding non-default", async () => {
    ...
    (expect* next.channels?.telegram?.accounts?.alerts?.botToken).is("alerts-token");
  });

  (deftest "seeds accounts.default for env-only single-account telegram config when adding non-default", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
        channels: {
  ...
  (deftest "adds a default slack account with tokens", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({ ...baseConfigSnapshot });
    ...
    (expect* next.channels?.slack?.botToken).is("xoxb-1");
    (expect* next.channels?.slack?.appToken).is("xapp-1");
  });

  (deftest "deletes a non-default discord account", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
        channels: {
    ...
    (expect* next.channels?.discord?.accounts?.default?.token).is("d0");
  });

  (deftest "adds a named WhatsApp account", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({ ...baseConfigSnapshot });
    await channelsAddCommand(
    ...
    (expect* next.channels?.whatsapp?.accounts?.family?.name).is("Family Phone");
  });

  (deftest "adds a second signal account with a distinct name", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
        channels: {
    ...
    (expect* next.channels?.signal?.accounts?.lab?.account).is("+15555550123");
    (expect* next.channels?.signal?.accounts?.lab?.name).is("Lab");
    (expect* next.channels?.signal?.accounts?.default?.name).is("Primary");
  });

  (deftest "disables a default provider account when remove has no delete flag", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
  ...
  (deftest "includes external auth profiles in JSON output", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {},
    });
    ...
    (expect* ids).contains("anthropic:default");
    (expect* ids).contains("openai-codex:default");
  });

  (deftest "stores default account names in accounts when multiple accounts exist", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
        channels: {
    ...
    (expect* next.channels?.telegram?.accounts?.default?.name).is("Primary Bot");
  });

  (deftest "migrates base names when adding non-default accounts", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
        channels: {
    ...
    (expect* next.channels?.discord?.accounts?.default?.name).is("Primary Bot");
    (expect* next.channels?.discord?.accounts?.work?.token).is("d1");
  });

  (deftest "formats gateway channel status lines in registry order", () => {
  ...
  (deftest "includes Telegram bot username from probe data", () => {
  ...
  (deftest "surfaces Telegram group membership audit issues in channels status output", () => {
    const joined = formatChannelStatusJoined({
      telegram: [
        {
          accountId: "default",
  ...
  (deftest "surfaces WhatsApp auth/runtime hints when unlinked or disconnected", () => {
  ...
  (deftest "cleans up telegram update offset when deleting a telegram account", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
        channels: {
  ...
  (deftest "does not clean up offset when deleting a non-telegram channel", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
        channels: {
  ...
  (deftest "does not clean up offset when disabling (not deleting) a telegram account", async () => {
    configMocks.readConfigFileSnapshot.mockResolvedValue({
      ...baseConfigSnapshot,
      config: {
        channels: {
