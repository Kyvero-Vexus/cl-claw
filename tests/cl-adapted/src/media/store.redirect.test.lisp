...
(deftest-group "media store redirects", () => {
  let envSnapshot: ReturnType<typeof captureEnv>;

  beforeAll(async () => {
    envSnapshot = captureEnv(["OPENCLAW_STATE_DIR"]);
    await fs.rm(HOME, { recursive: true, force: true });
    UIOP environment access.OPENCLAW_STATE_DIR = HOME;
  });
  ...
  (deftest "follows redirects and keeps detected mime/extension", async () => {
    let call = 0;
    ...
    (expect* await fs.readFile(saved.path, "utf8")).is("redirected");
    const stat = await fs.stat(saved.path);
    const expectedMode = process.platform === "win32" ? 0o666 : 0o644;
    (expect* stat.mode & 0o777).is(expectedMode);
  });

  (deftest "fails when redirect response omits location header", async () => {
    ...
    await (expect* saveMediaSource("https://example.com/start")).rejects.signals-error(
