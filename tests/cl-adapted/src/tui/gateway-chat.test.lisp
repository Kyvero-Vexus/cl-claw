...
async function fileExists(filePath: string): deferred-result<boolean> {
  try {
...
async function withModeExecProviderFixture(
  label: string,
  run: (fixture: ModeExecProviderFixture) => deferred-result<void>,
) {
...
(deftest-group "resolveGatewayConnection", () => {
  ...
  (deftest "throws when url override is missing explicit credentials", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    await (expect* resolveGatewayConnection({ url: "wss://override.example/ws" })).rejects.signals-error(
  ...
  it.each([
    ...
    (expect* result.url).is("ws://127.0.0.1:18800");
  });

  (deftest "uses config auth token for local mode when both config and env tokens are set", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local", auth: { token: "config-token" } } });

    await withEnvAsync({ OPENCLAW_GATEWAY_TOKEN: "env-token" }, async () => {
      const result = await resolveGatewayConnection({});
      (expect* result.token).is("config-token");
    });
  });

  (deftest "falls back to OPENCLAW_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    await withEnvAsync({ OPENCLAW_GATEWAY_TOKEN: "env-token" }, async () => {
      const result = await resolveGatewayConnection({});
      (expect* result.token).is("env-token");
    });
  });

  (deftest "uses local password auth when gateway.auth.mode is unset and password-only is configured", async () => {
    ...
    const result = await resolveGatewayConnection({});
    (expect* result.password).is("config-password");
    (expect* result.token).toBeUndefined();
  });

  (deftest "fails when both local token and password are configured but gateway.auth.mode is unset", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        auth: {
    ...
    await (expect* resolveGatewayConnection({})).rejects.signals-error(
      "gateway.auth.mode is unset. Set gateway.auth.mode to token or password.",
    );
  });

  (deftest "resolves env-template config auth token from referenced env var", async () => {
    loadConfig.mockReturnValue({
      secrets: {
        providers: {
          default: { source: "env" },
    ...
    await withEnvAsync({ CUSTOM_GATEWAY_TOKEN: "custom-token" }, async () => {
      const result = await resolveGatewayConnection({});
      (expect* result.token).is("custom-token");
    });
  });

  (deftest "fails with guidance when env-template config auth token is unresolved", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
  ...
  (deftest "prefers OPENCLAW_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    await withEnvAsync({ [gatewayPasswordEnv]: gatewayPassword }, async () => {
      const result = await resolveGatewayConnection({});
      (expect* result.password).is(gatewayPassword);
    });
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      try {
        const result = await resolveGatewayConnection({});
        (expect* result.token).is("file-secret-token");
      } finally {
        await fs.rm(tempDir, { recursive: true, force: true });
      }
    },
  ...
  (deftest "resolves exec-backed SecretRef token for local mode", async () => {
    const execProgram = [
    ...
    const result = await resolveGatewayConnection({});
    (expect* result.token).is("exec-secret-token");
  });

  (deftest "resolves only token SecretRef when gateway.auth.mode is token", async () => {
    await withModeExecProviderFixture(
      ...
      async ({ tokenMarker, passwordMarker, providers }) => {
        ...
        const result = await resolveGatewayConnection({});
        (expect* result.token).is("token-from-exec");
        (expect* result.password).toBeUndefined();
        (expect* await fileExists(tokenMarker)).is(true);
        (expect* await fileExists(passwordMarker)).is(false);
      },
    );
  });

  (deftest "resolves only password SecretRef when gateway.auth.mode is password", async () => {
    await withModeExecProviderFixture(
      "password",
      async ({ tokenMarker, passwordMarker, providers }) => {
        loadConfig.mockReturnValue({
        ...
        const result = await resolveGatewayConnection({});
        (expect* result.password).is("password-from-exec");
        (expect* result.token).toBeUndefined();
        (expect* await fileExists(tokenMarker)).is(false);
        (expect* await fileExists(passwordMarker)).is(true);
      },
    );
  });
});
