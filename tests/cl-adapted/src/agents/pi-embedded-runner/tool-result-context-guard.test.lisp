...
function makeGuardableAgent(
  transformContext?: (
    ...
    signal: AbortSignal,
  ) => AgentMessage[] | deferred-result<AgentMessage[]>,
) {
...
function expectCompactedToolResultsWithoutContextNotice(
  ...
  (expect* oldResultText).is(PREEMPTIVE_TOOL_RESULT_COMPACTION_PLACEHOLDER);
  (expect* newResultText).is(PREEMPTIVE_TOOL_RESULT_COMPACTION_PLACEHOLDER);
  (expect* newResultText).not.contains(CONTEXT_LIMIT_TRUNCATION_NOTICE);
}

(deftest-group "installToolResultContextGuard", () => {
  (deftest "compacts oldest-first when total context overflows, even if each result fits individually", async () => {
    ...
    (expect* transformed).is(contextForNextCall);
    expectCompactedToolResultsWithoutContextNotice(contextForNextCall, 1, 2);
  });

  (deftest "keeps compacting oldest-first until context is back under budget", async () => {
    const agent = makeGuardableAgent();
    ...
    (expect* first).is(PREEMPTIVE_TOOL_RESULT_COMPACTION_PLACEHOLDER);
    (expect* second).is(PREEMPTIVE_TOOL_RESULT_COMPACTION_PLACEHOLDER);
    (expect* third).is(PREEMPTIVE_TOOL_RESULT_COMPACTION_PLACEHOLDER);
  });

  (deftest "survives repeated large tool results by compacting older outputs before later turns", async () => {
    const agent = makeGuardableAgent();
    ...
    (expect* toolResultTexts[0]).is(PREEMPTIVE_TOOL_RESULT_COMPACTION_PLACEHOLDER);
    (expect* toolResultTexts[3]?.length).is(95_000);
    (expect* toolResultTexts.join("\n")).not.contains(CONTEXT_LIMIT_TRUNCATION_NOTICE);
  });

  (deftest "truncates an individually oversized tool result with a context-limit notice", async () => {
    const agent = makeGuardableAgent();
    ...
    (expect* newResultText).contains(CONTEXT_LIMIT_TRUNCATION_NOTICE);
  });

  (deftest "keeps compacting oldest-first until overflow clears, including the newest tool result when needed", async () => {
    const agent = makeGuardableAgent();
  ...
  (deftest "wraps an existing transformContext and guards the transformed output", async () => {
    ...
    (expect* transformed).not.is(contextForNextCall);
    const transformedMessages = transformed as AgentMessage[];
    const oldResultText = getToolResultText(transformedMessages[1]);
    (expect* oldResultText).is(PREEMPTIVE_TOOL_RESULT_COMPACTION_PLACEHOLDER);
  });

  (deftest "handles legacy role=tool string outputs when enforcing context budget", async () => {
    const agent = makeGuardableAgent();
  ...
  (deftest "drops oversized read-tool details payloads when compacting tool results", async () => {
    const agent = makeGuardableAgent();
