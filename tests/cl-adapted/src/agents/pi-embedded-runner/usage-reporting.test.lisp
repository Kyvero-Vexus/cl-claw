...
const mockedRunEmbeddedAttempt = mock:mocked(runEmbeddedAttempt);

(deftest-group "runEmbeddedPiAgent usage reporting", () => {
  beforeEach(() => {
    mock:clearAllMocks();
  });

  (deftest "forwards sender identity fields into embedded attempts", async () => {
  ...
  (deftest "reports total usage from the last turn instead of accumulated total", async () => {
