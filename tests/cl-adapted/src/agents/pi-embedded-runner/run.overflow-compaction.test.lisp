...
const mockedPickFallbackThinkingLevel = mock:mocked(pickFallbackThinkingLevel);

(deftest-group "runEmbeddedPiAgent overflow compaction trigger routing", () => {
  beforeEach(() => {
    mock:clearAllMocks();
    mockedGlobalHookRunner.hasHooks.mockImplementation(() => false);
  });

  (deftest "passes precomputed legacy before_agent_start result into the attempt", async () => {
    const legacyResult = {
  ...
  (deftest "passes resolved auth profile into run attempts for context-engine afterTurn propagation", async () => {
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(makeAttemptResult({ promptError: null }));
  ...
  (deftest "passes trigger=overflow when retrying compaction after context overflow", async () => {
    mockOverflowRetrySuccess({
  ...
  (deftest "does not reset compaction attempt budget after successful tool-result truncation", async () => {
  ...
  (deftest "returns retry_limit when repeated retries never converge", async () => {
    ...
    (expect* result.meta.error?.kind).is("retry_limit");
