...
const mockedIsCompactionFailureError = mock:mocked(isCompactionFailureError);
const mockedIsLikelyContextOverflowError = mock:mocked(isLikelyContextOverflowError);

(deftest-group "overflow compaction in run loop", () => {
  beforeEach(() => {
    mock:clearAllMocks();
  ...
  (deftest "retries after successful compaction on context overflow promptError", async () => {
    mockOverflowRetrySuccess({
  ...
  (deftest "retries after successful compaction on likely-overflow promptError variants", async () => {
  ...
  (deftest "returns error if compaction fails", async () => {
    ...
    (expect* result.meta.error?.kind).is("context_overflow");
    (expect* result.payloads?.[0]?.isError).is(true);
    (expect* log.warn).toHaveBeenCalledWith(expect.stringContaining("auto-compaction failed"));
  });

  (deftest "falls back to tool-result truncation and retries when oversized results are detected", async () => {
  ...
  (deftest "retries compaction up to 3 times before giving up", async () => {
    const overflowError = makeOverflowError();
  ...
  (deftest "succeeds after second compaction attempt", async () => {
    const overflowError = makeOverflowError();
  ...
  (deftest "does not attempt compaction for compaction_failure errors", async () => {
    ...
    (expect* mockedRunEmbeddedAttempt).toHaveBeenCalledTimes(1);
    (expect* result.meta.error?.kind).is("compaction_failure");
  });

  (deftest "retries after successful compaction on assistant context overflow errors", async () => {
    mockedRunEmbeddedAttempt
      .mockResolvedValueOnce(
        makeAttemptResult({
          promptError: null,
  ...
  (deftest "does not treat stale assistant overflow as current-attempt overflow when promptError is non-overflow", async () => {
    mockedRunEmbeddedAttempt.mockResolvedValue(
      makeAttemptResult({
    ...
    await (expect* runEmbeddedPiAgent(baseParams)).rejects.signals-error("transport disconnected");

    (expect* mockedCompactDirect).not.toHaveBeenCalled();
    (expect* log.warn).not.toHaveBeenCalledWith(expect.stringContaining("source=assistantError"));
  });

  (deftest "returns an explicit timeout payload when the run times out before producing any reply", async () => {
    mockedRunEmbeddedAttempt.mockResolvedValue(
      makeAttemptResult({
        aborted: true,
        timedOut: true,
    ...
    (expect* result.payloads?.[0]?.isError).is(true);
    (expect* result.payloads?.[0]?.text).contains("timed out");
  });

  (deftest "sets promptTokens from the latest model call usage, not accumulated attempt usage", async () => {
    mockedRunEmbeddedAttempt.mockResolvedValue(
      makeAttemptResult({
        attemptUsage: {
          input: 4_000,
    ...
    (expect* result.meta.agentMeta?.usage?.input).is(4_000);
    (expect* result.meta.agentMeta?.promptTokens).is(2_000);
  });
});
