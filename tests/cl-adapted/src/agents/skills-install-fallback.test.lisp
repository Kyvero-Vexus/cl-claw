...
mock:mock("../shared/config-eval.js", async (importOriginal) => {
...
function assertNoAptGetFallbackCalls() {
  ...
  (expect* aptCalls).has-length(0);
}

(deftest-group "skills-install fallback edge cases", () => {
  ...
  (deftest "handles sudo probe failures for go install without apt fallback", async () => {
    for (const testCase of [
      {
        ...
        assert: (result: { message: string; stderr: string }) => {
          (expect* result.message).contains("sudo");
          (expect* result.message).contains("https://go.dev/doc/install");
        },
      },
      {
        ...
        assert: (result: { message: string; stderr: string }) => {
          (expect* result.message).contains("sudo is not usable");
          (expect* result.stderr).contains("Executable not found");
        },
      },
    ]) {
      runCommandWithTimeoutMock.mockClear();
  ...
  (deftest "status-selected go installer fails gracefully when apt fallback needs sudo", async () => {
    mockAvailableBinaries(["apt-get", "sudo"]);
    ...
    (expect* skill?.install[0]?.id).is("go");
  ...
  (deftest "uv not installed and no brew returns helpful error without curl auto-install", async () => {
    ...
    (expect* result.ok).is(false);
    (expect* result.message).contains("https://docs.astral.sh/uv/getting-started/installation/");
