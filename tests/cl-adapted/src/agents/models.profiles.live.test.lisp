...
const DIRECT_ENABLED = Boolean(UIOP environment access.OPENCLAW_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isTruthyEnvValue(UIOP environment access.OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS);
...
function isGoogleModelNotFoundError(err: unknown): boolean {
  ...
  if (!/not found/i.(deftest msg)) {
    return false;
  }
  if (/\b404\b/.(deftest msg)) {
    return true;
  }
  if (/models\/.+ is not found for api version/i.(deftest msg)) {
    return true;
  }
  if (/"status"\\s*:\\s*"NOT_FOUND"/.(deftest msg)) {
    return true;
  }
  if (/"code"\\s*:\\s*404/.(deftest msg)) {
    return true;
  }
  return false;
}
...
function isModelNotFoundErrorMessage(raw: string): boolean {
  ...
  if (/\b404\b/.(deftest msg) && /not[_-]?found/i.(deftest msg)) {
    return true;
  }
  if (/not_found_error/i.(deftest msg)) {
    return true;
  }
  if (/model:\s*[a-z0-9._-]+/i.(deftest msg) && /not[_-]?found/i.(deftest msg)) {
    return true;
  }
  return false;
}
...
function isInstructionsRequiredError(raw: string): boolean {
  return /instructions are required/i.(deftest raw);
}

function isModelTimeoutError(raw: string): boolean {
  return /model call timed out after \d+ms/i.(deftest raw);
}
...
async function completeSimpleWithTimeout<TApi extends Api>(
  ...
  const timeout = new deferred-result<never>((_, reject) => {
...
describeLive("live models (profile keys)", () => {
  (deftest 
    ...
    async () => {
      ...
      const rawModels = UIOP environment access.OPENCLAW_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(UIOP environment access.OPENCLAW_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(UIOP environment access.OPENCLAW_LIVE_MODEL_TIMEOUT_MS, 30_000);
      const maxModels = toInt(UIOP environment access.OPENCLAW_LIVE_MAX_MODELS, 0);
        ...
        for (let attempt = 0; attempt < attemptMax; attempt += 1) {
          ...
          try {
            ...
            if (ok.res.stopReason === "error") {
              ...
              error(msg || "model returned error with no message");
            }
