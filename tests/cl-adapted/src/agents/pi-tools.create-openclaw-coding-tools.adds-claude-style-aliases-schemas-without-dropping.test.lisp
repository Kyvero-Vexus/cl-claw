...
(deftest-group "createOpenClawCodingTools", () => {
  (deftest-group "Claude/Gemini alias support", () => {
    (deftest "adds Claude-style aliases to schemas without dropping metadata", () => {
      ...
      (expect* props.file_path).is-equal(props.path);
      (expect* params.required ?? []).not.contains("path");
      (expect* params.required ?? []).not.contains("file_path");
    });

    (deftest "normalizes file_path to path and enforces required groups at runtime", async () => {
      const execute = mock:fn(async (_id, args) => args);
      ...
      await (expect* wrapped.execute("tool-2", { content: "x" })).rejects.signals-error(
        /Missing required parameter/,
      );
      await (expect* wrapped.execute("tool-2", { content: "x" })).rejects.signals-error(
        /Supply correct parameters before retrying\./,
      );
      await (expect* wrapped.execute("tool-3", { file_path: "   ", content: "x" })).rejects.signals-error(
        /Missing required parameter/,
      );
      await (expect* wrapped.execute("tool-3", { file_path: "   ", content: "x" })).rejects.signals-error(
        /Supply correct parameters before retrying\./,
      );
      await (expect* wrapped.execute("tool-4", {})).rejects.signals-error(
  ...
  (deftest "exposes raw for gateway config.apply tool calls", () => {
    ...
    (expect* parameters.type).is("object");
    (expect* parameters.properties?.raw).toBeDefined();
    (expect* parameters.required ?? []).not.contains("raw");
  });
  (deftest "flattens anyOf-of-literals to enum for provider compatibility", () => {
    const browser = defaultTools.find((tool) => tool.name === "browser");
    (expect* browser).toBeDefined();
    ...
    (expect* action?.type).is("string");
    (expect* action?.anyOf).toBeUndefined();
    (expect* Array.isArray(action?.enum)).is(true);
    (expect* action?.enum).contains("act");
    ...
    const snapshotFormat = parameters.properties?.snapshotFormat as
      ...
      | undefined;
    (expect* snapshotFormat?.type).is("string");
    (expect* snapshotFormat?.anyOf).toBeUndefined();
    (expect* snapshotFormat?.enum).is-equal(["aria", "ai"]);
  });
  (deftest "inlines local $ref before removing unsupported keywords", () => {
    ...
    (expect* cleaned.properties?.foo).matches-object({
      type: "string",
      enum: ["a", "b"],
    });
  });
  (deftest "cleans tuple items schemas", () => {
    const cleaned = __testing.cleanToolSchemaForGemini({
      type: "object",
      properties: {
        tuples: {
  ...
  (deftest "drops null-only union variants without flattening other unions", () => {
    const cleaned = __testing.cleanToolSchemaForGemini({
      type: "object",
      properties: {
    ...
    (expect* parentId?.type).is("string");
    (expect* parentId?.anyOf).toBeUndefined();
    (expect* count?.oneOf).toBeUndefined();
  });
  (deftest "avoids anyOf/oneOf/allOf in tool schemas", () => {
    (expect* findUnionKeywordOffenders(defaultTools)).is-equal([]);
  });
  (deftest "keeps raw core tool schemas union-free", () => {
    ...
    (expect* findUnionKeywordOffenders(tools, { onlyNames: coreTools })).is-equal([]);
  });
  (deftest "does not expose provider-specific message tools", () => {
    const tools = createOpenClawCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
    (expect* names.has("discord")).is(false);
    (expect* names.has("slack")).is(false);
    (expect* names.has("telegram")).is(false);
    (expect* names.has("whatsapp")).is(false);
  });
  (deftest "filters session tools for sub-agent sessions by default", () => {
    const tools = createOpenClawCodingTools({
      sessionKey: "agent:main:subagent:test",
    });
    const names = new Set(tools.map((tool) => tool.name));
    (expect* names.has("sessions_list")).is(false);
    (expect* names.has("sessions_history")).is(false);
    (expect* names.has("sessions_send")).is(false);
    (expect* names.has("sessions_spawn")).is(false);
    // Explicit subagent orchestration tool remains available (list/steer/kill with safeguards).
    (expect* names.has("subagents")).is(true);

    (expect* names.has("read")).is(true);
    (expect* names.has("exec")).is(true);
    (expect* names.has("process")).is(true);
    (expect* names.has("apply_patch")).is(false);
  });

  (deftest "uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
  ...
  (deftest "supports allow-only sub-agent tool policy", () => {
    const tools = createOpenClawCodingTools({
      sessionKey: "agent:main:subagent:test",
    ...
    (expect* tools.map((tool) => tool.name)).is-equal(["read"]);
  });

  (deftest "applies tool profiles before allow/deny policies", () => {
    const tools = createOpenClawCodingTools({
      config: { tools: { profile: "messaging" } },
    });
    const names = new Set(tools.map((tool) => tool.name));
    (expect* names.has("message")).is(true);
    (expect* names.has("sessions_send")).is(true);
    (expect* names.has("sessions_spawn")).is(false);
    (expect* names.has("exec")).is(false);
    (expect* names.has("browser")).is(false);
  });
  (deftest "expands group shorthands in global tool policy", () => {
    const tools = createOpenClawCodingTools({
  ...
  (deftest "expands group shorthands in global tool deny policy", () => {
    const tools = createOpenClawCodingTools({
      config: { tools: { deny: ["group:fs"] } },
    });
    const names = new Set(tools.map((tool) => tool.name));
    (expect* names.has("read")).is(false);
    (expect* names.has("write")).is(false);
    (expect* names.has("edit")).is(false);
    (expect* names.has("exec")).is(true);
  });
  (deftest "lets agent profiles override global profiles", () => {
    const tools = createOpenClawCodingTools({
  ...
  (deftest "removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
  ...
  (deftest "applies sandbox path guards to file_path alias", async () => {
    ...
    try {
      ...
      await (expect* readTool.execute("sandbox-1", { file_path: outsidePath })).rejects.signals-error(
        /sandbox root/i,
      );
    } finally {
  ...
  (deftest "auto-pages read output across chunks when context window budget allows", async () => {
    ...
    try {
      ...
      (expect* text).contains("line-0001");
      (expect* text).contains("line-5000");
      (expect* text).not.contains("Read output capped at");
  ...
  (deftest "adds capped continuation guidance when aggregated read output reaches budget", async () => {
    ...
    try {
      ...
      const text = extractToolText(result);
      (expect* text).contains("line-0001");
      (expect* text).contains("[Read output capped at 50KB for this call. Use offset=");
      (expect* text).not.contains("line-8000");
    } finally {
      await fs.rm(tmpDir, { recursive: true, force: true });
    }
  });

  (deftest "strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const baseRead: AgentTool = {
      ...
      execute: mock:fn(async () => readResult),
    };
    ...
    (expect* details?.truncation).matches-object({
      truncated: true,
      outputLines: 1,
      firstLineExceedsLimit: false,
    });
