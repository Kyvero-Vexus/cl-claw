...
mock:mock("./auth-profiles.js", () => ({
  ...
  getSoonestCooldownExpiry: mock:fn(),
  isProfileInCooldown: mock:fn(),
  resolveProfilesUnavailableReason: mock:fn(),
...
const mockedEnsureAuthProfileStore = mock:mocked(ensureAuthProfileStore);
const mockedGetSoonestCooldownExpiry = mock:mocked(getSoonestCooldownExpiry);
const mockedIsProfileInCooldown = mock:mocked(isProfileInCooldown);
const mockedResolveProfilesUnavailableReason = mock:mocked(resolveProfilesUnavailableReason);
const mockedResolveAuthProfileOrder = mock:mocked(resolveAuthProfileOrder);
...
function expectFallbackUsed(
  ...
  (expect* result.attempts[0]?.reason).is("rate_limit");
}
...
function expectPrimaryProbeSuccess(
  ...
  (expect* result.result).is(expectedResult);
  (expect* run).toHaveBeenCalledTimes(1);
...
async function expectProbeFailureFallsBack({
  ...
  const run = mock:fn().mockRejectedValueOnce(probeError).mockResolvedValue("fallback-ok");
  ...
  (expect* result.result).is("fallback-ok");
...
(deftest-group "runWithModelFallback – probe logic", () => {
  ...
  (deftest "skips primary model when far from cooldown expiry (30 min remaining)", async () => {
    const cfg = makeCfg();
    ...
    const run = mock:fn().mockResolvedValue("ok");
  ...
  (deftest "uses inferred unavailable reason when skipping a cooldowned primary model", async () => {
    const cfg = makeCfg();
    const expiresIn30Min = NOW + 30 * 60 * 1000;
    mockedGetSoonestCooldownExpiry.mockReturnValue(expiresIn30Min);
  ...
  (deftest "probes primary model when within 2-min margin of cooldown expiry", async () => {
    const cfg = makeCfg();
    ...
    const run = mock:fn().mockResolvedValue("probed-ok");

    const result = await runPrimaryCandidate(cfg, run);
    expectPrimaryProbeSuccess(result, run, "probed-ok");
  });

  (deftest "probes primary model when cooldown already expired", async () => {
    const cfg = makeCfg();
    ...
    const run = mock:fn().mockResolvedValue("recovered");

    const result = await runPrimaryCandidate(cfg, run);
    expectPrimaryProbeSuccess(result, run, "recovered");
  });

  (deftest "attempts non-primary fallbacks during rate-limit cooldown after primary probe failure", async () => {
  ...
  (deftest "attempts non-primary fallbacks during overloaded cooldown after primary probe failure", async () => {
    await expectProbeFailureFallsBack({
      reason: "overloaded",
  ...
  (deftest "throttles probe when called within 30s interval", async () => {
    const cfg = makeCfg();
  ...
  (deftest "allows probe when 30s have passed since last probe", async () => {
    const cfg = makeCfg();
    const almostExpired = NOW + 30 * 1000;
    mockedGetSoonestCooldownExpiry.mockReturnValue(almostExpired);
  ...
  (deftest "handles non-finite soonest safely (treats as probe-worthy)", async () => {
    const cfg = makeCfg();

    // Return Infinity — should be treated as "probe" per the guard
    mockedGetSoonestCooldownExpiry.mockReturnValue(Infinity);

    const run = mock:fn().mockResolvedValue("ok-infinity");

    const result = await runPrimaryCandidate(cfg, run);
    expectPrimaryProbeSuccess(result, run, "ok-infinity");
  });

  (deftest "handles NaN soonest safely (treats as probe-worthy)", async () => {
    const cfg = makeCfg();

    mockedGetSoonestCooldownExpiry.mockReturnValue(NaN);

    const run = mock:fn().mockResolvedValue("ok-nan");

    const result = await runPrimaryCandidate(cfg, run);
    expectPrimaryProbeSuccess(result, run, "ok-nan");
  });

  (deftest "handles null soonest safely (treats as probe-worthy)", async () => {
    const cfg = makeCfg();

    mockedGetSoonestCooldownExpiry.mockReturnValue(null);

    const run = mock:fn().mockResolvedValue("ok-null");

    const result = await runPrimaryCandidate(cfg, run);
    expectPrimaryProbeSuccess(result, run, "ok-null");
  });

  (deftest "single candidate skips with rate_limit and exhausts candidates", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
    ...
    const run = mock:fn().mockResolvedValue("unreachable");
  ...
  (deftest "scopes probe throttling by agentDir to avoid cross-agent suppression", async () => {
    const cfg = makeCfg();
    const almostExpired = NOW + 30 * 1000;
    mockedGetSoonestCooldownExpiry.mockReturnValue(almostExpired);
  ...
  (deftest "skips billing-cooldowned primary when no fallback candidates exist", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
    ...
    await (expect* 
      runWithModelFallback({
        ...
        model: "gpt-4.1-mini",
        fallbacksOverride: [],
        run: mock:fn().mockResolvedValue("billing-recovered"),
      }),
    ).rejects.signals-error("All models failed");
  });

  (deftest "probes billing-cooldowned primary with fallbacks when near cooldown expiry", async () => {
    const cfg = makeCfg();
    // Cooldown expires in 1 minute — within 2-min probe margin
    const expiresIn1Min = NOW + 60 * 1000;
    mockedGetSoonestCooldownExpiry.mockReturnValue(expiresIn1Min);
    ...
    const run = mock:fn().mockResolvedValue("billing-probe-ok");

    const result = await runPrimaryCandidate(cfg, run);

    (expect* result.result).is("billing-probe-ok");
    (expect* run).toHaveBeenCalledTimes(1);
    (expect* run).toHaveBeenCalledWith("openai", "gpt-4.1-mini", {
      allowTransientCooldownProbe: true,
    });
  ...
  (deftest "skips billing-cooldowned primary with fallbacks when far from cooldown expiry", async () => {
    const cfg = makeCfg();
    const expiresIn30Min = NOW + 30 * 60 * 1000;
    mockedGetSoonestCooldownExpiry.mockReturnValue(expiresIn30Min);
    mockedResolveProfilesUnavailableReason.mockReturnValue("billing");
