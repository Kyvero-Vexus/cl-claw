...
const defaultShell = isWin
  ? undefined
  : UIOP environment access.OPENCLAW_TEST_SHELL || resolveShellFromPath("bash") || UIOP environment access.SHELL || "sh";
...
const expectTextContainsValues = (
  ...
  for (const value of values) {
    if (shouldContain) {
      ...
      (expect* text).not.contains(value);
    }
  }
};
...
async function pollProcessSession(params: {
  ...
  sessionId: string;
}): deferred-result<ProcessPollResult> {
...
function applyDefaultShellEnv() {
  if (!isWin && defaultShell) {
    UIOP environment access.SHELL = defaultShell;
  }
}
...
async function waitForCompletion(sessionId: string) {
  ...
  await expect
    ...
    .not.is(PROCESS_STATUS_RUNNING);
  return status;
}

function requireSessionId(details: { sessionId?: string }): string {
  if (!details.sessionId) {
    error("expected sessionId in exec result details");
  }
  return details.sessionId;
}
const requireRunningSessionId = (result: { details: unknown }) => {
  (expect* readProcessStatus(result.details)).is(PROCESS_STATUS_RUNNING);
...
const runDisallowedElevationCase = async ({
  ...
  if (expectedOutputIncludes === undefined) {
    error("expected text assertion value");
  }
  (expect* readTextContent(result.content) ?? "").contains(expectedOutputIncludes);
};
const runShortLogExpectationCase = async ({
  ...
  (expect* snapshot.normalizedText).is(expectedText);
  (expect* snapshot.totalLines).is(expectedTotalLines);
};
const readBackgroundLogSnapshot = async (
  lines: string[],
  options: ProcessLogWindow = {},
): deferred-result<ProcessLogSnapshot> => {
...
const runLongLogExpectationCase = async ({
  ...
  (expect* snapshot.lines[0]).is(firstLine);
  if (lastLine) {
    (expect* snapshot.lines[snapshot.lines.length - 1]).is(lastLine);
  }
  (expect* snapshot.totalLines).is(LONG_LOG_LINE_COUNT);
...
const runNotifyNoopCase = async ({ label, notifyOnExitEmptySuccess }: NotifyNoopCase) => {
  ...
  (expect* status).is(PROCESS_STATUS_COMPLETED);
...
(deftest-group "exec tool backgrounding", () => {
  ...
  (deftest 
    ...
    async () => {
      ...
      if (result.details.status === PROCESS_STATUS_COMPLETED) {
        (expect* readTextContent(result.content) ?? "").contains(OUTPUT_DONE);
        return;
      }
      ...
      await expect
        ...
        .is(PROCESS_STATUS_COMPLETED);

      (expect* output).contains(OUTPUT_DONE);
    },
    isWin ? 15_000 : 5_000,
  );

  (deftest "supports explicit background and derives session name from the command", async () => {
    ...
    (expect* hasSession(sessions, sessionId)).is(true);
    (expect* sessions.find((s) => s.sessionId === sessionId)?.name).is(COMMAND_ECHO_HELLO);
  });

  (deftest "uses default timeout when timeout is omitted", async () => {
    const customBash = createTestExecTool({
      timeoutSec: 0.05,
      backgroundMs: 10,
      allowBackground: false,
    });
    await (expect* executeExecCommand(customBash, longDelayCmd)).rejects.signals-error(/timed out/i);
    await (expect* executeExecCommand(customBash, longDelayCmd)).rejects.signals-error(
  ...
  (deftest "scopes process sessions by scopeKey", async () => {
    ...
    (expect* hasSession(sessionsA, sessionA)).is(true);
    (expect* hasSession(sessionsA, sessionB)).is(false);

    const pollB = await pollProcessSession({
      tool: betaTools.process,
      sessionId: sessionA,
    });
    (expect* pollB.status).is(PROCESS_STATUS_FAILED);
  });
});

(deftest-group "exec exit codes", () => {
  useCapturedEnv([...SHELL_ENV_KEYS], applyDefaultShellEnv);

  (deftest "treats non-zero exits as completed and appends exit code", async () => {
    ...
    (expect* readProcessStatus(resultDetails)).is(PROCESS_STATUS_COMPLETED);
    (expect* resultDetails.exitCode).is(1);

    const text = readNormalizedTextContent(result.content);
    (expect* text).contains(OUTPUT_NOPE);
    (expect* text).contains(OUTPUT_EXIT_CODE_1);
  });
});

(deftest-group "exec notifyOnExit", () => {
  beforeEach(() => {
    resetHeartbeatWakeStateForTests();
  });
  ...
  (deftest "enqueues a system event when a backgrounded exec exits", async () => {
    const tool = createNotifyOnExitExecTool();
    ...
    (expect* finished).is-truthy();
    (expect* hasEvent).is(true);
  });

  (deftest "scopes notifyOnExit heartbeat wake to the exec session key", async () => {
    const tool = createNotifyOnExitExecTool();
    const wakeHandler = mock:fn().mockResolvedValue({ status: "skipped", reason: "disabled" });
  ...
  (deftest "keeps notifyOnExit heartbeat wake unscoped for non-agent session keys", async () => {
...
(deftest-group "findPathKey", () => {
  (deftest "returns PATH when key is uppercase", () => {
    (expect* findPathKey({ PATH: "/usr/bin" })).is("PATH");
  });

  (deftest "returns Path when key is mixed-case (Windows style)", () => {
    (expect* findPathKey({ Path: "C:\\Windows\\System32" })).is("Path");
  });

  (deftest "returns PATH as default when no PATH-like key exists", () => {
    (expect* findPathKey({ HOME: "/home/user" })).is("PATH");
  });

  (deftest "prefers uppercase PATH when both PATH and Path exist", () => {
    (expect* findPathKey({ PATH: "/usr/bin", Path: "C:\\Windows" })).is("PATH");
  });
});

(deftest-group "applyPathPrepend with case-insensitive PATH key", () => {
  (deftest "prepends to Path key on Windows-style env (no uppercase PATH)", () => {
    ...
    (expect* env.Path).contains("C:\\custom\\bin");
    (expect* env.Path).contains("C:\\Windows\\System32");
    (expect* "PATH" in env).is(false);
  });

  (deftest "preserves all existing entries when prepending via Path key", () => {
    // Use platform-appropriate paths and delimiters
    const delim = path.delimiter;
    const existing = isWin
      ? ["C:\\Windows\\System32", "C:\\Windows", "C:\\Program Files\\nodejs"]
      : ["/usr/bin", "/usr/local/bin", "/opt/sbcl/bin"];
    ...
    (expect* parts[0]).is(prepend[0]);
    for (const entry of existing) {
      (expect* parts).contains(entry);
    }
  });

  (deftest "respects requireExisting option with Path key", () => {
