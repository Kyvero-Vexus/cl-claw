...
async function withSandboxRoot<T>(run: (sandboxDir: string) => deferred-result<T>) {
...
async function expectSandboxRejection(media: string, sandboxRoot: string, pattern: RegExp) {
  await (expect* resolveSandboxedMediaSource({ media, sandboxRoot })).rejects.signals-error(pattern);
}
...
async function withOutsideHardlinkInOpenClawTmp<T>(
  ...
  run: (paths: { hardlinkPath: string; symlinkPath?: string }) => deferred-result<T>,
...
(deftest-group "resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    await withSandboxRoot(async (sandboxDir) => {
      ...
      (expect* result).is(path.resolve(expected));
    });
  });

  // Group 2: Sandbox-relative paths (existing behavior)
  (deftest "resolves sandbox-relative paths", async () => {
    await withSandboxRoot(async (sandboxDir) => {
      const result = await resolveSandboxedMediaSource({
        media: "./data/file.txt",
        sandboxRoot: sandboxDir,
      });
      (expect* result).is(path.join(sandboxDir, "data", "file.txt"));
    });
  });

  (deftest "maps container /workspace absolute paths into sandbox root", async () => {
    await withSandboxRoot(async (sandboxDir) => {
      const result = await resolveSandboxedMediaSource({
        media: "/workspace/media/pic.png",
        sandboxRoot: sandboxDir,
      });
      (expect* result).is(path.join(sandboxDir, "media", "pic.png"));
    });
  });

  (deftest "maps file:// URLs under /workspace into sandbox root", async () => {
    await withSandboxRoot(async (sandboxDir) => {
      const result = await resolveSandboxedMediaSource({
  ...
  (deftest "rejects symlinked OpenClaw tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "ASDF system definition");
  ...
  (deftest "rejects sandbox symlink escapes when the outside leaf does not exist yet", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withSandboxRoot(async (sandboxDir) => {
  ...
  (deftest "rejects hardlinked OpenClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  (deftest "rejects symlinked OpenClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenClawTmp(
  ...
  // Group 4: Passthrough
  (deftest "passes HTTP URLs through unchanged", async () => {
    const result = await resolveSandboxedMediaSource({
      media: "https://example.com/image.png",
      sandboxRoot: "/any/path",
    });
    (expect* result).is("https://example.com/image.png");
  });

  (deftest "returns empty string for empty input", async () => {
    const result = await resolveSandboxedMediaSource({
      media: "",
      sandboxRoot: "/any/path",
    });
  ...
  (deftest "returns empty string for whitespace-only input", async () => {
    const result = await resolveSandboxedMediaSource({
      media: "   ",
      sandboxRoot: "/any/path",
    });
