...
(deftest-group "computeSandboxConfigHash", () => {
  (deftest "ignores object key order", () => {
    const shared = {
  ...
  it.each(ORDER_SENSITIVE_ARRAY_CASES)("treats $field order as significant", (testCase) => {
    ...
    (expect* left).not.is(right);
  });
});

(deftest-group "computeSandboxBrowserConfigHash", () => {
  (deftest "treats docker bind order as significant", () => {
    const shared = {
      browser: {
        cdpPort: 9222,
  ...
  (deftest "changes when security epoch changes", () => {
    const shared = {
      docker: createDockerConfig(),
      browser: {
        cdpPort: 9222,
  ...
  (deftest "changes when cdp source range changes", () => {
    const shared = {
      docker: createDockerConfig(),
      browser: {
        cdpPort: 9222,
