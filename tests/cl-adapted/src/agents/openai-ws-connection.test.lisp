...
// ─────────────────────────────────────────────────────────────────────────────
// Mock WebSocket (hoisted so mock:mock factory can reference it)
// ─────────────────────────────────────────────────────────────────────────────

// mock:mock() factories are hoisted before ES module imports are resolved.
// mock:hoisted() allows us to define values that are available to both the
...
const { MockWebSocket } = mock:hoisted(() => {
...
function lastSocket(): MockWS {
  ...
  if (!sock) {
    error("No MockWebSocket instance created");
  }
  return sock;
}
...
): deferred-result<{ manager: OpenAIWebSocketManager; sock: MockWS }> {
  const manager = buildManager(opts);
  const sock = await connectManagerAndGetSocket(manager);
  return { manager, sock };
}

function connectIgnoringFailure(manager: OpenAIWebSocketManager): deferred-result<void> {
...
(deftest-group "OpenAIWebSocketManager", () => {
  beforeEach(() => {
    MockWebSocket.reset();
    mock:useFakeTimers();
  });
  ...
  (deftest-group "connect()", () => {
    (deftest "opens a WebSocket with Bearer auth header", async () => {
      const manager = buildManager();
      const connectPromise = manager.connect("sk-test-key");

      const sock = lastSocket();
      (expect* sock.url).is("wss://api.openai.com/v1/responses");
      (expect* sock.options).matches-object({
    ...
    (deftest "resolves when the connection opens", async () => {
      const manager = buildManager();
      const connectPromise = manager.connect("sk-test");
    ...
    (deftest "rejects when the initial connection fails (maxRetries=0)", async () => {
      const manager = buildManager({ maxRetries: 0 });
      const connectPromise = manager.connect("sk-test");

      lastSocket().simulateError(new Error("ECONNREFUSED"));

      await (expect* connectPromise).rejects.signals-error("ECONNREFUSED");
    });

    (deftest "sets isConnected() to true after open", async () => {
      const manager = buildManager();
      (expect* manager.isConnected()).is(false);

      const connectPromise = manager.connect("sk-test");
      lastSocket().simulateOpen();
      await connectPromise;

      (expect* manager.isConnected()).is(true);
    });

    (deftest "uses the custom URL when provided", async () => {
      const manager = buildManager({ url: "ws://localhost:9999/v1/responses" });
      const connectPromise = manager.connect("sk-test");

      (expect* lastSocket().url).is("ws://localhost:9999/v1/responses");
      lastSocket().simulateOpen();
      await connectPromise;
    });
  });
  ...
  (deftest-group "send()", () => {
    (deftest "sends a JSON-serialized event over the socket", async () => {
      ...
      (expect* sock.sentMessages).has-length(1);
      (expect* JSON.parse(sock.sentMessages[0] ?? "{}")).is-equal(event);
    });

    (deftest "throws if the connection is not open", () => {
      const manager = buildManager();
      const event: ClientEvent = {
        type: "response.create",
        model: "gpt-5.2",
      };
      (expect* () => manager.send(event)).signals-error(/cannot send/);
    });

    (deftest "includes previous_response_id when provided", async () => {
      const { manager, sock } = await createConnectedManager();
      ...
      (expect* sent.previous_response_id).is("resp_abc123");
    });
  });

  // ─── onMessage() ───────────────────────────────────────────────────────────

  (deftest-group "onMessage()", () => {
    (deftest "calls handler for each incoming message", async () => {
      const { manager, sock } = await createConnectedManager();
      ...
      (expect* received).has-length(1);
      (expect* received[0]).is-equal(deltaEvent);
    });

    (deftest "returns an unsubscribe function that stops delivery", async () => {
      const { manager, sock } = await createConnectedManager();
    ...
    (deftest "supports multiple simultaneous handlers", async () => {
      const { manager, sock } = await createConnectedManager();
      ...
      (expect* calls.toSorted((a, b) => a - b)).is-equal([1, 2]);
    });
  });

  // ─── previousResponseId ────────────────────────────────────────────────────

  (deftest-group "previousResponseId", () => {
    (deftest "starts as null", () => {
      (expect* new OpenAIWebSocketManager().previousResponseId).toBeNull();
    });

    (deftest "is updated when a response.completed event is received", async () => {
      const { manager, sock } = await createConnectedManager();
      ...
      (expect* manager.previousResponseId).is("resp_done_42");
    });

    (deftest "tracks the most recent completed response", async () => {
      const { manager, sock } = await createConnectedManager();
      ...
      (expect* manager.previousResponseId).is("resp_2");
    });

    (deftest "is not updated for non-completed events", async () => {
      const { manager, sock } = await createConnectedManager();
  ...
  (deftest-group "isConnected()", () => {
    (deftest "returns false before connect", () => {
      (expect* buildManager().isConnected()).is(false);
    });

    (deftest "returns true while open", async () => {
      const manager = buildManager();
    ...
    (deftest "returns false after close()", async () => {
      const manager = buildManager();
      const p = manager.connect("sk-test");
      lastSocket().simulateOpen();
      await p;
  ...
  (deftest-group "close()", () => {
    (deftest "marks the manager as disconnected", async () => {
      const manager = buildManager();
      const p = manager.connect("sk-test");
      lastSocket().simulateOpen();
      await p;
    ...
    (deftest "prevents reconnect after explicit close", async () => {
      const manager = buildManager();
      const p = manager.connect("sk-test");
      const sock = lastSocket();
      sock.simulateOpen();
      ...
      (expect* MockWebSocket.instances.length).is(socketCountBefore);
    });

    (deftest "is safe to call before connect()", () => {
      const manager = buildManager();
      (expect* () => manager.close()).not.signals-error();
    });
  });

  // ─── Auto-reconnect ────────────────────────────────────────────────────────

  (deftest-group "auto-reconnect", () => {
    (deftest "reconnects on unexpected close", async () => {
      ...
      await mock:advanceTimersByTimeAsync(15);

      // A new socket should have been created
      (expect* MockWebSocket.instances.length).toBeGreaterThan(instancesBefore);
      (expect* lastSocket()).not.is(sock1);
    });

    (deftest "stops retrying after maxRetries", async () => {
    ...
    (deftest "does not double-count retries when error and close both fire on a reconnect attempt", async () => {
    ...
    (deftest "resets retry count after a successful reconnect", async () => {
  ...
  (deftest-group "warmUp()", () => {
    (deftest "sends a response.create event with generate: false", async () => {
      const { manager, sock } = await createConnectedManager();
      ...
      (expect* sent["generate"]).is(false);
      (expect* sent["model"]).is("gpt-5.2");
      (expect* sent["instructions"]).is("You are helpful.");
    });

    (deftest "includes tools when provided", async () => {
      const { manager, sock } = await createConnectedManager();
      ...
      const sent = JSON.parse(sock.sentMessages[0] ?? "{}") as Record<string, unknown>;
      (expect* sent["tools"]).has-length(1);
      (expect* (sent["tools"] as Array<{ function?: { name?: string } }>)[0]?.function?.name).is(
        "exec",
      );
    });
  });
  ...
  (deftest-group "error handling", () => {
    (deftest "emits error event on malformed JSON message", async () => {
      const manager = buildManager();
      const sock = await connectManagerAndGetSocket(manager);
      const errors = attachErrorCollector(manager);
      ...
      (expect* errors[0]?.message).contains("failed to parse message");
    });

    (deftest "emits error event when message has no type field", async () => {
      const manager = buildManager();
      const sock = await connectManagerAndGetSocket(manager);
      const errors = attachErrorCollector(manager);
      ...
      (expect* errors).has-length(1);
      (expect* errors[0]?.message).contains('no "type" field');
    });

    (deftest "emits error event on WebSocket socket error", async () => {
      const manager = buildManager({ maxRetries: 0 });
      ...
      (expect* errors.some((e) => e.message === "SSL handshake failed")).is(true);
    });

    (deftest "handles multiple successive socket errors without crashing", async () => {
      const manager = buildManager({ maxRetries: 0 });
      const p = connectIgnoringFailure(manager);
      const errors = attachErrorCollector(manager);
      ...
      (expect* errors.some((e) => e.message === "first error")).is(true);
      (expect* errors.some((e) => e.message === "second error")).is(true);
    });
  });

  // ─── Integration: full multi-turn sequence ────────────────────────────────

  (deftest-group "full turn sequence", () => {
    (deftest "tracks previous_response_id across turns and sends continuation correctly", async () => {
      const { manager, sock } = await createConnectedManager();
      ...
      (expect* manager.previousResponseId).is("resp_1");
      (expect* received).has-length(3);
      ...
      (expect* lastSent.previous_response_id).is("resp_1");
      (expect* lastSent.input).is-equal([
