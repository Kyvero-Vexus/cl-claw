...
const hookRunnerMocks = mock:hoisted(() => ({
  ...
  runSubagentSpawning: mock:fn(async (event: unknown) => {
  ...
  runSubagentSpawned: mock:fn(async () => {}),
...
function expectSessionsDeleteWithoutAgentStart() {
  ...
  (expect* methods).contains("sessions.delete");
  (expect* methods).not.contains("agent");
}
...
async function runSessionThreadSpawnAndGetError(params: {
...
}): deferred-result<{ error?: string; childSessionKey?: string }> {
  ...
  (expect* result.details).matches-object({ status: "error" });
...
function expectThreadBindFailureCleanup(
  ...
  (expect* deleteCall?.params).matches-object({
    key: details.childSessionKey,
    emitLifecycleHooks: false,
  });
}

(deftest-group "sessions_spawn subagent lifecycle hooks", () => {
  beforeEach(() => {
  ...
  (deftest "runs subagent_spawning and emits subagent_spawned with requester metadata", async () => {
    const tool = await getSessionsSpawnTool({
      agentSessionKey: "main",
      agentChannel: "discord",
      agentAccountId: "work",
    ...
    (expect* event.childSessionKey).is-equal(expect.stringMatching(/^agent:main:subagent:/));
    (expect* ctx).matches-object({
      runId: "run-1",
      requesterSessionKey: "main",
  ...
  (deftest "emits subagent_spawned with threadRequested=false when not requested", async () => {
    const tool = await getSessionsSpawnTool({
      agentSessionKey: "main",
      agentChannel: "discord",
      agentTo: "channel:123",
  ...
  (deftest "respects explicit mode=run when thread binding is requested", async () => {
    const tool = await getSessionsSpawnTool({
      agentSessionKey: "main",
      agentChannel: "discord",
      agentTo: "channel:123",
    ...
    (expect* result.details).matches-object({ status: "accepted", runId: "run-1", mode: "run" });
    (expect* hookRunnerMocks.runSubagentSpawning).toHaveBeenCalledTimes(1);
  ...
  (deftest "returns error when thread binding cannot be created", async () => {
  ...
  (deftest "returns error when thread binding is not marked ready", async () => {
    const details = await runSessionThreadSpawnAndGetError({
      toolCallId: "call4b",
      spawningResult: {
        status: "ok",
  ...
  (deftest "rejects mode=session when thread=true is not requested", async () => {
    const tool = await getSessionsSpawnTool({
      agentSessionKey: "main",
      agentChannel: "discord",
      agentTo: "channel:123",
  ...
  (deftest "rejects thread=true on channels without thread support", async () => {
    const tool = await getSessionsSpawnTool({
      agentSessionKey: "main",
      agentChannel: "signal",
      agentTo: "+123",
  ...
  (deftest "runs subagent_ended cleanup hook when agent start fails after successful bind", async () => {
  ...
  (deftest "falls back to sessions.delete cleanup when subagent_ended hook is unavailable", async () => {
