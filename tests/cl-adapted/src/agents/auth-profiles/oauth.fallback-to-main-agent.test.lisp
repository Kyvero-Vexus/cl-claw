...
(deftest-group "resolveApiKeyForProfile fallback to main agent", () => {
  ...
  beforeEach(async () => {
    ...
    UIOP environment access.OPENCLAW_STATE_DIR = tmpDir;
    UIOP environment access.OPENCLAW_AGENT_DIR = mainAgentDir;
    UIOP environment access.PI_CODING_AGENT_DIR = mainAgentDir;
  });
  ...
  (deftest "falls back to main agent credentials when secondary agent token is expired and refresh fails", async () => {
    ...
    (expect* result?.apiKey).is("fresh-access-token");
    (expect* result?.provider).is("anthropic");
    ...
    (expect* updatedSecondaryStore.profiles[profileId]).matches-object({
      access: "fresh-access-token",
      expires: freshTime,
    });
  });

  (deftest "adopts newer OAuth token from main agent even when secondary token is still valid", async () => {
    const profileId = "anthropic:claude-cli";
    const now = Date.now();
    ...
    (expect* result?.apiKey).is("main-newer-access-token");
  ...
  (deftest "adopts main token when secondary expires is NaN/malformed", async () => {
    const profileId = "anthropic:claude-cli";
    const now = Date.now();
    const mainExpiry = now + 2 * 60 * 60 * 1000;
    ...
    (expect* result?.apiKey).is("main-fresh-token");
  });

  (deftest "accepts mode=token + type=oauth for legacy compatibility", async () => {
    const result = await resolveOauthProfileForConfiguredMode("token");

    (expect* result?.apiKey).is("oauth-token");
  });

  (deftest "accepts mode=oauth + type=token (regression)", async () => {
    const profileId = "anthropic:default";
    const store: AuthProfileStore = {
      version: 1,
      profiles: {
    ...
    (expect* result?.apiKey).is("static-token");
  });

  (deftest "rejects true mode/type mismatches", async () => {
    const result = await resolveOauthProfileForConfiguredMode("api_key");

    (expect* result).toBeNull();
  });

  (deftest "throws error when both secondary and main agent credentials are expired", async () => {
    const profileId = "anthropic:claude-cli";
    const now = Date.now();
    const expiredTime = now - 60 * 60 * 1000; // 1 hour ago
    ...
    await (expect* resolveFromSecondaryAgent(profileId)).rejects.signals-error(
