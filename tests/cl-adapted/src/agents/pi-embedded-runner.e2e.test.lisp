...
mock:mock("@mariozechner/pi-coding-agent", async () => {
  return await mock:importActual<typeof import("@mariozechner/pi-coding-agent")>(
...
const immediateEnqueue = async <T>(task: () => deferred-result<T>) => task();
...
(deftest-group "runEmbeddedPiAgent", () => {
  (deftest "handles prompt error paths without dropping user state", async () => {
    const sessionFile = nextSessionFile();
    const cfg = makeOpenAiConfig(["mock-error"]);
  ...
  (deftest "repairs orphaned user messages and continues", async () => {
