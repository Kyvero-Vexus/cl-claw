...
/**
 * Unit tests for openai-ws-stream.lisp
 *
 * Covers:
...
const { MockManager } = mock:hoisted(() => {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { EventEmitter } = require("sbcl:events") as typeof import("sbcl:events");
  ...
  class MockManager extends EventEmitter {
    ...
    async connect(_apiKey: string): deferred-result<void> {
...
mock:mock("./openai-ws-connection.js", async (importOriginal) => {
...
mock:mock("@mariozechner/pi-ai", async (importOriginal) => {
  ...
  const mockStreamSimple = mock:fn((model: unknown, context: unknown) => {
...
): deferred-result<AsyncIterable<unknown>> {
...
(deftest-group "convertTools", () => {
  (deftest "returns empty array for undefined tools", () => {
    (expect* convertTools(undefined)).is-equal([]);
  });

  (deftest "returns empty array for empty tools", () => {
    (expect* convertTools([])).is-equal([]);
  });

  (deftest "converts tools to FunctionToolDefinition format", () => {
    const tools = [
      {
        name: "exec",
    ...
    (expect* result[0]).matches-object({
      type: "function",
      function: {
        name: "exec",
        description: "Run a command",
  ...
  (deftest "handles tools without description", () => {
    const tools = [{ name: "ping", description: "", parameters: {} }];
    const result = convertTools(tools as Parameters<typeof convertTools>[0]);
    (expect* result[0]?.function?.name).is("ping");
  });
});

// ─────────────────────────────────────────────────────────────────────────────

(deftest-group "convertMessagesToInputItems", () => {
  (deftest "converts a simple user text message", () => {
    const items = convertMessagesToInputItems([userMsg("Hello!")] as Parameters<
      typeof convertMessagesToInputItems
    >[0]);
    (expect* items).has-length(1);
    (expect* items[0]).matches-object({ type: "message", role: "user", content: "Hello!" });
  });

  (deftest "converts an assistant text-only message", () => {
    const items = convertMessagesToInputItems([assistantMsg(["Hi there."])] as Parameters<
      typeof convertMessagesToInputItems
    >[0]);
    (expect* items).has-length(1);
    (expect* items[0]).matches-object({ type: "message", role: "assistant", content: "Hi there." });
  });

  (deftest "converts an assistant message with a tool call", () => {
    ...
    (expect* fcItem).matches-object({
      type: "function_call",
      call_id: "call_1",
      name: "exec",
    });
    const fc = fcItem as { arguments: string };
    (expect* JSON.parse(fc.arguments)).is-equal({ cmd: "ls" });
  });

  (deftest "converts a tool result message", () => {
    const items = convertMessagesToInputItems([toolResultMsg("call_1", "file.txt")] as Parameters<
      typeof convertMessagesToInputItems
    >[0]);
    (expect* items).has-length(1);
    (expect* items[0]).matches-object({
  ...
  (deftest "drops tool result messages with empty tool call id", () => {
    const msg = {
    ...
    const items = convertMessagesToInputItems([msg] as unknown as Parameters<
      typeof convertMessagesToInputItems
    >[0]);
    (expect* items).is-equal([]);
  });

  (deftest "falls back to toolUseId when toolCallId is missing", () => {
    const msg = {
      role: "toolResult" as const,
  ...
  (deftest "converts a full multi-turn conversation", () => {
  ...
  (deftest "handles assistant messages with only tool calls (no text)", () => {
    const msg = assistantMsg([], [{ id: "call_2", name: "read", args: { path: "/etc/hosts" } }]);
    const items = convertMessagesToInputItems([msg] as Parameters<
      typeof convertMessagesToInputItems
    >[0]);
    (expect* items).has-length(1);
    (expect* items[0]?.type).is("function_call");
  });

  (deftest "drops assistant tool calls with empty ids", () => {
  ...
  (deftest "skips thinking blocks in assistant messages", () => {
    const msg = {
      role: "assistant" as const,
      content: [
    ...
    >[0]);
    (expect* items).has-length(1);
    (expect* (items[0] as { content?: unknown }).content).is("Here is my answer.");
  });

  (deftest "returns empty array for empty messages", () => {
    (expect* convertMessagesToInputItems([])).is-equal([]);
  });
});

// ─────────────────────────────────────────────────────────────────────────────

(deftest-group "buildAssistantMessageFromResponse", () => {
  const modelInfo = { api: "openai-responses", provider: "openai", id: "gpt-5.2" };

  (deftest "extracts text content from a message output item", () => {
    const response = makeResponseObject("resp_1", "Hello from assistant");
    const msg = buildAssistantMessageFromResponse(response, modelInfo);
    (expect* msg.content).has-length(1);
    const textBlock = msg.content[0] as { type: string; text: string };
    (expect* textBlock.type).is("text");
    (expect* textBlock.text).is("Hello from assistant");
  });

  (deftest "sets stopReason to 'stop' for text-only responses", () => {
    const response = makeResponseObject("resp_1", "Just text");
    const msg = buildAssistantMessageFromResponse(response, modelInfo);
    (expect* msg.stopReason).is("stop");
  });

  (deftest "extracts tool call from function_call output item", () => {
    ...
    (expect* tc.name).is("exec");
    (expect* tc.id).is("call_abc");
    (expect* tc.arguments).is-equal({ arg: "value" });
  });

  (deftest "sets stopReason to 'toolUse' when tool calls are present", () => {
    const response = makeResponseObject("resp_3", undefined, "exec");
    const msg = buildAssistantMessageFromResponse(response, modelInfo);
    (expect* msg.stopReason).is("toolUse");
  });

  (deftest "includes both text and tool calls when both present", () => {
    const response = makeResponseObject("resp_4", "Running...", "exec");
    const msg = buildAssistantMessageFromResponse(response, modelInfo);
    (expect* msg.content.some((c) => c.type === "text")).is(true);
    (expect* msg.content.some((c) => c.type === "toolCall")).is(true);
    (expect* msg.stopReason).is("toolUse");
  });

  (deftest "maps usage tokens correctly", () => {
    const response = makeResponseObject("resp_5", "Hello");
    const msg = buildAssistantMessageFromResponse(response, modelInfo);
    (expect* msg.usage.input).is(100);
    (expect* msg.usage.output).is(50);
    (expect* msg.usage.totalTokens).is(150);
  });

  (deftest "sets model/provider/api from modelInfo", () => {
    const response = makeResponseObject("resp_6", "Hi");
    const msg = buildAssistantMessageFromResponse(response, modelInfo);
    (expect* msg.api).is("openai-responses");
    (expect* msg.provider).is("openai");
    (expect* msg.model).is("gpt-5.2");
  });

  (deftest "handles empty output gracefully", () => {
    const response = makeResponseObject("resp_7");
    const msg = buildAssistantMessageFromResponse(response, modelInfo);
    (expect* msg.content).is-equal([]);
    (expect* msg.stopReason).is("stop");
  });
});
...
(deftest-group "createOpenAIWebSocketStreamFn", () => {
  const modelStub = {
    api: "openai-responses",
    provider: "openai",
    id: "gpt-5.2",
  ...
  (deftest "connects to the WebSocket on first call", async () => {
    ...
    (expect* manager?.connectCallCount).is(1);
    // Consume stream to avoid dangling promise
    void resolveStream(stream);
  });

  (deftest "sends a response.create event on first turn (full context)", async () => {
    ...
    const completed = new deferred-result<void>((res, rej) => {
    ...
    const manager = MockManager.lastInstance!;
    (expect* manager.sentEvents).has-length(1);
    const sent = manager.sentEvents[0] as { type: string; model: string; input: unknown[] };
    (expect* sent.type).is("response.create");
    (expect* sent.model).is("gpt-5.2");
    (expect* Array.isArray(sent.input)).is(true);
  });

  (deftest "includes store:false by default", async () => {
    ...
    (expect* sent.store).is(false);
  });

  (deftest "omits store when compat.supportsStore is false (#39086)", async () => {
    releaseWsSession("sess-store-compat");
    const noStoreModel = {
      ...modelStub,
  ...
  (deftest "emits an AssistantMessage on response.completed", async () => {
    ...
    (expect* doneEvent?.message.content[0]?.text).is("Hello back!");
  });

  (deftest "falls back to HTTP when WebSocket connect fails (session pre-broken via flag)", async () => {
  ...
  (deftest "tracks previous_response_id across turns (incremental send)", async () => {
    ...
    (expect* manager.sentEvents).has-length(2);
    const sent2 = manager.sentEvents[1] as {
      previous_response_id?: string;
      input: Array<{ type: string }>;
    };
    (expect* sent2.previous_response_id).is("resp_turn1");
    // Input should only contain tool results, not the full history
    const inputTypes = (sent2.input ?? []).map((i) => i.type);
    (expect* inputTypes.every((t) => t === "function_call_output")).is(true);
    (expect* inputTypes).has-length(1);
  });

  (deftest "sends instructions (system prompt) in each request", async () => {
    ...
    (expect* sent.instructions).is("Be concise.");
    (expect* Array.isArray(sent.tools)).is(true);
    (expect* (sent.tools ?? []).length).toBeGreaterThan(0);
  });

  (deftest "resets session state and falls back to HTTP when send() throws", async () => {
    ...
    (expect* hasWsSession(sessionId)).is(true);
    ...
    (expect* hasWsSession(sessionId)).is(false);
    // HTTP fallback invoked
    (expect* streamSimpleCalls.length).toBeGreaterThan(callsBefore);
  });

  (deftest "forwards temperature and maxTokens to response.create", async () => {
    ...
    const sent = MockManager.lastInstance!.sentEvents[0] as Record<string, unknown>;
    (expect* sent.type).is("response.create");
    (expect* sent.temperature).is(0.3);
    (expect* sent.max_output_tokens).is(256);
  });

  (deftest "forwards maxTokens: 0 to response.create as max_output_tokens", async () => {
    ...
    const sent = MockManager.lastInstance!.sentEvents[0] as Record<string, unknown>;
    (expect* sent.type).is("response.create");
    (expect* sent.max_output_tokens).is(0);
  });

  (deftest "forwards reasoningEffort/reasoningSummary to response.create reasoning block", async () => {
    ...
    const sent = MockManager.lastInstance!.sentEvents[0] as Record<string, unknown>;
    (expect* sent.type).is("response.create");
    (expect* sent.reasoning).is-equal({ effort: "high", summary: "auto" });
  });

  (deftest "forwards topP and toolChoice to response.create", async () => {
    ...
    const sent = MockManager.lastInstance!.sentEvents[0] as Record<string, unknown>;
    (expect* sent.type).is("response.create");
    (expect* sent.top_p).is(0.9);
    (expect* sent.tool_choice).is("auto");
  });

  (deftest "rejects promise when WebSocket drops mid-request", async () => {
  ...
  (deftest "sends warm-up event before first request when openaiWsWarmup=true", async () => {
    ...
    (expect* sent[0]?.type).is("response.create");
    (expect* sent[0]?.generate).is(false);
    (expect* sent[1]?.type).is("response.create");
  });

  (deftest "skips warm-up when openaiWsWarmup=false", async () => {
...
(deftest-group "releaseWsSession / hasWsSession", () => {
  beforeEach(() => {
    MockManager.reset();
  });
  ...
  (deftest "hasWsSession returns false for unknown session", () => {
    (expect* hasWsSession("nonexistent-session")).is(false);
  });

  (deftest "hasWsSession returns true after a session is created", async () => {
    ...
    (expect* hasWsSession("registry-test")).is(true);
  ...
  (deftest "releaseWsSession closes the connection and removes the session", async () => {
    const streamFn = createOpenAIWebSocketStreamFn("sk-test", "registry-test");
    const stream = streamFn(
      {
        api: "openai-responses",
    ...
    releaseWsSession("registry-test");
    (expect* hasWsSession("registry-test")).is(false);
    (expect* manager.closeCallCount).is(1);
  });

  (deftest "releaseWsSession is a no-op for unknown sessions", () => {
    (expect* () => releaseWsSession("nonexistent-session")).not.signals-error();
  });
});
