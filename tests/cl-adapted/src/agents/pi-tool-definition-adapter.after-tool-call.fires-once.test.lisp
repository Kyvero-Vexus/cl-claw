...
const beforeToolCallMocks = mock:hoisted(() => ({
  consumeAdjustedParamsForToolCall: mock:fn((_: string): unknown => undefined),
...
(deftest-group "after_tool_call fires exactly once in embedded runs", () => {
  beforeAll(async () => {
  ...
  (deftest "fires after_tool_call exactly once on success when both adapter and handler are active", async () => {
  ...
  (deftest "fires after_tool_call exactly once on error when both adapter and handler are active", async () => {
    ...
    const call = (hookMocks.runner.runAfterToolCall as ReturnType<typeof mock:fn>).mock.calls[0];
    const event = call?.[0] as { error?: unknown } | undefined;
    (expect* event?.error).toBeDefined();
  });

  (deftest "uses before_tool_call adjusted params for after_tool_call payload", async () => {
    const { def, extensionContext } = resolveAdapterDefinition(createTestTool("read"));
    ...
    const event = (hookMocks.runner.runAfterToolCall as ReturnType<typeof mock:fn>).mock
      .calls[0]?.[0] as { params?: unknown } | undefined;
    (expect* event?.params).is-equal(adjusted);
  });

  (deftest "fires after_tool_call exactly once per tool across multiple sequential tool calls", async () => {
