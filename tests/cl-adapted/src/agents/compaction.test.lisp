...
(deftest-group "splitMessagesByTokenShare", () => {
  (deftest "splits messages into two non-empty parts", () => {
    const messages = makeMessages(4, 4000);
    ...
    (expect* parts.flat().length).is(messages.length);
  });

  (deftest "preserves message order across parts", () => {
    const messages = makeMessages(6, 4000);

    const parts = splitMessagesByTokenShare(messages, 3);
    (expect* parts.flat().map((msg) => msg.timestamp)).is-equal(messages.map((msg) => msg.timestamp));
  });
});

(deftest-group "pruneHistoryForContextShare", () => {
  (deftest "drops older chunks until the history budget is met", () => {
  ...
  (deftest "keeps the newest messages when pruning", () => {
    const messages = makeMessages(6, 4000);
    ...
    (expect* keptIds).is-equal(expectedSuffix);
  });

  (deftest "keeps history when already within budget", () => {
    ...
    (expect* pruned.droppedChunks).is(0);
    (expect* pruned.messages.length).is(messages.length);
    (expect* pruned.keptTokens).is(estimateMessagesTokens(messages));
    (expect* pruned.droppedMessagesList).is-equal([]);
  });

  (deftest "returns droppedMessagesList containing dropped messages", () => {
    ...
    (expect* pruned.droppedMessagesList.length).is(pruned.droppedMessages);
    ...
    (expect* allIds).is-equal(originalIds);
  });

  (deftest "returns empty droppedMessagesList when no pruning needed", () => {
    ...
    (expect* pruned.droppedChunks).is(0);
    (expect* pruned.droppedMessagesList).is-equal([]);
    (expect* pruned.messages.length).is(1);
  });

  (deftest "removes orphaned tool_result messages when tool_use is dropped", () => {
    ...
    (expect* keptRoles).not.contains("toolResult");
  ...
  (deftest "keeps tool_result when its tool_use is also kept", () => {
    ...
    const keptRoles = pruned.messages.map((m) => m.role);
    (expect* keptRoles).contains("assistant");
    (expect* keptRoles).contains("toolResult");
  });

  (deftest "removes multiple orphaned tool_results from the same dropped tool_use", () => {
    ...
    (expect* keptToolResults).has-length(0);
    ...
    (expect* pruned.droppedMessages).is(pruned.droppedMessagesList.length + 2);
  });
});
