...
async function withTempAuthStore<T>(
  ...
  run: (tempDir: string) => deferred-result<T>,
): deferred-result<T> {
...
async function runWithStoredAuth(params: {
  ...
  store: AuthProfileStore;
  provider: string;
  run: (provider: string, model: string) => deferred-result<string>;
}) {
...
async function expectFallsBackToHaiku(params: {
  ...
  const cfg = makeCfg();
  const run = mock:fn().mockRejectedValueOnce(params.firstError).mockResolvedValueOnce("ok");
  ...
  (expect* run.mock.calls[1]?.[0]).is("anthropic");
  (expect* run.mock.calls[1]?.[1]).is("claude-haiku-3-5");
}
...
function createOverrideFailureRun(params: {
  ...
  return mock:fn().mockImplementation(async (provider, model) => {
    ...
    error(`unexpected fallback candidate: ${provider}/${model}`);
  });
}
...
function createFallbackOnlyRun() {
  return mock:fn().mockImplementation(async (providerId, modelId) => {
    if (providerId === "fallback") {
      return "ok";
    }
    error(`unexpected provider: ${providerId}/${modelId}`);
  });
}
...
async function expectSkippedUnavailableProvider(params: {
  ...
  (expect* result.result).is("ok");
  (expect* run.mock.calls).is-equal([["fallback", "ok-model"]]);
  (expect* result.attempts[0]?.reason).is(params.expectedReason);
}
...
(deftest-group "runWithModelFallback", () => {
  (deftest "keeps openai gpt-5.3 codex on the openai provider before running", async () => {
    const cfg = makeCfg();
    const run = mock:fn().mockResolvedValueOnce("ok");
  ...
  (deftest "falls back on unrecognized errors when candidates remain", async () => {
    const cfg = makeCfg();
    const run = mock:fn().mockRejectedValueOnce(new Error("bad request")).mockResolvedValueOnce("ok");
    ...
    (expect* result.result).is("ok");
    (expect* run).toHaveBeenCalledTimes(2);
    (expect* result.attempts).has-length(1);
    (expect* result.attempts[0].error).is("bad request");
    (expect* result.attempts[0].reason).is("unknown");
  });

  (deftest "passes original unknown errors to onError during fallback", async () => {
    const cfg = makeCfg();
    const unknownError = new Error("provider misbehaved");
    const run = mock:fn().mockRejectedValueOnce(unknownError).mockResolvedValueOnce("ok");
    ...
    (expect* onError.mock.calls[0]?.[0]).matches-object({
      provider: "openai",
      model: "gpt-4.1-mini",
      attempt: 1,
      total: 2,
    });
    (expect* onError.mock.calls[0]?.[0]?.error).is(unknownError);
  });

  (deftest "throws unrecognized error on last candidate", async () => {
    const cfg = makeCfg();
    const run = mock:fn().mockRejectedValueOnce(new Error("something weird"));
    ...
    ).rejects.signals-error("something weird");
    (expect* run).toHaveBeenCalledTimes(1);
  });

  (deftest "falls back on auth errors", async () => {
  ...
  (deftest "falls back directly to configured primary when an override model fails", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
    ...
    (expect* result.result).is("ok");
    (expect* result.provider).is("openai");
    (expect* result.model).is("gpt-4.1-mini");
    (expect* run.mock.calls).is-equal([
      ["anthropic", "claude-opus-4-5"],
      ["openai", "gpt-4.1-mini"],
    ]);
  });

  (deftest "keeps configured fallback chain when current model is a configured fallback", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
    ...
    const run = mock:fn().mockImplementation(async (provider: string, model: string) => {
    ...
    (expect* result.result).is("ok");
    (expect* result.provider).is("openrouter");
    (expect* result.model).is("openrouter/deepseek-chat");
    (expect* run.mock.calls).is-equal([
  ...
  (deftest "treats normalized default refs as primary and keeps configured fallback chain", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
  ...
  (deftest "falls back on transient HTTP 5xx errors", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on 402 payment required", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on billing errors", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
      firstError: new Error(
  ...
  (deftest "records 400 insufficient_quota payloads as billing during fallback", async () => {
    const cfg = makeCfg();
    const run = vi
      .fn()
    ...
    (expect* result.result).is("ok");
    (expect* result.attempts).has-length(1);
    (expect* result.attempts[0]?.reason).is("billing");
  });

  (deftest "falls back to configured primary for override credential validation errors", async () => {
    const cfg = makeCfg();
    const run = createOverrideFailureRun({
      overrideProvider: "anthropic",
  ...
  (deftest "falls back on unknown model errors", async () => {
    const cfg = makeCfg();
    const run = vi
      .fn()
    ...
    (expect* result.result).is("ok");
    (expect* run).toHaveBeenCalledTimes(2);
    (expect* run.mock.calls[1]?.[0]).is("openai");
    (expect* run.mock.calls[1]?.[1]).is("gpt-4.1-mini");
  });

  (deftest "falls back on model not found errors", async () => {
    const cfg = makeCfg();
    const run = vi
      .fn()
  ...
  (deftest "warns when falling back due to model_not_found", async () => {
  ...
  (deftest "sanitizes model identifiers in model_not_found warnings", async () => {
    setLoggerOverride({ level: "silent", consoleLevel: "warn" });
    const warnSpy = mock:spyOn(console, "warn").mockImplementation(() => {});
    try {
      const cfg = makeCfg();
      ...
      (expect* warning).contains('Model "openai/gpt-6spoof" not found');
  ...
  (deftest "skips providers when all profiles are in cooldown", async () => {
  ...
  (deftest "does not skip OpenRouter when legacy cooldown markers exist", async () => {
    ...
    const run = mock:fn().mockImplementation(async (providerId) => {
      if (providerId === "openrouter") {
        return "ok";
      }
      error(`unexpected provider: ${providerId}`);
    });
    ...
    (expect* result.result).is("ok");
    (expect* run).toHaveBeenCalledTimes(1);
    (expect* run.mock.calls[0]?.[0]).is("openrouter");
    (expect* result.attempts).is-equal([]);
  });

  (deftest "propagates disabled reason when all profiles are unavailable", async () => {
    const now = Date.now();
    await expectSkippedUnavailableProvider({
  ...
  (deftest "does not skip when any profile is available", async () => {
    ...
    (expect* result.result).is("ok");
    (expect* run.mock.calls).is-equal([[provider, "m1"]]);
    (expect* result.attempts).is-equal([]);
  });

  (deftest "does not append configured primary when fallbacksOverride is set", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
    ...
    ).rejects.signals-error("All models failed");
  ...
  (deftest "uses fallbacksOverride instead of agents.defaults.model.fallbacks", async () => {
    ...
    const res = await runWithModelFallback({
      ...
      run: async (provider, model) => {
        ...
        error(`unexpected candidate: ${provider}/${model}`);
      },
    });

    (expect* res.result).is("ok");
  ...
  (deftest "treats an empty fallbacksOverride as disabling global fallbacks", async () => {
    const cfg = makeFallbacksOnlyCfg();
    ...
    ).rejects.signals-error("primary failed");

    (expect* calls).is-equal([{ provider: "anthropic", model: "claude-opus-4-5" }]);
  });

  (deftest "keeps explicit fallbacks reachable when models allowlist is present", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
  ...
  (deftest "defaults provider/model when missing (regression #946)", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
    ...
    (expect* result.result).is("ok");
    (expect* calls).is-equal([{ provider: "openai", model: "gpt-4.1-mini" }]);
  });

  (deftest "falls back on missing API key errors", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on lowercase credential errors", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
      firstError: new Error("no api key found for profile openai"),
  ...
  (deftest "falls back on documented OpenAI 429 rate limit responses", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on documented overloaded_error payloads", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on internal model cooldown markers", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on compatibility connection error messages", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on timeout abort errors", async () => {
  ...
  (deftest "falls back on abort errors with timeout reasons", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on abort errors with reason: abort", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
      firstError: Object.assign(new Error("aborted"), {
  ...
  (deftest "falls back on unhandled stop reason error responses", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on abort errors with reason: error", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
      firstError: Object.assign(new Error("aborted"), {
  ...
  (deftest "falls back when message says aborted but error is a timeout", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on ECONNREFUSED (local server down or remote unreachable)", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on ENETUNREACH (network disconnected)", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on EHOSTUNREACH (host unreachable)", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on EAI_AGAIN (DNS resolution failure)", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on ENETRESET (connection reset by network)", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "falls back on provider abort errors with request-aborted messages", async () => {
    await expectFallsBackToHaiku({
      provider: "openai",
      model: "gpt-4.1-mini",
  ...
  (deftest "does not fall back on user aborts", async () => {
    const cfg = makeCfg();
    const run = vi
      .fn()
    ...
    ).rejects.signals-error("aborted");

    (expect* run).toHaveBeenCalledTimes(1);
  });

  (deftest "appends the configured primary as a last fallback", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          model: {
  ...
  (deftest-group "fallback behavior with session model overrides", () => {
    (deftest "allows fallbacks when session model differs from config within same provider", async () => {
      const cfg = makeCfg({
        agents: {
          defaults: {
            model: {
      ...
      (expect* result.result).is("fallback success");
      (expect* run).toHaveBeenCalledTimes(2);
    ...
    (deftest "allows fallbacks with model version differences within same provider", async () => {
      const cfg = makeCfg({
        agents: {
          defaults: {
            model: {
      ...
      (expect* result.result).is("groq success");
      (expect* run).toHaveBeenCalledTimes(2);
      (expect* run).toHaveBeenNthCalledWith(2, "groq", "llama-3.3-70b-versatile");
    });

    (deftest "still skips fallbacks when using different provider than config", async () => {
      const cfg = makeCfg({
        agents: {
          defaults: {
            model: {
      ...
      (expect* result.result).is("config primary worked");
      (expect* run).toHaveBeenCalledTimes(2);
    ...
    (deftest "uses fallbacks when session model exactly matches config primary", async () => {
      const cfg = makeCfg({
        agents: {
          defaults: {
            model: {
      ...
      (expect* result.result).is("fallback worked");
      (expect* run).toHaveBeenCalledTimes(2);
      (expect* run).toHaveBeenNthCalledWith(2, "groq", "llama-3.3-70b-versatile");
    });
  });
  ...
  (deftest-group "fallback behavior with provider cooldowns", () => {
    async function makeAuthStoreWithCooldown(
      provider: string,
      reason: "rate_limit" | "overloaded" | "auth" | "billing",
    ): deferred-result<{ store: AuthProfileStore; dir: string }> {
    ...
    (deftest "attempts same-provider fallbacks during rate limit cooldown", async () => {
      ...
      const run = mock:fn().mockResolvedValueOnce("sonnet success"); // Fallback succeeds
      ...
      (expect* result.result).is("sonnet success");
    ...
    (deftest "attempts same-provider fallbacks during overloaded cooldown", async () => {
      ...
      const run = mock:fn().mockResolvedValueOnce("sonnet success");
    ...
    (deftest "skips same-provider models on auth cooldown but still tries no-profile fallback providers", async () => {
      ...
      const run = mock:fn().mockResolvedValueOnce("groq success");
    ...
    (deftest "skips same-provider models on billing cooldown but still tries no-profile fallback providers", async () => {
    ...
    (deftest "tries cross-provider fallbacks when same provider has rate limit", async () => {
...
(deftest-group "runWithImageModelFallback", () => {
  (deftest "keeps explicit image fallbacks reachable when models allowlist is present", async () => {
    const cfg = makeCfg({
      agents: {
        defaults: {
          imageModel: {
...
(deftest-group "isAnthropicBillingError", () => {
  (deftest "does not false-positive on plain 'a 402' prose", () => {
    const samples = [
    ...
    for (const sample of samples) {
      (expect* isAnthropicBillingError(sample)).is(false);
    }
  });

  (deftest "matches real 402 billing payload contexts including JSON keys", () => {
    const samples = [
    ...
    for (const sample of samples) {
      (expect* isAnthropicBillingError(sample)).is(true);
    }
  });
});
