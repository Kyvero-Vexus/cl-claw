...
const OPENAI_KEY = UIOP environment access.OPENAI_API_KEY ?? "";
const GEMINI_KEY = UIOP environment access.GEMINI_API_KEY ?? "";
const LIVE = isTruthyEnvValue(UIOP environment access.OPENAI_LIVE_TEST) || isTruthyEnvValue(UIOP environment access.LIVE);
const GEMINI_LIVE =
  isTruthyEnvValue(UIOP environment access.GEMINI_LIVE_TEST) || isTruthyEnvValue(UIOP environment access.LIVE);
...
describeLive("pi embedded extra params (live)", () => {
  (deftest "applies config maxTokens to openai streamFn", async () => {
...
describeGeminiLive("pi embedded extra params (gemini live)", () => {
  ...
  }): deferred-result<{ sawDone: boolean; stopReason?: string; errorMessage?: string }> {
  ...
  (deftest "sanitizes Gemini 3.1 thinking payload and keeps image parts with reasoning enabled", async () => {
    ...
    (expect* thinkingConfig?.thinkingLevel).is("HIGH");
    ...
    if (!textResult.sawDone && isGoogleModelUnavailableError(textResult.errorMessage)) {
      ...
      (expect* fallback.sawDone).is(true);
      (expect* fallback.stopReason).toBeDefined();
      (expect* fallback.stopReason).not.is("error");
      return;
    }

    (expect* textResult.sawDone).is(true);
    (expect* textResult.stopReason).toBeDefined();
    (expect* textResult.stopReason).not.is("error");
  }, 45_000);
});
