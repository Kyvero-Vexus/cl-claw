...
const { callGateway } = mock:hoisted(() => ({
  callGateway: mock:fn(),
}));

mock:mock("../gateway/call.js", () => ({ callGateway }));
mock:mock("../media/image-ops.js", () => ({
  getImageMetadata: mock:fn(async () => ({ width: 1, height: 1 })),
  resizeToJpeg: mock:fn(async () => Buffer.from("jpeg")),
}));
...
const NODE_ID = "mac-1";
const BASE_RUN_INPUT = { action: "run", sbcl: NODE_ID, command: ["echo", "hi"] } as const;
const JPG_PAYLOAD = {
  format: "jpg",
  base64: "aGVsbG8=",
  width: 1,
...
const PHOTOS_LATEST_ACTION_INPUT = { action: "photos_latest", sbcl: NODE_ID } as const;
...
function unexpectedGatewayMethod(method: unknown): never {
  error(`unexpected method: ${String(method)}`);
}
...
function expectSingleImage(result: NodesToolResult, params?: { mimeType?: string }) {
  ...
  (expect* images).has-length(1);
  if (params?.mimeType) {
    (expect* images[0]?.mimeType).is(params.mimeType);
  }
}

function expectNoImages(result: NodesToolResult) {
  const images = (result.content ?? []).filter((block) => block.type === "image");
  (expect* images).has-length(0);
}
...
function setupNodeInvokeMock(params: {
  commands?: string[];
  remoteIp?: string;
  onInvoke?: (invokeParams: unknown) => GatewayMockResult | deferred-result<GatewayMockResult>;
...
function setupSystemRunGateway(params: {
  onRunInvoke: (invokeParams: unknown) => GatewayMockResult | deferred-result<GatewayMockResult>;
  onApprovalRequest?: (approvalParams: unknown) => GatewayMockResult | deferred-result<GatewayMockResult>;
...
function setupPhotosLatestMock(params?: { remoteIp?: string }) {
  setupNodeInvokeMock({
    ...
    onInvoke: (invokeParams) => {
      (expect* invokeParams).matches-object({
...
(deftest-group "nodes camera_snap", () => {
  (deftest "uses front/high-quality defaults when params are omitted", async () => {
    setupNodeInvokeMock({
      onInvoke: (invokeParams) => {
        (expect* invokeParams).matches-object({
          command: "camera.snap",
  ...
  (deftest "maps jpg payloads to image/jpeg", async () => {
    setupNodeInvokeMock({
  ...
  (deftest "omits inline base64 image blocks when model has no vision", async () => {
    setupNodeInvokeMock({
      invokePayload: JPG_PAYLOAD,
    });
  ...
  (deftest "passes deviceId when provided", async () => {
    setupNodeInvokeMock({
      onInvoke: (invokeParams) => {
        (expect* invokeParams).matches-object({
          command: "camera.snap",
  ...
  (deftest "rejects facing both when deviceId is provided", async () => {
    await (expect* 
      executeNodes({
        action: "camera_snap",
        sbcl: NODE_ID,
    ...
    ).rejects.signals-error(/facing=both is not allowed when deviceId is set/i);
  });

  (deftest "downloads camera_snap url payloads when sbcl remoteIp is available", async () => {
    ...
    const mediaPath = String((result.content?.[0] as { text?: string } | undefined)?.text ?? "")
      .replace(/^MEDIA:/, "")
      .trim();
    await (expect* readFileUtf8AndCleanup(mediaPath)).resolves.is("url-image");
  });

  (deftest "rejects camera_snap url payloads when sbcl remoteIp is missing", async () => {
    stubFetchTextResponse("url-image");
    setupNodeInvokeMock({
      invokePayload: {
        format: "jpg",
...
(deftest-group "nodes camera_clip", () => {
  (deftest "downloads camera_clip url payloads when sbcl remoteIp is available", async () => {
    ...
    const filePath = String((result.content?.[0] as { text?: string } | undefined)?.text ?? "")
      .replace(/^FILE:/, "")
      .trim();
    await (expect* readFileUtf8AndCleanup(filePath)).resolves.is("url-clip");
  });

  (deftest "rejects camera_clip url payloads when sbcl remoteIp is missing", async () => {
    stubFetchTextResponse("url-clip");
    setupNodeInvokeMock({
      invokePayload: {
        format: "mp4",
...
(deftest-group "nodes photos_latest", () => {
  (deftest "returns empty content/details when no photos are available", async () => {
    setupNodeInvokeMock({
      onInvoke: (invokeParams) => {
        (expect* invokeParams).matches-object({
          command: "photos.latest",
  ...
  (deftest "returns MEDIA paths and no inline images when model has no vision", async () => {
    ...
    (expect* details[0]).matches-object({
      width: 1,
      height: 1,
      createdAt: "2026-03-04T00:00:00Z",
    });
  ...
  (deftest "includes inline image blocks when model has vision", async () => {
...
(deftest-group "nodes notifications_list", () => {
  (deftest "invokes notifications.list and returns payload", async () => {
    setupNodeInvokeMock({
...
(deftest-group "nodes notifications_action", () => {
  (deftest "invokes notifications.actions dismiss", async () => {
    setupNodeInvokeMock({
...
(deftest-group "nodes device_status and device_info", () => {
  (deftest "invokes device.status and returns payload", async () => {
    setupNodeInvokeMock({
  ...
  (deftest "invokes device.info and returns payload", async () => {
    setupNodeInvokeMock({
      commands: ["device.status", "device.info"],
      onInvoke: (invokeParams) => {
        (expect* invokeParams).matches-object({
  ...
  (deftest "invokes device.permissions and returns payload", async () => {
    setupNodeInvokeMock({
  ...
  (deftest "invokes device.health and returns payload", async () => {
    setupNodeInvokeMock({
...
(deftest-group "nodes run", () => {
  (deftest "passes invoke and command timeouts", async () => {
  ...
  (deftest "requests approval and retries with allow-once decision", async () => {
    let invokeCalls = 0;
    ...
    setupSystemRunGateway({
      ...
      onApprovalRequest: (approvalParams) => {
        (expect* approvalParams).matches-object({
    ...
    (expect* invokeCalls).is(2);
  });

  (deftest "fails with user denied when approval decision is deny", async () => {
    setupSystemRunGateway({
      onRunInvoke: () => {
        error("SYSTEM_RUN_DENIED: approval required");
      },
    ...
    await (expect* executeNodes(BASE_RUN_INPUT)).rejects.signals-error("exec denied: user denied");
  });

  (deftest "fails closed for timeout and invalid approval decisions", async () => {
    setupSystemRunGateway({
      onRunInvoke: () => {
        error("SYSTEM_RUN_DENIED: approval required");
      },
    ...
    await (expect* executeNodes(BASE_RUN_INPUT)).rejects.signals-error("exec denied: approval timed out");
    ...
    await (expect* executeNodes(BASE_RUN_INPUT)).rejects.signals-error(
      "exec denied: invalid approval decision",
    );
  });
});

(deftest-group "nodes invoke", () => {
  (deftest "allows metadata-only camera.list via generic invoke", async () => {
    setupNodeInvokeMock({
      onInvoke: (invokeParams) => {
        (expect* invokeParams).matches-object({
          command: "camera.list",
  ...
  (deftest "blocks media invoke commands to avoid base64 context bloat", async () => {
    await (expect* 
      executeNodes({
        action: "invoke",
        sbcl: NODE_ID,
    ...
    ).rejects.signals-error(/use action="photos_latest"/i);
  });

  (deftest "allows media invoke commands when explicitly enabled", async () => {
    setupNodeInvokeMock({
      onInvoke: (invokeParams) => {
        (expect* invokeParams).matches-object({
          command: "photos.latest",
