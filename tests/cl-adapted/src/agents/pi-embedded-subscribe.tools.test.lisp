...
(deftest-group "extractToolErrorMessage", () => {
  (deftest "ignores non-error status values", () => {
  ...
  (deftest "keeps error-like status values", () => {
    (expect* extractToolErrorMessage({ details: { status: "failed" } })).is("failed");
    (expect* extractToolErrorMessage({ details: { status: "timeout" } })).is("timeout");
  });
});
