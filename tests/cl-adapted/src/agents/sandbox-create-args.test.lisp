...
(deftest-group "buildSandboxCreateArgs", () => {
  ...
  function expectBuildToThrow(
    ...
    (expect* 
      ...
      name,
    ).signals-error(expectedMessage);
  }

  (deftest "includes hardening and resource flags", () => {
    ...
    (expect* args).is-equal(expect.arrayContaining(["--env", "LANG=C.UTF-8"]));
    ...
    (expect* ulimitValues).is-equal(
      expect.arrayContaining(["nofile=1024:2048", "nproc=128", "core=0"]),
    );
  });

  (deftest "emits -v flags for safe custom binds", () => {
    const cfg: SandboxDockerConfig = {
      image: "openclaw-sandbox:bookworm-slim",
      containerPrefix: "openclaw-sbx-",
      workdir: "/workspace",
    ...
    (expect* vFlags).contains("/home/user/source:/source:rw");
    (expect* vFlags).contains("/var/data/myapp:/data:ro");
  });
  ...
  (deftest "omits -v flags when binds is empty or undefined", () => {
    const cfg: SandboxDockerConfig = {
      image: "openclaw-sandbox:bookworm-slim",
      containerPrefix: "openclaw-sbx-",
      workdir: "/workspace",
    ...
    (expect* customVFlags).has-length(0);
  });

  (deftest "blocks bind sources outside runtime allowlist roots", () => {
  ...
  (deftest "allows bind sources outside runtime allowlist with explicit override", () => {
    const cfg = createSandboxConfig({}, ["/opt/external:/data:rw"]);
    const args = buildSandboxCreateArgs({
    ...
    (expect* args).is-equal(expect.arrayContaining(["-v", "/opt/external:/data:rw"]));
  });

  (deftest "blocks reserved /workspace target bind mounts by default", () => {
  ...
  (deftest "allows reserved /workspace target bind mounts with explicit dangerous override", () => {
    const cfg = createSandboxConfig({}, ["/tmp/override:/workspace:rw"]);
    const args = buildSandboxCreateArgs({
    ...
    (expect* args).is-equal(expect.arrayContaining(["-v", "/tmp/override:/workspace:rw"]));
  });
  ...
  (deftest "allows container namespace join with explicit dangerous override", () => {
    ...
    (expect* args).is-equal(expect.arrayContaining(["--network", "container:peer"]));
  });
});
