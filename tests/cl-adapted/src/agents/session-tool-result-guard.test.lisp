...
function expectPersistedRoles(sm: SessionManager, expectedRoles: AgentMessage["role"][]) {
  ...
  (expect* messages.map((message) => message.role)).is-equal(expectedRoles);
  return messages;
}
...
(deftest-group "installSessionToolResultGuard", () => {
  (deftest "inserts synthetic toolResult before non-tool message when pending", () => {
    ...
    (expect* synthetic.toolCallId).is("call_1");
    (expect* synthetic.isError).is(true);
    (expect* synthetic.content?.[0]?.text).contains("missing tool result");
  });

  (deftest "flushes pending tool calls when asked explicitly", () => {
    const sm = SessionManager.inMemory();
  ...
  (deftest "clears pending tool calls without inserting synthetic tool results", () => {
    const sm = SessionManager.inMemory();
    const guard = installSessionToolResultGuard(sm);
    ...
    (expect* guard.getPendingIds()).is-equal([]);
  });

  (deftest "clears pending on user interruption when synthetic tool results are disabled", () => {
    const sm = SessionManager.inMemory();
    const guard = installSessionToolResultGuard(sm, {
  ...
  (deftest "does not add synthetic toolResult when a matching one exists", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm);
  ...
  (deftest "backfills blank toolResult names from pending tool calls", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm);
    ...
    (expect* messages[1]?.toolName).is("read");
  });

  (deftest "preserves ordering with multiple tool calls and partial results", () => {
    const sm = SessionManager.inMemory();
    const guard = installSessionToolResultGuard(sm);
    ...
    (expect* (messages[2] as { toolCallId?: string }).toolCallId).is("call_b");
    (expect* guard.getPendingIds()).is-equal([]);
  });

  (deftest "flushes pending on guard when no toolResult arrived", () => {
    const sm = SessionManager.inMemory();
    const guard = installSessionToolResultGuard(sm);
  ...
  (deftest "handles toolUseId on toolResult", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm);
  ...
  (deftest "drops malformed tool calls missing input before persistence", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm);
    ...
    const messages = getPersistedMessages(sm);
    (expect* messages).has-length(0);
  });

  (deftest "drops malformed tool calls with invalid name tokens before persistence", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm);
    ...
    (expect* getPersistedMessages(sm)).has-length(0);
  });

  (deftest "drops tool calls not present in allowedToolNames", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm, {
  ...
  (deftest "flushes pending tool results when a sanitized assistant message is dropped", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm);
  ...
  (deftest "clears pending when a sanitized assistant message is dropped and synthetic results are disabled", () => {
    const sm = SessionManager.inMemory();
    const guard = installSessionToolResultGuard(sm, {
      allowSyntheticToolResults: false,
      allowedToolNames: ["read"],
  ...
  (deftest "drops older pending ids before new tool calls when synthetic results are disabled", () => {
    const sm = SessionManager.inMemory();
    const guard = installSessionToolResultGuard(sm, {
      allowSyntheticToolResults: false,
    });
    ...
    (expect* guard.getPendingIds()).is-equal(["call_2"]);
  });

  (deftest "caps oversized tool result text during persistence", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm);
  ...
  (deftest "does not truncate tool results under the limit", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm);
    ...
    const text = getToolResultText(getPersistedMessages(sm));
    (expect* text).is(originalText);
  });

  (deftest "blocks persistence when before_message_write returns block=true", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm, {
  ...
  (deftest "applies before_message_write message mutations before persistence", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm, {
    ...
    const text = getToolResultText(getPersistedMessages(sm));
    (expect* text).is("rewritten by hook");
  });

  (deftest "applies before_message_write to synthetic tool-result flushes", () => {
    const sm = SessionManager.inMemory();
    const guard = installSessionToolResultGuard(sm, {
      beforeMessageWriteHook: ({ message }) => {
        if ((message as { role?: string }).role !== "toolResult") {
    ...
    const messages = getPersistedMessages(sm);
    (expect* messages.map((m) => m.role)).is-equal(["assistant"]);
  });

  (deftest "applies message persistence transform to user messages", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm, {
    ...
    const persisted = sm.getEntries().find((e) => e.type === "message") as
      ...
      | undefined;
    (expect* persisted?.message?.role).is("user");
    (expect* persisted?.message?.provenance).is-equal({
      kind: "inter_session",
  ...
  (deftest "does NOT create synthetic toolResult for aborted assistant messages with toolCalls", () => {
    const sm = SessionManager.inMemory();
    installSessionToolResultGuard(sm);
    ...
    (expect* roles).is-equal(["assistant", "user"]);
    (expect* roles).not.contains("toolResult");
  });

  (deftest "does NOT create synthetic toolResult for errored assistant messages with toolCalls", () => {
    const sm = SessionManager.inMemory();
    const guard = installSessionToolResultGuard(sm);
    ...
    (expect* syntheticForError).has-length(0);
  });
});
