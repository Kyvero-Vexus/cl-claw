...
const { callGatewayMock } = mock:hoisted(() => ({
  callGatewayMock: mock:fn(),
}));
...
(deftest-group "cron tool", () => {
  ...
  }): deferred-result<string | undefined> {
    const tool = createCronTool({ agentSessionKey: params.agentSessionKey });
    await tool.execute(params.callId, {
      action: "add",
      job: {
  ...
  (deftest "marks cron as owner-only", async () => {
  ...
  it.each([
    ...
    (expect* params).is-equal(expectedParams);
  });

  (deftest "prefers jobId over id when both are provided", async () => {
    const tool = createCronTool();
    ...
    (expect* readGatewayCall().params).is-equal({ id: "job-primary", mode: "force" });
  });

  (deftest "supports due-only run mode", async () => {
    const tool = createCronTool();
    ...
    (expect* readGatewayCall().params).is-equal({ id: "job-due", mode: "due" });
  });

  (deftest "normalizes cron.add job payloads", async () => {
    const tool = createCronTool();
  ...
  (deftest "does not default agentId when job.agentId is null", async () => {
    const tool = createCronTool({ agentSessionKey: "main" });
  ...
  (deftest "stamps cron.add with caller sessionKey when missing", async () => {
    ...
    (expect* sessionKey).is(callerSessionKey);
  });

  (deftest "preserves explicit job.sessionKey on add", async () => {
    callGatewayMock.mockResolvedValueOnce({ ok: true });
    ...
    (expect* sessionKey).is("agent:main:telegram:group:-100123:topic:99");
  });

  (deftest "adds recent context for systemEvent reminders when contextMessages > 0", async () => {
    callGatewayMock
    ...
    (expect* historyCall.method).is("chat.history");

    const cronCall = readGatewayCall(1);
    (expect* cronCall.method).is("cron.add");
    const text = readCronPayloadText(1);
    (expect* text).contains("Recent context:");
    (expect* text).contains("User: Discussed Q2 budget");
    (expect* text).contains("Assistant: We agreed to review on Tuesday.");
    (expect* text).contains("User: Remind me about the thing at 2pm");
  });

  (deftest "caps contextMessages at 10", async () => {
    ...
    (expect* historyParams?.limit).is(10);

    const text = readCronPayloadText(1);
    (expect* text).not.toMatch(/Message 1\\b/);
    (expect* text).not.toMatch(/Message 2\\b/);
    (expect* text).contains("Message 3");
    (expect* text).contains("Message 12");
  });

  (deftest "does not add context when contextMessages is 0 (default)", async () => {
    callGatewayMock.mockResolvedValueOnce({ ok: true });
    ...
    (expect* text).not.contains("Recent context:");
  });

  (deftest "preserves explicit agentId null on add", async () => {
    callGatewayMock.mockResolvedValueOnce({ ok: true });
    ...
    (expect* call.method).is("cron.add");
    (expect* call.params?.agentId).toBeNull();
  });

  (deftest "infers delivery from threaded session keys", async () => {
    (expect* 
  ...
  (deftest "preserves telegram forum topics when inferring delivery", async () => {
    (expect* 
      await executeAddAndReadDelivery({
  ...
  (deftest "infers delivery when delivery is null", async () => {
    (expect* 
      await executeAddAndReadDelivery({
  ...
  (deftest "recovers flat params when job is missing", async () => {
    callGatewayMock.mockResolvedValueOnce({ ok: true });
    ...
    const params = expectSingleGatewayCallMethod("cron.add") as
      ...
      | undefined;
    (expect* params?.name).is("flat-job");
    (expect* params?.sessionTarget).is("isolated");
    (expect* params?.payload?.kind).is("agentTurn");
  });

  (deftest "recovers flat params when job is empty object", async () => {
    callGatewayMock.mockResolvedValueOnce({ ok: true });
    ...
    const params = expectSingleGatewayCallMethod("cron.add") as
      ...
      | undefined;
    (expect* params?.name).is("empty-job");
    (expect* params?.sessionTarget).is("main");
    (expect* params?.payload?.text).is("wake up");
  });

  (deftest "recovers flat message shorthand as agentTurn payload", async () => {
    callGatewayMock.mockResolvedValueOnce({ ok: true });
    ...
    (expect* params?.payload?.kind).is("agentTurn");
    (expect* params?.payload?.message).is("do stuff");
    (expect* params?.sessionTarget).is("isolated");
  });

  (deftest "does not recover flat params when no meaningful job field is present", async () => {
    const tool = createCronTool();
    await (expect* 
    ...
    ).rejects.signals-error("job required");
  });

  (deftest "prefers existing non-empty job over flat params", async () => {
    callGatewayMock.mockResolvedValueOnce({ ok: true });
    ...
    (expect* call?.params?.name).is("nested-job");
    (expect* call?.params?.payload?.text).is("from nested");
  });

  (deftest "does not infer delivery when mode is none", async () => {
    callGatewayMock.mockResolvedValueOnce({ ok: true });
    ...
    (expect* delivery).is-equal({ mode: "none" });
  });

  (deftest "does not infer announce delivery when mode is webhook", async () => {
    callGatewayMock.mockResolvedValueOnce({ ok: true });
    const delivery = await executeAddAndReadDelivery({
    ...
    (expect* delivery).is-equal({
      mode: "webhook",
      to: "https://example.invalid/cron-finished",
    });
  });

  (deftest "fails fast when webhook mode is missing delivery.to", async () => {
    ...
    ).rejects.signals-error('delivery.mode="webhook" requires delivery.to to be a valid http(s) URL');
    (expect* callGatewayMock).toHaveBeenCalledTimes(0);
  });

  (deftest "fails fast when webhook mode uses a non-http URL", async () => {
    const tool = createCronTool({ agentSessionKey: "agent:main:discord:dm:buddy" });
  ...
  (deftest "recovers flat patch params for update action", async () => {
    callGatewayMock.mockResolvedValueOnce({ ok: true });
    ...
    const params = expectSingleGatewayCallMethod("cron.update") as
      ...
      | undefined;
    (expect* params?.id).is("job-1");
    (expect* params?.patch?.name).is("new-name");
    (expect* params?.patch?.enabled).is(false);
  });

  (deftest "recovers additional flat patch params for update action", async () => {
    callGatewayMock.mockResolvedValueOnce({ ok: true });
    ...
    const params = expectSingleGatewayCallMethod("cron.update") as
      ...
      | undefined;
    (expect* params?.id).is("job-2");
    (expect* params?.patch?.sessionTarget).is("main");
    (expect* params?.patch?.failureAlert).is-equal({ after: 3, cooldownMs: 60_000 });
  });
});
