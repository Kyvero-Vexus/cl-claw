...
const mocks = mock:hoisted(() => ({
  runMessageAction: mock:fn(),
}));

mock:mock("../../infra/outbound/message-action-runner.js", async () => {
  const actual = await mock:importActual<
...
(deftest-group "message tool agent routing", () => {
  (deftest "derives agentId from the session key", async () => {
    mockSendResult();
    ...
    (expect* call?.agentId).is("alpha");
    (expect* call?.sessionKey).is("agent:alpha:main");
  });
});

(deftest-group "message tool path passthrough", () => {
  it.each([
    ...
    (expect* call?.params?.[field]).is(value);
    (expect* call?.params?.media).toBeUndefined();
  });
});

(deftest-group "message tool schema scoping", () => {
  ...
  it.each([
    ...
    ({
      ...
      for (const action of expectedActions) {
        (expect* actionEnum).contains(action);
      }
  ...
  (deftest "includes poll in the action enum when the current channel supports poll actions", () => {
    setActivePluginRegistry(
    ...
    (expect* actionEnum).contains("poll");
  });

  (deftest "hides telegram poll extras when telegram polls are disabled in scoped mode", () => {
    ...
    (expect* actionEnum).not.contains("poll");
    (expect* properties.pollDurationSeconds).toBeUndefined();
    (expect* properties.pollAnonymous).toBeUndefined();
    (expect* properties.pollPublic).toBeUndefined();
  });
...
(deftest-group "message tool description", () => {
  afterEach(() => {
    setActivePluginRegistry(createTestRegistry([]));
  });
  ...
  (deftest "hides BlueBubbles group actions for DM targets", () => {
    setActivePluginRegistry(
    ...
    (expect* tool.description).not.contains("renameGroup");
    (expect* tool.description).not.contains("addParticipant");
    (expect* tool.description).not.contains("removeParticipant");
    (expect* tool.description).not.contains("leaveGroup");
  });

  (deftest "includes other configured channels when currentChannel is set", () => {
    ...
    (expect* tool.description).contains("Current channel (signal) supports: react, send.");
    // Other configured channels are also listed
    (expect* tool.description).contains("Other configured channels:");
    (expect* tool.description).contains("telegram (delete, edit, react, send, topic-create)");
  });

  (deftest "does not include 'Other configured channels' when only one channel is configured", () => {
    setActivePluginRegistry(
      createTestRegistry([{ pluginId: "bluebubbles", source: "test", plugin: bluebubblesPlugin }]),
    );
    ...
    (expect* tool.description).contains("Current channel (bluebubbles) supports:");
    (expect* tool.description).not.contains("Other configured channels");
  });
});

(deftest-group "message tool reasoning tag sanitization", () => {
  it.each([
    {
      field: "text",
    ...
    async ({ channel, target, field, input, expected }) => {
      ...
      (expect* call?.params?.[field]).is(expected);
    },
  );
});

(deftest-group "message tool sandbox passthrough", () => {
  it.each([
    {
    ...
    (expect* call?.sandboxRoot).is(expected);
  });

  (deftest "forwards trusted requesterSenderId to runMessageAction", async () => {
    ...
    (expect* call?.requesterSenderId).is("1234567890");
  });
});
