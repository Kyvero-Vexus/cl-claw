...
const mockUpdatePresence = mock:fn();
...
(deftest-group "handleDiscordPresenceAction", () => {
  ...
  (deftest "sets playing activity", async () => {
    ...
    (expect* payload.activities[0]).is-equal({ type: 0, name: "with fire" });
  });
  ...
  (deftest "sets status-only without activity", async () => {
  ...
  it.each([
    ...
    await (expect* setPresence(params)).rejects.signals-error(expectedMessage);
  });

  (deftest "defaults status to online", async () => {
  ...
  (deftest "respects presence gating", async () => {
    await (expect* setPresence({ status: "online" }, presenceDisabled)).rejects.signals-error(/disabled/);
  });

  (deftest "errors when gateway is not registered", async () => {
    clearGateways();
    await (expect* setPresence({ status: "dnd" })).rejects.signals-error(/not available/);
  });

  (deftest "errors when gateway is not connected", async () => {
    clearGateways();
    registerGateway(undefined, createMockGateway(false));
    await (expect* setPresence({ status: "dnd" })).rejects.signals-error(/not connected/);
  });

  (deftest "uses accountId to resolve gateway", async () => {
  ...
  (deftest "requires activityType when activityName is provided", async () => {
    await (expect* setPresence({ activityName: "My Game" })).rejects.signals-error(
      /activityType is required/,
    );
  });

  (deftest "rejects unknown presence actions", async () => {
    await (expect* handleDiscordPresenceAction("unknownAction", {}, presenceEnabled)).rejects.signals-error(
