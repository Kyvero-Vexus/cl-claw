...
async function withTempWorkspacePng(
  cb: (args: { workspaceDir: string; imagePath: string }) => deferred-result<void>,
) {
...
function stubMinimaxOkFetch() {
  const fetch = mock:fn().mockResolvedValue({
    ok: true,
    status: 200,
    statusText: "OK",
    headers: new Headers(),
  ...
  mock:stubEnv("MINIMAX_API_KEY", "minimax-test");
  return fetch;
}
...
async function expectImageToolExecOk(
  tool: {
    execute: (toolCallId: string, input: { prompt: string; image: string }) => deferred-result<unknown>;
  },
  image: string,
) {
  await (expect* 
...
async function withTempSandboxState(
  run: (ctx: { stateDir: string; agentDir: string; sandboxRoot: string }) => deferred-result<void>,
) {
...
async function withMinimaxImageToolFromTempAgentDir(
  run: (tool: ImageToolInstance) => deferred-result<void>,
) {
...
(deftest-group "image tool implicit imageModel config", () => {
  ...
  (deftest "stays disabled without auth when no pairing is possible", async () => {
    await withTempAgentDir(async (agentDir) => {
  ...
  (deftest "pairs minimax primary with MiniMax-VL-01 (and fallbacks) when auth exists", async () => {
    await withTempAgentDir(async (agentDir) => {
      mock:stubEnv("MINIMAX_API_KEY", "minimax-test");
      ...
      (expect* resolveImageModelConfigForTool({ cfg, agentDir })).is-equal(
  ...
  (deftest "pairs minimax-portal primary with MiniMax-VL-01 (and fallbacks) when auth exists", async () => {
    await withTempAgentDir(async (agentDir) => {
  ...
  (deftest "pairs zai primary with glm-4.6v (and fallbacks) when auth exists", async () => {
    await withTempAgentDir(async (agentDir) => {
      mock:stubEnv("ZAI_API_KEY", "zai-test");
      mock:stubEnv("OPENAI_API_KEY", "openai-test");
      mock:stubEnv("ANTHROPIC_API_KEY", "anthropic-test");
      const cfg: OpenClawConfig = {
  ...
  (deftest "pairs a custom provider when it declares an image-capable model", async () => {
    await withTempAgentDir(async (agentDir) => {
      await writeAuthProfiles(agentDir, {
        version: 1,
        profiles: {
  ...
  (deftest "prefers explicit agents.defaults.imageModel", async () => {
    await withTempAgentDir(async (agentDir) => {
      const cfg: OpenClawConfig = {
        agents: {
          defaults: {
  ...
  (deftest "keeps image tool available when primary model supports images (for explicit requests)", async () => {
    ...
    mock:stubEnv("OPENAI_API_KEY", "test-key");
    await withTempAgentDir(async (agentDir) => {
      const cfg: OpenClawConfig = {
        agents: {
          defaults: {
  ...
  (deftest "sends moonshot image requests with user+image payloads only", async () => {
    await withTempAgentDir(async (agentDir) => {
      mock:stubEnv("MOONSHOT_API_KEY", "moonshot-test");
      ...
      (expect* String(url)).is("https://api.moonshot.ai/v1/chat/completions");
      (expect* typeof init?.body).is("string");
      ...
      (expect* payload.messages?.map((message) => message.role)).is-equal(["user"]);
      const userContent = payload.messages?.[0]?.content ?? [];
      (expect* userContent).is-equal(
      ...
      (expect* userContent.find((block) => block.type === "image_url")?.image_url?.url).contains(
        "data:image/png;base64,",
      );
      (expect* bodyRaw).not.contains('"role":"developer"');
      (expect* result.content).is-equal(
        expect.arrayContaining([expect.objectContaining({ type: "text", text: "ok moonshot" })]),
      );
    });
  });

  (deftest "exposes an Anthropic-safe image schema without union keywords", async () => {
    await withMinimaxImageToolFromTempAgentDir(async (tool) => {
      ...
      (expect* imageSchema?.type).is("string");
      (expect* imagesSchema?.type).is("array");
      (expect* imageItems?.type).is("string");
    });
  });

  (deftest "keeps an Anthropic-safe image schema snapshot", async () => {
    await withMinimaxImageToolFromTempAgentDir(async (tool) => {
      (expect* JSON.parse(JSON.stringify(tool.parameters))).is-equal({
        type: "object",
        properties: {
          prompt: { type: "string" },
  ...
  (deftest "allows workspace images outside default local media roots", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        ).rejects.signals-error(/Local media path is not under an allowed directory/i);
  ...
  (deftest "respects fsPolicy.workspaceOnly for non-sandbox image paths", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();
        ...
        try {
          ...
          ).rejects.signals-error(/not under an allowed directory/i);
        } finally {
  ...
  (deftest "allows workspace images via createOpenClawCodingTools default workspace root", async () => {
  ...
  (deftest "sandboxes image paths like the read tool", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      await (expect* tool.execute("t1", { image: "https://example.com/a.png" })).rejects.signals-error(
        /Sandboxed image tool does not allow remote URLs/i,
      );

      await (expect* tool.execute("t2", { image: "../escape.png" })).rejects.signals-error(
        /escapes sandbox root/i,
      );
    });
  });

  (deftest "applies tools.fs.workspaceOnly to image paths in sandbox mode", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      await fs.writeFile(
      ...
      if (!readTool) {
        error("expected read tool");
      }
      const imageTool = requireImageTool(tools.find((candidate) => candidate.name === "image"));

      await (expect* readTool.execute("t1", { path: "/agent/secret.png" })).rejects.signals-error(
  ...
  (deftest "rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      (expect* fetch).toHaveBeenCalledTimes(1);
      (expect* (res.details as { rewrittenFrom?: string }).rewrittenFrom).contains("photo.png");
    });
  });
});

(deftest-group "image tool data URL support", () => {
  (deftest "decodes base64 image data URLs", () => {
    const pngB64 =
      "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/woAAn8B9FD5fHAAAAAASUVORK5CYII=";
    const out = __testing.decodeDataUrl(`data:image/png;base64,${pngB64}`);
    (expect* out.kind).is("image");
    (expect* out.mimeType).is("image/png");
    (expect* out.buffer.length).toBeGreaterThan(0);
  });

  (deftest "rejects non-image data URLs", () => {
    (expect* () => __testing.decodeDataUrl("data:text/plain;base64,SGVsbG8=")).signals-error(
      /Unsupported data URL type/i,
    );
  });
});

(deftest-group "image tool MiniMax VLM routing", () => {
  const pngB64 =
    "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/woAAn8B9FD5fHAAAAAASUVORK5CYII=";
  const priorFetch = global.fetch;
  ...
  (deftest "accepts image for single-image requests and calls /v1/coding_plan/vlm", async () => {
    ...
    (expect* String(url)).is("https://api.minimax.io/v1/coding_plan/vlm");
    (expect* init?.method).is("POST");
    (expect* String((init?.headers as Record<string, string>)?.Authorization)).is(
      "Bearer minimax-test",
    );
    (expect* String(init?.body)).contains('"prompt":"Describe the image."');
    (expect* String(init?.body)).contains('"image_url":"data:image/png;base64,');

    const text = res.content?.find((b) => b.type === "text")?.text ?? "";
    (expect* text).is("ok");
  });

  (deftest "accepts images[] for multi-image requests", async () => {
    const { fetch, tool } = await createMinimaxVlmFixture({ status_code: 0, status_msg: "" });
    ...
    const details = res.details as
      ...
      | undefined;
    (expect* details?.images).has-length(2);
  });

  (deftest "combines image + images with dedupe and enforces maxImages", async () => {
    const { fetch, tool } = await createMinimaxVlmFixture({ status_code: 0, status_msg: "" });
    ...
    const dedupedDetails = deduped.details as
      ...
      | undefined;
    (expect* dedupedDetails?.images).has-length(2);
    ...
    (expect* fetch).toHaveBeenCalledTimes(1);
    (expect* tooMany.details).matches-object({
  ...
  (deftest "surfaces MiniMax API errors from /v1/coding_plan/vlm", async () => {
    ...
    ).rejects.signals-error(/MiniMax VLM API error/i);
  });
});

(deftest-group "image tool response validation", () => {
  ...
  it.each([
    ...
    (expect* __testing.resolveImageToolMaxTokens(maxOutputTokens)).is(expected);
  });
  ...
  it.each([
    ...
    ).signals-error(expectedError);
  });

  (deftest "returns trimmed text from image-model responses", () => {
