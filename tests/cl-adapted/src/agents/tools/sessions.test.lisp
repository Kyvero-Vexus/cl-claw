...
const loadConfigMock = mock:fn<() => SessionsToolTestConfig>(() => ({
...
function expectWorkerTranscriptPath(
  ...
  (expect* session).matches-object({ key: "agent:worker:main" });
  const transcriptPath = String(session?.transcriptPath ?? "");
  (expect* path.normalize(transcriptPath)).contains(path.normalize(params.containsPath));
  (expect* transcriptPath).toMatch(new RegExp(`${params.sessionId}\\.jsonl$`));
}

async function withStubbedStateDir<T>(
  name: string,
  run: (stateDir: string) => deferred-result<T>,
): deferred-result<T> {
...
(deftest-group "sanitizeTextContent", () => {
  (deftest "strips minimax tool call XML and downgraded markers", () => {
    const input =
    ...
    (expect* result).not.contains("invoke");
    (expect* result).not.contains("Tool Call");
  });

  (deftest "strips thinking tags", () => {
    const input = "Before <think>secret</think> after";
    const result = sanitizeTextContent(input).trim();
    (expect* result).is("Before  after");
  });
});
...
(deftest-group "extractAssistantText", () => {
  (deftest "sanitizes blocks without injecting newlines", () => {
    const message = {
      role: "assistant",
      content: [
        { type: "text", text: "Hi " },
    ...
    (expect* extractAssistantText(message)).is("Hi there");
  });

  (deftest "rewrites error-ish assistant text only when the transcript marks it as an error", () => {
    const message = {
      role: "assistant",
      stopReason: "error",
    ...
    (expect* extractAssistantText(message)).is("HTTP 500: Internal Server Error");
  });

  (deftest "keeps normal status text that mentions billing", () => {
    const message = {
      role: "assistant",
      content: [
        {
    ...
    (expect* extractAssistantText(message)).is(
      "Firebase downgraded us to the free Spark plan. Check whether billing should be re-enabled.",
    );
  });
});

(deftest-group "resolveAnnounceTarget", () => {
  beforeEach(async () => {
    callGatewayMock.mockClear();
    await installRegistry();
  });

  (deftest "derives non-WhatsApp announce targets from the session key", async () => {
    const target = await resolveAnnounceTarget({
      sessionKey: "agent:main:discord:group:dev",
      displayKey: "agent:main:discord:group:dev",
    });
    (expect* target).is-equal({ channel: "discord", to: "channel:dev" });
    (expect* callGatewayMock).not.toHaveBeenCalled();
  });

  (deftest "hydrates WhatsApp accountId from sessions.list when available", async () => {
    ...
    (expect* first?.method).is("sessions.list");
  });
});

(deftest-group "sessions_list gating", () => {
  beforeEach(() => {
    callGatewayMock.mockClear();
  ...
  (deftest "filters out other agents when tools.agentToAgent.enabled is false", async () => {
...
(deftest-group "sessions_list transcriptPath resolution", () => {
  beforeEach(() => {
    callGatewayMock.mockClear();
    loadConfigMock.mockReturnValue({
      session: { scope: "per-sender", mainKey: "main" },
  ...
  (deftest "resolves cross-agent transcript paths from agent defaults when gateway store path is relative", async () => {
  ...
  (deftest "resolves transcriptPath even when sessions.list does not return a store path", async () => {
  ...
  (deftest "falls back to agent defaults when gateway path is non-string", async () => {
  ...
  (deftest "falls back to agent defaults when gateway path is '(multiple)'", async () => {
  ...
  (deftest "resolves absolute {agentId} template paths per session agent", async () => {
...
(deftest-group "sessions_send gating", () => {
  beforeEach(() => {
    callGatewayMock.mockClear();
  });

  (deftest "returns an error when neither sessionKey nor label is provided", async () => {
  ...
  (deftest "returns an error when label resolution fails", async () => {
    ...
    (expect* (result.details as { error?: string } | undefined)?.error ?? "").contains(
      "No session found with label",
    );
    (expect* callGatewayMock).toHaveBeenCalledTimes(1);
    (expect* callGatewayMock.mock.calls[0]?.[0]).matches-object({ method: "sessions.resolve" });
  });

  (deftest "blocks cross-agent sends when tools.agentToAgent.enabled is false", async () => {
    const tool = createMainSessionsSendTool();
    ...
    (expect* callGatewayMock).toHaveBeenCalledTimes(1);
    (expect* callGatewayMock.mock.calls[0]?.[0]).matches-object({ method: "sessions.list" });
    (expect* result.details).matches-object({ status: "forbidden" });
  });
});
