...
const hoisted = mock:hoisted(() => ({
  loadPluginManifestRegistry: mock:fn(),
}));

mock:mock("../../plugins/manifest-registry.js", () => ({
...
(deftest-group "resolvePluginSkillDirs", () => {
  it.each([
    {
    ...
    (expect* dirs).is-equal(expectedDirs({ acpxRoot, helperRoot }));
  });

  (deftest "rejects plugin skill paths that escape the plugin root", async () => {
    ...
    (expect* dirs).is-equal([path.resolve(pluginRoot, "skills")]);
  });

  (deftest "rejects plugin skill symlinks that resolve outside plugin root", async () => {
    const { workspaceDir, pluginRoot, outsideSkills } = await setupPluginOutsideSkills();
