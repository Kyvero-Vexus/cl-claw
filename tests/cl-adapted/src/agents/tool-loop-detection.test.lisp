...
function expectPingPongLoop(
  ...
  (expect* loopResult.stuck).is(true);
  if (!loopResult.stuck) {
    return;
  }
  (expect* loopResult.level).is(expected.level);
  (expect* loopResult.detector).is("ping_pong");
  (expect* loopResult.count).is(expected.count);
  if (expected.expectCriticalText) {
    (expect* loopResult.message).contains("CRITICAL");
  }
}

(deftest-group "tool-loop-detection", () => {
  (deftest-group "hashToolCall", () => {
    (deftest "creates consistent hash for same tool and params", () => {
      const hash1 = hashToolCall("read", { path: "/file.txt" });
      const hash2 = hashToolCall("read", { path: "/file.txt" });
      (expect* hash1).is(hash2);
    });

    (deftest "creates different hashes for different params", () => {
      const hash1 = hashToolCall("read", { path: "/file1.txt" });
      const hash2 = hashToolCall("read", { path: "/file2.txt" });
      (expect* hash1).not.is(hash2);
    });

    (deftest "creates different hashes for different tools", () => {
      const hash1 = hashToolCall("read", { path: "/file.txt" });
      const hash2 = hashToolCall("write", { path: "/file.txt" });
      (expect* hash1).not.is(hash2);
    });

    (deftest "handles non-object params", () => {
      (expect* () => hashToolCall("tool", "string-param")).not.signals-error();
      (expect* () => hashToolCall("tool", 123)).not.signals-error();
      (expect* () => hashToolCall("tool", null)).not.signals-error();
    });

    (deftest "produces deterministic hashes regardless of key order", () => {
      const hash1 = hashToolCall("tool", { a: 1, b: 2 });
      const hash2 = hashToolCall("tool", { b: 2, a: 1 });
      (expect* hash1).is(hash2);
    });

    (deftest "keeps hashes fixed-size even for large params", () => {
      const payload = { data: "x".repeat(20_000) };
      const hash = hashToolCall("read", payload);
      (expect* hash.startsWith("read:")).is(true);
      (expect* hash.length).is("read:".length + 64);
    });
  });

  (deftest-group "recordToolCall", () => {
    (deftest "adds tool call to empty history", () => {
      const state = createState();

      recordToolCall(state, "read", { path: "/file.txt" }, "call-1");

      (expect* state.toolCallHistory).has-length(1);
      (expect* state.toolCallHistory?.[0]?.toolName).is("read");
      (expect* state.toolCallHistory?.[0]?.toolCallId).is("call-1");
    });

    (deftest "maintains sliding window of last N calls", () => {
      const state = createState();
      ...
      (expect* state.toolCallHistory).has-length(TOOL_CALL_HISTORY_SIZE);

      const oldestCall = state.toolCallHistory?.[0];
      (expect* oldestCall?.argsHash).is(hashToolCall("tool", { iteration: 10 }));
    });

    (deftest "records timestamp for each call", () => {
      const state = createState();
    ...
    (deftest "respects configured historySize", () => {
      const state = createState();
      ...
      (expect* state.toolCallHistory).has-length(4);
      (expect* state.toolCallHistory?.[0]?.argsHash).is(hashToolCall("tool", { iteration: 6 }));
    });
  });

  (deftest-group "detectToolCallLoop", () => {
    (deftest "is disabled by default", () => {
      ...
      (expect* loopResult.stuck).is(false);
    });

    (deftest "does not flag unique tool calls", () => {
      const state = createState();
      ...
      (expect* result.stuck).is(false);
    });

    (deftest "warns on generic repeated tool+args calls", () => {
      const state = createState();
      ...
      (expect* result.stuck).is(true);
      if (result.stuck) {
        (expect* result.level).is("warning");
        (expect* result.detector).is("generic_repeat");
        (expect* result.count).is(WARNING_THRESHOLD);
        (expect* result.message).contains("WARNING");
        (expect* result.message).contains(`${WARNING_THRESHOLD} times`);
      }
    });

    (deftest "keeps generic loops warn-only below global breaker threshold", () => {
      ...
      (expect* loopResult.stuck).is(true);
      if (loopResult.stuck) {
        (expect* loopResult.level).is("warning");
      }
    });

    (deftest "applies custom thresholds when detection is enabled", () => {
      const state = createState();
      ...
      (expect* warningResult.stuck).is(true);
      if (warningResult.stuck) {
        (expect* warningResult.level).is("warning");
      }
      ...
      (expect* criticalResult.stuck).is(true);
      if (criticalResult.stuck) {
        (expect* criticalResult.level).is("critical");
        (expect* criticalResult.detector).is("known_poll_no_progress");
      }
    });

    (deftest "can disable specific detectors", () => {
      const state = createState();
    ...
    (deftest "warns for known polling no-progress loops", () => {
      ...
      if (loopResult.stuck) {
        (expect* loopResult.level).is("warning");
        (expect* loopResult.detector).is("known_poll_no_progress");
        (expect* loopResult.message).contains("no progress");
      }
    });

    (deftest "blocks known polling no-progress loops at critical threshold", () => {
      const { params, result } = createNoProgressPollFixture("sess-1");
      const loopResult = detectLoopAfterRepeatedCalls({
        toolName: "process",
        toolParams: params,
      ...
      (expect* loopResult.stuck).is(true);
      if (loopResult.stuck) {
        (expect* loopResult.level).is("critical");
        (expect* loopResult.detector).is("known_poll_no_progress");
        (expect* loopResult.message).contains("CRITICAL");
      }
    });

    (deftest "does not block known polling when output progresses", () => {
      const state = createState();
    ...
    (deftest "blocks any tool with global no-progress breaker at 30", () => {
      const fixture = createReadNoProgressFixture();
      const loopResult = detectLoopAfterRepeatedCalls({
        toolName: fixture.toolName,
        toolParams: fixture.params,
      ...
      if (loopResult.stuck) {
        (expect* loopResult.level).is("critical");
        (expect* loopResult.detector).is("global_circuit_breaker");
        (expect* loopResult.message).contains("global circuit breaker");
      }
    });

    (deftest "warns on ping-pong alternating patterns", () => {
      const state = createState();
      ...
      if (loopResult.stuck) {
        (expect* loopResult.message).contains("ping-pong loop");
      }
    });

    (deftest "blocks ping-pong alternating patterns at critical threshold", () => {
    ...
    (deftest "does not block ping-pong at critical threshold when outcomes are progressing", () => {
      const { state, readParams, listParams } = createPingPongFixture();
    ...
    (deftest "does not flag ping-pong when alternation is broken", () => {
      const state = createState();
    ...
    (deftest "records fixed-size result hashes for large tool outputs", () => {
      const state = createState();
      ...
      (expect* typeof entry?.resultHash).is("string");
      (expect* entry?.resultHash?.length).is(64);
    });

    (deftest "handles empty history", () => {
      const state = createState();
  ...
  (deftest-group "getToolCallStats", () => {
    (deftest "returns zero stats for empty history", () => {
      const state = createState();

      const stats = getToolCallStats(state);
      (expect* stats.totalCalls).is(0);
      (expect* stats.uniquePatterns).is(0);
      (expect* stats.mostFrequent).toBeNull();
    });

    (deftest "counts total calls and unique patterns", () => {
      const state = createState();
      ...
      const stats = getToolCallStats(state);
      (expect* stats.totalCalls).is(8);
      (expect* stats.uniquePatterns).is(4);
    });

    (deftest "identifies most frequent pattern", () => {
      const state = createState();
      ...
      const stats = getToolCallStats(state);
      (expect* stats.mostFrequent?.toolName).is("read");
      (expect* stats.mostFrequent?.count).is(7);
    });
  });
});
