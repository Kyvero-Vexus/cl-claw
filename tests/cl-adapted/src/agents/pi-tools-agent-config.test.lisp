...
(deftest-group "Agent-specific tool filtering", () => {
  ...
  function expectReadOnlyToolSet(toolNames: string[], extraDenied: string[] = []) {
    ...
    (expect* toolNames).not.contains("exec");
    (expect* toolNames).not.contains("write");
    (expect* toolNames).not.contains("apply_patch");
    for (const toolName of extraDenied) {
      (expect* toolNames).not.contains(toolName);
    }
  }
...
*** End Patch`;
  ...
  (deftest "should apply global tool policy when no agent-specific policy exists", () => {
  ...
  (deftest "should keep global tool policy when agent only sets tools.elevated", () => {
    const cfg = createMainAgentConfig({
      tools: {
        deny: ["write"],
      },
    ...
    const toolNames = tools.map((t) => t.name);
    (expect* toolNames).contains("exec");
    (expect* toolNames).contains("read");
    (expect* toolNames).not.contains("write");
    (expect* toolNames).not.contains("apply_patch");
  });

  (deftest "should allow apply_patch when exec is allow-listed and applyPatch is enabled", () => {
    const cfg: OpenClawConfig = {
      tools: {
        allow: ["read", "exec"],
        exec: {
  ...
  (deftest "defaults apply_patch to workspace-only (blocks traversal)", async () => {
    await withApplyPatchEscapeCase({}, async ({ applyPatchTool, escapedPath, patch }) => {
      await (expect* applyPatchTool.execute("tc1", { input: patch })).rejects.signals-error(
  ...
  (deftest "allows disabling apply_patch workspace-only via config (dangerous)", async () => {
  ...
  (deftest "should apply agent-specific tool policy", () => {
    const cfg: OpenClawConfig = {
      tools: {
  ...
  (deftest "should apply provider-specific tool policy", () => {
    const cfg: OpenClawConfig = {
      tools: {
        allow: ["read", "write", "exec"],
        byProvider: {
  ...
  (deftest "should apply provider-specific tool profile overrides", () => {
    const cfg: OpenClawConfig = {
      tools: {
        profile: "coding",
        byProvider: {
    ...
    const toolNames = tools.map((t) => t.name);
    (expect* toolNames).is-equal(["session_status"]);
  });

  (deftest "should allow different tool policies for different agents", () => {
    const cfg: OpenClawConfig = {
      agents: {
        list: [
          {
    ...
    (expect* mainToolNames).contains("exec");
    (expect* mainToolNames).contains("write");
    (expect* mainToolNames).contains("edit");
    (expect* mainToolNames).not.contains("apply_patch");
    ...
    (expect* familyToolNames).contains("read");
    (expect* familyToolNames).not.contains("exec");
    (expect* familyToolNames).not.contains("write");
    (expect* familyToolNames).not.contains("edit");
    (expect* familyToolNames).not.contains("apply_patch");
  });

  (deftest "should apply group tool policy overrides (group-specific beats wildcard)", () => {
    const cfg: OpenClawConfig = {
      channels: {
        whatsapp: {
          groups: {
    ...
    (expect* trustedNames).contains("read");
    (expect* trustedNames).contains("exec");
    ...
    (expect* defaultNames).contains("read");
    (expect* defaultNames).not.contains("exec");
  });

  (deftest "should apply per-sender tool policies for group tools", () => {
    const cfg: OpenClawConfig = {
      channels: {
        whatsapp: {
          groups: {
    ...
    (expect* aliceNames).contains("read");
    (expect* aliceNames).contains("exec");
    ...
    (expect* bobNames).contains("read");
    (expect* bobNames).not.contains("exec");
  });

  (deftest "should not let default sender policy override group tools", () => {
    const cfg: OpenClawConfig = {
      channels: {
        whatsapp: {
          groups: {
    ...
    (expect* adminNames).contains("read");
    (expect* adminNames).not.contains("exec");
  });

  (deftest "should resolve telegram group tool policy for topic session keys", () => {
    const cfg: OpenClawConfig = {
      channels: {
        telegram: {
          groups: {
    ...
    (expect* names).not.contains("exec");
  });

  (deftest "should inherit group tool policy for subagents from spawnedBy session keys", () => {
    const cfg: OpenClawConfig = {
      channels: {
        whatsapp: {
          groups: {
  ...
  (deftest "should apply global tool policy before agent-specific policy", () => {
    const cfg: OpenClawConfig = {
      tools: {
    ...
    (expect* toolNames).not.contains("browser");
    (expect* toolNames).not.contains("exec");
    (expect* toolNames).not.contains("process");
    (expect* toolNames).not.contains("apply_patch");
  });

  (deftest "should work with sandbox tools filtering", () => {
  ...
  (deftest "should run exec synchronously when process is denied", async () => {
    const cfg: OpenClawConfig = {
      tools: {
        deny: ["process"],
        exec: {
  ...
  (deftest "keeps sandbox as the implicit exec host default without forcing gateway approvals", async () => {
    const tools = createOpenClawCodingTools({
      config: {},
      sessionKey: "agent:main:main",
    ...
    ).rejects.signals-error("exec host not allowed");
  });

  (deftest "fails closed when exec host=sandbox is requested without sandbox runtime", async () => {
    const tools = createOpenClawCodingTools({
      config: {},
      sessionKey: "agent:main:main",
    ...
    ).rejects.signals-error("exec host=sandbox is configured");
  });

  (deftest "should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    (expect* mainDetails?.status).is("completed");
    await (expect* 
  ...
  (deftest "applies explicit agentId exec defaults when sessionKey is opaque", async () => {
