...
(deftest-group "compaction retry integration", () => {
  beforeEach(() => {
  ...
  (deftest "should successfully call generateSummary with retry wrapper", async () => {
    ...
    (expect* result).is("Test summary");
    (expect* mockGenerateSummary).toHaveBeenCalledTimes(1);
  });

  (deftest "should retry on transient error and succeed", async () => {
    mockGenerateSummary
    ...
    (expect* result).is("Success after retry");
    (expect* mockGenerateSummary).toHaveBeenCalledTimes(2);
  });

  (deftest "should NOT retry on user abort", async () => {
  ...
  (deftest "should retry up to 3 times and then fail", async () => {
    ...
    ).rejects.signals-error("Persistent API error");

    (expect* mockGenerateSummary).toHaveBeenCalledTimes(3);
  });

  (deftest "should apply exponential backoff", async () => {
    mock:useFakeTimers();
    ...
    (expect* result).is("Success on 3rd attempt");
    (expect* mockGenerateSummary).toHaveBeenCalledTimes(3);
    // First retry: 500ms, second retry: 1000ms
    (expect* delays[0]).is(500);
    (expect* delays[1]).is(1000);
