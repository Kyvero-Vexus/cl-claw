...
(deftest-group "sanitizeToolUseResultPairing", () => {
  ...
  (deftest "moves tool results directly after tool calls and inserts missing results", () => {
    ...
    (expect* (out[1] as { toolCallId?: string }).toolCallId).is("call_1");
    (expect* out[2]?.role).is("toolResult");
    (expect* (out[2] as { toolCallId?: string }).toolCallId).is("call_2");
    (expect* out[3]?.role).is("user");
  });

  (deftest "repairs blank tool result names from matching tool calls", () => {
    const input = castAgentMessages([
      {
        role: "assistant",
        content: [{ type: "toolCall", id: "call_1", name: "read", arguments: {} }],
    ...
    (expect* toolResult?.toolName).is("read");
  });

  (deftest "drops duplicate tool results for the same id within a span", () => {
    const input = castAgentMessages([
    ...
    const out = sanitizeToolUseResultPairing(input);
    (expect* out.filter((m) => m.role === "toolResult")).has-length(1);
  });

  (deftest "drops duplicate tool results for the same id across the transcript", () => {
    ...
    (expect* results[0]?.toolCallId).is("call_1");
  });

  (deftest "drops orphan tool results that do not match any tool call", () => {
    const input = castAgentMessages([
    ...
    const out = sanitizeToolUseResultPairing(input);
    (expect* out.some((m) => m.role === "toolResult")).is(false);
    (expect* out.map((m) => m.role)).is-equal(["user", "assistant"]);
  });

  (deftest "skips tool call extraction for assistant messages with stopReason 'error'", () => {
    ...
    (expect* result.messages[0]?.role).is("assistant");
    (expect* result.messages[1]?.role).is("user");
    (expect* result.messages).has-length(2);
  });

  (deftest "skips tool call extraction for assistant messages with stopReason 'aborted'", () => {
  ...
  (deftest "still repairs tool results for normal assistant messages with stopReason 'toolUse'", () => {
    ...
    (expect* result.added[0]?.toolCallId).is("call_normal");
  });

  (deftest "drops orphan tool results that follow an aborted assistant message", () => {
    ...
    (expect* result.droppedOrphanCount).is(1);
    (expect* result.messages).has-length(2);
    (expect* result.messages[0]?.role).is("assistant");
    (expect* result.messages[1]?.role).is("user");
...
(deftest-group "sanitizeToolCallInputs", () => {
  ...
  (deftest "drops tool calls missing input or arguments", () => {
    const input = castAgentMessages([
      {
        role: "assistant",
    ...
    (expect* out.map((m) => m.role)).is-equal(["user"]);
  });
  ...
  it.each([
    ...
    (expect* ids).is-equal(expectedIds);
  });

  (deftest "keeps valid tool calls and preserves text blocks", () => {
    const input = castAgentMessages([
      {
        role: "assistant",
        content: [
    ...
    const types = Array.isArray(assistant.content)
      ...
      : [];
    (expect* types).is-equal(["text", "toolUse"]);
  });
  ...
  it.each([
    ...
    (expect* names).is-equal(expectedNames);
  });

  (deftest "preserves toolUse input shape for sessions_spawn when no attachments are present", () => {
    const input = castAgentMessages([
      {
        role: "assistant",
        content: [
    ...
    (expect* toolCalls).has-length(1);
    (expect* Object.hasOwn(toolCalls[0] ?? {}, "input")).is(true);
    (expect* Object.hasOwn(toolCalls[0] ?? {}, "arguments")).is(false);
    (expect* (toolCalls[0] ?? {}).input).is-equal({ task: "hello" });
  });

  (deftest "redacts sessions_spawn attachments for mixed-case and padded tool names", () => {
    const input = castAgentMessages([
      {
        role: "assistant",
        content: [
    ...
    (expect* toolCalls).has-length(1);
    (expect* (toolCalls[0] ?? {}).name).is("SESSIONS_SPAWN");
    ...
    (expect* attachments[0]?.content).is("__OPENCLAW_REDACTED__");
  });
  (deftest "preserves other block properties when trimming tool names", () => {
    const toolCalls = sanitizeAssistantToolCalls([
      { type: "toolCall", id: "call_1", name: " read ", arguments: { path: "/tmp/test" } },
    ]);

    (expect* toolCalls).has-length(1);
    (expect* (toolCalls[0] as { name?: unknown }).name).is("read");
    (expect* (toolCalls[0] as { id?: unknown }).id).is("call_1");
    (expect* (toolCalls[0] as { arguments?: unknown }).arguments).is-equal({ path: "/tmp/test" });
  });
});

(deftest-group "stripToolResultDetails", () => {
  (deftest "removes details only from toolResult messages", () => {
    const input = castAgentMessages([
      {
        role: "toolResult",
        toolCallId: "call_1",
    ...
    (expect* Object.hasOwn(out[0] ?? {}, "details")).is(false);
    (expect* (out[0] ?? {}).role).is("toolResult");

    // Non-toolResult messages are preserved as-is.
    (expect* Object.hasOwn(out[1] ?? {}, "details")).is(true);
    (expect* (out[1] ?? {}).role).is("assistant");
    (expect* (out[2] ?? {}).role).is("user");
  });

  (deftest "returns the same array reference when there are no toolResult details", () => {
    const input = castAgentMessages([
