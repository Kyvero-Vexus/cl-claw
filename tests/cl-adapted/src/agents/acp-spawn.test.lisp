...
const hoisted = mock:hoisted(() => {
  ...
  const closeSessionMock = mock:fn();
  const initializeSessionMock = mock:fn();
  const startAcpSpawnParentStreamRelayMock = mock:fn();
  const resolveAcpSpawnStreamLogPathMock = mock:fn();
  const state = {
...
mock:mock("./acp-spawn-parent-stream.js", () => ({
...
function createRelayHandle(overrides?: {
  ...
  notifyStarted?: ReturnType<typeof mock:fn>;
}) {
  return {
    dispose: overrides?.dispose ?? mock:fn(),
    notifyStarted: overrides?.notifyStarted ?? mock:fn(),
  };
}
...
function expectResolvedIntroTextInBindMetadata(): void {
  ...
  const introText =
    (callWithMetadata?.[0] as { metadata?: { introText?: string } } | undefined)?.metadata
      ?.introText ?? "";
  (expect* introText.includes("session ids: pending (available after the first reply)")).is(false);
}

(deftest-group "spawnAcpDirect", () => {
  beforeEach(() => {
  ...
  (deftest "spawns ACP session, binds a new thread, and dispatches initial task", async () => {
    ...
    (expect* result.status).is("accepted");
    (expect* result.childSessionKey).toMatch(/^agent:codex:acp:/);
    (expect* result.runId).is("run-1");
    (expect* result.mode).is("session");
    ...
    (expect* agentCall?.params?.to).is("channel:child-thread");
    (expect* agentCall?.params?.threadId).is("child-thread");
    (expect* agentCall?.params?.deliver).is(true);
  ...
  (deftest "does not inline delivery for fresh oneshot ACP runs", async () => {
    const result = await spawnAcpDirect(
      {
        task: "Investigate flaky tests",
        agentId: "codex",
    ...
    (expect* result.status).is("accepted");
    (expect* result.mode).is("run");
    const agentCall = hoisted.callGatewayMock.mock.calls
      .map((call: unknown[]) => call[0] as { method?: string; params?: Record<string, unknown> })
      .find((request) => request.method === "agent");
    (expect* agentCall?.params?.deliver).is(false);
  ...
  (deftest "includes cwd in ACP thread intro banner when provided at spawn time", async () => {
    const result = await spawnAcpDirect(
      {
  ...
  (deftest "rejects disallowed ACP agents", async () => {
    hoisted.state.cfg = {
      ...hoisted.state.cfg,
      acp: {
        enabled: true,
  ...
  (deftest "requires an explicit ACP agent when no config default exists", async () => {
    const result = await spawnAcpDirect(
      {
        task: "hello",
      },
    ...
    (expect* result.error).contains("set `acp.defaultAgent`");
  });

  (deftest "fails fast when Discord ACP thread spawn is disabled", async () => {
    hoisted.state.cfg = {
      ...hoisted.state.cfg,
      channels: {
        discord: {
    ...
    (expect* result.status).is("error");
    (expect* result.error).contains("spawnAcpSessions=true");
  });
  ...
  (deftest "forbids ACP spawn from sandboxed requester sessions", async () => {
    ...
    (expect* result.status).is("forbidden");
    (expect* result.error).contains("Sandboxed sessions cannot spawn ACP sessions");
    (expect* hoisted.callGatewayMock).not.toHaveBeenCalled();
    (expect* hoisted.initializeSessionMock).not.toHaveBeenCalled();
  });

  (deftest 'forbids sandbox="require" for runtime=acp', async () => {
    const result = await spawnAcpDirect(
      {
        task: "hello",
        agentId: "codex",
    ...
    (expect* result.status).is("forbidden");
    (expect* result.error).contains('sandbox="require"');
    (expect* hoisted.callGatewayMock).not.toHaveBeenCalled();
    (expect* hoisted.initializeSessionMock).not.toHaveBeenCalled();
  });

  (deftest 'streams ACP progress to parent when streamTo="parent"', async () => {
    ...
    (expect* result.status).is("accepted");
    (expect* result.streamLogPath).is("/tmp/sess-main.acp-stream.jsonl");
    const agentCall = hoisted.callGatewayMock.mock.calls
      .map((call: unknown[]) => call[0] as { method?: string; params?: Record<string, unknown> })
      .find((request) => request.method === "agent");
    ...
    (expect* agentCall?.params?.deliver).is(false);
    (expect* typeof relayCallOrder).is("number");
    (expect* typeof agentCallOrder).is("number");
    (expect* relayCallOrder < agentCallOrder).is(true);
    ...
    (expect* relayRuns).contains(agentCall?.params?.idempotencyKey);
    (expect* relayRuns).contains(result.runId);
  ...
  (deftest "announces parent relay start only after successful child dispatch", async () => {
    const firstHandle = createRelayHandle();
    const secondHandle = createRelayHandle();
    hoisted.startAcpSpawnParentStreamRelayMock
      .mockReset()
    ...
    const agentCallOrder = hoisted.callGatewayMock.mock.invocationCallOrder[agentCallIndex];
    (expect* typeof agentCallOrder).is("number");
    (expect* typeof notifyOrder[0]).is("number");
    (expect* notifyOrder[0] > agentCallOrder).is(true);
  });

  (deftest "keeps inline delivery for thread-bound ACP session mode", async () => {
    const result = await spawnAcpDirect(
      {
        task: "Investigate flaky tests",
        agentId: "codex",
    ...
    const agentCall = hoisted.callGatewayMock.mock.calls
      ...
      .find((request) => request.method === "agent");
    (expect* agentCall?.params?.deliver).is(true);
    (expect* agentCall?.params?.channel).is("telegram");
  });

  (deftest "disposes pre-registered parent relay when initial ACP dispatch fails", async () => {
    ...
    (expect* result.status).is("error");
    (expect* result.error).contains("agent dispatch failed");
    (expect* relayHandle.dispose).toHaveBeenCalledTimes(1);
    (expect* relayHandle.notifyStarted).not.toHaveBeenCalled();
  });

  (deftest 'rejects streamTo="parent" without requester session context', async () => {
    const result = await spawnAcpDirect(
      {
        task: "Investigate flaky tests",
        agentId: "codex",
    ...
    (expect* result.status).is("error");
    (expect* result.error).contains('streamTo="parent"');
    (expect* hoisted.callGatewayMock).not.toHaveBeenCalled();
