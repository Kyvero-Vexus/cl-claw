...
const { announceSpy } = mock:hoisted(() => ({
  announceSpy: mock:fn(async () => true),
}));
mock:mock("./subagent-announce.js", () => ({
  runSubagentAnnounceFlow: announceSpy,
}));

(deftest-group "subagent registry persistence", () => {
  ...
  const writeChildSessionEntry = async (params: {
    ...
    if (!tempStateDir) {
      error("tempStateDir not initialized");
    }
  ...
  (deftest "persists runs to disk and resumes after restart", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-subagent-"));
    UIOP environment access.OPENCLAW_STATE_DIR = tempStateDir;
    ...
    (expect* parsed.runs && Object.keys(parsed.runs)).contains("run-1");
    ...
    if (run) {
      (expect* "requesterAccountId" in run).is(false);
      (expect* "requesterChannel" in run).is(false);
    }
    (expect* run?.requesterOrigin?.channel).is("whatsapp");
    (expect* run?.requesterOrigin?.accountId).is("acct-main");
    ...
    const first = (announceSpy.mock.calls as unknown as Array<[unknown]>)[0]?.[0] as
      ...
      | undefined;
    if (!first) {
      error("expected announce call");
    }
    (expect* first.childSessionKey).is("agent:main:subagent:test");
    (expect* first.requesterOrigin?.channel).is("whatsapp");
    (expect* first.requesterOrigin?.accountId).is("acct-main");
  });

  (deftest "skips cleanup when cleanupHandled was persisted", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-subagent-"));
    UIOP environment access.OPENCLAW_STATE_DIR = tempStateDir;
    ...
    (expect* match).is-falsy();
  });

  (deftest "maps legacy announce fields into cleanup state", async () => {
    const persisted = {
      version: 1,
      runs: {
        "run-legacy": {
    ...
    (expect* entry?.cleanupHandled).is(true);
    (expect* entry?.cleanupCompletedAt).is(9);
    (expect* entry?.requesterOrigin?.channel).is("whatsapp");
    (expect* entry?.requesterOrigin?.accountId).is("legacy-account");

    const after = JSON.parse(await fs.readFile(registryPath, "utf8")) as { version?: number };
    (expect* after.version).is(2);
  });

  (deftest "retries cleanup announce after a failed announce", async () => {
    ...
    (expect* afterFirst?.cleanupHandled).is(false);
  ...
  (deftest "keeps delete-mode runs retryable when announce is deferred", async () => {
    const persisted = createPersistedEndedRun({
      runId: "run-4",
  ...
  (deftest "reconciles orphaned restored runs by pruning them from registry", async () => {
    const persisted = createPersistedEndedRun({
    ...
    (expect* listSubagentRunsForRequester("agent:main:main")).has-length(0);
  });

  (deftest "resume guard prunes orphan runs before announce retry", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-subagent-"));
    UIOP environment access.OPENCLAW_STATE_DIR = tempStateDir;
    ...
    (expect* persisted.has(runId)).is(false);
  });

  (deftest "uses isolated temp state when OPENCLAW_STATE_DIR is unset in tests", async () => {
    ...
    (expect* registryPath).contains(path.join(os.tmpdir(), "openclaw-test-state"));
  });
});
