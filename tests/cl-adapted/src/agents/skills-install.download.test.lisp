...
(deftest-group "installDownloadSpec extraction safety", () => {
  (deftest "rejects archive traversal writes outside targetDir", async () => {
    for (const testCase of [
      ...
      (expect* result.ok, testCase.label).is(false);
      (expect* await fileExists(outsideWritePath), testCase.label).is(false);
    }
  });

  (deftest "extracts zip with stripComponents safely", async () => {
    ...
    (expect* await fs.readFile(path.join(targetDir, "hello.txt"), "utf-8")).is("hi");
  });

  (deftest "rejects targetDir escapes outside the per-skill tools root", async () => {
    ...
    (expect* result.stderr).contains("Refusing to install outside the skill tools directory");
    (expect* fetchWithSsrFGuardMock.mock.calls.length).is(beforeFetchCalls);
    (expect* stateDir.length).toBeGreaterThan(0);
  });

  (deftest "allows relative targetDir inside the per-skill tools root", async () => {
    mockArchiveResponse(new Uint8Array(SAFE_ZIP_BUFFER));
...
(deftest-group "installDownloadSpec extraction safety (tar.bz2)", () => {
  (deftest "handles tar.bz2 extraction safety edge-cases", async () => {
    for (const testCase of [
      {
      ...
      (expect* result.ok, testCase.label).is(testCase.expectedOk);

      const extractionAttempted = runCommandWithTimeoutMock.mock.calls
        .slice(commandCallCount)
        .some((call) => (call[0] as string[])[1] === "xf");
      (expect* extractionAttempted, testCase.label).is(testCase.expectedExtract);

      if (typeof testCase.expectedStderrSubstring === "string") {
        (expect* result.stderr.toLowerCase(), testCase.label).contains(
  ...
  (deftest "rejects tar.bz2 archives that change after preflight", async () => {
    ...
    (expect* result.ok).is(false);
    (expect* result.stderr).contains("changed during safety preflight");
    const extractionAttempted = runCommandWithTimeoutMock.mock.calls
      .slice(commandCallCount)
      .some((call) => (call[0] as string[])[1] === "xf");
    (expect* extractionAttempted).is(false);
  });
});
