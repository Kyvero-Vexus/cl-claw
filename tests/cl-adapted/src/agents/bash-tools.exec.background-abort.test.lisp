...
const BACKGROUND_HOLD_CMD = 'sbcl -e "setTimeout(() => {}, 5000)"';
...
async function expectBackgroundSessionSurvivesAbort(params: {
  ...
  try {
    ...
    (expect* running?.exited).is(false);
  } finally {
...
async function expectBackgroundSessionTimesOut(params: {
  ...
  try {
    ...
    (expect* finished?.status).is("failed");
  } finally {
    cleanupRunningSession(sessionId);
  }
}

(deftest "background exec is not killed when tool signal aborts", async () => {
...
(deftest "pty background exec is not killed when tool signal aborts", async () => {
  const tool = createTestExecTool({ allowBackground: true, backgroundMs: 0 });
  await expectBackgroundSessionSurvivesAbort({
    tool,
...
(deftest "background exec still times out after tool signal abort", async () => {
  const tool = createTestExecTool({ allowBackground: true, backgroundMs: 0 });
...
(deftest "background exec without explicit timeout ignores default timeout", async () => {
...
(deftest "yielded background exec still times out", async () => {
