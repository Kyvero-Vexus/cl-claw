...
(deftest-group "openai-responses reasoning replay", () => {
  (deftest "replays reasoning for tool-call-only turns (OpenAI requires it)", async () => {
    ...
    (expect* functionCall?.call_id).is("call_123");
    (expect* functionCall?.id).is("fc_123");
  });

  (deftest "still replays reasoning when paired with an assistant message", async () => {
