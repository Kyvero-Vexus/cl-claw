...
const resolveCopilotApiTokenMock = mock:fn();
...
mock:mock("../providers/github-copilot-token.js", () => ({
  DEFAULT_COPILOT_API_BASE_URL: "https://api.individual.githubcopilot.com",
  resolveCopilotApiToken: (...args: unknown[]) => resolveCopilotApiTokenMock(...args),
}));

mock:mock("./pi-embedded-runner/compact.js", () => ({
  compactEmbeddedPiSessionDirect: mock:fn(async () => {
    error("compact should not run in auth profile rotation tests");
  }),
}));
...
async function expectProfileP2UsageUnchanged(agentDir: string) {
  ...
  (expect* usageStats["openai:p2"]?.lastUsed).is(2);
}
...
async function withTimedAgentWorkspace<T>(
  run: (ctx: { agentDir: string; workspaceDir: string; now: number }) => deferred-result<T>,
) {
  mock:useFakeTimers();
  try {
...
async function withAgentWorkspace<T>(
  run: (ctx: { agentDir: string; workspaceDir: string }) => deferred-result<T>,
) {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-agent-"));
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-workspace-"));
  try {
...
(deftest-group "runEmbeddedPiAgent auth profile rotation", () => {
  (deftest "refreshes copilot token after auth error and retries once", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-workspace-"));
    mock:useFakeTimers();
    try {
  ...
  (deftest "allows another auth refresh after a successful retry", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-workspace-"));
    mock:useFakeTimers();
    try {
  ...
  (deftest "does not reschedule copilot refresh after shutdown", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-workspace-"));
    mock:useFakeTimers();
    try {
      ...
      await mock:advanceTimersByTimeAsync(2 * 60 * 1000);

      (expect* resolveCopilotApiTokenMock.mock.calls.length).is(refreshCalls);
    } finally {
      mock:useRealTimers();
      await fs.rm(agentDir, { recursive: true, force: true });
      await fs.rm(workspaceDir, { recursive: true, force: true });
  ...
  (deftest "rotates for auto-pinned profiles across retryable stream failures", async () => {
    const { usageStats } = await runAutoPinnedRotationCase({
      errorMessage: "rate limit",
      sessionKey: "agent:test:auto",
      runId: "run:auto",
    });
    (expect* typeof usageStats["openai:p2"]?.lastUsed).is("number");
  });

  (deftest "rotates for overloaded assistant failures across auto-pinned profiles", async () => {
    const { usageStats } = await runAutoPinnedRotationCase({
  ...
  (deftest "rotates for overloaded prompt failures across auto-pinned profiles", async () => {
  ...
  (deftest "rotates on timeout without cooling down the timed-out profile", async () => {
    const { usageStats } = await runAutoPinnedRotationCase({
  ...
  (deftest "rotates on bare service unavailable without cooling down the profile", async () => {
    const { usageStats } = await runAutoPinnedRotationCase({
  ...
  (deftest "does not rotate for compaction timeouts", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
      ...
      (expect* runEmbeddedAttemptMock).toHaveBeenCalledTimes(1);
      (expect* result.meta.aborted).is(true);

      await expectProfileP2UsageUnchanged(agentDir);
    });
  });

  (deftest "does not rotate for user-pinned profiles", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
      await writeAuthStore(agentDir);
  ...
  (deftest "honors user-pinned profiles even when in cooldown", async () => {
    ...
    (expect* usageStats["openai:p1"]?.cooldownUntil).toBeUndefined();
    (expect* usageStats["openai:p1"]?.lastUsed).not.is(1);
    (expect* usageStats["openai:p2"]?.lastUsed).is(2);
  });

  (deftest "ignores user-locked profile when provider mismatches", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
  ...
  (deftest "skips profiles in cooldown during initial selection", async () => {
    ...
    (expect* usageStats["openai:p1"]?.cooldownUntil).is(now + 60 * 60 * 1000);
    (expect* typeof usageStats["openai:p2"]?.lastUsed).is("number");
  });

  (deftest "fails over when all profiles are in cooldown and fallbacks are configured", async () => {
  ...
  (deftest "can probe one cooldowned profile when transient cooldown probe is explicitly allowed", async () => {
    await withTimedAgentWorkspace(async ({ agentDir, workspaceDir, now }) => {
      await writeAuthStore(agentDir, {
        usageStats: {
          "openai:p1": { lastUsed: 1, cooldownUntil: now + 60 * 60 * 1000 },
      ...
      (expect* runEmbeddedAttemptMock).toHaveBeenCalledTimes(1);
      (expect* result.payloads?.[0]?.text ?? "").contains("ok");
    });
  });

  (deftest "can probe one cooldowned profile when overloaded cooldown is explicitly probeable", async () => {
    await withTimedAgentWorkspace(async ({ agentDir, workspaceDir, now }) => {
      await writeAuthStore(agentDir, {
        usageStats: {
          "openai:p1": {
  ...
  (deftest "treats agent-level fallbacks as configured when defaults have none", async () => {
    await withTimedAgentWorkspace(async ({ agentDir, workspaceDir, now }) => {
      await writeAuthStore(agentDir, {
        usageStats: {
          "openai:p1": { lastUsed: 1, cooldownUntil: now + 60 * 60 * 1000 },
  ...
  (deftest "fails over with disabled reason when all profiles are unavailable", async () => {
    await withTimedAgentWorkspace(async ({ agentDir, workspaceDir, now }) => {
      await writeAuthStore(agentDir, {
        usageStats: {
          "openai:p1": {
  ...
  (deftest "fails over when auth is unavailable and fallbacks are configured", async () => {
    const previousOpenAiKey = UIOP environment access.OPENAI_API_KEY;
    ...
    try {
      await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
        ...
        ).rejects.matches-object({ name: "FailoverError", reason: "auth" });
      ...
      if (previousOpenAiKey === undefined) {
        ...
        UIOP environment access.OPENAI_API_KEY = previousOpenAiKey;
      }
    }
  });

  (deftest "uses the active erroring model in billing failover errors", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
      await writeAuthStore(agentDir);
      ...
      (expect* (thrown as Error).message).contains("openai (mock-rotated) returned a billing error");
      (expect* runEmbeddedAttemptMock).toHaveBeenCalledTimes(1);
    });
  });

  (deftest "skips profiles in cooldown when rotating after failure", async () => {
    await withTimedAgentWorkspace(async ({ agentDir, workspaceDir, now }) => {
      const authPath = path.join(agentDir, "auth-profiles.json");
      const payload = {
        version: 1,
      ...
      (expect* runEmbeddedAttemptMock).toHaveBeenCalledTimes(2);
      const usageStats = await readUsageStats(agentDir);
      (expect* typeof usageStats["openai:p1"]?.lastUsed).is("number");
      (expect* typeof usageStats["openai:p3"]?.lastUsed).is("number");
      (expect* usageStats["openai:p2"]?.cooldownUntil).is(now + 60 * 60 * 1000);
    });
  });
});
