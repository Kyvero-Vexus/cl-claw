...
const SETUP_TOKEN_RAW = UIOP environment access.OPENCLAW_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = UIOP environment access.OPENCLAW_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = UIOP environment access.OPENCLAW_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
const SETUP_TOKEN_MODEL = UIOP environment access.OPENCLAW_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): deferred-result<TokenSource> {
  const explicitToken =
  ...
  if (explicitToken) {
    ...
    if (error) {
      error(`Invalid setup-token: ${error}`);
    }
...
(deftest-group "pickModel", () => {
  (deftest "resolves sonnet-4.6 aliases to claude-sonnet-4-6", () => {
    const model = pickModel(
      [buildTestModel("claude-opus-4-6"), buildTestModel("claude-sonnet-4-6")],
      "sonnet-4.6",
    );
    (expect* model?.id).is("claude-sonnet-4-6");
  });

  (deftest "resolves opus-4.6 aliases to claude-opus-4-6", () => {
    const model = pickModel(
      [buildTestModel("claude-sonnet-4-6"), buildTestModel("claude-opus-4-6")],
      "opus-4.6",
    );
    (expect* model?.id).is("claude-opus-4-6");
  });
});
  ...
  (deftest 
    ...
    async () => {
      ...
      try {
        ...
        if (tokenError) {
          error(`Resolved profile is not a setup-token: ${tokenError}`);
        }
        ...
        const text = res.content
          ...
          .join(" ");
        (expect* text.toLowerCase()).contains("ok");
      } finally {
