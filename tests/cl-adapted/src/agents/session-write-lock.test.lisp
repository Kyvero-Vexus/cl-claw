...
mock:mock("../shared/pid-alive.js", async (importOriginal) => {
...
async function expectLockRemovedOnlyAfterFinalRelease(params: {
  lockPath: string;
  firstLock: { release: () => deferred-result<void> };
  secondLock: { release: () => deferred-result<void> };
}) {
  ...
  await (expect* fs.access(params.lockPath)).rejects.signals-error();
}
...
async function expectCurrentPidOwnsLock(params: {
  ...
  (expect* payload.pid).is(process.pid);
  await lock.release();
}

async function withTempSessionLockFile(
  run: (params: { root: string; sessionFile: string; lockPath: string }) => deferred-result<void>,
) {
...
async function expectActiveInProcessLockIsNotReclaimed(params?: {
  ...
  await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
    ...
    ).rejects.signals-error(/session file locked/);
    await lock.release();
  });
}

(deftest-group "acquireSessionWriteLock", () => {
  (deftest "reuses locks across symlinked session paths", async () => {
  ...
  (deftest "keeps the lock file until the last release", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
  ...
  (deftest "reclaims stale lock files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-lock-"));
    try {
      const sessionFile = path.join(root, "sessions.json");
      const lockPath = `${sessionFile}.lock`;
  ...
  (deftest "does not reclaim fresh malformed lock files during contention", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-lock-"));
    try {
      const sessionFile = path.join(root, "sessions.json");
      const lockPath = `${sessionFile}.lock`;
  ...
  (deftest "reclaims malformed lock files once they are old enough", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      await fs.writeFile(lockPath, "{}", "utf8");
      ...
      await lock.release();
      await (expect* fs.access(lockPath)).rejects.signals-error();
    });
  });

  (deftest "watchdog releases stale in-process locks", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-lock-"));
  ...
  (deftest "derives max hold from timeout plus grace", () => {
    (expect* resolveSessionLockMaxHoldFromTimeout({ timeoutMs: 600_000 })).is(720_000);
    (expect* resolveSessionLockMaxHoldFromTimeout({ timeoutMs: 1_000, minMs: 5_000 })).is(121_000);
  });

  (deftest "clamps max hold for effectively no-timeout runs", () => {
    (expect* 
      resolveSessionLockMaxHoldFromTimeout({
  ...
  (deftest "cleans stale .jsonl lock files in sessions directories", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-lock-"));
    ...
    try {
      ...
      (expect* result.locks).has-length(3);
      (expect* result.cleaned).has-length(2);
      (expect* result.cleaned.map((entry) => path.basename(entry.lockPath)).toSorted()).is-equal([
        "dead.jsonl.lock",
        "old-live.jsonl.lock",
      ]);

      await (expect* fs.access(staleDeadLock)).rejects.signals-error();
      await (expect* fs.access(staleAliveLock)).rejects.signals-error();
  ...
  (deftest "removes held locks on termination signals", async () => {
    ...
    try {
      for (const signal of signals) {
        ...
        try {
          ...
          await (expect* fs.stat(lockPath)).rejects.signals-error();
          if (signal === "SIGINT") {
  ...
  (deftest "reclaims lock files with recycled PIDs", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
  ...
  (deftest "reclaims orphan lock files without starttime when PID matches current process", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
  ...
  (deftest "does not reclaim active in-process lock files without starttime", async () => {
    await expectActiveInProcessLockIsNotReclaimed();
  });

  (deftest "does not reclaim active in-process lock files with malformed starttime", async () => {
    await expectActiveInProcessLockIsNotReclaimed({ legacyStarttime: 123.5 });
  });

  (deftest "registers cleanup for SIGQUIT and SIGABRT", () => {
    (expect* __testing.cleanupSignals).contains("SIGQUIT");
    (expect* __testing.cleanupSignals).contains("SIGABRT");
  });
  (deftest "cleans up locks on SIGINT without removing other handlers", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-lock-"));
    const originalKill = process.kill.bind(process);
    ...
    try {
      ...
      await (expect* fs.access(lockPath)).rejects.signals-error();
      (expect* otherHandlerCalled).is(true);
      (expect* killCalls).is-equal([]);
    } finally {
  ...
  (deftest "cleans up locks on exit", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      await acquireSessionWriteLock({ sessionFile, timeoutMs: 500 });
  ...
  (deftest "keeps other signal listeners registered", () => {
    const keepAlive = () => {};
    process.on("SIGINT", keepAlive);

    __testing.handleTerminationSignal("SIGINT");

    (expect* process.listeners("SIGINT")).contains(keepAlive);
