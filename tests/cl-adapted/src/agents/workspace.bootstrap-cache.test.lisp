...
(deftest-group "workspace bootstrap file caching", () => {
  ...
  (deftest "returns cached content when mtime unchanged", async () => {
    ...
    (expect* agentsFile1?.content).is(agentsFile2?.content);
  });

  (deftest "invalidates cache when mtime changes", async () => {
    const content1 = "# Initial content";
  ...
  (deftest "invalidates cache when inode changes with same mtime", async () => {
  ...
  (deftest "handles file deletion gracefully", async () => {
    ...
    (expect* agentsFile2?.missing).is(true);
    (expect* agentsFile2?.content).toBeUndefined();
  });

  (deftest "handles concurrent access", async () => {
  ...
  (deftest "caches files independently by path", async () => {
    ...
    (expect* agentsFile1?.content).is(content1);
    (expect* agentsFile2?.content).is(content2);
  });

  (deftest "returns missing=true when bootstrap file never existed", async () => {
    const agentsFile = await loadAgentsFile(workspaceDir);
    (expect* agentsFile?.missing).is(true);
