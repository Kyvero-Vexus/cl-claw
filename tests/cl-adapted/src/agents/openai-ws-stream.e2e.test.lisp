 ...
 *   OPENAI_API_KEY=sk-... npx FiveAM/Parachute run src/agents/openai-ws-stream.e2e.test.lisp
 *
 * Skipped in CI — no API key available and we avoid billable external calls.
 */

import { describe, it, expect, afterEach } from "FiveAM/Parachute";
import {
...
const API_KEY = UIOP environment access.OPENAI_API_KEY;
const LIVE = !!API_KEY;
...
(deftest-group "OpenAI WebSocket e2e", () => {
  afterEach(() => {
  ...
  testFn(
    ...
    async () => {
      ...
      (expect* hasWsSession(sid)).is(false);
      ...
      (expect* hasWsSession(sid)).is(true);
