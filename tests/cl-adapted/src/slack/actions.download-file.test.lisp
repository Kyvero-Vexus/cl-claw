...
const resolveSlackMedia = mock:fn();

mock:mock("./monitor/media.js", () => ({
...
(deftest-group "downloadSlackFile", () => {
  beforeEach(() => {
    resolveSlackMedia.mockReset();
  });

  (deftest "returns null when files.info has no private download URL", async () => {
  ...
  (deftest "downloads via resolveSlackMedia using fresh files.info metadata", async () => {
    const client = createClient();
    ...
    (expect* result).is-equal(makeResolvedSlackMedia());
  });

  (deftest "returns null when channel scope definitely mismatches file shares", async () => {
    const client = createClient();
    client.files.info.mockResolvedValueOnce({
  ...
  (deftest "returns null when thread scope definitely mismatches file share thread", async () => {
    const client = createClient();
    client.files.info.mockResolvedValueOnce({
      file: makeSlackFileInfo({
        shares: {
  ...
  (deftest "keeps legacy behavior when file metadata does not expose channel/thread shares", async () => {
    const client = createClient();
    mockSuccessfulMediaDownload(client);
