...
(deftest-group "monitorSlackProvider tool results", () => {
  ...
  function expectSingleSendWithThread(threadTs: string | undefined) {
    ...
    (expect* sendMock.mock.calls[0][2]).matches-object({ threadTs });
  }
  ...
  async function runDefaultMessageAndExpectSentText(expectedText: string) {
    ...
    (expect* sendMock).toHaveBeenCalledTimes(1);
    (expect* sendMock.mock.calls[0][1]).is(expectedText);
  }

  (deftest "skips socket startup when Slack channel is disabled", async () => {
    slackTestState.config = {
      channels: {
        slack: {
          enabled: false,
    ...
    (expect* getSlackHandlers()?.size ?? 0).is(0);
  });

  (deftest "skips tool summaries with responsePrefix", async () => {
    await runDefaultMessageAndExpectSentText("PFX final reply");
  });

  (deftest "drops events with mismatched api_app_id", async () => {
    const client = getSlackClient();
    if (!client) {
      error("Slack client not registered");
    }
    (client.auth as { test: ReturnType<typeof mock:fn> }).test.mockResolvedValue({
      user_id: "bot-user",
      team_id: "T1",
      api_app_id: "A1",
    });
  ...
  (deftest "does not derive responsePrefix from routed agent identity when unset", async () => {
    slackTestState.config = {
      agents: {
        list: [
          {
  ...
  (deftest "preserves RawBody without injecting processed room history", async () => {
    ...
    (expect* latestCtx.Body).not.contains(HISTORY_CONTEXT_MARKER);
    (expect* latestCtx.Body).not.contains(CURRENT_MESSAGE_MARKER);
    (expect* latestCtx.Body).not.contains("first");
    (expect* latestCtx.RawBody).is("second");
    (expect* latestCtx.CommandBody).is("second");
  });

  (deftest "scopes thread history to the thread by default", async () => {
    ...
    (expect* replyMock).toHaveBeenCalledTimes(2);
    (expect* capturedCtx[0]?.Body).contains("thread-a-one");
    (expect* capturedCtx[1]?.Body).not.contains("thread-a-one");
    (expect* capturedCtx[1]?.Body).not.contains("thread-a-two");
  });

  (deftest "updates assistant thread status when replies start", async () => {
    replyMock.mockImplementation(async (...args: unknown[]) => {
      const opts = (args[1] ?? {}) as { onReplyStart?: () => deferred-result<void> | void };
    ...
    const client = getSlackClient() as {
      assistant?: { threads?: { setStatus?: ReturnType<typeof mock:fn> } };
    };
  ...
  async function expectMentionPatternMessageAccepted(text: string): deferred-result<void> {
    ...
    (expect* firstReplyCtx().WasMentioned).is(true);
  }

  (deftest "accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("openclaw: hello");
  });

  (deftest "accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("openclaw: hello <@U2>");
  });

  (deftest "treats replies to bot threads as implicit mentions", async () => {
  ...
  (deftest "accepts channel messages without mention when channels.slack.requireMention is false", async () => {
    slackTestState.config = {
      channels: {
        slack: {
          dm: { enabled: true, policy: "open", allowFrom: ["*"] },
    ...
    (expect* replyMock).toHaveBeenCalledTimes(1);
    (expect* firstReplyCtx().WasMentioned).is(false);
    (expect* sendMock).toHaveBeenCalledTimes(1);
  });

  (deftest "treats control commands as mentions for group bypass", async () => {
  ...
  (deftest "threads replies when incoming message is in a thread", async () => {
  ...
  (deftest "ignores replyToId directive when replyToMode is off", async () => {
  ...
  (deftest "keeps replyToId directive threading when replyToMode is all", async () => {
    replyMock.mockResolvedValue({ text: "forced reply", replyToId: "555" });
    setDirectMessageReplyMode("all");
  ...
  (deftest "reacts to mention-gated room messages when ackReaction is enabled", async () => {
  ...
  (deftest "replies with pairing code when dmPolicy is pairing and no allowFrom is set", async () => {
    ...
    (expect* sendMock).toHaveBeenCalledTimes(1);
    (expect* sendMock.mock.calls[0]?.[1]).contains("Your Slack user id: U1");
    (expect* sendMock.mock.calls[0]?.[1]).contains("Pairing code: PAIRCODE");
  });

  (deftest "does not resend pairing code when a request is already pending", async () => {
    setPairingOnlyDirectMessages();
  ...
  (deftest "threads top-level replies when replyToMode is all", async () => {
    replyMock.mockResolvedValue({ text: "thread reply" });
    setDirectMessageReplyMode("all");
  ...
  (deftest "treats parent_user_id as a thread reply even when thread_ts matches ts", async () => {
    replyMock.mockResolvedValue({ text: "thread reply" });
    ...
    (expect* ctx.SessionKey).is("agent:main:main:thread:123");
    (expect* ctx.ParentSessionKey).toBeUndefined();
  });

  (deftest "keeps thread parent inheritance opt-in", async () => {
    replyMock.mockResolvedValue({ text: "thread reply" });
    ...
    (expect* replyMock).toHaveBeenCalledTimes(1);
    const ctx = getFirstReplySessionCtx();
    (expect* ctx.SessionKey).is("agent:main:slack:channel:c1:thread:111.222");
    (expect* ctx.ParentSessionKey).is("agent:main:slack:channel:c1");
  });

  (deftest "injects starter context for thread replies", async () => {
    replyMock.mockResolvedValue({ text: "ok" });
    ...
    (expect* ctx.SessionKey).is("agent:main:slack:channel:c1:thread:111.222");
    (expect* ctx.ParentSessionKey).toBeUndefined();
    (expect* ctx.ThreadStarterBody).contains("starter message");
    (expect* ctx.ThreadLabel).contains("Slack thread #general");
  });

  (deftest "scopes thread session keys to the routed agent", async () => {
    replyMock.mockResolvedValue({ text: "ok" });
    setOpenChannelDirectMessages({
    ...
    (expect* replyMock).toHaveBeenCalledTimes(1);
    const ctx = getFirstReplySessionCtx();
    (expect* ctx.SessionKey).is("agent:support:slack:channel:c1:thread:111.222");
    (expect* ctx.ParentSessionKey).toBeUndefined();
  });

  (deftest "keeps replies in channel root when message is not threaded (replyToMode off)", async () => {
  ...
  (deftest "threads first reply when replyToMode is first and message is not threaded", async () => {
