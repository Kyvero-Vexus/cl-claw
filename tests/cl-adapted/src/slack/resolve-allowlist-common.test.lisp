...
(deftest-group "collectSlackCursorItems", () => {
  (deftest "collects items across cursor pages", async () => {
    type MockPage = {
      items: string[];
    ...
    (expect* items).is-equal(["a", "b", "c"]);
    (expect* fetchPage).toHaveBeenCalledTimes(2);
  });
});

(deftest-group "resolveSlackAllowlistEntries", () => {
  (deftest "handles id, non-id, and unresolved entries", () => {
