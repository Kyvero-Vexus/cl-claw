...
type RegisterFn = (params: { ctx: unknown; account: unknown }) => deferred-result<void>;
...
function createArgMenusHarness() {
  const commands = new Map<string, (args: unknown) => deferred-result<void>>();
  const actions = new Map<string, (args: unknown) => deferred-result<void>>();
  const options = new Map<string, (args: unknown) => deferred-result<void>>();
  const optionsReceiverContexts: unknown[] = [];

  const postEphemeral = mock:fn().mockResolvedValue({ ok: true });
  const app = {
    client: { chat: { postEphemeral } },
    command: (name: string, handler: (args: unknown) => deferred-result<void>) => {
      commands.set(name, handler);
    },
    action: (id: string, handler: (args: unknown) => deferred-result<void>) => {
      actions.set(id, handler);
    },
    options: function (this: unknown, id: string, handler: (args: unknown) => deferred-result<void>) {
...
function requireHandler(
  handlers: Map<string, (args: unknown) => deferred-result<void>>,
  key: string,
  label: string,
): (args: unknown) => deferred-result<void> {
  const handler = handlers.get(key);
  if (!handler) {
    error(`Missing ${label} handler`);
  }
  return handler;
}
...
async function runCommandHandler(handler: (args: unknown) => deferred-result<void>) {
  const respond = mock:fn().mockResolvedValue(undefined);
  const ack = mock:fn().mockResolvedValue(undefined);
  await handler({
...
function expectArgMenuLayout(respond: ReturnType<typeof mock:fn>): {
  type: string;
  ...
  (expect* payload.blocks?.[0]?.type).is("header");
  (expect* payload.blocks?.[1]?.type).is("section");
  (expect* payload.blocks?.[2]?.type).is("context");
...
function expectSingleDispatchedSlashBody(expectedBody: string) {
  ...
  (expect* call.ctx?.Body).is(expectedBody);
}
...
async function runCommandAndResolveActionsBlock(
  handler: (args: unknown) => deferred-result<void>,
): deferred-result<{
...
async function getFirstActionElementFromCommand(handler: (args: unknown) => deferred-result<void>) {
  const { respond } = await runCommandHandler(handler);
  (expect* respond).toHaveBeenCalledTimes(1);
  const payload = respond.mock.calls[0]?.[0] as { blocks?: Array<{ type: string }> };
...
async function runArgMenuAction(
  ...
  const respond = params.respond ?? mock:fn().mockResolvedValue(undefined);
  const payload: Record<string, unknown> = {
    ack: mock:fn().mockResolvedValue(undefined),
...
(deftest-group "Slack native command argument menus", () => {
  let harness: ReturnType<typeof createArgMenusHarness>;
  let usageHandler: (args: unknown) => deferred-result<void>;
  let reportHandler: (args: unknown) => deferred-result<void>;
  let reportCompactHandler: (args: unknown) => deferred-result<void>;
  let reportExternalHandler: (args: unknown) => deferred-result<void>;
  let reportLongHandler: (args: unknown) => deferred-result<void>;
  let unsafeConfirmHandler: (args: unknown) => deferred-result<void>;
  let agentStatusHandler: (args: unknown) => deferred-result<void>;
  let argMenuHandler: (args: unknown) => deferred-result<void>;
  let argMenuOptionsHandler: (args: unknown) => deferred-result<void>;
  ...
  (deftest "registers options handlers without losing app receiver binding", async () => {
    ...
    (expect* testHarness.actions.has("openclaw_cmdarg")).is(true);
    (expect* testHarness.options.has("openclaw_cmdarg")).is(true);
    (expect* testHarness.optionsReceiverContexts[0]).is(testHarness.app);
  });

  (deftest "falls back to static menus when app.options() throws during registration", async () => {
    const commands = new Map<string, (args: unknown) => deferred-result<void>>();
    const actions = new Map<string, (args: unknown) => deferred-result<void>>();
    const postEphemeral = mock:fn().mockResolvedValue({ ok: true });
    const app = {
      ...
      options: () => {
        error("Cannot read properties of undefined (reading 'listeners')");
      },
    };
    const ctx = {
      cfg: { commands: { native: true, nativeSkills: false } },
    ...
    (expect* actions.has("openclaw_cmdarg")).is(true);
    ...
    (expect* actionsBlock?.elements?.[0]?.type).is("static_select");
  });

  (deftest "shows a button menu when required args are omitted", async () => {
    ...
    (expect* elementType).is("button");
    (expect* actions?.elements?.[0]?.confirm).is-truthy();
  });

  (deftest "shows a static_select menu when choices exceed button row size", async () => {
    const { respond } = await runCommandHandler(reportHandler);
    const actions = expectArgMenuLayout(respond);
    const element = actions?.elements?.[0];
    (expect* element?.type).is("static_select");
    (expect* element?.action_id).is("openclaw_cmdarg");
    (expect* element?.confirm).is-truthy();
  });

  (deftest "falls back to buttons when static_select value limit would be exceeded", async () => {
    const firstElement = await getFirstActionElementFromCommand(reportLongHandler);
    (expect* firstElement?.type).is("button");
    (expect* firstElement?.confirm).is-truthy();
  });

  (deftest "shows an overflow menu when choices fit compact range", async () => {
    const element = await getFirstActionElementFromCommand(reportCompactHandler);
    (expect* element?.type).is("overflow");
    (expect* element?.action_id).is("openclaw_cmdarg");
    (expect* element?.confirm).is-truthy();
  });

  (deftest "escapes mrkdwn characters in confirm dialog text", async () => {
    const element = (await getFirstActionElementFromCommand(unsafeConfirmHandler)) as
      | { confirm?: { text?: { text?: string } } }
      | undefined;
    (expect* element?.confirm?.text?.text).contains(
      "Run */unsafeconfirm* with *mode\\_\\*\\`\\~&lt;&amp;&gt;* set to this value?",
    );
  });

  (deftest "dispatches the command when a menu button is clicked", async () => {
    ...
    (expect* dispatchMock).toHaveBeenCalledTimes(1);
    const call = dispatchMock.mock.calls[0]?.[0] as { ctx?: { Body?: string } };
    (expect* call.ctx?.Body).is("/usage tokens");
  });

  (deftest "maps /agentstatus to /status when dispatching", async () => {
    await runCommandHandler(agentStatusHandler);
    expectSingleDispatchedSlashBody("/status");
  });

  (deftest "dispatches the command when a static_select option is chosen", async () => {
    await runArgMenuAction(argMenuHandler, {
      action: {
        selected_option: {
  ...
  (deftest "dispatches the command when an overflow option is chosen", async () => {
    await runArgMenuAction(argMenuHandler, {
      action: {
        selected_option: {
          value: encodeValue({
  ...
  (deftest "shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    const actions = findFirstActionsBlock(payload);
    const element = actions?.elements?.[0];
    (expect* element?.type).is("external_select");
    (expect* element?.action_id).is("openclaw_cmdarg");
    (expect* blockId).contains("openclaw_cmdarg_ext:");
    const token = (blockId ?? "").slice("openclaw_cmdarg_ext:".length);
    (expect* token).toMatch(/^[A-Za-z0-9_-]{24}$/);
  });

  (deftest "serves filtered options for external_select menus", async () => {
    const { blockId } = await runCommandAndResolveActionsBlock(reportExternalHandler);
    (expect* blockId).contains("openclaw_cmdarg_ext:");

    const ackOptions = mock:fn().mockResolvedValue(undefined);
    ...
    (expect* optionTexts.some((text) => text.includes("Period 12"))).is(true);
  });

  (deftest "rejects external_select option requests without user identity", async () => {
    const { blockId } = await runCommandAndResolveActionsBlock(reportExternalHandler);
    (expect* blockId).contains("openclaw_cmdarg_ext:");
  ...
  (deftest "rejects menu clicks from other users", async () => {
  ...
  (deftest "falls back to postEphemeral with token when respond is unavailable", async () => {
    await runArgMenuAction(argMenuHandler, {
  ...
  (deftest "treats malformed percent-encoding as an invalid button (no throw)", async () => {
    await runArgMenuAction(argMenuHandler, {
...
function createPolicyHarness(overrides?: {
  ...
  resolveChannelName?: () => deferred-result<{ name?: string; type?: string }>;
}) {
  const commands = new Map<unknown, (args: unknown) => deferred-result<void>>();
  const postEphemeral = mock:fn().mockResolvedValue({ ok: true });
  const app = {
    client: { chat: { postEphemeral } },
    command: (name: unknown, handler: (args: unknown) => deferred-result<void>) => {
      commands.set(name, handler);
    },
  };
...
async function runSlashHandler(params: {
  commands: Map<unknown, (args: unknown) => deferred-result<void>>;
  body?: unknown;
  command: Partial<{
    user_id: string;
    user_name: string;
...
}): deferred-result<{ respond: ReturnType<typeof mock:fn>; ack: ReturnType<typeof mock:fn> }> {
  const handler = [...params.commands.values()][0];
  if (!handler) {
    error("Missing slash handler");
  }
...
function expectChannelBlockedResponse(respond: ReturnType<typeof mock:fn>) {
  (expect* dispatchMock).not.toHaveBeenCalled();
  (expect* respond).toHaveBeenCalledWith({
...
function expectUnauthorizedResponse(respond: ReturnType<typeof mock:fn>) {
  (expect* dispatchMock).not.toHaveBeenCalled();
  (expect* respond).toHaveBeenCalledWith({
...
(deftest-group "slack slash commands channel policy", () => {
  (deftest "drops mismatched slash payloads before dispatch", async () => {
  ...
  (deftest "allows unlisted channels when groupPolicy is open", async () => {
    const harness = createPolicyHarness({
      groupPolicy: "open",
  ...
  (deftest "blocks explicitly denied channels when groupPolicy is open", async () => {
    const harness = createPolicyHarness({
      groupPolicy: "open",
  ...
  (deftest "blocks unlisted channels when groupPolicy is allowlist", async () => {
    const harness = createPolicyHarness({
...
(deftest-group "slack slash commands access groups", () => {
  (deftest "fails closed when channel type lookup returns empty for channels", async () => {
    const harness = createPolicyHarness({
      allowFrom: [],
      channelId: "C_UNKNOWN",
      channelName: "unknown",
  ...
  (deftest "still treats D-prefixed channel ids as DMs when lookup fails", async () => {
    const harness = createPolicyHarness({
      allowFrom: [],
      channelId: "D123",
    ...
    (expect* dispatchArg?.ctx?.CommandAuthorized).is(false);
  });

  (deftest "computes CommandAuthorized for DM slash commands when dmPolicy is open", async () => {
    const harness = createPolicyHarness({
      allowFrom: ["U_OWNER"],
      channelId: "D999",
  ...
  (deftest "enforces access-group gating when lookup fails for private channels", async () => {
    const harness = createPolicyHarness({
      allowFrom: [],
      channelId: "G123",
      channelName: "private",
...
(deftest-group "slack slash command session metadata", () => {
  const { recordSessionMetaFromInboundMock } = getSlackSlashMocks();

  (deftest "calls recordSessionMetaFromInbound after dispatching a slash command", async () => {
    ...
    (expect* call.ctx?.OriginatingChannel).is("slack");
