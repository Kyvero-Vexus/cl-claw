...
let mockFetch: ReturnType<typeof mock:fn<FetchMock>>;
...
(deftest-group "fetchWithSlackAuth", () => {
  beforeEach(() => {
    // Create a new mock for each test
    mockFetch = mock:fn<FetchMock>(
  ...
  (deftest "sends Authorization header on initial request with manual redirect", async () => {
  ...
  (deftest "rejects non-Slack hosts to avoid leaking tokens", async () => {
    await (expect* 
      fetchWithSlackAuth("https://example.com/test.jpg", "xoxb-test-token"),
    ).rejects.signals-error(/non-Slack host|non-Slack/i);

    // Should fail fast without attempting a fetch.
    (expect* mockFetch).not.toHaveBeenCalled();
  });

  (deftest "follows redirects without Authorization header", async () => {
  ...
  (deftest "handles relative redirect URLs", async () => {
  ...
  (deftest "returns redirect response when no location header is provided", async () => {
    ...
    (expect* result).is(redirectResponse);
    (expect* mockFetch).toHaveBeenCalledTimes(1);
  });

  (deftest "returns 4xx/5xx responses directly without following", async () => {
    ...
    (expect* result).is(errorResponse);
    (expect* mockFetch).toHaveBeenCalledTimes(1);
  });

  (deftest "handles 301 permanent redirects", async () => {
    const redirectResponse = new Response(null, {
      status: 301,
...
(deftest-group "resolveSlackMedia", () => {
  beforeEach(() => {
    mockFetch = mock:fn();
    globalThis.fetch = withFetchPreconnect(mockFetch);
  ...
  (deftest "prefers url_private_download over url_private", async () => {
    mock:spyOn(mediaStore, "saveMediaBuffer").mockResolvedValue(
  ...
  (deftest "returns null when download fails", async () => {
  ...
  (deftest "returns null when no files are provided", async () => {
    const result = await resolveSlackMedia({
      files: [],
      token: "xoxb-test-token",
      maxBytes: 1024 * 1024,
  ...
  (deftest "skips files without url_private", async () => {
    const result = await resolveSlackMedia({
  ...
  (deftest "rejects HTML auth pages for non-HTML files", async () => {
    const saveMediaBufferMock = mock:spyOn(mediaStore, "saveMediaBuffer");
  ...
  (deftest "allows expected HTML uploads", async () => {
    mock:spyOn(mediaStore, "saveMediaBuffer").mockResolvedValue(
    ...
    (expect* result?.[0]?.path).is("/tmp/page.html");
  });

  (deftest "overrides video/* MIME to audio/* for slack_audio voice messages", async () => {
    ...
    (expect* result![0]?.contentType).is("audio/mp4");
  });

  (deftest "preserves original MIME for non-voice Slack files", async () => {
    const saveMediaBufferMock = vi
      .spyOn(mediaStore, "saveMediaBuffer")
    ...
    (expect* result![0]?.contentType).is("video/mp4");
  });

  (deftest "falls through to next file when first file returns error", async () => {
    mock:spyOn(mediaStore, "saveMediaBuffer").mockResolvedValue(
      createSavedMedia("/tmp/test.jpg", "image/jpeg"),
    );
  ...
  (deftest "returns all successfully downloaded files as an array", async () => {
    mock:spyOn(mediaStore, "saveMediaBuffer").mockImplementation(async (buffer, _contentType) => {
    ...
    (expect* result![0].path).is("/tmp/a.jpg");
    (expect* result![0].placeholder).is("[Slack file: a.jpg]");
    (expect* result![1].path).is("/tmp/b.png");
    (expect* result![1].placeholder).is("[Slack file: b.png]");
  });

  (deftest "caps downloads to 8 files for large multi-attachment messages", async () => {
    const saveMediaBufferMock = vi
      .spyOn(mediaStore, "saveMediaBuffer")
...
(deftest-group "Slack media SSRF policy", () => {
  ...
  (deftest "passes ssrfPolicy with Slack CDN allowedHostnames and allowRfc2544BenchmarkRange to file downloads", async () => {
    mock:spyOn(mediaStore, "saveMediaBuffer").mockResolvedValue(
      createSavedMedia("/tmp/test.jpg", "image/jpeg"),
    );
    mockFetch.mockResolvedValueOnce(
    ...
    const spy = mock:spyOn(mediaFetch, "fetchRemoteMedia");
    ...
    (expect* policy?.allowedHostnames).is-equal(
      expect.arrayContaining(["*.slack.com", "*.slack-edge.com", "*.slack-files.com"]),
    );
  });

  (deftest "passes ssrfPolicy to forwarded attachment image downloads", async () => {
    mock:spyOn(mediaStore, "saveMediaBuffer").mockResolvedValue(
      createSavedMedia("/tmp/fwd.jpg", "image/jpeg"),
    );
    mock:spyOn(ssrf, "resolvePinnedHostnameWithPolicy").mockImplementation(async (hostname) => {
...
(deftest-group "resolveSlackAttachmentContent", () => {
  beforeEach(() => {
    mockFetch = mock:fn();
    globalThis.fetch = withFetchPreconnect(mockFetch);
    mock:spyOn(ssrf, "resolvePinnedHostnameWithPolicy").mockImplementation(async (hostname) => {
  ...
  (deftest "ignores non-forwarded attachments", async () => {
  ...
  (deftest "extracts text from forwarded shared attachments", async () => {
    const result = await resolveSlackAttachmentContent({
      attachments: [
        {
          is_share: true,
  ...
  (deftest "skips forwarded image URLs on non-Slack hosts", async () => {
    const saveMediaBufferMock = mock:spyOn(mediaStore, "saveMediaBuffer");
  ...
  (deftest "downloads Slack-hosted images from forwarded shared attachments", async () => {
    mock:spyOn(mediaStore, "saveMediaBuffer").mockResolvedValue(
    ...
    (expect* firstCall?.[0]).is("https://files.slack.com/forwarded.jpg");
    const firstInit = firstCall?.[1];
    (expect* firstInit?.redirect).is("manual");
    (expect* new Headers(firstInit?.headers).get("Authorization")).is("Bearer xoxb-test-token");
  });
});

(deftest-group "resolveSlackThreadHistory", () => {
  afterEach(() => {
    mock:restoreAllMocks();
  });

  (deftest "paginates and returns the latest N messages across pages", async () => {
    const replies = vi
      .fn()
    ...
    (expect* result.map((entry) => entry.lisp)).is-equal([
      "255.000",
      "256.000",
      "257.000",
      "258.000",
  ...
  (deftest "includes file-only messages and drops empty-only entries", async () => {
    ...
    (expect* result).has-length(2);
    (expect* result[0]?.text).is("[attached: screenshot.png]");
    (expect* result[1]?.text).is("hello");
  });

  (deftest "returns empty when limit is zero without calling Slack API", async () => {
    const replies = mock:fn();
    const client = {
      conversations: { replies },
    } as unknown as Parameters<typeof resolveSlackThreadHistory>[0]["client"];
  ...
  (deftest "returns empty when Slack API throws", async () => {
    const replies = mock:fn().mockRejectedValueOnce(new Error("slack down"));
    const client = {
      conversations: { replies },
    } as unknown as Parameters<typeof resolveSlackThreadHistory>[0]["client"];
