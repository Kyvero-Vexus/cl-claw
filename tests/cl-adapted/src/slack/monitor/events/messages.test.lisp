...
const messageQueueMock = mock:fn();
const messageAllowMock = mock:fn();
...
type MessageHandler = (args: { event: Record<string, unknown>; body: unknown }) => deferred-result<void>;
...
function createMessageHandlers(overrides?: SlackSystemEventTestOverrides) {
  ...
  const handleSlackMessage = mock:fn(async () => {});
...
async function runMessageCase(input: MessageCase = {}): deferred-result<void> {
...
(deftest-group "registerSlackMessageEvents", () => {
  ...
  (deftest "passes regular message events to the message handler", async () => {
    messageQueueMock.mockClear();
    messageAllowMock.mockReset().mockResolvedValue([]);
  ...
  (deftest "handles channel and group messages via the unified message handler", async () => {
    messageQueueMock.mockClear();
    messageAllowMock.mockReset().mockResolvedValue([]);
  ...
  (deftest "applies subtype system-event handling for channel messages", async () => {
    messageQueueMock.mockClear();
    messageAllowMock.mockReset().mockResolvedValue([]);
    const { handler, handleSlackMessage } = createMessageHandlers({
      dmPolicy: "open",
  ...
  (deftest "skips app_mention events for DM channel ids even with contradictory channel_type", async () => {
  ...
  (deftest "routes app_mention events from channels to the message handler", async () => {
    const { handler, handleSlackMessage } = createAppMentionHandlers({ dmPolicy: "open" });
    (expect* handler).is-truthy();
