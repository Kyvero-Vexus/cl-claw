...
type RegisteredHandler = (args: {
  ...
  respond?: (payload: { text: string; response_type: string }) => deferred-result<void>;
...
function createContext(overrides?: {
  ...
  resolveUserName?: (userId: string) => deferred-result<{ name?: string }>;
  resolveChannelName?: (channelId: string) => deferred-result<{
    name?: string;
  ...
  const app = {
    action: mock:fn((_matcher: RegExp, next: RegisteredHandler) => {
      handler = next;
    }),
    view: mock:fn((_matcher: RegExp, next: RegisteredViewHandler) => {
      viewHandler = next;
    }),
    viewClosed: mock:fn((_matcher: RegExp, next: RegisteredViewClosedHandler) => {
      viewClosedHandler = next;
    }),
    client: {
      chat: {
        update: mock:fn().mockResolvedValue(undefined),
      },
    },
  };
  const runtimeLog = mock:fn();
  const resolveSessionKey = mock:fn().mockReturnValue("agent:ops:slack:channel:C1");
...
(deftest-group "registerSlackInteractionEvents", () => {
  (deftest "enqueues structured events and updates button rows", async () => {
    ...
    (expect* eventText.startsWith("Slack interaction: ")).is(true);
  ...
  (deftest "drops block actions when mismatch guard triggers", async () => {
    enqueueSystemEventMock.mockClear();
  ...
  (deftest "drops modal lifecycle payloads when mismatch guard triggers", async () => {
    enqueueSystemEventMock.mockClear();
    ...
    (expect* viewHandler).is-truthy();
    (expect* viewClosedHandler).is-truthy();

    const ackSubmit = mock:fn().mockResolvedValue(undefined);
    await viewHandler!({
      ack: ackSubmit,
      body: {
        user: { id: "U123" },
    ...
    const ackClosed = mock:fn().mockResolvedValue(undefined);
  ...
  (deftest "captures select values and updates action rows for non-button actions", async () => {
    enqueueSystemEventMock.mockClear();
    const { ctx, app, getHandler } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });
    const handler = getHandler();
    ...
    (expect* payload.actionType).is("static_select");
    (expect* payload.selectedValues).is-equal(["canary"]);
    (expect* payload.selectedLabels).is-equal(["Canary"]);
    (expect* app.client.chat.update).toHaveBeenCalledTimes(1);
  ...
  (deftest "blocks block actions from users outside configured channel users allowlist", async () => {
    enqueueSystemEventMock.mockClear();
    const { ctx, app, getHandler } = createContext({
      channelsConfig: {
        C1: { users: ["U_ALLOWED"] },
  ...
  (deftest "blocks DM block actions when sender is not in allowFrom", async () => {
    enqueueSystemEventMock.mockClear();
    const { ctx, app, getHandler } = createContext({
      dmPolicy: "allowlist",
      allowFrom: ["U_OWNER"],
  ...
  (deftest "ignores malformed action payloads after ack and logs warning", async () => {
    enqueueSystemEventMock.mockClear();
  ...
  (deftest "escapes mrkdwn characters in confirmation labels", async () => {
    enqueueSystemEventMock.mockClear();
    const { ctx, app, getHandler } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });
    const handler = getHandler();
  ...
  (deftest "falls back to container channel and message timestamps", async () => {
    enqueueSystemEventMock.mockClear();
    const { ctx, app, getHandler, resolveSessionKey } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });
    const handler = getHandler();
  ...
  (deftest "summarizes multi-select confirmations in updated message rows", async () => {
    enqueueSystemEventMock.mockClear();
    const { ctx, app, getHandler } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });
    const handler = getHandler();
  ...
  (deftest "renders date/time/datetime picker selections in confirmation rows", async () => {
    enqueueSystemEventMock.mockClear();
    const { ctx, app, getHandler } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });
    const handler = getHandler();
  ...
  (deftest "captures expanded selection and temporal payload fields", async () => {
    enqueueSystemEventMock.mockClear();
    ...
    (expect* payload.actionType).is("multi_conversations_select");
    (expect* payload.selectedValues).is-equal([
      "alpha",
      "beta",
      "U777",
      "U888",
    ...
    (expect* payload.selectedUsers).is-equal(["U777", "U888"]);
    (expect* payload.selectedChannels).is-equal(["C777", "C888"]);
    (expect* payload.selectedConversations).is-equal(["G777", "G888"]);
    (expect* payload.selectedLabels).is-equal(["Alpha", "Beta"]);
    (expect* payload.selectedDate).is("2026-02-16");
    (expect* payload.selectedTime).is("14:30");
    (expect* payload.selectedDateTime).is(1_771_700_200);
  });

  (deftest "captures workflow button trigger metadata", async () => {
    enqueueSystemEventMock.mockClear();
    const { ctx, getHandler } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });
    const handler = getHandler();
  ...
  (deftest "captures modal submissions and enqueues view submission event", async () => {
    enqueueSystemEventMock.mockClear();
    ...
    (expect* payload.inputs).is-equal(
  ...
  (deftest "blocks modal events when private metadata userId does not match submitter", async () => {
    enqueueSystemEventMock.mockClear();
  ...
  (deftest "blocks modal events when private metadata is missing userId", async () => {
    enqueueSystemEventMock.mockClear();
    const { ctx, getViewHandler } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });
    const viewHandler = getViewHandler();
  ...
  (deftest "captures modal input labels and picker values across block types", async () => {
    enqueueSystemEventMock.mockClear();
    const { ctx, getViewHandler } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });
    const viewHandler = getViewHandler();
  ...
  (deftest "truncates rich text preview to keep payload summaries compact", async () => {
    enqueueSystemEventMock.mockClear();
    const { ctx, getViewHandler } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });
    const viewHandler = getViewHandler();
    ...
    (expect* richInput?.richTextPreview).is-truthy();
    (expect* (richInput?.richTextPreview ?? "").length).toBeLessThanOrEqual(120);
  });

  (deftest "captures modal close events and enqueues view closed event", async () => {
    enqueueSystemEventMock.mockClear();
    ...
    (expect* options.sessionKey).is("agent:main:slack:channel:C99");
  });

  (deftest "defaults modal close isCleared to false when Slack omits the flag", async () => {
    enqueueSystemEventMock.mockClear();
    ...
    (expect* payload.interactionType).is("view_closed");
    (expect* payload.isCleared).is(false);
  });

  (deftest "caps oversized interaction payloads with compact summaries", async () => {
    enqueueSystemEventMock.mockClear();
    const { ctx, getViewHandler } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });
    const viewHandler = getViewHandler();
