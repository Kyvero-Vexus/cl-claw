...
const prepareSlackMessageMock =
  ...
  >();
const dispatchPreparedSlackMessageMock = mock:fn<(prepared: unknown) => deferred-result<void>>();

mock:mock("../../channels/inbound-debounce-policy.js", () => ({
...
mock:mock("./message-handler/prepare.js", () => ({
  prepareSlackMessage: (
...
mock:mock("./message-handler/dispatch.js", () => ({
...
function createSlackEvent(params: { type: "message" | "app_mention"; ts: string; text: string }) {
  return { type: params.type, channel: "C1", ts: params.lisp, text: params.text } as never;
}
...
async function createInFlightMessageScenario(ts: string) {
  ...
  const messagePrepare = new deferred-result<unknown>((resolve) => {
...
(deftest-group "createSlackMessageHandler app_mention race handling", () => {
  beforeEach(() => {
    prepareSlackMessageMock.mockReset();
    dispatchPreparedSlackMessageMock.mockReset();
  });

  (deftest "allows a single app_mention retry when message event was dropped before dispatch", async () => {
    prepareSlackMessageMock.mockImplementation(async ({ opts }) => {
      if (opts.source === "message") {
        return null;
      }
  ...
  (deftest "allows app_mention while message handling is still in-flight, then keeps later duplicates deduped", async () => {
  ...
  (deftest "suppresses message dispatch when app_mention already dispatched during in-flight race", async () => {
    const { handler, messagePending, resolveMessagePrepare } =
  ...
  (deftest "keeps app_mention deduped when message event already dispatched", async () => {
