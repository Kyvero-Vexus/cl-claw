...
const {
  ...
  return {
    createConnectionMock,
    joinVoiceChannelMock: mock:fn(() => createConnectionMock()),
    entersStateMock: mock:fn(async (_target?: unknown, _state?: string, _timeoutMs?: number) => {
      return undefined;
    }),
    createAudioPlayerMock: mock:fn(() => ({
      on: mock:fn(),
      off: mock:fn(),
      stop: mock:fn(),
      play: mock:fn(),
    ...
    resolveAgentRouteMock: mock:fn(() => ({ agentId: "agent-1", sessionKey: "discord:g1:c1" })),
    agentCommandMock: mock:fn(async (_opts?: unknown, _runtime?: unknown) => ({ payloads: [] })),
    buildProviderRegistryMock: mock:fn(() => ({})),
    createMediaAttachmentCacheMock: mock:fn(() => ({
      cleanup: mock:fn(async () => undefined),
    })),
    normalizeMediaAttachmentsMock: mock:fn(() => [{ kind: "audio", path: "/tmp/test.wav" }]),
    runCapabilityMock: mock:fn(async () => ({
      outputs: [{ kind: "audio.transcription", text: "hello from voice" }],
    })),
  };
});

mock:mock("@discordjs/voice", () => ({
...
mock:mock("../../media-understanding/runner.js", () => ({
...
function createClient() {
  return {
    ...
    getPlugin: mock:fn(() => ({
      getGatewayAdapterCreator: mock:fn(() => mock:fn()),
    })),
    fetchMember: mock:fn(),
    fetchUser: mock:fn(),
  };
...
(deftest-group "DiscordVoiceManager", () => {
  beforeAll(async () => {
  ...
  const expectConnectedStatus = (
    ...
    (expect* manager.status()).is-equal([
      {
        ok: true,
  ...
  (deftest "keeps the new session when an old disconnected handler fires", async () => {
  ...
  (deftest "keeps the new session when an old destroyed handler fires", async () => {
    const oldConnection = createConnectionMock();
    const newConnection = createConnectionMock();
    joinVoiceChannelMock.mockReturnValueOnce(oldConnection).mockReturnValueOnce(newConnection);
  ...
  (deftest "removes voice listeners on leave", async () => {
  ...
  (deftest "passes DAVE options to joinVoiceChannel", async () => {
    const manager = createManager({
      voice: {
  ...
  (deftest "attempts rejoin after repeated decrypt failures", async () => {
    const manager = createManager();
  ...
  (deftest "passes senderIsOwner=true for allowlisted voice speakers", async () => {
    ...
    const commandArgs = agentCommandMock.mock.calls.at(-1)?.[0] as
      ...
      | undefined;
    (expect* commandArgs?.senderIsOwner).is(true);
  });

  (deftest "passes senderIsOwner=false for non-owner voice speakers", async () => {
    const client = createClient();
    client.fetchMember.mockResolvedValue({
      nickname: "Guest Nick",
      user: {
    ...
    const commandArgs = agentCommandMock.mock.calls.at(-1)?.[0] as
      | { senderIsOwner?: boolean }
      | undefined;
    (expect* commandArgs?.senderIsOwner).is(false);
  });

  (deftest "reuses speaker context cache for repeated segments from the same speaker", async () => {
    const client = createClient();
    client.fetchMember.mockResolvedValue({
