...
function createThreadClient(
  params: {
    fetchChannel?: ReturnType<typeof mock:fn>;
    restGet?: ReturnType<typeof mock:fn>;
  } = {},
) {
  return {
    fetchChannel:
...
(deftest-group "discord tool result dispatch", () => {
  ...
  (deftest 
    ...
    async () => {
      ...
      const interaction = {
        ...
        guild: null,
        rawData: { id: "i1" },
        options: { getString: mock:fn().mockReturnValue("on") },
        reply,
        followUp,
      ...
      (expect* reply.mock.calls[0]?.[0]?.content).contains("final");
    },
  );

  (deftest "accepts guild reply-to-bot messages as implicit mentions", async () => {
    const cfg = createMentionRequiredGuildConfig();
  ...
  (deftest "forks thread sessions and injects starter context", async () => {
    ...
    (expect* capturedCtx?.SessionKey).is("agent:main:discord:channel:t1");
    (expect* capturedCtx?.ParentSessionKey).is("agent:main:discord:channel:p1");
    (expect* capturedCtx?.ThreadStarterBody).contains("starter message");
    (expect* capturedCtx?.ThreadLabel).contains("Discord thread #general");
  });

  (deftest "skips thread starter context when disabled", async () => {
  ...
  (deftest "treats forum threads as distinct sessions without channel payloads", async () => {
    const getCapturedCtx = captureThreadDispatchCtx();
    ...
    const restGet = mock:fn().mockResolvedValue({
      content: "starter message",
      author: { id: "u1", username: "Alice", discriminator: "0001" },
      timestamp: new Date().toISOString(),
    });
    ...
    const capturedCtx = getCapturedCtx();
    (expect* capturedCtx?.SessionKey).is("agent:main:discord:channel:t1");
    (expect* capturedCtx?.ParentSessionKey).is("agent:main:discord:channel:forum-1");
    (expect* capturedCtx?.ThreadStarterBody).contains("starter message");
    (expect* capturedCtx?.ThreadLabel).contains("Discord thread #support");
    (expect* restGet).toHaveBeenCalledWith(Routes.channelMessage("t1", "t1"));
  });

  (deftest "scopes thread sessions to the routed agent", async () => {
    ...
    await mock:waitFor(() => (expect* dispatchMock).toHaveBeenCalledTimes(1));
    const capturedCtx = getCapturedCtx();
    (expect* capturedCtx?.SessionKey).is("agent:support:discord:channel:t1");
    (expect* capturedCtx?.ParentSessionKey).is("agent:support:discord:channel:p1");
  });
});
