...
(deftest-group "preflightDiscordMessage configured ACP bindings", () => {
  beforeEach(() => {
  ...
  (deftest "does not initialize configured ACP bindings for rejected messages", async () => {
  ...
  (deftest "initializes configured ACP bindings only after preflight accepts the message", async () => {
    const result = await preflightDiscordMessage(
      createBasePreflightParams({
        guildEntries: {
          [GUILD_ID]: {
    ...
    (expect* result?.boundSessionKey).is("agent:codex:acp:binding:discord:default:abc123");
  });
});
