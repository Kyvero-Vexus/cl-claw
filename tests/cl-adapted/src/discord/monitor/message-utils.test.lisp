...
const fetchRemoteMedia = mock:fn();
const saveMediaBuffer = mock:fn();

mock:mock("../../media/fetch.js", () => ({
  fetchRemoteMedia: (...args: unknown[]) => fetchRemoteMedia(...args),
}));

mock:mock("../../media/store.js", () => ({
...
(deftest-group "resolveDiscordMessageChannelId", () => {
  it.each([
    {
    ...
    (expect* resolveDiscordMessageChannelId(params)).is(expected);
  });
});

(deftest-group "resolveForwardedMediaList", () => {
  beforeEach(() => {
    fetchRemoteMedia.mockClear();
    saveMediaBuffer.mockClear();
  });

  (deftest "downloads forwarded attachments", async () => {
    const attachment = {
      id: "att-1",
  ...
  (deftest "forwards fetchImpl to forwarded attachment downloads", async () => {
    const proxyFetch = mock:fn() as unknown as typeof fetch;
    const attachment = {
      id: "att-proxy",
  ...
  (deftest "keeps forwarded attachment metadata when download fails", async () => {
    const attachment = {
      id: "att-fallback",
  ...
  (deftest "downloads forwarded stickers", async () => {
    const sticker = {
      id: "sticker-1",
      name: "wave",
  ...
  (deftest "returns empty when no snapshots are present", async () => {
  ...
  (deftest "skips snapshots without attachments", async () => {
    const result = await resolveForwardedMediaList(
      asMessage({
        rawData: {
...
(deftest-group "resolveMediaList", () => {
  beforeEach(() => {
    fetchRemoteMedia.mockClear();
    saveMediaBuffer.mockClear();
  });

  (deftest "downloads stickers", async () => {
    const sticker = {
      id: "sticker-2",
      name: "hello",
      format_type: StickerFormatType.PNG,
  ...
  (deftest "forwards fetchImpl to sticker downloads", async () => {
    const proxyFetch = mock:fn() as unknown as typeof fetch;
    const sticker = {
      id: "sticker-proxy",
      name: "proxy-sticker",
  ...
  (deftest "keeps attachment metadata when download fails", async () => {
    const attachment = {
  ...
  (deftest "falls back to URL when saveMediaBuffer fails", async () => {
    const attachment = {
      id: "att-save-fail",
  ...
  (deftest "preserves downloaded attachments alongside failed ones", async () => {
  ...
  (deftest "keeps sticker metadata when sticker download fails", async () => {
    const sticker = {
      id: "sticker-fallback",
      name: "fallback",
      format_type: StickerFormatType.PNG,
...
(deftest-group "Discord media SSRF policy", () => {
  beforeEach(() => {
    fetchRemoteMedia.mockClear();
    saveMediaBuffer.mockClear();
  });

  (deftest "passes Discord CDN hostname allowlist with RFC2544 enabled", async () => {
    fetchRemoteMedia.mockResolvedValueOnce({
  ...
  (deftest "merges provided ssrfPolicy with Discord CDN defaults", async () => {
    fetchRemoteMedia.mockResolvedValueOnce({
      buffer: Buffer.from("img"),
      contentType: "image/png",
    });
...
(deftest-group "resolveDiscordMessageText", () => {
  (deftest "includes forwarded message snapshots in body text", () => {
    ...
    (expect* text).contains("[Forwarded message from @Bob]");
    (expect* text).contains("forwarded hello");
  });

  (deftest "resolves user mentions in content", () => {
    const text = resolveDiscordMessageText(
      asMessage({
    ...
    (expect* text).is("Hello @Alice Wonderland and @bob!");
  });

  (deftest "leaves content unchanged if no mentions present", () => {
    const text = resolveDiscordMessageText(
      asMessage({
        content: "Hello world",
        mentionedUsers: [],
  ...
  (deftest "uses sticker placeholders when content is empty", () => {
    const text = resolveDiscordMessageText(
      asMessage({
        content: "",
        stickers: [
    ...
    (expect* text).is("<media:sticker> (1 sticker)");
  });

  (deftest "uses embed title when content is empty", () => {
    const text = resolveDiscordMessageText(
      asMessage({
        content: "",
    ...
    (expect* text).is("Breaking");
  });

  (deftest "uses embed description when content is empty", () => {
    const text = resolveDiscordMessageText(
      asMessage({
        content: "",
    ...
    (expect* text).is("Details");
  });

  (deftest "joins embed title and description when content is empty", () => {
    const text = resolveDiscordMessageText(
      asMessage({
        content: "",
  ...
  (deftest "prefers message content over embed fallback text", () => {
    const text = resolveDiscordMessageText(
      asMessage({
    ...
    (expect* text).is("hello from content");
  });

  (deftest "joins forwarded snapshot embed title and description when content is empty", () => {
    const text = resolveDiscordMessageText(
      asForwardedSnapshotMessage({
        content: "",
    ...
    (expect* text).contains("[Forwarded message from @Bob]");
    (expect* text).contains("Forwarded title\nForwarded details");
  });
});

(deftest-group "resolveDiscordChannelInfo", () => {
  beforeEach(() => {
    __resetDiscordChannelInfoCacheForTest();
  });

  (deftest "caches channel lookups between calls", async () => {
    const fetchChannel = mock:fn().mockResolvedValue({
      type: ChannelType.DM,
      name: "dm",
    });
  ...
  (deftest "negative-caches missing channels", async () => {
    const fetchChannel = mock:fn().mockResolvedValue(null);
    const client = { fetchChannel } as unknown as Client;
