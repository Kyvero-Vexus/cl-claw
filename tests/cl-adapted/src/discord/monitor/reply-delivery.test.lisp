...
const sendMessageDiscordMock = mock:hoisted(() => mock:fn());
const sendVoiceMessageDiscordMock = mock:hoisted(() => mock:fn());
const sendWebhookMessageDiscordMock = mock:hoisted(() => mock:fn());
const sendDiscordTextMock = mock:hoisted(() => mock:fn());
...
mock:mock("../send.shared.js", () => ({
  sendDiscordText: (...args: unknown[]) => sendDiscordTextMock(...args),
}));

(deftest-group "deliverDiscordReply", () => {
  ...
  (deftest "routes audioAsVoice payloads through the voice API and sends text separately", async () => {
  ...
  (deftest "skips follow-up text when the voice payload text is blank", async () => {
    await deliverDiscordReply({
      replies: [
        {
          text: "   ",
  ...
  (deftest "passes mediaLocalRoots through media sends", async () => {
  ...
  (deftest "uses replyToId only for the first chunk when replyToMode is first", async () => {
    await deliverDiscordReply({
      replies: [
        {
          text: "1234567890",
    ...
    (expect* sendMessageDiscordMock).toHaveBeenCalledTimes(2);
    (expect* sendMessageDiscordMock.mock.calls[0]?.[2]?.replyTo).is("reply-1");
    (expect* sendMessageDiscordMock.mock.calls[1]?.[2]?.replyTo).toBeUndefined();
  });

  (deftest "does not consume replyToId for replyToMode=first on whitespace-only payloads", async () => {
    await deliverDiscordReply({
  ...
  (deftest "preserves leading whitespace in delivered text chunks", async () => {
    await deliverDiscordReply({
  ...
  (deftest "sends text chunks in order via sendDiscordText when rest is provided", async () => {
    ...
    (expect* callOrder).is-equal(["12345", "67890"]);
    (expect* sendDiscordTextMock.mock.calls[0]?.[1]).is("789");
    (expect* sendDiscordTextMock.mock.calls[1]?.[1]).is("789");
  });

  (deftest "falls back to sendMessageDiscord when rest is not provided", async () => {
    await deliverDiscordReply({
  ...
  (deftest "retries bot send on 429 rate limit then succeeds", async () => {
  ...
  (deftest "retries bot send on 500 server error then succeeds", async () => {
  ...
  (deftest "does not retry on 4xx client errors", async () => {
  ...
  (deftest "throws after exhausting retry attempts", async () => {
    const rateLimitErr = Object.assign(new Error("rate limited"), { status: 429 });
    ...
    ).rejects.signals-error("rate limited");

    (expect* sendMessageDiscordMock).toHaveBeenCalledTimes(3);
  });

  (deftest "delivers remaining chunks after a mid-sequence retry", async () => {
    sendMessageDiscordMock
  ...
  (deftest "sends bound-session text replies through webhook delivery", async () => {
  ...
  (deftest "touches bound-thread activity after outbound delivery", async () => {
    mock:useFakeTimers();
    try {
      mock:setSystemTime(new Date("2026-02-20T00:00:00.000Z"));
      const threadBindings = await createBoundThreadBindings();
      mock:setSystemTime(new Date("2026-02-20T00:02:00.000Z"));
      ...
      (expect* threadBindings.getByThreadId("thread-1")?.lastActivityAt).is(
  ...
  (deftest "falls back to bot send when webhook delivery fails", async () => {
    const threadBindings = await createBoundThreadBindings();
  ...
  (deftest "does not use thread webhook when outbound target is not a bound thread", async () => {
    const threadBindings = await createBoundThreadBindings();
