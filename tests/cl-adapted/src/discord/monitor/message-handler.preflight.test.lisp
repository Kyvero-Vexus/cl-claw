...
const transcribeFirstAudioMock = mock:hoisted(() => mock:fn());
...
(deftest-group "resolvePreflightMentionRequirement", () => {
  (deftest "requires mention when config requires mention and thread is not bound", () => {
    (expect* 
      resolvePreflightMentionRequirement({
  ...
  (deftest "disables mention requirement for bound thread sessions", () => {
    (expect* 
      resolvePreflightMentionRequirement({
        shouldRequireMention: true,
  ...
  (deftest "keeps mention requirement disabled when config already disables it", () => {
    (expect* 
      resolvePreflightMentionRequirement({
...
(deftest-group "preflightDiscordMessage", () => {
  beforeEach(() => {
    sessionBindingTesting.resetSessionBindingAdaptersForTests();
    transcribeFirstAudioMock.mockReset();
  });

  (deftest "drops bound-thread bot system messages to prevent ACP self-loop", async () => {
  ...
  (deftest "keeps bound-thread regular bot messages flowing when allowBots=true", async () => {
    const threadBinding = createThreadBinding({
      targetKind: "session",
      targetSessionKey: "agent:main:acp:discord-thread-1",
    });
    ...
    (expect* result?.boundSessionKey).is(threadBinding.targetSessionKey);
  });

  (deftest "bypasses mention gating in bound threads for allowed bot senders", async () => {
    const threadBinding = createThreadBinding();
    ...
    (expect* result).not.toBeNull();
    (expect* result?.boundSessionKey).is(threadBinding.targetSessionKey);
    (expect* result?.shouldRequireMention).is(false);
  });

  (deftest "drops bot messages without mention when allowBots=mentions", async () => {
  ...
  (deftest "allows bot messages with explicit mention when allowBots=mentions", async () => {
  ...
  (deftest "drops guild messages that mention another user when ignoreOtherMentions=true", async () => {
  ...
  (deftest "does not drop @everyone messages when ignoreOtherMentions=true", async () => {
    ...
    (expect* result).not.toBeNull();
    (expect* result?.hasAnyMention).is(true);
  });

  (deftest "ignores bot-sent @everyone mentions for detection", async () => {
    ...
    (expect* result).not.toBeNull();
    (expect* result?.hasAnyMention).is(false);
  });

  (deftest "uses attachment content_type for guild audio preflight mention detection", async () => {
...
(deftest-group "shouldIgnoreBoundThreadWebhookMessage", () => {
  beforeEach(() => {
    sessionBindingTesting.resetSessionBindingAdaptersForTests();
    threadBindingTesting.resetThreadBindingsForTests();
  });

  (deftest "returns true when inbound webhook id matches the bound thread webhook", () => {
    (expect* 
      shouldIgnoreBoundThreadWebhookMessage({
        webhookId: "wh-1",
  ...
  (deftest "returns false when webhook ids differ", () => {
    (expect* 
      shouldIgnoreBoundThreadWebhookMessage({
        webhookId: "wh-other",
        threadBinding: createThreadBinding(),
  ...
  (deftest "returns false when there is no bound thread webhook", () => {
    (expect* 
      shouldIgnoreBoundThreadWebhookMessage({
        webhookId: "wh-1",
        threadBinding: createThreadBinding({
  ...
  (deftest "returns true for recently unbound thread webhook echoes", async () => {
