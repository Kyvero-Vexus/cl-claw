...
function expectBoundSessionDispatch(
  ...
  (expect* dispatchCall.ctx?.SessionKey).is(boundSessionKey);
...
(deftest-group "Discord native plugin command dispatch", () => {
  beforeEach(() => {
  ...
  (deftest "executes matched plugin commands directly without invoking the agent dispatcher", async () => {
    ...
    const pluginMatch = {
      command: {
        ...
        pluginId: "cron-jobs",
        acceptsArgs: false,
        handler: mock:fn().mockResolvedValue({ text: "jobs" }),
      },
      args: undefined,
    };

    mock:spyOn(pluginCommandsModule, "matchPluginCommand").mockReturnValue(
  ...
  (deftest "routes native slash commands through configured ACP Discord channel bindings", async () => {
  ...
  (deftest "falls back to the routed slash and channel session keys when no bound session exists", async () => {
    const guildId = "1459246755253325866";
    const channelId = "1478836151241412759";
    const cfg = {
      commands: {
    ...
    (expect* dispatchCall.ctx?.SessionKey).is("agent:qwen:discord:slash:owner");
  ...
  (deftest "routes Discord DM native slash commands through configured ACP bindings", async () => {
