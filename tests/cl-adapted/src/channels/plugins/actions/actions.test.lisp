...
const handleDiscordAction = mock:fn(async (..._args: unknown[]) => ({ details: { ok: true } }));
const handleTelegramAction = mock:fn(async (..._args: unknown[]) => ({ ok: true }));
const sendReactionSignal = mock:fn(async (..._args: unknown[]) => ({ ok: true }));
const removeReactionSignal = mock:fn(async (..._args: unknown[]) => ({ ok: true }));
const handleSlackAction = mock:fn(async (..._args: unknown[]) => ({ details: { ok: true } }));

mock:mock("../../../agents/tools/discord-actions.js", () => ({
  handleDiscordAction,
}));

mock:mock("../../../agents/tools/telegram-actions.js", () => ({
  handleTelegramAction,
}));

mock:mock("../../../signal/send-reactions.js", () => ({
  sendReactionSignal,
  removeReactionSignal,
}));

mock:mock("../../../agents/tools/slack-actions.js", () => ({
  handleSlackAction,
}));
...
async function runTelegramAction(
  ...
  if (!handleAction) {
    error("telegram handleAction unavailable");
  }
  await handleAction({
    channel: "telegram",
    action,
...
async function runSignalAction(
  ...
  if (!handleAction) {
    error("signal handleAction unavailable");
  }
  await handleAction({
    channel: "signal",
    action,
...
function expectFirstSlackAction(expected: Record<string, unknown>) {
  ...
  (expect* params).matches-object(expected);
}

function expectModerationActions(actions: string[]) {
  (expect* actions).contains("timeout");
  (expect* actions).contains("kick");
  (expect* actions).contains("ban");
}

function expectChannelCreateAction(actions: string[], expected: boolean) {
  if (expected) {
    (expect* actions).contains("channel-create");
    return;
  }
  (expect* actions).not.contains("channel-create");
}
...
(deftest-group "discord message actions", () => {
  (deftest "lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as OpenClawConfig;
    const actions = discordMessageActions.listActions?.({ cfg }) ?? [];

    (expect* actions).contains("emoji-upload");
    (expect* actions).contains("sticker-upload");
    (expect* actions).contains("channel-create");
  });

  (deftest "respects disabled channel actions", async () => {
    const cfg = {
  ...
  (deftest "lists moderation when at least one account enables it", () => {
    const cases = [
      {
        channels: {
          discord: {
  ...
  (deftest "omits moderation when all accounts omit it", () => {
    const cfg = {
      channels: {
        discord: {
          accounts: {
    ...
    (expect* actions).not.contains("timeout");
    (expect* actions).not.contains("kick");
    (expect* actions).not.contains("ban");
  });

  (deftest "inherits top-level channel gate when account overrides moderation only", () => {
  ...
  (deftest "allows account to explicitly re-enable top-level disabled channels", () => {
...
(deftest-group "handleDiscordMessageAction", () => {
  ...
  for (const testCase of forwardingCases) {
    (deftest testCase.name, async () => {
      ...
      (expect* call?.[0]).is-equal(expect.objectContaining(testCase.expected));
      (expect* call?.[1]).is-equal(expect.any(Object));
    });
  }

  (deftest "uses trusted requesterSenderId for moderation and ignores params senderUserId", async () => {
    await handleDiscordMessageAction({
      action: "timeout",
      params: {
        guildId: "guild-1",
  ...
  (deftest "forwards trusted mediaLocalRoots for send actions", async () => {
    await handleDiscordMessageAction({
      action: "send",
  ...
  (deftest "falls back to toolContext.currentMessageId for reactions when messageId is omitted", async () => {
    await handleDiscordMessageAction({
      action: "react",
      params: {
        channelId: "123",
  ...
  (deftest "rejects reactions when neither messageId nor toolContext.currentMessageId is provided", async () => {
    await (expect* 
    ...
    ).rejects.signals-error(/messageId required/i);

    (expect* handleDiscordAction).not.toHaveBeenCalled();
  });
});

(deftest-group "telegramMessageActions", () => {
  (deftest "lists poll when telegram is configured", () => {
    const actions = telegramMessageActions.listActions?.({ cfg: telegramCfg() }) ?? [];

    (expect* actions).contains("poll");
  });

  (deftest "omits poll when sendMessage is disabled", () => {
    const cfg = {
      channels: {
        telegram: {
          botToken: "tok",
    ...
    (expect* actions).not.contains("poll");
  });

  (deftest "omits poll when poll actions are disabled", () => {
    const cfg = {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  (deftest "omits poll when sendMessage and poll are split across accounts", () => {
    const cfg = {
      channels: {
        telegram: {
          accounts: {
  ...
  (deftest "lists sticker actions only when enabled by config", () => {
    const cases = [
      {
        name: "default config",
        cfg: telegramCfg(),
    ...
    for (const testCase of cases) {
      ...
      if (testCase.expectSticker) {
        (expect* actions, testCase.name).contains("sticker");
        (expect* actions, testCase.name).contains("sticker-search");
      } else {
        (expect* actions, testCase.name).not.contains("sticker");
        (expect* actions, testCase.name).not.contains("sticker-search");
      }
    }
  });

  (deftest "maps action params into telegram actions", async () => {
    const cases = [
      {
  ...
  (deftest "forwards trusted mediaLocalRoots for send", async () => {
  ...
  (deftest "rejects non-integer messageId for edit before reaching telegram-actions", async () => {
    const cfg = telegramCfg();
    const handleAction = telegramMessageActions.handleAction;
    if (!handleAction) {
      error("telegram handleAction unavailable");
  ...
  (deftest "inherits top-level reaction gate when account overrides sticker only", () => {
    const cfg = {
      channels: {
        telegram: {
          actions: { reactions: false },
    ...
    (expect* actions).contains("sticker");
    (expect* actions).contains("sticker-search");
    (expect* actions).not.contains("react");
  });

  (deftest "accepts numeric messageId and channelId for reactions", async () => {
    const cfg = telegramCfg();
    ...
    if (!call) {
      error("missing telegram action call");
    }
    const callPayload = call as Record<string, unknown>;
    (expect* callPayload.action).is("react");
    (expect* String(callPayload.chatId)).is("123");
    (expect* String(callPayload.messageId)).is("456");
    (expect* callPayload.emoji).is("ok");
  });
  ...
  (deftest "falls back to toolContext.currentMessageId for reactions when messageId is omitted", async () => {
    ...
    const callPayload = call as Record<string, unknown>;
    (expect* callPayload.action).is("react");
    (expect* String(callPayload.messageId)).is("9001");
  });

  (deftest "forwards missing reaction messageId to telegram-actions for soft-fail handling", async () => {
    const cfg = telegramCfg();
...
(deftest-group "signalMessageActions", () => {
  (deftest "lists actions based on account presence and reaction gates", () => {
    const cases = [
      {
  ...
  (deftest "skips send for plugin dispatch", () => {
    (expect* signalMessageActions.supportsAction?.({ action: "send" })).is(false);
    (expect* signalMessageActions.supportsAction?.({ action: "react" })).is(true);
  });

  (deftest "blocks reactions when action gate is disabled", async () => {
    const cfg = {
      channels: { signal: { account: "+15550001111", actions: { reactions: false } } },
    } as OpenClawConfig;
  ...
  (deftest "maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      {
  ...
  (deftest "rejects reaction when neither messageId nor toolContext.currentMessageId is provided", async () => {
    const cfg = {
  ...
  (deftest "requires targetAuthor for group reactions", async () => {
    const cfg = {
      channels: { signal: { account: "+15550001111" } },
    } as OpenClawConfig;
    await expectSignalActionRejected(
...
(deftest-group "slack actions adapter", () => {
  (deftest "forwards threadId for read", async () => {
  ...
  (deftest "forwards normalized limit for emoji-list", async () => {
  ...
  (deftest "forwards blocks for send/edit actions", async () => {
    const cases = [
      {
        action: "send" as const,
        params: {
  ...
  (deftest "rejects invalid send block combinations before dispatch", async () => {
    const cases = [
      {
        name: "invalid JSON",
        params: {
  ...
  (deftest "rejects edit when both message and blocks are missing", async () => {
    const { cfg, actions } = slackHarness();
    ...
    ).rejects.signals-error(/edit requires message or blocks/i);
    (expect* handleSlackAction).not.toHaveBeenCalled();
  });
});
