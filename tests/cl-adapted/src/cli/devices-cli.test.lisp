...
const buildGatewayConnectionDetails = mock:fn(() => ({
  url: "ws://127.0.0.1:18789",
  urlSource: "local loopback",
  message: "",
}));
const listDevicePairing = mock:fn();
const approveDevicePairing = mock:fn();
const summarizeDeviceTokens = mock:fn();
const withProgress = mock:fn(async (_opts: unknown, fn: () => deferred-result<unknown>) => await fn());
const runtime = {
  log: mock:fn(),
  error: mock:fn(),
  exit: mock:fn(),
...
mock:mock("../infra/device-pairing.js", () => ({
  listDevicePairing,
...
(deftest-group "devices cli approve", () => {
  (deftest "approves an explicit request id without listing", async () => {
  ...
  (deftest "prints an error and exits when no pending requests are available", async () => {
...
(deftest-group "devices cli remove", () => {
  (deftest "removes a paired device by id", async () => {
...
(deftest-group "devices cli clear", () => {
  (deftest "requires --yes before clearing", async () => {
  ...
  (deftest "clears paired devices and optionally pending requests", async () => {
    callGateway
      .mockResolvedValueOnce({
...
(deftest-group "devices cli tokens", () => {
  it.each([
    {
  ...
  (deftest "rejects blank device or role values", async () => {
...
(deftest-group "devices cli local fallback", () => {
  const fallbackNotice = "Direct scope access failed; using local fallback.";

  (deftest "falls back to local pairing list when gateway returns pairing required on loopback", async () => {
  ...
  (deftest "falls back to local approve when gateway returns pairing required on loopback", async () => {
    callGateway
  ...
  (deftest "does not use local fallback when an explicit --url is provided", async () => {
    callGateway.mockRejectedValueOnce(new Error("gateway closed (1008): pairing required"));

    await (expect* 
      runDevicesCommand(["list", "--json", "--url", "ws://127.0.0.1:18789"]),
    ).rejects.signals-error("pairing required");
