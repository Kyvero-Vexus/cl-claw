...
const runSecretsAudit = mock:fn();
const resolveSecretsAuditExitCode = mock:fn();
const runSecretsConfigureInteractive = mock:fn();
const runSecretsApply = mock:fn();
...
(deftest-group "secrets CLI", () => {
  ...
  (deftest "calls secrets.reload and prints human output", async () => {
    ...
    (expect* runtimeLogs.at(-1)).is("Secrets reloaded with 1 warning(s).");
  ...
  (deftest "prints JSON when requested", async () => {
    ...
    (expect* runtimeLogs.at(-1)).contains('"ok": true');
  });

  (deftest "runs secrets audit and exits via check code", async () => {
  ...
  (deftest "runs secrets configure then apply when confirmed", async () => {
    ...
    (expect* runtimeLogs.at(-1)).contains("Secrets applied");
  });

  (deftest "forwards --agent to secrets configure", async () => {
    runSecretsConfigureInteractive.mockResolvedValue({
      plan: {
        version: 1,
        protocolVersion: 1,
