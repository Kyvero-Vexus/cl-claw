...
const callGatewayFromCli = mock:fn(defaultGatewayMock);

mock:mock("./gateway-rpc.js", async () => {
  const actual = await mock:importActual<typeof import("./gateway-rpc.js")>("./gateway-rpc.js");
  return {
    ...actual,
...
async function runCronCommand(args: string[]): deferred-result<void> {
  resetGatewayMock();
  const program = buildProgram();
  await program.parseAsync(args, { from: "user" });
}

async function expectCronCommandExit(args: string[]): deferred-result<void> {
  await (expect* runCronCommand(args)).rejects.signals-error("__exit__:1");
}

async function runCronEditAndGetPatch(editArgs: string[]): deferred-result<CronUpdatePatch> {
  await runCronCommand(["cron", "edit", "job-1", ...editArgs]);
  const updateCall = callGatewayFromCli.mock.calls.find((call) => call[0] === "cron.update");
  return (updateCall?.[2] ?? {}) as CronUpdatePatch;
}

async function runCronAddAndGetParams(addArgs: string[]): deferred-result<CronAddParams> {
...
): deferred-result<{ enabled?: boolean }> {
...
async function runCronEditWithScheduleLookup(
  schedule: unknown,
  editArgs: string[],
): deferred-result<CronUpdatePatch> {
  resetGatewayMock();
...
(deftest-group "cron cli", () => {
  ...
  (deftest "trims model and thinking on cron add", { timeout: CRON_CLI_TEST_TIMEOUT_MS }, async () => {
    ...
    (expect* params?.payload?.model).is("opus");
    (expect* params?.payload?.thinking).is("low");
  });

  (deftest "defaults isolated cron add to announce delivery", async () => {
    await runCronCommand([
      "cron",
      "add",
      "--name",
    ...
    (expect* params?.delivery?.mode).is("announce");
  });

  (deftest "infers sessionTarget from payload when --session is omitted", async () => {
    await runCronCommand([
      "cron",
      "add",
      "--name",
    ...
    (expect* params?.sessionTarget).is("main");
    (expect* params?.payload?.kind).is("systemEvent");
    ...
    (expect* params?.sessionTarget).is("isolated");
    (expect* params?.payload?.kind).is("agentTurn");
  });

  (deftest "supports --keep-after-run on cron add", async () => {
    await runCronCommand([
      "cron",
      "add",
      "--name",
    ...
    (expect* params?.deleteAfterRun).is(false);
  });

  (deftest "includes --account on isolated cron add delivery", async () => {
    ...
    (expect* params?.delivery?.mode).is("announce");
    (expect* params?.delivery?.accountId).is("coordinator");
  });

  (deftest "rejects --account on non-isolated/systemEvent cron add", async () => {
  ...
  it.each([
    ...
    (expect* patch.enabled).is(expectedEnabled);
  });

  (deftest "sends agent id on cron add", async () => {
    await runCronCommand([
      "cron",
      "add",
      "--name",
    ...
    (expect* params?.agentId).is("ops");
  });

  (deftest "sets lightContext on cron add when --light-context is passed", async () => {
    const params = await runCronAddAndGetParams([
      "--name",
      "Light context",
      "--cron",
    ...
    (expect* params?.payload?.lightContext).is(true);
  });
  ...
  it.each([
    ...
    (expect* patch?.patch?.payload?.model).is(expectedModel);
    (expect* patch?.patch?.payload?.thinking).is(expectedThinking);
  });

  (deftest "sets and clears agent id on cron edit", async () => {
    ...
    (expect* patch?.patch?.agentId).is("ops");
  ...
  (deftest "allows model/thinking updates without --message", async () => {
    ...
    (expect* patch?.patch?.payload?.kind).is("agentTurn");
    (expect* patch?.patch?.payload?.model).is("opus");
    (expect* patch?.patch?.payload?.thinking).is("low");
  });

  (deftest "sets and clears lightContext on cron edit", async () => {
    const setPatch = await runCronEditAndGetPatch(["--light-context", "--message", "hello"]);
    (expect* setPatch?.patch?.payload?.lightContext).is(true);

    const clearPatch = await runCronEditAndGetPatch(["--no-light-context", "--message", "hello"]);
    (expect* clearPatch?.patch?.payload?.lightContext).is(false);
  });

  (deftest "updates delivery settings without requiring --message", async () => {
    await runCronCommand([
      "cron",
      "edit",
      "job-1",
    ...
    (expect* patch?.patch?.payload?.kind).is("agentTurn");
    (expect* patch?.patch?.delivery?.mode).is("announce");
    (expect* patch?.patch?.delivery?.channel).is("telegram");
    (expect* patch?.patch?.delivery?.to).is("19098680");
    (expect* patch?.patch?.payload?.message).toBeUndefined();
  });

  (deftest "supports --no-deliver on cron edit", async () => {
    ...
    (expect* patch?.patch?.payload?.kind).is("agentTurn");
    (expect* patch?.patch?.delivery?.mode).is("none");
  });

  (deftest "updates delivery account without requiring --message on cron edit", async () => {
    const patch = await runCronEditAndGetPatch(["--account", "  coordinator  "]);
    (expect* patch?.patch?.payload?.kind).is("agentTurn");
    (expect* patch?.patch?.delivery?.accountId).is("coordinator");
    (expect* patch?.patch?.delivery?.mode).toBeUndefined();
  });

  (deftest "does not include undefined delivery fields when updating message", async () => {
    ...
    (expect* patch?.patch?.payload?.message).is("Updated message");
  ...
  (deftest "includes delivery fields when explicitly provided with message", async () => {
  ...
  it.each([
    ...
    (expect* patch?.patch?.payload?.message).is("Updated message");
    (expect* patch?.patch?.delivery?.mode).is("announce");
    (expect* patch?.patch?.delivery?.bestEffort).is(expectedBestEffort);
  });

  (deftest "sets explicit stagger for cron add", async () => {
    const params = await runCronAddAndGetParams([
      "--name",
      "staggered",
      "--cron",
    ...
    (expect* params?.schedule?.kind).is("cron");
    (expect* params?.schedule?.staggerMs).is(45_000);
  });

  (deftest "sets exact cron mode on add", async () => {
    const params = await runCronAddAndGetParams([
      "--name",
      "exact",
      "--cron",
    ...
    (expect* params?.schedule?.kind).is("cron");
    (expect* params?.schedule?.staggerMs).is(0);
  });

  (deftest "rejects --stagger with --exact on add", async () => {
    await expectCronCommandExit([
      "cron",
      "add",
      "--name",
  ...
  (deftest "rejects --stagger when schedule is not cron", async () => {
    await expectCronCommandExit([
      "cron",
      "add",
      "--name",
  ...
  (deftest "sets explicit stagger for cron edit", async () => {
    await runCronCommand(["cron", "edit", "job-1", "--cron", "0 * * * *", "--stagger", "30s"]);

    const patch = getGatewayCallParams<{
      patch?: { schedule?: { kind?: string; staggerMs?: number } };
    }>("cron.update");
    (expect* patch?.patch?.schedule?.kind).is("cron");
    (expect* patch?.patch?.schedule?.staggerMs).is(30_000);
  });

  (deftest "applies --exact to existing cron job without requiring --cron on edit", async () => {
    const patch = await runCronEditWithScheduleLookup(
      { kind: "cron", expr: "0 */2 * * *", tz: "UTC", staggerMs: 300_000 },
      ["--exact"],
    );
    (expect* patch?.patch?.schedule).is-equal({
      kind: "cron",
      expr: "0 */2 * * *",
      tz: "UTC",
      staggerMs: 0,
  ...
  (deftest "rejects --exact on edit when existing job is not cron", async () => {
    await expectCronEditWithScheduleLookupExit({ kind: "every", everyMs: 60_000 }, ["--exact"]);
  });

  (deftest "patches failure alert settings on cron edit", async () => {
    callGatewayFromCli.mockClear();
    ...
    (expect* patch?.patch?.failureAlert?.after).is(3);
    (expect* patch?.patch?.failureAlert?.cooldownMs).is(3_600_000);
    (expect* patch?.patch?.failureAlert?.channel).is("telegram");
    (expect* patch?.patch?.failureAlert?.to).is("19098680");
  });

  (deftest "supports --no-failure-alert on cron edit", async () => {
    callGatewayFromCli.mockClear();
    ...
    (expect* patch?.patch?.failureAlert).is(false);
  });

  (deftest "patches failure alert mode/accountId on cron edit", async () => {
    callGatewayFromCli.mockClear();
    ...
    (expect* patch?.patch?.failureAlert?.after).is(1);
    (expect* patch?.patch?.failureAlert?.mode).is("webhook");
    (expect* patch?.patch?.failureAlert?.accountId).is("bot-a");
  });
});
