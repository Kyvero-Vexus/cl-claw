...
(deftest-group "logs cli", () => {
  afterEach(() => {
    callGatewayFromCli.mockClear();
    mock:restoreAllMocks();
  });

  (deftest "writes output directly to stdout/stderr", async () => {
    ...
    mock:spyOn(process.stdout, "write").mockImplementation((chunk: unknown) => {
      stdoutWrites.push(String(chunk));
      return true;
    });
    mock:spyOn(process.stderr, "write").mockImplementation((chunk: unknown) => {
    ...
    (expect* stdoutWrites.join("")).contains("Log file:");
    (expect* stdoutWrites.join("")).contains("raw line");
    (expect* stderrWrites.join("")).contains("Log tail truncated");
    (expect* stderrWrites.join("")).contains("Log cursor reset");
  });

  (deftest "wires --local-time through CLI parsing and emits local timestamps", async () => {
    callGatewayFromCli.mockResolvedValueOnce({
      file: "/tmp/openclaw.log",
      lines: [
        JSON.stringify({
    ...
    (expect* timestamp).is-truthy();
    (expect* timestamp?.endsWith("Z")).is(false);
  });

  (deftest "warns when the output pipe closes", async () => {
    callGatewayFromCli.mockResolvedValueOnce({
      file: "/tmp/openclaw.log",
      lines: ["line one"],
    });
    ...
    const stderrWrites: string[] = [];
    mock:spyOn(process.stdout, "write").mockImplementation(() => {
    ...
    (expect* stderrWrites.join("")).contains("output stdout closed");
  });

  (deftest-group "formatLogTimestamp", () => {
    (deftest "formats UTC timestamp in plain mode by default", () => {
      const result = formatLogTimestamp("2025-01-01T12:00:00.000Z");
      (expect* result).is("2025-01-01T12:00:00.000Z");
    });

    (deftest "formats UTC timestamp in pretty mode", () => {
      const result = formatLogTimestamp("2025-01-01T12:00:00.000Z", "pretty");
      (expect* result).is("12:00:00");
    });

    (deftest "formats local time in plain mode when localTime is true", () => {
      ...
      (expect* result).not.is(utcTime);
    });

    (deftest "formats local time in pretty mode when localTime is true", () => {
      const utcTime = "2025-01-01T12:00:00.000Z";
    ...
    it.each([
      ...
      (expect* formatLogTimestamp(input)).is(expected);
    });
  });
});
