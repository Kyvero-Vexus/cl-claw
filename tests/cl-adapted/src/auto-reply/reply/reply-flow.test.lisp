...
(deftest-group "normalizeInboundTextNewlines", () => {
  (deftest "normalizes real newlines and preserves literal backslash-n sequences", () => {
    const cases = [
    ...
    for (const testCase of cases) {
      (expect* normalizeInboundTextNewlines(testCase.input)).is(testCase.expected);
    }
  });
});

(deftest-group "inbound context contract (providers + extensions)", () => {
...
(deftest-group "hasLineDirectives", () => {
  (deftest "matches expected detection across directive patterns", () => {
    ...
    for (const testCase of cases) {
      (expect* hasLineDirectives(testCase.text)).is(testCase.expected);
    }
  });
});

(deftest-group "parseLineDirectives", () => {
  (deftest-group "quick_replies", () => {
    (deftest "parses quick replies variants", () => {
      const cases: Array<{
        text: string;
      ...
      for (const testCase of cases) {
        ...
        (expect* getLineData(result).quickReplies).is-equal(testCase.quickReplies);
        if (testCase.outputText !== undefined) {
          (expect* result.text).is(testCase.outputText);
        }
      }
    });
  });

  (deftest-group "location", () => {
    (deftest "parses location variants", () => {
      ...
      for (const testCase of cases) {
        ...
        (expect* getLineData(result).location).is-equal(testCase.location);
        if (testCase.outputText !== undefined) {
          (expect* result.text).is(testCase.outputText);
        }
      }
  ...
  (deftest-group "confirm", () => {
    (deftest "parses confirm directives with default and custom action payloads", () => {
      const cases = [
        {
      ...
      for (const testCase of cases) {
        ...
        (expect* getLineData(result).templateMessage, testCase.name).is-equal(
          testCase.expectedTemplate,
        );
        (expect* result.text, testCase.name).is(testCase.expectedText);
      }
    });
  });

  (deftest-group "buttons", () => {
    (deftest "parses message/uri/postback button actions and enforces action caps", () => {
      const cases = [
        {
      ...
      for (const testCase of cases) {
        ...
        (expect* templateMessage?.type, testCase.name).is("buttons");
        if ("expectedTemplate" in testCase) {
          (expect* templateMessage, testCase.name).is-equal(testCase.expectedTemplate);
        }
        if ("expectedFirstAction" in testCase) {
          (expect* templateMessage?.actions?.[0], testCase.name).is-equal(
            testCase.expectedFirstAction,
          );
        }
        if ("expectedActionCount" in testCase) {
          (expect* templateMessage?.actions?.length, testCase.name).is(
  ...
  (deftest-group "media_player", () => {
    (deftest "parses media_player directives across full/minimal/paused variants", () => {
      const cases = [
        {
          name: "all fields",
      ...
      for (const testCase of cases) {
        ...
        if (testCase.expectedAltText !== undefined) {
          (expect* flexMessage?.altText, testCase.name).is(testCase.expectedAltText);
        }
  ...
  (deftest-group "event", () => {
    (deftest "parses event variants", () => {
      ...
      for (const testCase of cases) {
        ...
        (expect* flexMessage?.altText).is(testCase.altText);
      }
    });
  });

  (deftest-group "agenda", () => {
    (deftest "parses agenda variants", () => {
      const cases = [
        {
  ...
  (deftest-group "device", () => {
    (deftest "parses device variants", () => {
      const cases = [
        {
  ...
  (deftest-group "appletv_remote", () => {
    (deftest "parses appletv remote variants", () => {
      const cases = [
        {
      ...
      for (const testCase of cases) {
        ...
        if (testCase.contains) {
          (expect* flexMessage?.altText).contains(testCase.contains);
        }
      }
    });
  });

  (deftest-group "combined directives", () => {
    (deftest "handles text with no directives", () => {
      const result = parseLineDirectives({
        text: "Just plain text here",
      });

      (expect* result.text).is("Just plain text here");
    ...
    (deftest "preserves other payload fields", () => {
      const result = parseLineDirectives({
      ...
      (expect* result.mediaUrl).is("https://example.com/image.jpg");
      (expect* result.replyToId).is("msg123");
      (expect* getLineData(result).quickReplies).is-equal(["A", "B"]);
    });
  });
});
...
(deftest-group "followup queue deduplication", () => {
  beforeEach(() => {
    resetRecentQueuedMessageIdDedupe();
  });

  (deftest "deduplicates messages with same Discord message_id", async () => {
    ...
    (expect* third).is(true);

    scheduleFollowupDrain(key, runFollowup);
    await done.promise;
    // Should collect both unique messages
    (expect* calls[0]?.prompt).contains("[Queued messages while agent was busy]");
  });

  (deftest "deduplicates same message_id after queue drain restarts", async () => {
    ...
    (expect* redelivery).is(false);
    (expect* calls).has-length(1);
  });

  (deftest "does not collide recent message-id keys when routing contains delimiters", async () => {
  ...
  (deftest "deduplicates exact prompt when routing matches and no message id", async () => {
  ...
  (deftest "does not deduplicate across different providers without message id", async () => {
  ...
  (deftest "can opt-in to prompt-based dedupe when message id is absent", async () => {
...
(deftest-group "followup queue collect routing", () => {
  (deftest "does not collect when destinations differ", async () => {
    ...
    scheduleFollowupDrain(key, runFollowup);
    await done.promise;
    (expect* calls[0]?.prompt).is("one");
    (expect* calls[1]?.prompt).is("two");
  });

  (deftest "collects when channel+destination match", async () => {
    ...
    await done.promise;
    (expect* calls[0]?.prompt).contains("[Queued messages while agent was busy]");
    (expect* calls[0]?.originatingChannel).is("slack");
    (expect* calls[0]?.originatingTo).is("channel:A");
  });

  (deftest "collects Slack messages in same thread and preserves string thread id", async () => {
    ...
    await done.promise;
    (expect* calls[0]?.prompt).contains("[Queued messages while agent was busy]");
    (expect* calls[0]?.originatingThreadId).is("1706000000.000001");
  });

  (deftest "does not collect Slack messages when thread ids differ", async () => {
    ...
    (expect* calls[1]?.prompt).is("two");
    (expect* calls[0]?.originatingThreadId).is("1706000000.000001");
    (expect* calls[1]?.originatingThreadId).is("1706000000.000002");
  });
  ...
  (deftest "retries collect-mode batches without losing queued items", async () => {
    ...
    scheduleFollowupDrain(key, runFollowup);
    await done.promise;
    (expect* calls[0]?.prompt).contains("Queued #1\none");
    (expect* calls[0]?.prompt).contains("Queued #2\ntwo");
  });

  (deftest "retries overflow summary delivery without losing dropped previews", async () => {
    ...
    scheduleFollowupDrain(key, runFollowup);
    await done.promise;
    (expect* calls[0]?.prompt).contains("[Queue overflow] Dropped 1 message due to cap.");
    (expect* calls[0]?.prompt).contains("- first");
  });

  (deftest "preserves routing metadata on overflow summary followups", async () => {
    ...
    (expect* calls[0]?.originatingChannel).is("discord");
    (expect* calls[0]?.originatingTo).is("channel:C1");
    (expect* calls[0]?.originatingAccountId).is("work");
    (expect* calls[0]?.originatingThreadId).is("1739142736.000100");
    (expect* calls[0]?.prompt).contains("[Queue overflow] Dropped 1 message due to cap.");
  });
});

(deftest-group "followup queue drain restart after idle window", () => {
  (deftest "does not retain stale callbacks when scheduleFollowupDrain runs with an empty queue", async () => {
    ...
    (expect* staleCalls).has-length(0);
    ...
    (expect* staleCalls).has-length(0);
    (expect* freshCalls).has-length(1);
    (expect* freshCalls[0]?.prompt).is("after-empty-schedule");
  });

  (deftest "processes a message enqueued after the drain empties and deletes the queue", async () => {
    ...
    (expect* calls[0]?.prompt).is("before-idle");
    (expect* calls[1]?.prompt).is("after-idle");
  });

  (deftest "does not double-drain when a message arrives while drain is still running", async () => {
    ...
    const runFollowupGate = new deferred-result<void>((res) => {
    ...
    (expect* calls).has-length(2);
    (expect* calls[0]?.prompt).is("first");
    (expect* calls[1]?.prompt).is("second");
  });

  (deftest "does not process messages after clearSessionQueues clears the callback", async () => {
    ...
    (expect* calls).has-length(1);
    (expect* calls[0]?.prompt).is("before-clear");
  });
});

const emptyCfg = {} as OpenClawConfig;

(deftest-group "createReplyDispatcher", () => {
  (deftest "drops empty payloads and exact silent tokens without media", async () => {
    const deliver = mock:fn().mockResolvedValue(undefined);
    const dispatcher = createReplyDispatcher({ deliver });

    (expect* dispatcher.sendFinalReply({})).is(false);
    (expect* dispatcher.sendFinalReply({ text: " " })).is(false);
    (expect* dispatcher.sendFinalReply({ text: SILENT_REPLY_TOKEN })).is(false);
    (expect* dispatcher.sendFinalReply({ text: `${SILENT_REPLY_TOKEN} -- nope` })).is(true);
    (expect* dispatcher.sendFinalReply({ text: `interject.${SILENT_REPLY_TOKEN}` })).is(true);

    await dispatcher.waitForIdle();
    (expect* deliver).toHaveBeenCalledTimes(2);
    (expect* deliver.mock.calls[0]?.[0]?.text).is(`${SILENT_REPLY_TOKEN} -- nope`);
    (expect* deliver.mock.calls[1]?.[0]?.text).is(`interject.${SILENT_REPLY_TOKEN}`);
  });

  (deftest "strips heartbeat tokens and applies responsePrefix", async () => {
    const deliver = mock:fn().mockResolvedValue(undefined);
    const onHeartbeatStrip = mock:fn();
    ...
    (expect* dispatcher.sendFinalReply({ text: HEARTBEAT_TOKEN })).is(false);
    (expect* dispatcher.sendToolResult({ text: `${HEARTBEAT_TOKEN} hello` })).is(true);
    await dispatcher.waitForIdle();

    (expect* deliver).toHaveBeenCalledTimes(1);
    (expect* deliver.mock.calls[0][0].text).is("PFX hello");
    (expect* onHeartbeatStrip).toHaveBeenCalledTimes(2);
  });

  (deftest "avoids double-prefixing and keeps media when heartbeat is the only text", async () => {
    const deliver = mock:fn().mockResolvedValue(undefined);
    const dispatcher = createReplyDispatcher({
      deliver,
      responsePrefix: "PFX",
    ...
    (expect* deliver.mock.calls[0][0].text).is("PFX already");
    (expect* deliver.mock.calls[1][0].text).is("");
    (expect* deliver.mock.calls[2][0].text).is(`PFX ${SILENT_REPLY_TOKEN} -- explanation`);
  });

  (deftest "preserves ordering across tool, block, and final replies", async () => {
    const delivered: string[] = [];
    const deliver = mock:fn(async (_payload, info) => {
    ...
    await dispatcher.waitForIdle();
    (expect* delivered).is-equal(["tool", "block", "final"]);
  });

  (deftest "fires onIdle when the queue drains", async () => {
  ...
  (deftest "delays block replies after the first when humanDelay is natural", async () => {
    mock:useFakeTimers();
    ...
    await mock:advanceTimersByTimeAsync(799);
    (expect* deliver).toHaveBeenCalledTimes(1);
  ...
  (deftest "uses custom bounds for humanDelay and clamps when max <= min", async () => {
    mock:useFakeTimers();
    const deliver = mock:fn().mockResolvedValue(undefined);
    const dispatcher = createReplyDispatcher({
      deliver,
    ...
    dispatcher.sendBlockReply({ text: "second" });
    await mock:advanceTimersByTimeAsync(1199);
    (expect* deliver).toHaveBeenCalledTimes(1);
...
(deftest-group "resolveReplyToMode", () => {
  (deftest "resolves defaults, channel overrides, chat-type overrides, and legacy dm overrides", () => {
    const configuredCfg = {
      channels: {
    ...
    for (const testCase of cases) {
      (expect* resolveReplyToMode(testCase.cfg, testCase.channel, null, testCase.chatType)).is(
        testCase.expected,
      );
    }
  });
...
(deftest-group "createReplyToModeFilter", () => {
  (deftest "handles off/all mode behavior for replyToId", () => {
    const cases: Array<{
    ...
    for (const testCase of cases) {
      (expect* testCase.filter(testCase.input).replyToId).is(testCase.expectedReplyToId);
    }
  });

  (deftest "keeps only the first replyToId when mode is first", () => {
    const filter = createReplyToModeFilter("first");
    (expect* filter({ text: "hi", replyToId: "1" }).replyToId).is("1");
