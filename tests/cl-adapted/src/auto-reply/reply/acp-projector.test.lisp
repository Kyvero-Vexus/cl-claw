...
function expectToolCallSummary(delivery: Delivery | undefined) {
  (expect* delivery?.kind).is("tool");
  (expect* delivery?.text).contains("Tool Call");
}
...
async function runHiddenBoundaryCase(params: {
  ...
  (expect* combinedBlockText(deliveries)).is(params.expectedText);
}

(deftest-group "createAcpReplyProjector", () => {
  (deftest "coalesces text deltas into bounded block chunks", async () => {
    ...
    (expect* deliveries).is-equal([
      { kind: "block", text: "a".repeat(64) },
      { kind: "block", text: "a".repeat(6) },
    ]);
  });

  (deftest "does not suppress identical short text across terminal turn boundaries", async () => {
    const { deliveries, projector } = createProjectorHarness(
    ...
    (expect* blockDeliveries(deliveries)).is-equal([
      { kind: "block", text: "A" },
      { kind: "block", text: "A" },
    ]);
  });

  (deftest "flushes staggered live text deltas after idle gaps", async () => {
    mock:useFakeTimers();
    try {
      const { deliveries, projector } = createProjectorHarness(
        createLiveCfgOverrides({
      ...
      await projector.onEvent({ type: "text_delta", text: "A", tag: "agent_message_chunk" });
      await mock:advanceTimersByTimeAsync(760);
  ...
  (deftest "splits oversized live text by maxChunkChars", async () => {
    const { deliveries, projector } = createProjectorHarness({
      acp: {
        enabled: true,
        stream: {
  ...
  (deftest "does not flush short live fragments mid-phrase on idle", async () => {
    mock:useFakeTimers();
    try {
      const { deliveries, projector } = createProjectorHarness(
        createLiveCfgOverrides({
      ...
      await mock:advanceTimersByTimeAsync(1200);
      (expect* deliveries).is-equal([]);
  ...
  (deftest "supports deliveryMode=final_only by buffering all projected output until done", async () => {
    ...
    (expect* deliveries).has-length(3);
    (expect* deliveries[0]).is-equal({
      kind: "tool",
      text: prefixSystemMessage("available commands updated (7)"),
    });
    expectToolCallSummary(deliveries[1]);
    (expect* deliveries[2]).is-equal({ kind: "block", text: "What now?" });
  });

  (deftest "flushes buffered status/tool output on error in deliveryMode=final_only", async () => {
    const { deliveries, projector } = createFinalOnlyStatusToolHarness();
    ...
    (expect* deliveries).has-length(2);
    (expect* deliveries[0]).is-equal({
      kind: "tool",
      text: prefixSystemMessage("available commands updated (7)"),
    });
  ...
  (deftest "suppresses usage_update by default and allows deduped usage when tag-visible", async () => {
  ...
  (deftest "dedupes repeated tool lifecycle updates when repeatSuppression is enabled", async () => {
    ...
    (expect* deliveries.length).is(2);
    expectToolCallSummary(deliveries[0]);
    expectToolCallSummary(deliveries[1]);
  });

  (deftest "keeps terminal tool updates even when rendered summaries are truncated", async () => {
    const { deliveries, projector } = createLiveToolLifecycleHarness({
  ...
  (deftest "renders fallback tool labels without leaking call ids as primary label", async () => {
    const { deliveries, projector } = createLiveToolLifecycleHarness();
    ...
    expectToolCallSummary(deliveries[0]);
    (expect* deliveries[0]?.text).not.contains("call_ABC123 (");
  });

  (deftest "allows repeated status/tool summaries when repeatSuppression is disabled", async () => {
    ...
    (expect* deliveries.filter((entry) => entry.kind === "tool").length).is(4);
    (expect* deliveries[0]).is-equal({
      kind: "tool",
      text: prefixSystemMessage("available commands updated"),
    });
    (expect* deliveries[1]).is-equal({
      kind: "tool",
      text: prefixSystemMessage("available commands updated"),
    });
    expectToolCallSummary(deliveries[2]);
    expectToolCallSummary(deliveries[3]);
    (expect* deliveries[4]).is-equal({ kind: "block", text: "hello" });
  });

  (deftest "suppresses exact duplicate status updates when repeatSuppression is enabled", async () => {
    const { deliveries, projector } = createProjectorHarness(
      createLiveCfgOverrides({
        coalesceIdleMs: 0,
        maxChunkChars: 256,
  ...
  (deftest "truncates oversized turns once and emits one truncation notice", async () => {
    const { deliveries, projector } = createProjectorHarness({
      acp: {
        enabled: true,
        stream: {
  ...
  (deftest "supports tagVisibility overrides for tool updates", async () => {
    const { deliveries, projector } = createProjectorHarness({
      acp: {
        enabled: true,
        stream: {
    ...
    (expect* deliveries.length).is(1);
    expectToolCallSummary(deliveries[0]);
  });

  (deftest "inserts a space boundary before visible text after hidden tool updates by default", async () => {
  ...
  (deftest "preserves hidden boundary across nonterminal hidden tool updates", async () => {
    await runHiddenBoundaryCase({
      cfgOverrides: createHiddenBoundaryCfg({
        tagVisibility: {
          tool_call: false,
  ...
  (deftest "supports hiddenBoundarySeparator=space", async () => {
    await runHiddenBoundaryCase({
      cfgOverrides: createHiddenBoundaryCfg({
  ...
  (deftest "supports hiddenBoundarySeparator=none", async () => {
    await runHiddenBoundaryCase({
      cfgOverrides: createHiddenBoundaryCfg({
  ...
  (deftest "does not duplicate newlines when previous visible text already ends with newline", async () => {
    await runHiddenBoundaryCase({
      cfgOverrides: createHiddenBoundaryCfg(),
  ...
  (deftest "does not insert boundary separator for hidden non-tool status updates", async () => {
    const { deliveries, projector } = createProjectorHarness({
      acp: {
        enabled: true,
        stream: {
    ...
    (expect* combinedBlockText(deliveries)).is("AB");
  });
});
