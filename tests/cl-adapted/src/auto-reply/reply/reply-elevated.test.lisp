...
(deftest-group "resolveElevatedPermissions", () => {
  (deftest "authorizes when sender matches allowFrom", () => {
    ...
    (expect* result.failures).has-length(0);
  });

  (deftest "does not authorize when only recipient matches allowFrom", () => {
    const result = resolveElevatedPermissions({
  ...
  (deftest "does not authorize untyped mutable sender fields", () => {
    const result = resolveElevatedPermissions({
  ...
  (deftest "authorizes mutable sender fields only with explicit prefix", () => {
    const result = resolveElevatedPermissions({
