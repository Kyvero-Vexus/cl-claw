...
(deftest-group "export html security hardening", () => {
  (deftest "escapes raw HTML from markdown blocks", () => {
    ...
    (expect* messages).is-truthy();
    (expect* messages?.querySelector("img[onerror]")).toBeNull();
    (expect* messages?.innerHTML).contains("&lt;img src=x onerror=alert(1)&gt;");
  });

  (deftest "escapes tree and header metadata fields", () => {
    ...
    (expect* tree).is-truthy();
    (expect* header).is-truthy();
    (expect* tree?.querySelector("img[onerror]")).toBeNull();
    (expect* header?.querySelector("img[onerror]")).toBeNull();
    (expect* tree?.innerHTML).contains("&lt;img src=x onerror=alert(9)&gt;");
    (expect* header?.innerHTML).contains("&lt;img src=x onerror=alert(9)&gt;");
    ...
    (expect* modelLeaf.getElementById("tree-container")?.innerHTML).contains(
    ...
    (expect* thinkingLeaf.getElementById("tree-container")?.innerHTML).contains(
      "&lt;img src=x onerror=alert(9)&gt;",
    );
  });

  (deftest "sanitizes image MIME types used in data URLs", () => {
    const session: SessionData = {
    ...
    (expect* img).is-truthy();
    (expect* img?.getAttribute("onerror")).toBeNull();
    (expect* img?.getAttribute("src")).is("data:application/octet-stream;base64,AAAA");
  });

  (deftest "flattens remote markdown images but keeps data-image markdown", () => {
    ...
    (expect* messages?.textContent).contains("exfil");
    (expect* messages?.querySelector(`img[src="${dataImage}"]`)).is-truthy();
  });

  (deftest "escapes markdown data-image attributes", () => {
    const dataImage = "data:image/png;base64,AAAA";
    const session: SessionData = {
    ...
    (expect* img).is-truthy();
    (expect* img?.getAttribute("onerror")).toBeNull();
    (expect* img?.getAttribute("alt")).is('x" onerror="alert(1)');
    (expect* img?.getAttribute("src")).is(dataImage);
  });
});
