...
const handleCommandsMock = mock:fn();

mock:mock("./commands.js", () => ({
  handleCommands: (...args: unknown[]) => handleCommandsMock(...args),
  buildStatusReply: mock:fn(),
  buildCommandContext: mock:fn(),
}));
...
(deftest-group "handleInlineActions", () => {
  beforeEach(() => {
    handleCommandsMock.mockReset();
  });

  (deftest "skips whatsapp replies when config is empty and From !== To", async () => {
    ...
    (expect* result).is-equal({ kind: "reply", reply: undefined });
    (expect* typing.cleanup).toHaveBeenCalled();
    (expect* handleCommandsMock).not.toHaveBeenCalled();
  });

  (deftest "forwards agentDir into handleCommands", async () => {
    const typing = createTypingController();
    ...
    (expect* result).is-equal({ kind: "reply", reply: { text: "done" } });
  ...
  (deftest "skips stale queued messages that are at or before the /stop cutoff", async () => {
    const typing = createTypingController();
  ...
  (deftest "clears /stop cutoff when a newer message arrives", async () => {
    const typing = createTypingController();
    const sessionEntry: SessionEntry = {
      sessionId: "session-2",
      updatedAt: Date.now(),
    ...
    (expect* result).is-equal({ kind: "reply", reply: { text: "ok" } });
