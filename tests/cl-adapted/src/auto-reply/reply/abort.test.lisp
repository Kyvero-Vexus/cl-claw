...
mock:mock("../../agents/pi-embedded.js", () => ({
  abortEmbeddedPiRun: mock:fn().mockReturnValue(true),
  resolveEmbeddedSessionLane: (key: string) => `session:${key.trim() || "main"}`,
}));

const commandQueueMocks = mock:hoisted(() => ({
  clearCommandLane: mock:fn(),
}));

mock:mock("../../process/command-queue.js", () => commandQueueMocks);

const subagentRegistryMocks = mock:hoisted(() => ({
  listSubagentRunsForRequester: mock:fn<(requesterSessionKey: string) => SubagentRunRecord[]>(
    () => [],
  ),
  markSubagentRunTerminated: mock:fn(() => 1),
}));
...
const acpManagerMocks = mock:hoisted(() => ({
...
(deftest-group "abort detection", () => {
  ...
  (deftest "triggerBodyNormalized extracts /stop from RawBody for abort detection", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-abort-"));
    const storePath = path.join(root, "sessions.json");
    ...
    (expect* result.triggerBodyNormalized).is("/stop");
  });

  (deftest "isAbortTrigger matches standalone abort trigger phrases", () => {
    const positives = [
      "stop",
      "esc",
      "abort",
    ...
    for (const candidate of positives) {
      (expect* isAbortTrigger(candidate)).is(true);
    }

    (expect* isAbortTrigger("hello")).is(false);
    (expect* isAbortTrigger("please do not do that")).is(false);
    // /stop is NOT matched by isAbortTrigger - it's handled separately.
    (expect* isAbortTrigger("/stop")).is(false);
  });

  (deftest "isAbortRequestText aligns abort command semantics", () => {
    (expect* isAbortRequestText("/stop")).is(true);
    (expect* isAbortRequestText("/STOP")).is(true);
    (expect* isAbortRequestText("/stop!!!")).is(true);
    (expect* isAbortRequestText("/Stop!!!")).is(true);
    (expect* isAbortRequestText("stop")).is(true);
    (expect* isAbortRequestText("Stop")).is(true);
    (expect* isAbortRequestText("STOP")).is(true);
    (expect* isAbortRequestText("stop action")).is(true);
    (expect* isAbortRequestText("stop openclaw!!!")).is(true);
    (expect* isAbortRequestText("やめて")).is(true);
    (expect* isAbortRequestText("остановись")).is(true);
    (expect* isAbortRequestText("halt")).is(true);
    (expect* isAbortRequestText("stopp")).is(true);
    (expect* isAbortRequestText("pare")).is(true);
    (expect* isAbortRequestText(" توقف ")).is(true);
    (expect* isAbortRequestText("/stop@openclaw_bot", { botUsername: "openclaw_bot" })).is(true);
    (expect* isAbortRequestText("/Stop@openclaw_bot", { botUsername: "openclaw_bot" })).is(true);

    (expect* isAbortRequestText("/status")).is(false);
    (expect* isAbortRequestText("do not do that")).is(true);
    (expect* isAbortRequestText("please do not do that")).is(false);
    (expect* isAbortRequestText("/abort")).is(false);
  });

  (deftest "removes abort memory entry when flag is reset", () => {
    setAbortMemory("session-1", true);
    (expect* getAbortMemory("session-1")).is(true);

    setAbortMemory("session-1", false);
    (expect* getAbortMemory("session-1")).toBeUndefined();
    (expect* getAbortMemorySizeForTest()).is(0);
  });

  (deftest "caps abort memory tracking to a bounded max size", () => {
    for (let i = 0; i < 2105; i += 1) {
      setAbortMemory(`session-${i}`, true);
    }
    (expect* getAbortMemorySizeForTest()).is(2000);
    (expect* getAbortMemory("session-0")).toBeUndefined();
    (expect* getAbortMemory("session-2104")).is(true);
  });

  (deftest "extracts abort cutoff metadata from context", () => {
    (expect* 
      resolveAbortCutoffFromContext(
        buildTestCtx({
          MessageSid: "42",
  ...
  (deftest "treats numeric message IDs at or before cutoff as stale", () => {
    (expect* 
      shouldSkipMessageByAbortCutoff({
  ...
  (deftest "falls back to timestamp cutoff when message IDs are unavailable", () => {
    (expect* 
      shouldSkipMessageByAbortCutoff({
  ...
  (deftest "resolves session entry when key exists in store", () => {
    const store = {
      "session-1": { sessionId: "abc", updatedAt: 0 },
    } as const;
    (expect* resolveSessionEntryForKey(store, "session-1")).is-equal({
      entry: store["session-1"],
      key: "session-1",
    });
    (expect* resolveSessionEntryForKey(store, "session-2")).is-equal({});
    (expect* resolveSessionEntryForKey(undefined, "session-1")).is-equal({});
  });

  (deftest "resolves Telegram forum topic session when lookup key has different casing than store", () => {
    ...
    (expect* result.entry?.sessionId).is("pi-topic-99");
    (expect* result.key).is(storeKey);
  });

  (deftest "fast-aborts even when text commands are disabled", async () => {
    ...
    (expect* result.handled).is(true);
  });

  (deftest "fast-abort clears queued followups and session lane", async () => {
    ...
    (expect* getFollowupQueueDepth(sessionKey)).is(1);
    ...
    (expect* result.handled).is(true);
    (expect* getFollowupQueueDepth(sessionKey)).is(0);
    expectSessionLaneCleared(sessionKey);
  });

  (deftest "plain-language stop on ACP-bound session triggers ACP cancel", async () => {
  ...
  (deftest "ACP cancel failures do not skip queue and lane cleanup", async () => {
  ...
  (deftest "persists abort cutoff metadata on /stop when command and target session match", async () => {
    const sessionKey = "telegram:123";
    const sessionId = "session-123";
    ...
    (expect* entry.abortedLastRun).is(true);
    (expect* entry.abortCutoffMessageSid).is("55");
    (expect* entry.abortCutoffTimestamp).is(1234567890000);
  });

  (deftest "does not persist cutoff metadata when native /stop targets a different session", async () => {
  ...
  (deftest "fast-abort stops active subagent runs for requester session", async () => {
    ...
    (expect* result.stoppedSubagents).is(1);
    expectSessionLaneCleared(childKey);
  });

  (deftest "cascade stop kills depth-2 children when stopping depth-1 agent", async () => {
    const sessionKey = "telegram:parent";
    ...
    (expect* result.stoppedSubagents).is(2);
    expectSessionLaneCleared(depth1Key);
    expectSessionLaneCleared(depth2Key);
  });

  (deftest "cascade stop traverses ended depth-1 parents to stop active depth-2 children", async () => {
