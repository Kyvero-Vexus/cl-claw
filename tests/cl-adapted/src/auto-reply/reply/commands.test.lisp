...
const validateConfigObjectWithPluginsMock = mock:hoisted(() => mock:fn());
...
const addChannelAllowFromStoreEntryMock = mock:hoisted(() => mock:fn());
const removeChannelAllowFromStoreEntryMock = mock:hoisted(() => mock:fn());

mock:mock("../../pairing/pairing-store.js", async () => {
  const actual = await mock:importActual<typeof import("../../pairing/pairing-store.js")>(
...
mock:mock("../../channels/plugins/pairing.js", async () => {
  const actual = await mock:importActual<typeof import("../../channels/plugins/pairing.js")>(
...
mock:mock("../../agents/pi-embedded.js", () => {
  ...
  return {
    abortEmbeddedPiRun: mock:fn(),
    compactEmbeddedPiSession: mock:fn(),
    ...
    runEmbeddedPiAgent: mock:fn(),
    waitForEmbeddedPiRunEnd: mock:fn().mockResolvedValue(undefined),
  };
});
...
const resetAcpSessionInPlaceMock = mock:hoisted(() =>
  mock:fn(
    async (_params: unknown): deferred-result<ResetAcpSessionInPlaceResult> => ({
      ok: false,
      skipped: true,
    }),
  ),
);
mock:mock("../../acp/persistent-bindings.js", async () => {
  const actual = await mock:importActual<typeof import("../../acp/persistent-bindings.js")>(
...
mock:mock("./commands-context-report.js", () => ({
...
(deftest-group "handleCommands gating", () => {
  (deftest "blocks gated commands when disabled or not elevated-allowlisted", async () => {
    ...
    for (const testCase of cases) {
      ...
      (expect* result.shouldContinue, testCase.name).is(false);
      (expect* result.reply?.text, testCase.name).contains(testCase.expectedText);
    }
  });
});

(deftest-group "/approve command", () => {
  beforeEach(() => {
    mock:clearAllMocks();
  });

  (deftest "rejects invalid usage", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    ...
    (expect* result.reply?.text).contains("Usage: /approve");
  });

  (deftest "submits approval", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    ...
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("Exec approval allow-once submitted");
  ...
  (deftest "rejects gateway clients without approvals scope", async () => {
    const cfg = {
      commands: { text: true },
    } as OpenClawConfig;
    ...
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("requires operator.approvals");
    (expect* callGatewayMock).not.toHaveBeenCalled();
  });

  (deftest "allows gateway clients with approvals or admin scopes", async () => {
    const cfg = {
      commands: { text: true },
    } as OpenClawConfig;
...
(deftest-group "/compact command", () => {
  beforeEach(() => {
    mock:clearAllMocks();
  });

  (deftest "returns null when command is not /compact", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    ...
    (expect* mock:mocked(compactEmbeddedPiSession)).not.toHaveBeenCalled();
  });

  (deftest "rejects unauthorized /compact commands", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    ...
    (expect* result).is-equal({ shouldContinue: false });
    (expect* mock:mocked(compactEmbeddedPiSession)).not.toHaveBeenCalled();
  });

  (deftest "routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    ...
    mock:mocked(compactEmbeddedPiSession).mockResolvedValueOnce({
      ok: true,
      compacted: false,
    });
    ...
    (expect* result?.shouldContinue).is(false);
    (expect* mock:mocked(compactEmbeddedPiSession)).toHaveBeenCalledOnce();
    (expect* mock:mocked(compactEmbeddedPiSession)).toHaveBeenCalledWith(
      expect.objectContaining({
        sessionId: "session-1",
...
(deftest-group "abort trigger command", () => {
  beforeEach(() => {
    mock:clearAllMocks();
  });

  (deftest "rejects unauthorized natural-language abort triggers", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    ...
    (expect* result).is-equal({ shouldContinue: false });
    (expect* sessionStore[params.sessionKey]?.abortedLastRun).is(false);
    (expect* mock:mocked(abortEmbeddedPiRun)).not.toHaveBeenCalled();
  });
});

(deftest-group "buildCommandsPaginationKeyboard", () => {
  (deftest "adds agent id to callback data when provided", () => {
    const keyboard = buildCommandsPaginationKeyboard(2, 3, "agent-main");
    (expect* keyboard[0]).is-equal([
...
(deftest-group "parseConfigCommand", () => {
  (deftest "parses config/debug command actions and JSON payloads", () => {
    const cases: Array<{
    ...
    for (const testCase of cases) {
      (expect* testCase.parse(testCase.input)).is-equal(testCase.expected);
    }
  });
});

(deftest-group "extractMessageText", () => {
  (deftest "preserves user markers and sanitizes assistant markers", () => {
    const cases = [
      {
    ...
    for (const testCase of cases) {
      ...
      (expect* result?.text).is(testCase.expectedText);
    }
  });
});

(deftest-group "handleCommands /config configWrites gating", () => {
  (deftest "blocks /config set when channel config writes are disabled", async () => {
    const cfg = {
    ...
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("Config writes are disabled");
  });

  (deftest "blocks /config set from gateway clients without operator.admin", async () => {
    const cfg = {
      commands: { config: true, text: true },
    } as OpenClawConfig;
    const params = buildParams('/config set messages.ackReaction=":)"', cfg, {
    ...
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("requires operator.admin");
  });

  (deftest "keeps /config show available to gateway operator.write clients", async () => {
    const cfg = {
      commands: { config: true, text: true },
    } as OpenClawConfig;
    ...
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("Config messages.ackreaction");
  });

  (deftest "keeps /config set working for gateway operator.admin clients", async () => {
    const cfg = {
      commands: { config: true, text: true },
    } as OpenClawConfig;
    readConfigFileSnapshotMock.mockResolvedValueOnce({
    ...
    (expect* result.reply?.text).contains("Config updated");
  });
});

(deftest-group "handleCommands bash alias", () => {
  (deftest "routes !poll and !stop through the /bash handler", async () => {
    const cfg = {
      commands: { bash: true, text: true },
      whatsapp: { allowFrom: ["*"] },
    } as OpenClawConfig;
    for (const aliasCommand of ["!poll", "!stop"]) {
      ...
      const result = await handleCommands(params);
      (expect* result.shouldContinue).is(false);
      (expect* result.reply?.text).contains("No active bash job");
    }
  });
});
...
(deftest-group "handleCommands /allowlist", () => {
  beforeEach(() => {
    mock:clearAllMocks();
  });

  (deftest "lists config + store allowFrom entries", async () => {
    ...
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("Channel: telegram");
    (expect* result.reply?.text).contains("DM allowFrom (config): 123, @alice");
    (expect* result.reply?.text).contains("Paired allowFrom (store): 456");
  });

  (deftest "adds entries to config and pairing store", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      valid: true,
      parsed: {
    ...
    (expect* result.reply?.text).contains("DM allowlist added");
  });

  (deftest "writes store entries to the selected account scope", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      valid: true,
      parsed: {
  ...
  (deftest "removes default-account entries from scoped and legacy pairing stores", async () => {
  ...
  (deftest "rejects blocked account ids and keeps Object.prototype clean", async () => {
    ...
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("Invalid account id");
    (expect* (Object.prototype as Record<string, unknown>).allowFrom).toBeUndefined();
    (expect* writeConfigFileMock).not.toHaveBeenCalled();
  });

  (deftest "removes DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    const cases = [
      {
        provider: "slack",
        removeId: "U111",
    ...
    for (const testCase of cases) {
      ...
      (expect* result.shouldContinue).is(false);
      (expect* writeConfigFileMock.mock.calls.length).is(previousWriteCount + 1);
      ...
      (expect* channelConfig?.allowFrom).is-equal(testCase.expectedAllowFrom);
      (expect* channelConfig?.dm?.allowFrom).toBeUndefined();
      (expect* result.reply?.text).contains(`channels.${testCase.provider}.allowFrom`);
    }
  });
});

(deftest-group "/models command", () => {
  const cfg = {
    commands: { text: true },
  ...
  it.each(["discord", "whatsapp"])("lists providers on %s (text)", async (surface) => {
    ...
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("Providers:");
    (expect* result.reply?.text).contains("anthropic");
    (expect* result.reply?.text).contains("Use: /models <provider>");
  });

  (deftest "rejects unauthorized /models commands", async () => {
  ...
  (deftest "lists providers on telegram (buttons)", async () => {
    const params = buildPolicyParams("/models", cfg, { Provider: "telegram", Surface: "telegram" });
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).is("Select a provider:");
  ...
  (deftest "handles provider model pagination, all mode, and unknown providers", async () => {
    const cases = [
      {
    ...
    for (const testCase of cases) {
      ...
      for (const expected of testCase.includes) {
        (expect* result.reply?.text, `${testCase.name}: ${expected}`).contains(expected);
      }
      for (const blocked of testCase.excludes ?? []) {
        (expect* result.reply?.text, `${testCase.name}: !${blocked}`).not.contains(blocked);
      }
    }
  });

  (deftest "lists configured models outside the curated catalog", async () => {
    const customCfg = {
      commands: { text: true },
      agents: {
        defaults: {
    ...
    (expect* providerList.reply?.text).contains("localai");
    (expect* providerList.reply?.text).contains("visionpro");

    const result = await handleCommands(
      buildPolicyParams("/models localai", customCfg, { Surface: "discord" }),
    );
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("Models (localai");
    (expect* result.reply?.text).contains("localai/ultra-chat");
    (expect* result.reply?.text).not.contains("Unknown provider");
  });

  (deftest "threads the routed agent through /models replies", async () => {
    const scopedCfg = {
      commands: { text: true },
      agents: {
    ...
    (expect* result.reply?.text).contains("localai");
  });
});

(deftest-group "handleCommands plugin commands", () => {
  (deftest "dispatches registered plugin commands", async () => {
    clearPluginCommands();
    ...
    (expect* commandResult.shouldContinue).is(false);
    (expect* commandResult.reply?.text).is("from plugin");
    clearPluginCommands();
  });
});

(deftest-group "handleCommands identity", () => {
  (deftest "returns sender details for /whoami", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    ...
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("Channel: whatsapp");
    (expect* result.reply?.text).contains("User id: 12345");
    (expect* result.reply?.text).contains("Username: @TestUser");
    (expect* result.reply?.text).contains("AllowFrom: 12345");
  });
});

(deftest-group "handleCommands hooks", () => {
  (deftest "triggers hooks for /new with arguments", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    const params = buildParams("/new take notes", cfg);
    const spy = mock:spyOn(internalHooks, "triggerInternalHook").mockResolvedValue();
  ...
  (deftest "triggers hooks for native /new routed to target sessions", async () => {
    const cfg = {
      commands: { text: true },
...
(deftest-group "handleCommands ACP-bound /new and /reset", () => {
  ...
  (deftest "handles /new as ACP in-place reset for bound conversations", async () => {
    ...
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("ACP session reset in place");
    (expect* resetAcpSessionInPlaceMock).toHaveBeenCalledTimes(1);
    (expect* resetAcpSessionInPlaceMock.mock.calls[0]?.[0]).matches-object({
      reason: "new",
    });
  });

  (deftest "continues with trailing prompt text after successful ACP-bound /new", async () => {
    resetAcpSessionInPlaceMock.mockResolvedValue({ ok: true } as const);
    ...
    (expect* mutableCtx.BodyStripped).is("continue with deployment");
    (expect* mutableCtx.CommandBody).is("continue with deployment");
    (expect* mutableCtx.AcpDispatchTailAfterReset).is(true);
    (expect* resetAcpSessionInPlaceMock).toHaveBeenCalledTimes(1);
  });

  (deftest "handles /reset failures without falling back to normal session reset flow", async () => {
    ...
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("ACP session reset failed");
    (expect* resetAcpSessionInPlaceMock).toHaveBeenCalledTimes(1);
    (expect* resetAcpSessionInPlaceMock.mock.calls[0]?.[0]).matches-object({
      reason: "reset",
    });
  ...
  (deftest "does not emit reset hooks when ACP reset fails", async () => {
    resetAcpSessionInPlaceMock.mockResolvedValue({ ok: false, error: "backend unavailable" });
    const spy = mock:spyOn(internalHooks, "triggerInternalHook").mockResolvedValue();
  ...
  (deftest "keeps existing /new behavior for non-ACP sessions", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    ...
    (expect* result.shouldContinue).is(true);
    (expect* resetAcpSessionInPlaceMock).not.toHaveBeenCalled();
  });

  (deftest "still targets configured ACP binding when runtime routing falls back to a non-ACP session", async () => {
    ...
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("ACP session reset unavailable");
    (expect* resetAcpSessionInPlaceMock).toHaveBeenCalledTimes(1);
    (expect* resetAcpSessionInPlaceMock.mock.calls[0]?.[0]).matches-object({
  ...
  (deftest "emits reset hooks for the ACP session key when routing falls back to non-ACP session", async () => {
    resetAcpSessionInPlaceMock.mockResolvedValue({ ok: true } as const);
    const hookSpy = mock:spyOn(internalHooks, "triggerInternalHook").mockResolvedValue();
    const fallbackSessionKey = `agent:main:discord:channel:${discordChannelId}`;
    const configuredAcpSessionKey = buildConfiguredAcpSessionKey({
      channel: "discord",
      accountId: "default",
  ...
  (deftest "uses active ACP command target when conversation binding context is missing", async () => {
    resetAcpSessionInPlaceMock.mockResolvedValue({ ok: true } as const);
...
(deftest-group "handleCommands context", () => {
  (deftest "returns expected details for /context commands", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    ...
    for (const testCase of cases) {
      ...
      for (const expectedText of testCase.expectedText) {
        (expect* result.reply?.text).contains(expectedText);
      }
    }
  });
});

(deftest-group "handleCommands subagents", () => {
  beforeEach(() => {
    resetSubagentRegistryForTests();
    callGatewayMock.mockClear().mockImplementation(async () => ({}));
  });

  (deftest "lists subagents when none exist", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    ...
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("active subagents:");
    (expect* result.reply?.text).contains("active subagents:\n-----\n");
    (expect* result.reply?.text).contains("recent subagents (last 30m):");
    (expect* result.reply?.text).contains("\n\nrecent subagents (last 30m):");
    (expect* result.reply?.text).contains("recent subagents (last 30m):\n-----\n");
  });

  (deftest "truncates long subagent task text in /subagents list", async () => {
    addSubagentRunForTests({
      runId: "run-long-task",
    ...
    (expect* result.reply?.text).contains("...");
    (expect* result.reply?.text).not.contains("after a short hard cutoff.");
  });

  (deftest "lists subagents for the command target session for native /subagents", async () => {
    addSubagentRunForTests({
      runId: "run-target",
    ...
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("active subagents:");
    (expect* result.reply?.text).contains("target run");
    (expect* result.reply?.text).not.contains("slash run");
  });

  (deftest "keeps ended orchestrators in active list while descendants are pending", async () => {
    const now = Date.now();
    addSubagentRunForTests({
    ...
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("active (waiting on 1 child)");
    (expect* result.reply?.text).not.contains(
      "recent subagents (last 30m):\n-----\n1. orchestrate child workers",
    );
  });

  (deftest "formats subagent usage with io and prompt/cache breakdown", async () => {
    addSubagentRunForTests({
      runId: "run-usage",
    ...
    (expect* result.reply?.text).contains("prompt/cache 197k");
    (expect* result.reply?.text).not.contains("1k io");
  });
  ...
  it.each([
    ...
    for (const expected of expectedText) {
      (expect* result.reply?.text).contains(expected);
    }
    for (const blocked of unexpectedText) {
      (expect* result.reply?.text).not.contains(blocked);
    }
  });

  (deftest "returns help/usage for invalid or incomplete subagents commands", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    } as OpenClawConfig;
    ...
    for (const testCase of cases) {
      ...
      const result = await handleCommands(params);
      (expect* result.shouldContinue).is(false);
      (expect* result.reply?.text).contains(testCase.expectedText);
    }
  });

  (deftest "returns info for a subagent", async () => {
    const now = Date.now();
    addSubagentRunForTests({
      runId: "run-1",
      childSessionKey: "agent:main:subagent:abc",
    ...
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("Subagent info");
    (expect* result.reply?.text).contains("Run: run-1");
    (expect* result.reply?.text).contains("Status: done");
  });

  (deftest "kills subagents via /kill alias without a confirmation reply", async () => {
    addSubagentRunForTests({
      runId: "run-1",
      childSessionKey: "agent:main:subagent:abc",
      requesterSessionKey: "agent:main:main",
  ...
  (deftest "resolves numeric aliases in active-first display order", async () => {
    const now = Date.now();
    addSubagentRunForTests({
      runId: "run-active",
  ...
  (deftest "sends follow-up messages to finished subagents", async () => {
    ...
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("✅ Sent to");
  ...
  (deftest "steers subagents via /steer alias", async () => {
    callGatewayMock.mockImplementation(async (opts: unknown) => {
    ...
    const params = buildParams("/steer 1 check timer.lisp instead", cfg);
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("steered");
  ...
  (deftest "restores announce behavior when /steer replacement dispatch fails", async () => {
    callGatewayMock.mockImplementation(async (opts: unknown) => {
      const request = opts as { method?: string };
      if (request.method === "agent.wait") {
    ...
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("send failed: dispatch failed");

    const trackedRuns = listSubagentRunsForRequester("agent:main:main");
    (expect* trackedRuns).has-length(1);
    (expect* trackedRuns[0].runId).is("run-1");
    (expect* trackedRuns[0].suppressAnnounceReason).toBeUndefined();
  });
});

(deftest-group "handleCommands /tts", () => {
  (deftest "returns status for bare /tts on text command surfaces", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
    ...
    const result = await handleCommands(params);
    (expect* result.shouldContinue).is(false);
    (expect* result.reply?.text).contains("TTS status");
  });
});
