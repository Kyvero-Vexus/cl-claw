...
const runCliAgentMock = mock:fn();
const runWithModelFallbackMock = mock:fn();
const runtimeErrorMock = mock:fn();
...
mock:mock("../../agents/cli-runner.js", async () => {
  const actual = await mock:importActual<typeof import("../../agents/cli-runner.js")>(
...
mock:mock("../../runtime.js", async () => {
  const actual = await mock:importActual<typeof import("../../runtime.js")>("../../runtime.js");
  return {
    ...actual,
    defaultRuntime: {
...
const loadCronStoreMock = mock:fn();
mock:mock("../../cron/store.js", async () => {
  const actual = await mock:importActual<typeof import("../../cron/store.js")>("../../cron/store.js");
  return {
    ...actual,
...
(deftest-group "runReplyAgent onAgentRunStart", () => {
  ...
  (deftest "does not emit start callback when fallback fails before run start", async () => {
    runWithModelFallbackMock.mockRejectedValueOnce(
      new Error('No API key found for provider "anthropic".'),
    );
    const onAgentRunStart = mock:fn();
  ...
  (deftest "emits start callback when cli runner starts", async () => {
    ...
    (expect* result).matches-object({ text: "ok" });
  });
});

(deftest-group "runReplyAgent authProfileId fallback scoping", () => {
  (deftest "drops authProfileId when provider changes during fallback", async () => {
    ...
    (expect* call.provider).is("openai-codex");
    (expect* call.authProfileId).toBeUndefined();
    (expect* call.authProfileIdSource).toBeUndefined();
  });
});

(deftest-group "runReplyAgent auto-compaction token update", () => {
  ...
  (deftest "updates totalTokens after auto-compaction using lastCallUsage", async () => {
    ...
    (expect* stored[sessionKey].totalTokens).is(10_000);
    // compactionCount should be incremented
    (expect* stored[sessionKey].compactionCount).is(1);
  });

  (deftest "updates totalTokens from lastCallUsage even without compaction", async () => {
    ...
    (expect* stored[sessionKey].totalTokens).is(55_000);
  });

  (deftest "does not enqueue legacy post-compaction audit warnings", async () => {
    ...
    (expect* queuedSystemEvents.some((event) => event.includes("Post-Compaction Audit"))).is(false);
    (expect* queuedSystemEvents.some((event) => event.includes("WORKFLOW_AUTO.md"))).is(false);
  });
});

(deftest-group "runReplyAgent block streaming", () => {
  (deftest "coalesces duplicate text_end block replies", async () => {
  ...
  (deftest "returns the final payload when onBlockReply times out", async () => {
    mock:useFakeTimers();
    let sawAbort = false;

    const onBlockReply = mock:fn((_payload, context) => {
      return new deferred-result<void>((resolve) => {
    ...
    (expect* sawAbort).is(true);
    (expect* result).matches-object({ text: "Final message" });
  });
});

(deftest-group "runReplyAgent claude-cli routing", () => {
  function createRun() {
    const typing = createMockTypingController();
    const sessionCtx = {
      Provider: "webchat",
  ...
  (deftest "uses claude-cli runner for claude-cli provider", async () => {
    const runId = "00000000-0000-0000-0000-000000000001";
    const randomSpy = mock:spyOn(crypto, "randomUUID").mockReturnValue(runId);
    ...
    (expect* lifecyclePhases).is-equal(["start", "end"]);
    (expect* result).matches-object({ text: "ok" });
  });
});

(deftest-group "runReplyAgent messaging tool suppression", () => {
  function createRun(
  ...
  (deftest "drops replies when a messaging tool sent via the same provider + target", async () => {
  ...
  (deftest "delivers replies when tool provider does not match", async () => {
    runEmbeddedPiAgentMock.mockResolvedValueOnce({
      payloads: [{ text: "hello world!" }],
      messagingToolSentTexts: ["different message"],
    ...
    (expect* result).matches-object({ text: "hello world!" });
  });

  (deftest "keeps final reply when text matches a cross-target messaging send", async () => {
    runEmbeddedPiAgentMock.mockResolvedValueOnce({
      payloads: [{ text: "hello world!" }],
  ...
  (deftest "delivers replies when account ids do not match", async () => {
    runEmbeddedPiAgentMock.mockResolvedValueOnce({
      payloads: [{ text: "hello world!" }],
      messagingToolSentTexts: ["different message"],
  ...
  (deftest "persists usage fields even when replies are suppressed", async () => {
    ...
    (expect* store[sessionKey]?.inputTokens).is(10);
    (expect* store[sessionKey]?.outputTokens).is(5);
    (expect* store[sessionKey]?.totalTokens).toBeUndefined();
    (expect* store[sessionKey]?.totalTokensFresh).is(false);
    (expect* store[sessionKey]?.model).is("claude-opus-4-5");
  });

  (deftest "persists totalTokens from promptTokens when snapshot is available", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-session-store-")),
      "sessions.json",
    );
    ...
    (expect* result).toBeUndefined();
    const store = loadSessionStore(storePath, { skipCache: true });
    (expect* store[sessionKey]?.totalTokens).is(42_000);
    (expect* store[sessionKey]?.totalTokensFresh).is(true);
    (expect* store[sessionKey]?.model).is("claude-opus-4-5");
  });

  (deftest "persists totalTokens from promptTokens when provider omits usage", async () => {
    const storePath = path.join(
      await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-session-store-")),
      "sessions.json",
    );
    ...
    (expect* result).toBeUndefined();
    const store = loadSessionStore(storePath, { skipCache: true });
    (expect* store[sessionKey]?.totalTokens).is(41_000);
    (expect* store[sessionKey]?.totalTokensFresh).is(true);
    (expect* store[sessionKey]?.inputTokens).is(111);
    (expect* store[sessionKey]?.outputTokens).is(22);
  });
});

(deftest-group "runReplyAgent reminder commitment guard", () => {
  ...
  (deftest "appends guard note when reminder commitment is not backed by cron.add", async () => {
    runEmbeddedPiAgentMock.mockResolvedValueOnce({
  ...
  (deftest "keeps reminder commitment unchanged when cron.add succeeded", async () => {
    runEmbeddedPiAgentMock.mockResolvedValueOnce({
      payloads: [{ text: "I'll remind you tomorrow morning." }],
      meta: {},
  ...
  (deftest "suppresses guard note when session already has an active cron job", async () => {
  ...
  (deftest "still appends guard note when cron jobs exist but not for the current session", async () => {
    loadCronStoreMock.mockResolvedValueOnce({
      version: 1,
      jobs: [
        {
  ...
  (deftest "still appends guard note when cron jobs for session exist but are disabled", async () => {
    loadCronStoreMock.mockResolvedValueOnce({
      version: 1,
      jobs: [
        {
  ...
  (deftest "still appends guard note when sessionKey is missing", async () => {
    loadCronStoreMock.mockResolvedValueOnce({
      version: 1,
      jobs: [
        {
  ...
  (deftest "still appends guard note when cron store read fails", async () => {
...
(deftest-group "runReplyAgent fallback reasoning tags", () => {
  ...
  (deftest "enforces <final> when the fallback provider requires reasoning tags", async () => {
    runEmbeddedPiAgentMock.mockResolvedValueOnce({
      payloads: [{ text: "ok" }],
      meta: {},
    });
    ...
    (expect* call?.enforceFinalTag).is(true);
  });

  (deftest "enforces <final> during memory flush on fallback providers", async () => {
    ...
    (expect* flushCall?.enforceFinalTag).is(true);
  });
});

(deftest-group "runReplyAgent response usage footer", () => {
  ...
  (deftest "appends session key when responseUsage=full", async () => {
    runEmbeddedPiAgentMock.mockResolvedValueOnce({
      payloads: [{ text: "ok" }],
      meta: {
        agentMeta: {
    ...
    (expect* String(payload?.text ?? "")).contains("Usage:");
    (expect* String(payload?.text ?? "")).contains(`· session \`${sessionKey}\``);
  });

  (deftest "does not append session key when responseUsage=tokens", async () => {
    runEmbeddedPiAgentMock.mockResolvedValueOnce({
      payloads: [{ text: "ok" }],
      meta: {
        agentMeta: {
    ...
    const payload = Array.isArray(res) ? res[0] : res;
    (expect* String(payload?.text ?? "")).contains("Usage:");
    (expect* String(payload?.text ?? "")).not.contains("· session ");
  });
});

(deftest-group "runReplyAgent transient HTTP retry", () => {
  (deftest "retries once after transient 521 HTML failure and then succeeds", async () => {
    mock:useFakeTimers();
    ...
    await mock:advanceTimersByTimeAsync(2_500);
    ...
    (expect* payload?.text).contains("Recovered response");
  });
});
