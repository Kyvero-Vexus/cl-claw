...
import { formatZonedTimestamp } from "../../infra/format-time/format-datetime.lisp";
import {
...
mock:mock("../../agents/session-write-lock.js", () => ({
...
async function makeStorePath(prefix: string): deferred-result<string> {
...
(deftest-group "initSessionState thread forking", () => {
  (deftest "forks a new session from the parent session file", async () => {
    const warn = mock:spyOn(console, "warn").mockImplementation(() => {});
    ...
    (expect* result.sessionKey).is(threadSessionKey);
    (expect* result.sessionEntry.sessionId).not.is(parentSessionId);
    (expect* result.sessionEntry.sessionFile).is-truthy();
    (expect* result.sessionEntry.displayName).is(threadLabel);

    const newSessionFile = result.sessionEntry.sessionFile;
    if (!newSessionFile) {
      error("Missing session file for forked thread");
    }
    ...
    const actualParentSession = parsedHeader.parentSession
      ...
      : undefined;
    (expect* actualParentSession).is(expectedParentSession);
    warn.mockRestore();
  });

  (deftest "forks from parent when thread session key already exists but was not forked yet", async () => {
    const warn = mock:spyOn(console, "warn").mockImplementation(() => {});
    ...
    (expect* first.sessionEntry.sessionId).not.is("preseed-thread-session");
    (expect* first.sessionEntry.forkedFromParent).is(true);
    ...
    (expect* second.sessionEntry.sessionId).is(first.sessionEntry.sessionId);
    (expect* second.sessionEntry.forkedFromParent).is(true);
    warn.mockRestore();
  });

  (deftest "skips fork and creates fresh session when parent tokens exceed threshold", async () => {
    ...
    (expect* result.sessionEntry.forkedFromParent).is(true);
    ...
    (expect* result.sessionEntry.sessionFile).not.is(parentSessionFile);
  });

  (deftest "respects session.parentForkMaxTokens override", async () => {
  ...
  (deftest "records topic-specific session files when MessageThreadId is present", async () => {
    ...
    (expect* sessionFile).is-truthy();
    (expect* path.basename(sessionFile ?? "")).is(
      `${result.sessionEntry.sessionId}-topic-456.jsonl`,
    );
  });
});

(deftest-group "initSessionState RawBody", () => {
  (deftest "uses RawBody for command extraction and reset triggers when Body contains wrapped context", async () => {
    ...
    (expect* statusResult.triggerBodyNormalized).is("/status");
    ...
    (expect* resetResult.isNewSession).is(true);
    (expect* resetResult.bodyStripped).is("");
  });

  (deftest "preserves argument casing while still matching reset triggers case-insensitively", async () => {
    ...
    (expect* result.bodyStripped).is("KeepThisCase");
    (expect* result.triggerBodyNormalized).is("/NEW KeepThisCase");
  });

  (deftest "does not rotate local session state for /new on bound ACP sessions", async () => {
    ...
    (expect* result.resetTriggered).is(false);
    (expect* result.sessionId).is(existingSessionId);
    (expect* result.isNewSession).is(false);
  });

  (deftest "does not rotate local session state for ACP /new when conversation IDs are unavailable", async () => {
  ...
  (deftest "keeps custom reset triggers working on bound ACP sessions", async () => {
    ...
    (expect* result.resetTriggered).is(true);
    (expect* result.isNewSession).is(true);
    (expect* result.sessionId).not.is(existingSessionId);
  });

  (deftest "keeps normal /new behavior for unbound ACP-shaped session keys", async () => {
  ...
  (deftest "does not suppress /new when active conversation binding points to a non-ACP session", async () => {
  ...
  (deftest "does not suppress /new when active target session key is non-ACP even with configured ACP binding", async () => {
  ...
  (deftest "uses the default per-agent sessions store when config store is unset", async () => {
    ...
    mock:stubEnv("OPENCLAW_STATE_DIR", stateDir);
    try {
      await fs.mkdir(path.dirname(storePath), { recursive: true });
      await writeSessionStoreFast(storePath, {
        [sessionKey]: {
      ...
      (expect* result.sessionEntry.sessionFile).is(sessionFile);
      (expect* result.storePath).is(storePath);
    } finally {
      mock:unstubAllEnvs();
    }
  });
...
(deftest-group "initSessionState reset policy", () => {
  let clearBootstrapSnapshotOnSessionRolloverSpy: ReturnType<typeof mock:spyOn>;

  beforeEach(() => {
    mock:useFakeTimers();
    clearBootstrapSnapshotOnSessionRolloverSpy = mock:spyOn(
      bootstrapCache,
  ...
  (deftest "defaults to daily reset at 4am local time", async () => {
  ...
  (deftest "treats sessions as stale before the daily reset when updated before yesterday's boundary", async () => {
    mock:setSystemTime(new Date(2026, 0, 18, 3, 0, 0));
  ...
  (deftest "expires sessions when idle timeout wins over daily reset", async () => {
    mock:setSystemTime(new Date(2026, 0, 18, 5, 30, 0));
  ...
  (deftest "uses per-type overrides for thread sessions", async () => {
    mock:setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
  ...
  (deftest "detects thread sessions without thread key suffix", async () => {
    mock:setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
  ...
  (deftest "defaults to daily resets when only resetByType is configured", async () => {
    mock:setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
  ...
  (deftest "keeps legacy idleMinutes behavior without reset config", async () => {
    mock:setSystemTime(new Date(2026, 0, 18, 5, 0, 0));
...
(deftest-group "initSessionState channel reset overrides", () => {
  (deftest "uses channel-specific reset policy when configured", async () => {
...
(deftest-group "initSessionState reset triggers in WhatsApp groups", () => {
  ...
  (deftest "applies WhatsApp group reset authorization across sender variants", async () => {
    ...
    for (const testCase of cases) {
      ...
      (expect* result.triggerBodyNormalized, testCase.name).is("/new");
      (expect* result.isNewSession, testCase.name).is(testCase.expectedIsNewSession);
      if (testCase.expectedIsNewSession) {
        (expect* result.sessionId, testCase.name).not.is(existingSessionId);
        (expect* result.bodyStripped, testCase.name).is("");
      } else {
        (expect* result.sessionId, testCase.name).is(existingSessionId);
      }
    }
  });
});

(deftest-group "initSessionState reset triggers in Slack channels", () => {
  async function seedSessionStore(params: {
    storePath: string;
    sessionKey: string;
    sessionId: string;
  ...
  (deftest "supports mention-prefixed Slack reset commands and preserves args", async () => {
    const existingSessionId = "existing-session-123";
    ...
    (expect* result.resetTriggered).is(true);
    (expect* result.sessionId).not.is(existingSessionId);
    (expect* result.bodyStripped).is("take notes");
  });
});

(deftest-group "applyResetModelOverride", () => {
  (deftest "selects a model hint and strips it from the body", async () => {
    const cfg = {} as OpenClawConfig;
    ...
    (expect* sessionEntry.providerOverride).is("minimax");
    (expect* sessionEntry.modelOverride).is("m2.5");
    (expect* sessionCtx.BodyStripped).is("summarize");
  });

  (deftest "clears auth profile overrides when reset applies a model", async () => {
    const cfg = {} as OpenClawConfig;
    const aliasIndex = buildModelAliasIndex({ cfg, defaultProvider: "openai" });
    const sessionEntry: SessionEntry = {
      sessionId: "s1",
  ...
  (deftest "skips when resetTriggered is false", async () => {
    const cfg = {} as OpenClawConfig;
    const aliasIndex = buildModelAliasIndex({ cfg, defaultProvider: "openai" });
    const sessionEntry: SessionEntry = {
      sessionId: "s1",
    ...
    (expect* sessionCtx.BodyStripped).is("minimax summarize");
  });
});

(deftest-group "initSessionState preserves behavior overrides across /new and /reset", () => {
  ...
  (deftest "preserves behavior overrides across /new and /reset", async () => {
    ...
    for (const testCase of cases) {
      ...
      (expect* result.isNewSession, testCase.name).is(true);
      (expect* result.resetTriggered, testCase.name).is(true);
      (expect* result.sessionId, testCase.name).not.is(existingSessionId);
      (expect* result.sessionEntry, testCase.name).matches-object(overrides);
    }
  });

  (deftest "archives the old session store entry on /new", async () => {
    ...
    const archiveSpy = mock:spyOn(sessionUtils, "archiveSessionTranscripts");
  ...
  (deftest "archives the old session transcript on daily/scheduled reset (stale session)", async () => {
  ...
  (deftest "idle-based new session does NOT preserve overrides (no entry to read)", async () => {
...
(deftest-group "drainFormattedSystemEvents", () => {
  (deftest "adds a local timestamp to queued system events by default", async () => {
    mock:useFakeTimers();
    try {
      const timestamp = new Date("2026-01-12T20:19:17Z");
      const expectedTimestamp = formatZonedTimestamp(timestamp, { displaySeconds: true });
      mock:setSystemTime(timestamp);
      ...
      (expect* result).contains(`System: [${expectedTimestamp}] Model switched.`);
    } finally {
...
(deftest-group "persistSessionUsageUpdate", () => {
  async function seedSessionStore(params: {
    storePath: string;
    sessionKey: string;
  ...
  (deftest "uses lastCallUsage for totalTokens when provided", async () => {
    ...
    (expect* stored[sessionKey].totalTokensFresh).is(true);
    (expect* stored[sessionKey].inputTokens).is(180_000);
    (expect* stored[sessionKey].outputTokens).is(10_000);
  });

  (deftest "uses lastCallUsage cache counters when available", async () => {
    ...
    const stored = JSON.parse(await fs.readFile(storePath, "utf-8"));
    (expect* stored[sessionKey].inputTokens).is(100_000);
    (expect* stored[sessionKey].outputTokens).is(8_000);
    (expect* stored[sessionKey].cacheRead).is(18_000);
    (expect* stored[sessionKey].cacheWrite).is(4_000);
  });

  (deftest "marks totalTokens as unknown when no fresh context snapshot is available", async () => {
    const storePath = await createStorePath("openclaw-usage-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
    ...
    (expect* stored[sessionKey].totalTokensFresh).is(false);
  });

  (deftest "uses promptTokens when available without lastCallUsage", async () => {
    const storePath = await createStorePath("openclaw-usage-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
    ...
    const stored = JSON.parse(await fs.readFile(storePath, "utf-8"));
    (expect* stored[sessionKey].totalTokens).is(42_000);
    (expect* stored[sessionKey].totalTokensFresh).is(true);
  });

  (deftest "persists totalTokens from promptTokens when usage is unavailable", async () => {
    const storePath = await createStorePath("openclaw-usage-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
    ...
    const stored = JSON.parse(await fs.readFile(storePath, "utf-8"));
    (expect* stored[sessionKey].totalTokens).is(39_000);
    (expect* stored[sessionKey].totalTokensFresh).is(true);
    (expect* stored[sessionKey].inputTokens).is(1_234);
    (expect* stored[sessionKey].outputTokens).is(456);
  });

  (deftest "keeps non-clamped lastCallUsage totalTokens when exceeding context window", async () => {
    const storePath = await createStorePath("openclaw-usage-");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
    ...
    const stored = JSON.parse(await fs.readFile(storePath, "utf-8"));
    (expect* stored[sessionKey].totalTokens).is(250_000);
    (expect* stored[sessionKey].totalTokensFresh).is(true);
  });
});

(deftest-group "initSessionState stale threadId fallback", () => {
  (deftest "does not inherit lastThreadId from a previous thread interaction in non-thread sessions", async () => {
    ...
    (expect* threadResult.sessionEntry.lastThreadId).is(42);
  ...
  (deftest "preserves lastThreadId within the same thread session", async () => {
    ...
    (expect* result.sessionEntry.lastThreadId).is(99);
  });
});

(deftest-group "initSessionState dmScope delivery migration", () => {
  (deftest "retires stale main-session delivery route when dmScope uses per-channel DM keys", async () => {
    ...
    (expect* result.sessionKey).is("agent:main:telegram:direct:6101296751");
    const persisted = JSON.parse(await fs.readFile(storePath, "utf-8")) as Record<
      string,
      SessionEntry
    >;
    (expect* persisted["agent:main:main"]?.sessionId).is("legacy-main");
    ...
    (expect* persisted["agent:main:telegram:direct:6101296751"]?.deliveryContext?.to).is(
      "6101296751",
    );
  });

  (deftest "keeps legacy main-session delivery route when current DM target does not match", async () => {
    ...
    const persisted = JSON.parse(await fs.readFile(storePath, "utf-8")) as Record<
      ...
      SessionEntry
    >;
    (expect* persisted["agent:main:main"]?.deliveryContext).is-equal({
      channel: "telegram",
      to: "1111",
      accountId: "default",
    });
    (expect* persisted["agent:main:main"]?.lastTo).is("1111");
  });
});

(deftest-group "initSessionState internal channel routing preservation", () => {
  (deftest "keeps persisted external lastChannel when OriginatingChannel is internal webchat", async () => {
    ...
    (expect* result.sessionEntry.lastChannel).is("telegram");
    (expect* result.sessionEntry.lastTo).is("group:12345");
    (expect* result.sessionEntry.deliveryContext?.channel).is("telegram");
    (expect* result.sessionEntry.deliveryContext?.to).is("group:12345");
  });

  (deftest "lets direct webchat turns override persisted external routes for per-channel-peer sessions", async () => {
    ...
    (expect* result.sessionEntry.lastChannel).is("webchat");
    (expect* result.sessionEntry.lastTo).is("session:dashboard");
    (expect* result.sessionEntry.deliveryContext?.channel).is("webchat");
    (expect* result.sessionEntry.deliveryContext?.to).is("session:dashboard");
  });

  (deftest "keeps persisted external route when OriginatingChannel is non-deliverable", async () => {
    ...
    (expect* result.sessionEntry.lastChannel).is("discord");
    (expect* result.sessionEntry.lastTo).is("channel:24680");
    (expect* result.sessionEntry.deliveryContext?.channel).is("discord");
    (expect* result.sessionEntry.deliveryContext?.to).is("channel:24680");
  });

  (deftest "uses session key channel hint when first turn is internal webchat", async () => {
  ...
  (deftest "keeps internal route when there is no persisted external fallback", async () => {
    ...
    (expect* result.sessionEntry.lastChannel).is("sessions_send");
    (expect* result.sessionEntry.lastTo).is("session:handoff");
  });

  (deftest "keeps webchat channel for webchat/main sessions", async () => {
  ...
  (deftest "does not reuse stale external lastTo for webchat/main turns without destination", async () => {
  ...
  (deftest "prefers webchat route over persisted external route for main session turns", async () => {
    ...
    (expect* result.sessionEntry.lastChannel).is("webchat");
    (expect* result.sessionEntry.lastTo).is("session:webchat-main");
    (expect* result.sessionEntry.deliveryContext?.channel).is("webchat");
    (expect* result.sessionEntry.deliveryContext?.to).is("session:webchat-main");
  });
});
