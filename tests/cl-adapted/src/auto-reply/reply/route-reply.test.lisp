...
const mocks = mock:hoisted(() => ({
  sendMessageDiscord: mock:fn(async () => ({ messageId: "m1", channelId: "c1" })),
  sendMessageIMessage: mock:fn(async () => ({ messageId: "ok" })),
  sendMessageMSTeams: mock:fn(async (_params: unknown) => ({
    messageId: "m1",
    conversationId: "c1",
  })),
  sendMessageSignal: mock:fn(async () => ({ messageId: "t1" })),
  sendMessageSlack: mock:fn(async () => ({ messageId: "m1", channelId: "c1" })),
  sendMessageTelegram: mock:fn(async () => ({ messageId: "m1", chatId: "c1" })),
  sendMessageWhatsApp: mock:fn(async () => ({ messageId: "m1", toJid: "jid" })),
...
mock:mock("../../infra/outbound/deliver.js", async () => {
  const actual = await mock:importActual<typeof import("../../infra/outbound/deliver.js")>(
...
const actualDeliver = await mock:importActual<typeof import("../../infra/outbound/deliver.js")>(
  "../../infra/outbound/deliver.js",
);
...
(deftest-group "routeReply", () => {
  ...
  (deftest "skips sends when abort signal is already aborted", async () => {
    ...
    (expect* res.error).contains("aborted");
    (expect* mocks.sendMessageSlack).not.toHaveBeenCalled();
  });

  (deftest "no-ops on empty payload", async () => {
    mocks.sendMessageSlack.mockClear();
    const res = await routeReply({
      payload: {},
      channel: "slack",
  ...
  (deftest "suppresses reasoning payloads", async () => {
    mocks.sendMessageSlack.mockClear();
    const res = await routeReply({
  ...
  (deftest "drops silent token payloads", async () => {
    mocks.sendMessageSlack.mockClear();
    const res = await routeReply({
  ...
  (deftest "does not drop payloads that merely start with the silent token", async () => {
    mocks.sendMessageSlack.mockClear();
    const res = await routeReply({
  ...
  (deftest "applies responsePrefix when routing", async () => {
    mocks.sendMessageSlack.mockClear();
    const cfg = {
  ...
  (deftest "does not derive responsePrefix from agent identity when routing", async () => {
    mocks.sendMessageSlack.mockClear();
    const cfg = {
      agents: {
        list: [
  ...
  (deftest "uses threadId for Slack when replyToId is missing", async () => {
    mocks.sendMessageSlack.mockClear();
    await routeReply({
      payload: { text: "hi" },
      channel: "slack",
  ...
  (deftest "passes thread id to Telegram sends", async () => {
  ...
  (deftest "passes replyToId to Telegram sends", async () => {
    mocks.sendMessageTelegram.mockClear();
    await routeReply({
  ...
  (deftest "uses replyToId as threadTs for Slack", async () => {
    mocks.sendMessageSlack.mockClear();
    await routeReply({
  ...
  (deftest "uses threadId as threadTs for Slack when replyToId is missing", async () => {
    mocks.sendMessageSlack.mockClear();
    await routeReply({
      payload: { text: "hi" },
      channel: "slack",
  ...
  (deftest "sends multiple mediaUrls (caption only on first)", async () => {
    mocks.sendMessageSlack.mockClear();
    await routeReply({
  ...
  (deftest "routes WhatsApp via outbound sender (accountId honored)", async () => {
  ...
  (deftest "routes MS Teams via proactive sender", async () => {
  ...
  (deftest "passes mirror data when sessionKey is set", async () => {
  ...
  (deftest "skips mirror data when mirror is false", async () => {
    mocks.deliverOutboundPayloads.mockResolvedValue([]);
    await routeReply({
      payload: { text: "hi" },
      channel: "slack",
