...
const hoisted = mock:hoisted(() => {
  ...
  const ensureSessionMock = mock:fn();
  const runTurnMock = mock:fn();
  const cancelMock = mock:fn();
  const closeMock = mock:fn();
  const getCapabilitiesMock = mock:fn();
  const getStatusMock = mock:fn();
  const setModeMock = mock:fn();
  const setConfigOptionMock = mock:fn();
  const doctorMock = mock:fn();
  return {
    callGatewayMock,
...
mock:mock("../../discord/monitor/gateway-plugin.js", () => ({
...
function expectBoundIntroTextToExclude(match: string): void {
  ...
  (expect* (introText ?? "").includes(match)).is(false);
}
...
(deftest-group "/acp command", () => {
  ...
  (deftest "returns null when the message is not /acp", async () => {
    const result = await runDiscordAcpCommand("/status");
    (expect* result).toBeNull();
  });

  (deftest "shows help by default", async () => {
    const result = await runDiscordAcpCommand("/acp");
    (expect* result?.reply?.text).contains("ACP commands:");
    (expect* result?.reply?.text).contains("/acp spawn");
  });

  (deftest "spawns an ACP session and binds a Discord thread", async () => {
    ...
    (expect* result?.reply?.text).contains("Spawned ACP session agent:codex:acp:");
    (expect* result?.reply?.text).contains("Created thread thread-created and bound it");
    ...
    (expect* seededWithoutEntry?.backend).is("acpx");
    (expect* seededWithoutEntry?.runtimeSessionName).contains(":runtime");
  });

  (deftest "accepts unicode dash option prefixes in /acp spawn args", async () => {
    const result = await runThreadAcpCommand(
      "/acp spawn codex \u2014mode oneshot \u2014thread here \u2014cwd /home/bob/clawd \u2014label jeerreview",
    );

    (expect* result?.reply?.text).contains("Spawned ACP session agent:codex:acp:");
    (expect* result?.reply?.text).contains("Bound this thread to");
    (expect* hoisted.ensureSessionMock).toHaveBeenCalledWith(
      expect.objectContaining({
        agent: "codex",
        mode: "oneshot",
  ...
  (deftest "binds Telegram topic ACP spawns to full conversation ids", async () => {
    const result = await runTelegramAcpCommand("/acp spawn codex --thread here");

    (expect* result?.reply?.text).contains("Spawned ACP session agent:codex:acp:");
    (expect* result?.reply?.text).contains("Bound this conversation to");
    (expect* result?.reply?.channelData).is-equal({ telegram: { pin: true } });
    (expect* hoisted.sessionBindingBindMock).toHaveBeenCalledWith(
      expect.objectContaining({
        placement: "current",
  ...
  (deftest "binds Telegram DM ACP spawns to the DM conversation id", async () => {
  ...
  (deftest "requires explicit ACP target when acp.defaultAgent is not configured", async () => {
    const result = await runDiscordAcpCommand("/acp spawn");

    (expect* result?.reply?.text).contains("ACP target harness id is required");
    (expect* hoisted.ensureSessionMock).not.toHaveBeenCalled();
  });

  (deftest "rejects thread-bound ACP spawn when spawnAcpSessions is disabled", async () => {
    const cfg = {
      ...baseCfg,
      channels: {
        discord: {
    ...
    (expect* result?.reply?.text).contains("spawnAcpSessions=true");
  ...
  (deftest "forbids /acp spawn from sandboxed requester sessions", async () => {
    const cfg = {
      ...baseCfg,
      agents: {
        defaults: {
    ...
    (expect* result?.reply?.text).contains("Sandboxed sessions cannot spawn ACP sessions");
  ...
  (deftest "cancels the ACP session bound to the current thread", async () => {
  ...
  (deftest "sends steer instructions via ACP runtime", async () => {
    ...
    (expect* result?.reply?.text).contains("Applied steering.");
  });

  (deftest "resolves bound Telegram topic ACP sessions for /acp steer without explicit target", async () => {
    ...
    (expect* result?.reply?.text).contains("Viewed diver package.");
  });

  (deftest "blocks /acp steer when ACP dispatch is disabled by policy", async () => {
    const cfg = {
      ...baseCfg,
      acp: {
        ...baseCfg.acp,
    ...
    (expect* result?.reply?.text).contains("ACP dispatch is disabled by policy");
    (expect* hoisted.runTurnMock).not.toHaveBeenCalled();
  });

  (deftest "closes an ACP session, unbinds thread targets, and clears metadata", async () => {
    ...
    (expect* hoisted.upsertAcpSessionMetaMock).toHaveBeenCalled();
    (expect* result?.reply?.text).contains("Removed 1 binding");
  });

  (deftest "lists ACP sessions from the session store", async () => {
    ...
    (expect* result?.reply?.text).contains("ACP sessions:");
    (expect* result?.reply?.text).contains("codex-main");
    (expect* result?.reply?.text).contains(`thread:${defaultThreadId}`);
  });

  (deftest "shows ACP status for the thread-bound ACP session", async () => {
    mockBoundThreadSession({
      identity: {
        state: "resolved",
        source: "status",
    ...
    (expect* result?.reply?.text).contains("ACP status:");
    (expect* result?.reply?.text).contains(`session: ${defaultAcpSessionKey}`);
    (expect* result?.reply?.text).contains("agent session id: codex-sid-1");
    (expect* result?.reply?.text).contains("acpx session id: acpx-sid-1");
    (expect* result?.reply?.text).contains("capabilities:");
    (expect* hoisted.getStatusMock).toHaveBeenCalledTimes(1);
  });

  (deftest "updates ACP runtime mode via /acp set-mode", async () => {
    mockBoundThreadSession();
    ...
    (expect* result?.reply?.text).contains("Updated ACP runtime mode");
  });

  (deftest "updates ACP config options and keeps cwd local when using /acp set", async () => {
    mockBoundThreadSession();
    ...
    (expect* setModel?.reply?.text).contains("Updated ACP config option");
    ...
    (expect* setCwd?.reply?.text).contains("Updated ACP cwd");
  });

  (deftest "rejects non-absolute cwd values via ACP runtime option validation", async () => {
    mockBoundThreadSession();

    const result = await runThreadAcpCommand("/acp cwd relative/path", baseCfg);

    (expect* result?.reply?.text).contains("ACP error (ACP_INVALID_RUNTIME_OPTION)");
    (expect* result?.reply?.text).contains("absolute path");
  });

  (deftest "rejects invalid timeout values before backend config writes", async () => {
    mockBoundThreadSession();
  ...
  (deftest "returns actionable doctor output when backend is missing", async () => {
    ...
    (expect* result?.reply?.text).contains("ACP doctor:");
    (expect* result?.reply?.text).contains("healthy: no");
    (expect* result?.reply?.text).contains("next:");
  });

  (deftest "shows deterministic install instructions via /acp install", async () => {
    const result = await runDiscordAcpCommand("/acp install", baseCfg);

    (expect* result?.reply?.text).contains("ACP install:");
    (expect* result?.reply?.text).contains("run:");
    (expect* result?.reply?.text).contains("then: /acp doctor");
  });
});
