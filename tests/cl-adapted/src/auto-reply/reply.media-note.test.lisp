...
(deftest-group "getReplyFromConfig media note plumbing", () => {
  (deftest "includes all MediaPaths in the agent prompt", async () => {
    await withTempHome(async (home) => {
      let seenPrompt: string | undefined;
      mock:mocked(runEmbeddedPiAgent).mockImplementation(async (params) => {
      ...
      (expect* text).is("ok");
      (expect* seenPrompt).is-truthy();
      (expect* seenPrompt).contains("[media attached: 2 files]");
      const idxA = seenPrompt?.indexOf("[media attached 1/2: /tmp/a.png");
      const idxB = seenPrompt?.indexOf("[media attached 2/2: /tmp/b.png");
      (expect* typeof idxA).is("number");
      (expect* typeof idxB).is("number");
      (expect* (idxA ?? -1) >= 0).is(true);
      (expect* (idxB ?? -1) >= 0).is(true);
      (expect* (idxA ?? 0) < (idxB ?? 0)).is(true);
    });
  });
});
