...
async function runThinkDirectiveAndGetText(home: string): deferred-result<string | undefined> {
  const res = await getReplyFromConfig(
...
async function runInFlightVerboseToggleCase(params: {
  ...
  mock:mocked(runEmbeddedPiAgent).mockImplementation(async (agentParams) => {
    const shouldEmit = agentParams.shouldEmitToolResult;
    (expect* shouldEmit?.()).is(params.shouldEmitBefore);
...
(deftest-group "directive behavior", () => {
  ...
  (deftest "keeps reasoning acks out of mixed messages, including rapid repeats", async () => {
    await withTempHome(async (home) => {
      ...
      (expect* replyTexts(firstRes)).contains("done");
      ...
      (expect* blockReplies.length).is(0);
    });
  });
  (deftest "handles standalone verbose directives and persistence", async () => {
    await withTempHome(async (home) => {
      const storePath = sessionStorePath(home);
      ...
      (expect* entry?.verboseLevel).is("off");
      (expect* runEmbeddedPiAgent).not.toHaveBeenCalled();
    });
  });
  (deftest "updates tool verbose during in-flight runs for toggle on/off", async () => {
    await withTempHome(async (home) => {
  ...
  (deftest "covers think status and /thinking xhigh support matrix", async () => {
    await withTempHome(async (home) => {
      const text = await runThinkDirectiveAndGetText(home);
      (expect* text).contains("Current thinking level: high");
      ...
      for (const model of ["openai-codex/gpt-5.2-codex", "openai/gpt-5.2"]) {
        ...
        (expect* texts).contains("Thinking level set to xhigh.");
      }

      const unsupportedModelTexts = await runThinkingDirective(home, "openai/gpt-4.1-mini");
      (expect* unsupportedModelTexts).contains(
  ...
  (deftest "keeps reserved command aliases from matching after trimming", async () => {
    await withTempHome(async (home) => {
      const res = await getReplyFromConfig(
        {
          Body: "/help",
  ...
  (deftest "treats skill commands as reserved for model aliases", async () => {
    await withTempHome(async (home) => {
      ...
      const prompt = mock:mocked(runEmbeddedPiAgent).mock.calls[0]?.[0]?.prompt ?? "";
      (expect* prompt).contains('Use the "demo-skill" skill');
    });
  });
  (deftest "reports invalid queue options and current queue settings", async () => {
    await withTempHome(async (home) => {
      ...
      (expect* invalidText).contains("Invalid debounce");
      (expect* invalidText).contains("Invalid cap");
      (expect* invalidText).contains("Invalid drop policy");
