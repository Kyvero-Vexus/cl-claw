...
async function writeStoredModelOverride(cfg: ReturnType<typeof makeCfg>): deferred-result<void> {
  await fs.writeFile(
...
async function expectResetBlockedForNonOwner(params: { home: string }): deferred-result<void> {
  const { home } = params;
...
(deftest-group "trigger handling", () => {
  ...
  (deftest "handles trigger command and heartbeat flows end-to-end", async () => {
    await withTempHome(async (home) => {
      ...
      for (const testCase of errorCases) {
        ...
        (expect* maybeReplyText(errorRes), testCase.error).is(testCase.expected);
      ...
      for (const testCase of tokenCases) {
        ...
        (expect* maybeReplyText(res)).is(testCase.expected);
      ...
      for (const testCase of thinkCases) {
        ...
        (expect* text, testCase.label).is("ok");
        ...
        if (testCase.assertPrompt) {
          ...
          (expect* prompt).contains("Give me the status");
          (expect* prompt).not.contains("/thinking high");
      ...
      for (const testCase of modelCases) {
        ...
        (expect* call?.provider).is(testCase.expected.provider);
        (expect* call?.model).is(testCase.expected.model);
      }
      {
...

      {
        ...
        (expect* text?.startsWith("⚙️ Compacted")).is(true);
        (expect* getCompactEmbeddedPiSessionMock()).toHaveBeenCalledOnce();
        (expect* getCompactEmbeddedPiSessionMock().mock.calls[0]?.[0]?.sessionFile).contains(
...

      {
        ...
        if (!storePath) {
          error("missing session store path");
        }
        ...
        (expect* getFollowupQueueDepth(targetSessionKey)).is(1);
        ...
        (expect* text).is("⚙️ Agent was aborted.");
        (expect* getAbortEmbeddedPiRunMock()).toHaveBeenCalledWith(targetSessionId);
        const store = loadSessionStore(storePath);
        (expect* store[targetSessionKey]?.abortedLastRun).is(true);
        (expect* getFollowupQueueDepth(targetSessionKey)).is(0);
      }

      {
        ...
        const text = Array.isArray(res) ? res[0]?.text : res?.text;
        (expect* text).contains("Model set to openai/gpt-4.1-mini");

        const store = loadSessionStore(storePath);
        (expect* store[targetSessionKey]?.providerOverride).is("openai");
        (expect* store[targetSessionKey]?.modelOverride).is("gpt-4.1-mini");
        ...
        (expect* getRunEmbeddedPiAgentMock().mock.calls[0]?.[0]).is-equal(
