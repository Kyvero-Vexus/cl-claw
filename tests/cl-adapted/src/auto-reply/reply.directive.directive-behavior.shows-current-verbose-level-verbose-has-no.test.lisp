...
(deftest-group "directive behavior", () => {
  ...
  (deftest "reports current directive defaults when no arguments are provided", async () => {
    await withTempHome(async (home) => {
      const verboseText = await runCommand(home, "/verbose", {
        defaults: { verboseDefault: "on" },
      });
      (expect* verboseText).contains("Current verbose level: on");
      (expect* verboseText).contains("Options: on, full, off.");

      const reasoningText = await runCommand(home, "/reasoning");
      (expect* reasoningText).contains("Current reasoning level: off");
      (expect* reasoningText).contains("Options: on, off, stream.");

      const elevatedText = replyText(await runElevatedCommand(home, "/elevated"));
      (expect* elevatedText).contains("Current elevated level: on");
      (expect* elevatedText).contains("Options: on, off, ask, full.");
      ...
      (expect* execText).contains(
        "Current exec defaults: host=gateway, security=allowlist, ask=always, sbcl=mac-1.",
      );
      (expect* execText).contains(
        "Options: host=sandbox|gateway|sbcl, security=deny|allowlist|full, ask=off|on-miss|always, sbcl=<id>.",
      );
      (expect* runEmbeddedPiAgent).not.toHaveBeenCalled();
    });
  });
  (deftest "persists elevated toggles across /status and /elevated", async () => {
    await withTempHome(async (home) => {
      const storePath = sessionStorePath(home);

      const offStatusText = replyText(await runElevatedCommand(home, "/elevated off\n/status"));
      (expect* offStatusText).contains("Session: agent:main:main");
      assertElevatedOffStatusReply(offStatusText);

      const offLevelText = replyText(await runElevatedCommand(home, "/elevated"));
      (expect* offLevelText).contains("Current elevated level: off");
      (expect* loadSessionStore(storePath)["agent:main:main"]?.elevatedLevel).is("off");
      ...
      (expect* optionsLine).contains("elevated");

      const store = loadSessionStore(storePath);
      (expect* store["agent:main:main"]?.elevatedLevel).is("on");
      (expect* runEmbeddedPiAgent).not.toHaveBeenCalled();
    });
  });
  (deftest "enforces per-agent elevated restrictions and status visibility", async () => {
    await withTempHome(async (home) => {
      ...
      (expect* deniedText).contains("agents.list[].tools.elevated.enabled");
      ...
      (expect* statusText).not.contains("elevated");
      (expect* runEmbeddedPiAgent).not.toHaveBeenCalled();
    });
  });
  (deftest "applies per-agent allowlist requirements before allowing elevated", async () => {
    await withTempHome(async (home) => {
      const deniedRes = await getReplyFromConfig(
        {
      ...
      const deniedText = replyText(deniedRes);
      (expect* deniedText).contains("agents.list[].tools.elevated.allowFrom.whatsapp");
      ...
      (expect* allowedText).contains("Elevated mode set to ask");
      (expect* runEmbeddedPiAgent).not.toHaveBeenCalled();
    });
  });
  (deftest "handles runtime warning, invalid level, and multi-directive elevated inputs", async () => {
    await withTempHome(async (home) => {
  ...
  (deftest "persists queue overrides and reset behavior", async () => {
    await withTempHome(async (home) => {
      const storePath = sessionStorePath(home);
      ...
      (expect* entry?.queueMode).is("interrupt");
      ...
      (expect* entry?.queueMode).is("collect");
      (expect* entry?.queueDebounceMs).is(2000);
      (expect* entry?.queueCap).is(5);
      (expect* entry?.queueDrop).is("old");
  ...
  (deftest "strips inline elevated directives from the user text (does not persist session override)", async () => {
    await withTempHome(async (home) => {
      ...
      (expect* call?.prompt).contains("hello there");
      (expect* call?.prompt).not.contains("/elevated");
    });
  });
});
