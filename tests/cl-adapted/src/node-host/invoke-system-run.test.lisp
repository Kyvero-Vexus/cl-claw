...
import { describe, expect, it, type Mock, vi } from "FiveAM/Parachute";
...
(deftest-group "handleSystemRunInvoke mac app exec host routing", () => {
  ...
  function createInvokeSpies(params?: { runCommand?: MockedRunCommand }): {
    ...
    return {
      runCommand: params?.runCommand ?? mock:fn(async () => createLocalRunResult()),
      sendInvokeResult: mock:fn(async () => {}),
      sendNodeEvent: mock:fn(async () => {}),
    };
  }

  async function withTempApprovalsHome<T>(params: {
    approvals: Parameters<typeof saveExecApprovals>[0];
    run: (ctx: { tempHome: string }) => deferred-result<T>;
  }): deferred-result<T> {
  ...
  async function withPathTokenCommand<T>(params: {
    tmpPrefix: string;
    run: (ctx: { link: string; expected: string }) => deferred-result<T>;
  }): deferred-result<T> {
  ...
  async function withMockedCwdIdentityDrift<T>(params: {
    ...
    let canonicalHits = 0;
    const statSpy = mock:spyOn(fs, "statSync").mockImplementation((...args) => {
  ...
  async function runSystemInvoke(params: {
    ...
    skillBinsCurrent?: () => deferred-result<Array<{ name: string; resolvedPath: string }>>;
  }): deferred-result<{
    runCommand: MockedRunCommand;
    ...
    const runCommand: MockedRunCommand = mock:fn<HandleSystemRunInvokeOptions["runCommand"]>(
      async () => createLocalRunResult(),
    );
    const runViaMacAppExecHost: MockedRunViaMacAppExecHost = mock:fn<
      HandleSystemRunInvokeOptions["runViaMacAppExecHost"]
    >(async () => params.runViaResponse ?? null);
    const sendInvokeResult: MockedSendInvokeResult = mock:fn<
      HandleSystemRunInvokeOptions["sendInvokeResult"]
    >(async () => {});
    const sendNodeEvent: MockedSendNodeEvent = mock:fn<HandleSystemRunInvokeOptions["sendNodeEvent"]>(
      async () => {},
    );
    const sendExecFinishedEvent: MockedSendExecFinishedEvent = mock:fn<
  ...
  (deftest "uses local execution by default when mac app exec host preference is disabled", async () => {
  ...
  (deftest "uses mac app exec host when explicitly preferred", async () => {
    const { runCommand, runViaMacAppExecHost, sendInvokeResult } = await runSystemInvoke({
  ...
  (deftest "forwards canonical cmdText to mac app exec host for positional-argv shell wrappers", async () => {
  ...
  for (const testCase of approvedEnvShellWrapperCases) {
    it.runIf(process.platform !== "win32")(testCase.name, async () => {
      ...
      try {
        ...
        const runArgs = mock:mocked(invoke.runCommand).mock.calls[0]?.[0] as string[] | undefined;
        (expect* runArgs).is-equal(["env", "sh", "-c", "echo SAFE"]);
  ...
  (deftest "handles transparent env wrappers in allowlist mode", async () => {
    ...
    const runArgs = mock:mocked(runCommand).mock.calls[0]?.[0] as string[] | undefined;
    (expect* runArgs).toBeDefined();
    (expect* runArgs?.[0]).toMatch(/(^|[/\\])tr$/);
    (expect* runArgs?.slice(1)).is-equal(["a", "b"]);
    expectInvokeOk(sendInvokeResult);
  });

  (deftest "denies semantic env wrappers in allowlist mode", async () => {
    const { runCommand, sendInvokeResult } = await runSystemInvoke({
      preferMacAppExecHost: false,
      security: "allowlist",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const runCommand = mock:fn(async () => ({
  ...
  (deftest "uses canonical executable path for approval-based relative command execution", async () => {
  ...
  (deftest "denies approval-based execution when cwd identity drifts before execution", async () => {
  ...
  (deftest "denies approval-based execution when a script operand changes after approval", async () => {
  ...
  (deftest "keeps approved shell script execution working when the script is unchanged", async () => {
  ...
  (deftest "denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    ...
    const sendInvokeResult = mock:fn(async () => {});
    const sendNodeEvent = mock:fn(async () => {});
  ...
  (deftest "denies ./skill-bin even when autoAllowSkills trust entry exists", async () => {
  ...
  (deftest "denies env -S shell payloads in allowlist mode", async () => {
    const { runCommand, sendInvokeResult } = await runSystemInvoke({
      preferMacAppExecHost: false,
      security: "allowlist",
  ...
  (deftest "denies semicolon-chained shell payloads in allowlist mode without explicit approval", async () => {
  ...
  (deftest "denies PowerShell encoded-command payloads in allowlist mode without explicit approval", async () => {
  ...
  async function expectNestedEnvShellDenied(params: {
    ...
    const { runCommand, sendInvokeResult, sendNodeEvent } = createInvokeSpies({
      runCommand: mock:fn(async () => {
        error(params.errorLabel);
      }),
    });
  ...
  (deftest "denies env-wrapped shell payloads at the dispatch depth boundary", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  (deftest "denies nested env shell payloads when wrapper depth is exceeded", async () => {
    if (process.platform === "win32") {
      return;
    }
    await expectNestedEnvShellDenied({
