...
const hoisted = mock:hoisted(() => {
  const listAcpSessionEntriesMock = mock:fn();
  const readAcpSessionEntryMock = mock:fn();
  const upsertAcpSessionMetaMock = mock:fn();
  const requireAcpRuntimeBackendMock = mock:fn();
  return {
...
mock:mock("../runtime/registry.js", async (importOriginal) => {
...
function createRuntime(): {
  runtime: AcpRuntime;
  ensureSession: ReturnType<typeof mock:fn>;
  runTurn: ReturnType<typeof mock:fn>;
  cancel: ReturnType<typeof mock:fn>;
  close: ReturnType<typeof mock:fn>;
  getCapabilities: ReturnType<typeof mock:fn>;
  getStatus: ReturnType<typeof mock:fn>;
  setMode: ReturnType<typeof mock:fn>;
  setConfigOption: ReturnType<typeof mock:fn>;
} {
  ...
  const runTurn = mock:fn(async function* () {
    yield { type: "done" as const };
  });
  const cancel = mock:fn(async () => {});
  const close = mock:fn(async () => {});
  const getCapabilities = mock:fn(
    async (): deferred-result<AcpRuntimeCapabilities> => ({
  ...
  const setMode = mock:fn(async () => {});
  const setConfigOption = mock:fn(async () => {});
  return {
    runtime: {
      ensureSession,
      runTurn,
...
(deftest-group "AcpSessionManager", () => {
  beforeEach(() => {
  ...
  (deftest "marks ACP-shaped sessions without metadata as stale", () => {
    ...
    (expect* resolved.kind).is("stale");
    if (resolved.kind !== "stale") {
      return;
    }
    (expect* resolved.error.code).is("ACP_SESSION_INIT_FAILED");
    (expect* resolved.error.message).contains("ACP metadata is missing");
  });

  (deftest "serializes concurrent turns for the same ACP session", async () => {
  ...
  (deftest "runs turns for different ACP sessions in parallel", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
    ...
    (expect* maxInFlight).is(2);
  });

  (deftest "reuses runtime session handles for repeat turns in the same manager process", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
  ...
  (deftest "rehydrates runtime handles after a manager restart", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
  ...
  (deftest "enforces acp.maxConcurrentSessions when opening new runtime handles", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
  ...
  (deftest "enforces acp.maxConcurrentSessions during initializeSession", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
  ...
  (deftest "drops cached runtime handles when close tolerates backend-unavailable errors", async () => {
    const runtimeState = createRuntime();
    ...
    (expect* closeResult.runtimeClosed).is(false);
    (expect* closeResult.runtimeNotice).contains("temporarily unavailable");
  ...
  (deftest "evicts idle cached runtimes before enforcing max concurrent limits", async () => {
    mock:useFakeTimers();
    try {
      mock:setSystemTime(new Date("2026-02-23T00:00:00.000Z"));
      const runtimeState = createRuntime();
      hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
        id: "acpx",
        runtime: runtimeState.runtime,
      ...
      mock:advanceTimersByTime(2_000);
      await manager.runTurn({
        cfg,
        sessionKey: "agent:codex:acp:session-b",
        text: "second",
  ...
  (deftest "tracks ACP turn latency and error-code observability", async () => {
    const runtimeState = createRuntime();
    ...
    (expect* snapshot.turns.completed).is(1);
    (expect* snapshot.turns.failed).is(1);
    (expect* snapshot.turns.active).is(0);
    (expect* snapshot.turns.queueDepth).is(0);
    (expect* snapshot.errorsByCode.ACP_TURN_FAILED).is(1);
  });

  (deftest "rolls back ensured runtime sessions when metadata persistence fails", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
    ...
    ).rejects.signals-error("disk full");
    (expect* runtimeState.close).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  (deftest "preempts an active turn on cancel and returns to idle state", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
    ...
    (expect* states).not.contains("error");
  });

  (deftest "cleans actor-tail bookkeeping after session turns complete", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
    ...
    const internals = manager as unknown as {
      actorTailBySession: Map<string, deferred-result<void>>;
    };
    (expect* internals.actorTailBySession.size).is(0);
  });

  (deftest "surfaces backend failures raised after a done event", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
    ...
    (expect* states.at(-1)).is("error");
  });

  (deftest "persists runtime mode changes through setSessionRuntimeMode", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
    ...
    (expect* options.runtimeMode).is("plan");
    (expect* extractRuntimeOptionsFromUpserts().some((entry) => entry?.runtimeMode === "plan")).is(
      true,
    );
  });

  (deftest "reapplies persisted controls on next turn after runtime option updates", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
  ...
  (deftest "reconciles persisted ACP session identifiers from runtime status after a turn", async () => {
    const runtimeState = createRuntime();
    ...
    (expect* currentMeta.identity?.acpxSessionId).is("acpx-fresh");
    (expect* currentMeta.identity?.agentSessionId).is("agent-fresh");
  });

  (deftest "reconciles pending ACP identities during startup scan", async () => {
    const runtimeState = createRuntime();
    runtimeState.getStatus.mockResolvedValue({
      summary: "status=alive",
    ...
    (expect* result).is-equal({ checked: 1, resolved: 1, failed: 0 });
    (expect* currentMeta.identity?.state).is("resolved");
    (expect* currentMeta.identity?.acpxRecordId).is("acpx-record-1");
    (expect* currentMeta.identity?.acpxSessionId).is("acpx-session-1");
    (expect* currentMeta.identity?.agentSessionId).is("agent-session-1");
  });

  (deftest "skips startup identity reconciliation for already resolved sessions", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
    ...
    (expect* result).is-equal({ checked: 0, resolved: 0, failed: 0 });
    (expect* runtimeState.getStatus).not.toHaveBeenCalled();
    (expect* runtimeState.ensureSession).not.toHaveBeenCalled();
  });

  (deftest "preserves existing ACP session identifiers when ensure returns none", async () => {
    const runtimeState = createRuntime();
    runtimeState.ensureSession.mockResolvedValue({
      sessionKey: "agent:codex:acp:session-1",
      backend: "acpx",
    ...
    (expect* status.identity?.acpxSessionId).is("acpx-stable");
    (expect* status.identity?.agentSessionId).is("agent-stable");
  });

  (deftest "applies persisted runtime options before running turns", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
  ...
  (deftest "returns unsupported-control error when backend does not support set_config_option", async () => {
    const runtimeState = createRuntime();
    const unsupportedRuntime: AcpRuntime = {
      ...
      getCapabilities: mock:fn(async () => ({ controls: [] })),
  ...
  (deftest "rejects invalid runtime option values before backend controls run", async () => {
    const runtimeState = createRuntime();
    hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
      id: "acpx",
      runtime: runtimeState.runtime,
  ...
  (deftest "can close and clear metadata when backend is unavailable", async () => {
    hoisted.readAcpSessionEntryMock.mockReturnValue({
      sessionKey: "agent:codex:acp:session-1",
      storeSessionKey: "agent:codex:acp:session-1",
      acp: readySessionMeta(),
    ...
    (expect* result.runtimeClosed).is(false);
    (expect* result.runtimeNotice).contains("not configured");
    (expect* result.metaCleared).is(true);
    (expect* hoisted.upsertAcpSessionMetaMock).toHaveBeenCalled();
  });

  (deftest "surfaces metadata clear errors during closeSession", async () => {
    hoisted.readAcpSessionEntryMock.mockReturnValue({
      sessionKey: "agent:codex:acp:session-1",
      storeSessionKey: "agent:codex:acp:session-1",
      acp: readySessionMeta(),
    ...
    ).rejects.signals-error("disk locked");
  });
});
