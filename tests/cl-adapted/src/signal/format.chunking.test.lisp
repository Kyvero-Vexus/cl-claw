...
(deftest-group "splitSignalFormattedText", () => {
  ...
  (deftest-group "style-aware splitting - basic text", () => {
    (deftest "text with no styles splits correctly at whitespace", () => {
      ...
      (expect* joinedText).contains("hello");
      (expect* joinedText).contains("world");
      (expect* joinedText).contains("test");
    });

    (deftest "empty text returns empty array", () => {
      // Empty input produces no chunks (not an empty chunk)
      const chunks = markdownToSignalTextChunks("", 100);
      (expect* chunks).is-equal([]);
    });

    (deftest "text under limit returns single chunk unchanged", () => {
      const markdown = "short text";
      const chunks = markdownToSignalTextChunks(markdown, 100);

      (expect* chunks).has-length(1);
      (expect* chunks[0].text).is("short text");
    });
  });

  (deftest-group "style-aware splitting - style preservation", () => {
    (deftest "style fully within first chunk stays in first chunk", () => {
      ...
      (expect* firstChunk.text).contains("bold");
      (expect* firstChunk.styles.some((s) => s.style === "BOLD")).is(true);
      ...
      (expect* boldStyle!.start).is(0);
      (expect* boldStyle!.length).is(4); // "bold"
    });

    (deftest "style fully within second chunk has offset adjusted to chunk-local position", () => {
      ...
      (expect* chunkWithBold!.styles.some((s) => s.style === "BOLD")).is(true);
      ...
      (expect* boldStyle!.start).is(boldPos);
      (expect* boldStyle!.length).is(4);
    });

    (deftest "style spanning chunk boundary is split into two ranges", () => {
    ...
    (deftest "style starting exactly at split point goes entirely to second chunk", () => {
    ...
    (deftest "style ending exactly at split point stays entirely in first chunk", () => {
      const limit = 10;
    ...
    (deftest "multiple styles, some spanning boundary, some not", () => {
      const limit = 25;
      ...
      (expect* allStyles).contains("ITALIC");
      (expect* allStyles).contains("BOLD");
      (expect* allStyles).contains("MONOSPACE");
    });
  });

  (deftest-group "style-aware splitting - edge cases", () => {
    (deftest "handles zero-length text with styles gracefully", () => {
      // Edge case: empty markdown produces no chunks
      const chunks = markdownToSignalTextChunks("", 100);
      (expect* chunks).has-length(0);
    });

    (deftest "handles text that splits exactly at limit", () => {
      const limit = 10;
      const markdown = "1234567890"; // exactly 10 chars
      const chunks = markdownToSignalTextChunks(markdown, limit);

      (expect* chunks).has-length(1);
      (expect* chunks[0].text).is("1234567890");
    });

    (deftest "preserves style through whitespace trimming", () => {
      const limit = 30;
    ...
    (deftest "handles repeated substrings correctly (no indexOf fragility)", () => {
    ...
    (deftest "handles chunk that starts with whitespace after split", () => {
    ...
    (deftest "deterministically tracks position without indexOf fragility", () => {
...
(deftest-group "markdownToSignalTextChunks", () => {
  (deftest-group "link expansion chunk limit", () => {
    (deftest "does not exceed chunk limit after link expansion", () => {
    ...
    (deftest "handles multiple links near chunk boundary", () => {
      const limit = 100;
  ...
  (deftest-group "link expansion with style preservation", () => {
    (deftest "long message with links that expand beyond limit preserves all text", () => {
      const limit = 80;
      ...
      (expect* combined).contains(filler);
      (expect* combined).contains("click here");
      (expect* combined).contains("example.com");
    });

    (deftest "styles (bold, italic) survive chunking correctly after link expansion", () => {
      const limit = 60;
      const markdown =
    ...
    (deftest "multiple links near chunk boundary all get properly chunked", () => {
      const limit = 50;
      const markdown =
    ...
    (deftest "preserves spoiler style through link expansion and chunking", () => {
      const limit = 40;
      const markdown =
