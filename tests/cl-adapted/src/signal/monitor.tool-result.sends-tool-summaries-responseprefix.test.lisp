...
(deftest-group "monitorSignalProvider tool results", () => {
  (deftest "uses bounded readiness checks when auto-starting the daemon", async () => {
  ...
  (deftest "uses startupTimeoutMs override when provided", async () => {
    const runtime = createMonitorRuntime();
  ...
  (deftest "caps startupTimeoutMs at 2 minutes", async () => {
    const runtime = createMonitorRuntime();
  ...
  (deftest "fails fast when auto-started signal daemon exits during startup", async () => {
    const runtime = createMonitorRuntime();
    setSignalAutoStartConfig();
    ...
    waitForTransportReadyMock.mockImplementationOnce(
      async (params: { abortSignal?: AbortSignal | null }) => {
        await new deferred-result<void>((_resolve, reject) => {
    ...
    ).rejects.signals-error(/signal daemon exited/i);
  });

  (deftest "treats daemon exit after user abort as clean shutdown", async () => {
    const runtime = createMonitorRuntime();
    setSignalAutoStartConfig();
    const abortController = new AbortController();
    let exited = false;
    let resolveExit!: (value: SignalDaemonExitEvent) => void;
    const exitedPromise = new deferred-result<SignalDaemonExitEvent>((resolve) => {
      resolveExit = resolve;
    });
    const stop = mock:fn(() => {
      if (exited) {
  ...
  (deftest "skips tool summaries with responsePrefix", async () => {
    ...
    (expect* sendMock.mock.calls[0][1]).is("PFX final reply");
  });
  ...
  (deftest "replies with pairing code when dmPolicy is pairing and no allowFrom is set", async () => {
    ...
    (expect* sendMock).toHaveBeenCalledTimes(1);
    (expect* String(sendMock.mock.calls[0]?.[1] ?? "")).contains("Your Signal number: +15550001111");
    (expect* String(sendMock.mock.calls[0]?.[1] ?? "")).contains("Pairing code: PAIRCODE");
  });

  (deftest "ignores reaction-only messages", async () => {
  ...
  (deftest "ignores reaction-only dataMessage.reaction events (don’t treat as broken attachments)", async () => {
    await receiveSingleEnvelope({
      ...makeBaseEnvelope(),
      dataMessage: {
        reaction: {
  ...
  (deftest "enqueues system events for reaction notifications", async () => {
    ...
    (expect* events.some((text) => text.includes("Signal reaction added"))).is(true);
  });
  ...
  it.each([
    ...
    const events = getDirectSignalEventsFor("+15550001111");
    (expect* events.some((text) => text.includes("Signal reaction added"))).is(shouldEnqueue);
    (expect* sendMock).not.toHaveBeenCalled();
    (expect* upsertPairingRequestMock).not.toHaveBeenCalled();
  });

  (deftest "notifies on own reactions when target includes uuid + phone", async () => {
  ...
  (deftest "processes messages when reaction metadata is present", async () => {
