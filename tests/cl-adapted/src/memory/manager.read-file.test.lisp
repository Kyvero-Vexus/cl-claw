...
(deftest-group "MemoryIndexManager.readFile", () => {
  ...
  (deftest "returns empty text when the requested file does not exist", async () => {
    ...
    (expect* result).is-equal({ text: "", path: relPath });
  });

  (deftest "returns content slices when the file exists", async () => {
    ...
    (expect* result).is-equal({ text: "line 2", path: relPath });
  });

  (deftest "returns empty text when the requested slice is past EOF", async () => {
  ...
  (deftest "returns empty text when the file disappears after stat", async () => {
