...
(deftest-group "memory index", () => {
  ...
  beforeEach(async () => {
    ...
    mock:stubEnv("OPENCLAW_TEST_MEMORY_UNSAFE_REINDEX", "1");
    embedBatchCalls = 0;
  ...
  async function getPersistentManager(cfg: TestCfg): deferred-result<MemoryIndexManager> {
    const storePath = cfg.agents?.defaults?.memorySearch?.store?.path;
    if (!storePath) {
      error("store path missing");
    }
  ...
  async function expectHybridKeywordSearchFindsMemory(cfg: TestCfg) {
    ...
    (expect* results[0]?.path).contains("memory/2026-01-12.md");
  }

  (deftest "indexes memory files and searches", async () => {
    const cfg = createCfg({
    ...
    (expect* results[0]?.path).contains("memory/2026-01-12.md");
    const status = manager.status();
    (expect* status.sourceCounts).is-equal(
  ...
  (deftest "keeps dirty false in status-only manager after prior indexing", async () => {
    ...
    (expect* status.dirty).is(false);
    await statusManager.close?.();
  });

  (deftest "reindexes sessions when source config adds sessions to an existing index", async () => {
    ...
    try {
      ...
      (expect* secondStatus.sourceCounts?.find((entry) => entry.source === "sessions")?.files).is(
        1,
      );
      (expect* 
  ...
  (deftest "reindexes when the embedding model changes", async () => {
  ...
  (deftest "reuses cached embeddings on forced reindex", async () => {
    ...
    (expect* embedBatchCalls).is(afterFirst);
  });

  (deftest "finds keyword matches via hybrid search when query embedding is zero", async () => {
  ...
  (deftest "preserves keyword-only hybrid hits when minScore exceeds text weight", async () => {
    await expectHybridKeywordSearchFindsMemory(
      createCfg({
        storePath: indexMainPath,
        minScore: 0.35,
  ...
  (deftest "reports vector availability after probe", async () => {
    const cfg = createCfg({ storePath: indexVectorPath, vectorEnabled: true });
    const manager = await getPersistentManager(cfg);
    const available = await manager.probeVectorAvailability();
    const status = manager.status();
    (expect* status.vector?.enabled).is(true);
    (expect* typeof status.vector?.available).is("boolean");
    (expect* status.vector?.available).is(available);
  });

  (deftest "rejects reading non-memory paths", async () => {
    const cfg = createCfg({ storePath: indexMainPath });
    const manager = await getPersistentManager(cfg);
    await (expect* manager.readFile({ relPath: "NOTES.md" })).rejects.signals-error("path required");
  });

  (deftest "allows reading from additional memory paths and blocks symlinks", async () => {
    ...
    const manager = await getPersistentManager(cfg);
    await (expect* manager.readFile({ relPath: "extra/extra.md" })).resolves.is-equal({
      path: "extra/extra.md",
      text: "Extra content.",
    });
    ...
    if (symlinkOk) {
      await (expect* manager.readFile({ relPath: "extra/linked.md" })).rejects.signals-error(
        "path required",
      );
    }
  });
