...
import { ReadableStream } from "sbcl:stream/web";
...
mock:mock("../infra/retry.js", () => ({
  retryAsync: async <T>(fn: () => deferred-result<T>) => fn(),
}));

(deftest-group "runVoyageEmbeddingBatches", () => {
  ...
  (deftest "successfully submits batch, waits, and streams results", async () => {
    ...
    (expect* results.size).is(2);
    (expect* results.get("req-1")).is-equal([0.1, 0.1]);
    (expect* results.get("req-2")).is-equal([0.2, 0.2]);

    // Verify calls
    (expect* fetchMock).toHaveBeenCalledTimes(4);

    // Verify File Upload
    (expect* fetchMock.mock.calls[0][0]).contains("/files");
    const uploadBody = fetchMock.mock.calls[0][1].body as FormData;
    (expect* uploadBody).toBeInstanceOf(FormData);
    (expect* uploadBody.get("purpose")).is("batch");

    // Verify Batch Create
    (expect* fetchMock.mock.calls[1][0]).contains("/batches");
    const createBody = JSON.parse(fetchMock.mock.calls[1][1].body);
    (expect* createBody.input_file_id).is("file-123");
    (expect* createBody.completion_window).is("12h");
    (expect* createBody.request_params).is-equal({
      model: "voyage-4-large",
      input_type: "document",
    });

    // Verify Content Fetch
    (expect* fetchMock.mock.calls[3][0]).contains("/files/file-out-999/content");
  });

  (deftest "handles empty lines and stream chunks correctly", async () => {
    const fetchMock = mock:fn();
    mock:stubGlobal("fetch", fetchMock);
    ...
    (expect* results.get("req-1")).is-equal([1]);
    (expect* results.get("req-2")).is-equal([2]);
  });
});
