...
(deftest-group "memory embedding batches", () => {
  (deftest "splits large files across multiple embedding batches", async () => {
    ...
    (expect* totalTexts).is(status.chunks);
    ...
    (expect* inputs.every((text) => Buffer.byteLength(text, "utf8") <= 8000)).is(true);
    (expect* updates.length).toBeGreaterThan(0);
    (expect* updates.some((update) => update.label?.includes("/"))).is(true);
    const last = updates[updates.length - 1];
    (expect* last?.total).toBeGreaterThan(0);
    (expect* last?.completed).is(last?.total);
  });

  (deftest "keeps small files in a single embedding batch", async () => {
    const memoryDir = fx.getMemoryDir();
    ...
    (expect* embedBatch.mock.calls.length).is(1);
  });

  (deftest "retries embeddings on transient rate limit and 5xx errors", async () => {
    const memoryDir = fx.getMemoryDir();
    const managerSmall = fx.getManagerSmall();
  ...
  (deftest "skips empty chunks so embeddings input stays valid", async () => {
    const memoryDir = fx.getMemoryDir();
    const managerSmall = fx.getManagerSmall();
