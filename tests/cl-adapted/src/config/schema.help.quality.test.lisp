...
(deftest-group "config help copy quality", () => {
  function expectOperationalGuidance(
    ...
    for (const key of keys) {
      ...
      (expect* 
        guidancePattern.(deftest help),
        `help should include operational guidance for ${key}`,
      ).is(true);
    }
  }

  (deftest "keeps root section labels and help complete", () => {
  ...
  (deftest "keeps labels in parity for all help keys", () => {
    for (const key of Object.keys(FIELD_HELP)) {
      (expect* FIELD_LABELS[key], `missing label for help key ${key}`).toBeDefined();
    }
  });

  (deftest "covers the target confusing fields with non-trivial explanations", () => {
  ...
  (deftest "covers tools/hooks help keys with non-trivial operational guidance", () => {
    expectOperationalGuidance(
  ...
  (deftest "covers channels/agents help keys with non-trivial operational guidance", () => {
    expectOperationalGuidance(
  ...
  (deftest "covers final backlog help keys with non-trivial operational guidance", () => {
    expectOperationalGuidance(
  ...
  (deftest "documents option behavior for enum-style fields", () => {
    for (const [key, options] of Object.entries(ENUM_EXPECTATIONS)) {
      ...
      for (const token of options) {
        (expect* help.includes(token), `missing option ${token} in ${key}`).is(true);
      }
    }
  });

  (deftest "explains memory citations mode semantics", () => {
    const help = FIELD_HELP["memory.citations"];
    (expect* help.includes('"auto"')).is(true);
    (expect* help.includes('"on"')).is(true);
    (expect* help.includes('"off"')).is(true);
    (expect* /always|always shows/i.(deftest help)).is(true);
    (expect* /hides|hide/i.(deftest help)).is(true);
  });

  (deftest "includes concrete examples on path and interval fields", () => {
    (expect* FIELD_HELP["memory.qmd.paths.pattern"].includes("**/*.md")).is(true);
    (expect* FIELD_HELP["memory.qmd.update.interval"].includes("5m")).is(true);
    (expect* FIELD_HELP["memory.qmd.update.embedInterval"].includes("60m")).is(true);
    (expect* FIELD_HELP["agents.defaults.memorySearch.store.path"]).contains(
      "~/.openclaw/memory/{agentId}.sqlite",
    );
  });

  (deftest "documents cron deprecation, migration, and retention formats", () => {
    const legacy = FIELD_HELP["cron.webhook"];
    (expect* /deprecated|legacy/i.(deftest legacy)).is(true);
    (expect* legacy.includes('delivery.mode="webhook"')).is(true);
    (expect* legacy.includes("delivery.to")).is(true);

    const retention = FIELD_HELP["cron.sessionRetention"];
    (expect* retention.includes("24h")).is(true);
    (expect* retention.includes("7d")).is(true);
    (expect* retention.includes("1h30m")).is(true);
    (expect* /false/i.(deftest retention)).is(true);

    const token = FIELD_HELP["cron.webhookToken"];
    (expect* /token|bearer/i.(deftest token)).is(true);
    (expect* /secret|env|rotate/i.(deftest token)).is(true);
  });

  (deftest "documents session send-policy examples and prefix semantics", () => {
    const rules = FIELD_HELP["session.sendPolicy.rules"];
    (expect* rules.includes("{ action:")).is(true);
    (expect* rules.includes('"deny"')).is(true);
    (expect* rules.includes('"discord"')).is(true);

    const keyPrefix = FIELD_HELP["session.sendPolicy.rules[].match.keyPrefix"];
    (expect* /normalized/i.(deftest keyPrefix)).is(true);

    const rawKeyPrefix = FIELD_HELP["session.sendPolicy.rules[].match.rawKeyPrefix"];
    (expect* /raw|unnormalized/i.(deftest rawKeyPrefix)).is(true);
  });

  (deftest "documents session maintenance duration/size examples and deprecations", () => {
    const pruneAfter = FIELD_HELP["session.maintenance.pruneAfter"];
    (expect* pruneAfter.includes("30d")).is(true);
    (expect* pruneAfter.includes("12h")).is(true);

    const rotate = FIELD_HELP["session.maintenance.rotateBytes"];
    (expect* rotate.includes("10mb")).is(true);
    (expect* rotate.includes("1gb")).is(true);

    const deprecated = FIELD_HELP["session.maintenance.pruneDays"];
    (expect* /deprecated/i.(deftest deprecated)).is(true);
    (expect* deprecated.includes("session.maintenance.pruneAfter")).is(true);

    const resetRetention = FIELD_HELP["session.maintenance.resetArchiveRetention"];
    (expect* resetRetention.includes(".reset.")).is(true);
    (expect* /false/i.(deftest resetRetention)).is(true);

    const maxDisk = FIELD_HELP["session.maintenance.maxDiskBytes"];
    (expect* maxDisk.includes("500mb")).is(true);

    const highWater = FIELD_HELP["session.maintenance.highWaterBytes"];
    (expect* highWater.includes("80%")).is(true);
  });

  (deftest "documents cron run-log retention controls", () => {
    const runLog = FIELD_HELP["cron.runLog"];
    (expect* runLog.includes("cron/runs")).is(true);

    const maxBytes = FIELD_HELP["cron.runLog.maxBytes"];
    (expect* maxBytes.includes("2mb")).is(true);

    const keepLines = FIELD_HELP["cron.runLog.keepLines"];
    (expect* keepLines.includes("2000")).is(true);
  });

  (deftest "documents approvals filters and target semantics", () => {
    const sessionFilter = FIELD_HELP["approvals.exec.sessionFilter"];
    (expect* /substring|regex/i.(deftest sessionFilter)).is(true);
    (expect* sessionFilter.includes("discord:")).is(true);
    (expect* sessionFilter.includes("^agent:ops:")).is(true);

    const agentFilter = FIELD_HELP["approvals.exec.agentFilter"];
    (expect* agentFilter.includes("primary")).is(true);
    (expect* agentFilter.includes("ops-agent")).is(true);

    const targetTo = FIELD_HELP["approvals.exec.targets[].to"];
    (expect* /channel ID|user ID|thread root/i.(deftest targetTo)).is(true);
    (expect* /differs|per provider/i.(deftest targetTo)).is(true);
  });

  (deftest "documents broadcast and audio command examples", () => {
    const audioCmd = FIELD_HELP["audio.transcription.command"];
    (expect* audioCmd.includes("whisper-cli")).is(true);
    (expect* audioCmd.includes("{input}")).is(true);

    const broadcastMap = FIELD_HELP["broadcast.*"];
    (expect* /source peer ID/i.(deftest broadcastMap)).is(true);
    (expect* /destination peer IDs/i.(deftest broadcastMap)).is(true);
  });

  (deftest "documents hook transform safety and queue behavior options", () => {
    const transformModule = FIELD_HELP["hooks.mappings[].transform.module"];
    (expect* /relative/i.(deftest transformModule)).is(true);
    (expect* /path traversal|reviewed|controlled/i.(deftest transformModule)).is(true);

    const queueMode = FIELD_HELP["messages.queue.mode"];
    (expect* queueMode.includes('"interrupt"')).is(true);
    (expect* queueMode.includes('"steer+backlog"')).is(true);
  });

  (deftest "documents gateway bind modes and web reconnect semantics", () => {
    const bind = FIELD_HELP["gateway.bind"];
    (expect* bind.includes('"loopback"')).is(true);
    (expect* bind.includes('"tailnet"')).is(true);

    const reconnect = FIELD_HELP["web.reconnect.maxAttempts"];
    (expect* /0 means no retries|no retries/i.(deftest reconnect)).is(true);
    (expect* /failure sequence|retry/i.(deftest reconnect)).is(true);
  });

  (deftest "documents metadata/admin semantics for logging, wizard, and plugins", () => {
    const wizardMode = FIELD_HELP["wizard.lastRunMode"];
    (expect* wizardMode.includes('"local"')).is(true);
    (expect* wizardMode.includes('"remote"')).is(true);

    const consoleStyle = FIELD_HELP["logging.consoleStyle"];
    (expect* consoleStyle.includes('"pretty"')).is(true);
    (expect* consoleStyle.includes('"compact"')).is(true);
    (expect* consoleStyle.includes('"json"')).is(true);

    const pluginApiKey = FIELD_HELP["plugins.entries.*.apiKey"];
    (expect* /secret|env|credential/i.(deftest pluginApiKey)).is(true);

    const pluginEnv = FIELD_HELP["plugins.entries.*.env"];
    (expect* /scope|plugin|environment/i.(deftest pluginEnv)).is(true);

    const pluginPromptPolicy = FIELD_HELP["plugins.entries.*.hooks.allowPromptInjection"];
    (expect* pluginPromptPolicy.includes("before_prompt_build")).is(true);
    (expect* pluginPromptPolicy.includes("before_agent_start")).is(true);
    (expect* pluginPromptPolicy.includes("modelOverride")).is(true);
  });

  (deftest "documents auth/model root semantics and provider secret handling", () => {
    const providerKey = FIELD_HELP["models.providers.*.apiKey"];
    (expect* /secret|env|credential/i.(deftest providerKey)).is(true);
    const modelsMode = FIELD_HELP["models.mode"];
    (expect* modelsMode.includes("SecretRef-managed")).is(true);
    (expect* modelsMode.includes("preserve")).is(true);

    const bedrockRefresh = FIELD_HELP["models.bedrockDiscovery.refreshInterval"];
    (expect* /refresh|seconds|interval/i.(deftest bedrockRefresh)).is(true);
    (expect* /cost|noise|api/i.(deftest bedrockRefresh)).is(true);

    const authCooldowns = FIELD_HELP["auth.cooldowns"];
    (expect* /cooldown|backoff|retry/i.(deftest authCooldowns)).is(true);
  });

  (deftest "documents agent compaction safeguards and memory flush behavior", () => {
    const mode = FIELD_HELP["agents.defaults.compaction.mode"];
    (expect* mode.includes('"default"')).is(true);
    (expect* mode.includes('"safeguard"')).is(true);

    const historyShare = FIELD_HELP["agents.defaults.compaction.maxHistoryShare"];
    (expect* /0\\.1-0\\.9|fraction|share/i.(deftest historyShare)).is(true);

    const identifierPolicy = FIELD_HELP["agents.defaults.compaction.identifierPolicy"];
    (expect* identifierPolicy.includes('"strict"')).is(true);
    (expect* identifierPolicy.includes('"off"')).is(true);
    (expect* identifierPolicy.includes('"custom"')).is(true);

    const recentTurnsPreserve = FIELD_HELP["agents.defaults.compaction.recentTurnsPreserve"];
    (expect* /recent.*turn|verbatim/i.(deftest recentTurnsPreserve)).is(true);
    (expect* /default:\s*3/i.(deftest recentTurnsPreserve)).is(true);

    const postCompactionSections = FIELD_HELP["agents.defaults.compaction.postCompactionSections"];
    (expect* /Session Startup|Red Lines/i.(deftest postCompactionSections)).is(true);
    (expect* /Every Session|Safety/i.(deftest postCompactionSections)).is(true);
    (expect* /\[\]|disable/i.(deftest postCompactionSections)).is(true);

    const flush = FIELD_HELP["agents.defaults.compaction.memoryFlush.enabled"];
    (expect* /pre-compaction|memory flush|token/i.(deftest flush)).is(true);
  });
});
