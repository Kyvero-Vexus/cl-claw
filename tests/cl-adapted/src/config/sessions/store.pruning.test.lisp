...
(deftest-group "pruneStaleEntries", () => {
  (deftest "removes entries older than maxAgeDays", () => {
    const now = Date.now();
...
(deftest-group "capEntryCount", () => {
  (deftest "over limit: keeps N most recent by updatedAt, deletes rest", () => {
    const now = Date.now();
    const store = makeStore([
    ...
    (expect* evicted).is(2);
    (expect* Object.keys(store)).has-length(3);
...
(deftest-group "rotateSessionFile", () => {
  let testDir: string;
  ...
  (deftest "file over maxBytes: renamed to .bak.{timestamp}, returns true", async () => {
    const bigContent = "x".repeat(200);
    await fs.writeFile(storePath, bigContent, "utf-8");

    const rotated = await rotateSessionFile(storePath, 100);

    (expect* rotated).is(true);
    await (expect* fs.stat(storePath)).rejects.signals-error();
    const files = await fs.readdir(testDir);
    const bakFiles = files.filter((f) => f.startsWith("sessions.json.bak."));
    (expect* bakFiles).has-length(1);
    const bakContent = await fs.readFile(path.join(testDir, bakFiles[0]), "utf-8");
    (expect* bakContent).is(bigContent);
  });

  (deftest "multiple rotations: only keeps 3 most recent .bak files", async () => {
    let now = Date.now();
    const nowSpy = mock:spyOn(Date, "now").mockImplementation(() => (now += 5));
    try {
