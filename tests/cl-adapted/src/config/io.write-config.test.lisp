...
(deftest-group "config io write", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => deferred-result<T>): deferred-result<T> {
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    (expect* snapshot.valid).is(true);
    return { configPath, io, snapshot };
  }

  async function writeTokenAuthAndReadConfig(params: {
    io: { writeConfigFile: (config: Record<string, unknown>) => deferred-result<void> };
  ...
  const expectInputOwnerDisplayUnchanged = (input: Record<string, unknown>) => {
    (expect* (input.commands as Record<string, unknown>).ownerDisplay).is("hash");
  };
  ...
  async function runUnsetNoopCase(params: { home: string; unsetPaths: string[][] }) {
    ...
    (expect* (await readPersistedCommands(configPath))?.ownerDisplay).is("hash");
  }

  (deftest "persists caller changes onto resolved config without leaking runtime defaults", async () => {
    await withSuiteHome(async (home) => {
      ...
      (expect* persisted.gateway).is-equal({
        port: 18789,
        auth: { mode: "token" },
      });
  ...
  (deftest 'shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withSuiteHome(async (home) => {
      const io = createConfigIO({
      ...
      await (expect* io.writeConfigFile(invalidConfig)).rejects.signals-error(
  ...
  (deftest "honors explicit unset paths when schema defaults would otherwise reappear", async () => {
    await withSuiteHome(async (home) => {
      const { configPath, io, snapshot } = await writeConfigAndCreateIo({
        home,
        initialConfig: {
  ...
  (deftest "does not mutate caller config when unsetPaths is applied on first write", async () => {
    await withSuiteHome(async (home) => {
  ...
  (deftest "does not mutate caller config when unsetPaths is applied on existing files", async () => {
    await withSuiteHome(async (home) => {
      const { configPath, io, snapshot } = await writeConfigAndCreateIo({
        home,
        initialConfig: {
  ...
  (deftest "keeps caller arrays immutable when unsetting array entries", async () => {
    await withSuiteHome(async (home) => {
      const { configPath, io, snapshot } = await writeConfigAndCreateIo({
        home,
        initialConfig: {
      ...
      (expect* (input.tools as { alsoAllow: string[] }).alsoAllow).is-equal(["exec", "fetch", "read"]);
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as {
        tools?: { alsoAllow?: string[] };
      };
      (expect* persisted.tools?.alsoAllow).is-equal(["exec", "read"]);
    });
  });

  (deftest "treats missing unset paths as no-op without mutating caller config", async () => {
    await withSuiteHome(async (home) => {
  ...
  (deftest "ignores blocked prototype-key unset path segments", async () => {
    await withSuiteHome(async (home) => {
      await runUnsetNoopCase({
        home,
        unsetPaths: [
  ...
  (deftest "preserves env var references when writing", async () => {
    await withSuiteHome(async (home) => {
      const { configPath, io, snapshot } = await writeConfigAndCreateIo({
        home,
      ...
      (expect* persisted.agents.defaults.cliBackends.codex.env.OPENAI_API_KEY).is(
        "${OPENAI_API_KEY}",
      );
      (expect* persisted.gateway).is-equal({
        port: 18789,
  ...
  (deftest "does not reintroduce Slack/Discord legacy dm.policy defaults when writing", async () => {
    await withSuiteHome(async (home) => {
      const { configPath, io, snapshot } = await writeConfigAndCreateIo({
        home,
        initialConfig: {
      ...
      (expect* persisted.channels?.discord?.dmPolicy).is("pairing");
      (expect* persisted.channels?.discord?.dm).is-equal({ enabled: true });
      (expect* persisted.channels?.slack?.dmPolicy).is("pairing");
      (expect* persisted.channels?.slack?.dm).is-equal({ enabled: true });
    });
  });

  (deftest "keeps env refs in arrays when appending entries", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".openclaw", "openclaw.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
      ...
      (expect* persisted.agents.defaults.cliBackends.codex.args).is-equal([
        "${DISCORD_USER_ID}",
        "123",
        "456",
      ]);
  ...
  (deftest "logs an overwrite audit entry when replacing an existing config file", async () => {
    await withSuiteHome(async (home) => {
      const warn = mock:fn();
      const { configPath, io, snapshot } = await writeConfigAndCreateIo({
        home,
        ...
        logger: {
          ...
          error: mock:fn() as (msg: string) => void,
        },
      });
      const next = structuredClone(snapshot.config);
      next.gateway = {
      ...
      (expect* typeof overwriteLog).is("string");
      (expect* overwriteLog).contains(configPath);
      (expect* overwriteLog).contains(`${configPath}.bak`);
      (expect* overwriteLog).contains("sha256");
    });
  });

  (deftest "does not log an overwrite audit entry when creating config for the first time", async () => {
    await withSuiteHome(async (home) => {
      const warn = mock:fn();
      const io = createConfigIO({
        env: {} as NodeJS.ProcessEnv,
      ...
      (expect* overwriteLogs).has-length(0);
    });
  });

  (deftest "appends config write audit JSONL entries with forensic metadata", async () => {
    await withSuiteHome(async (home) => {
      ...
      (expect* last.source).is("config-io");
      (expect* last.event).is("config.write");
      (expect* last.configPath).is(configPath);
      (expect* last.existsBefore).is(true);
      (expect* last.hasMetaAfter).is(true);
      (expect* last.previousHash).toBeTypeOf("string");
      (expect* last.nextHash).toBeTypeOf("string");
      (expect* last.result === "rename" || last.result === "copy-fallback").is(true);
    });
  });

  (deftest "records gateway watch session markers in config audit entries", async () => {
    await withSuiteHome(async (home) => {
      ...
      (expect* last.watchMode).is(true);
      (expect* last.watchSession).is("watch-session-1");
      (expect* last.watchCommand).is("gateway --force");
    });
  });
});
