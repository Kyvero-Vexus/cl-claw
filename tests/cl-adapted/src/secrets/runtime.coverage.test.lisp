...
(deftest-group "secrets runtime target coverage", () => {
  afterEach(() => {
    clearSecretsRuntimeSnapshot();
  });

  (deftest "handles every openclaw.json registry target when configured as active", async () => {
    ...
    for (const [index, entry] of entries.entries()) {
      ...
      if (entry.expectedResolvedValue === "string") {
        (expect* resolved).is(expectedValue);
      } else {
        (expect* typeof resolved === "string" || (resolved && typeof resolved === "object")).is(
          true,
        );
      }
    }
  ...
  (deftest "handles every auth-profiles registry target", async () => {
    const entries = listSecretTargetRegistryEntries().filter(
