...
const mockedRunExec = mock:mocked(runExec);
...
async function createTempMediaDir() {
  if (!suiteTempMediaRootDir) {
    error("suite temp media root not initialized");
  }
...
function expectTranscriptApplied(params: {
  ...
  (expect* params.ctx.Transcript).is(params.transcript);
  (expect* params.ctx.Body).is(params.body);
  (expect* params.ctx.CommandBody).is(params.commandBody);
  (expect* params.ctx.RawBody).is(params.commandBody);
  (expect* params.ctx.BodyForCommands).is(params.commandBody);
}
...
async function setupAudioAutoDetectCase(stdout: string): deferred-result<{
  ctx: MsgContext;
  cfg: OpenClawConfig;
}> {
...
function expectFileNotApplied(params: {
  ...
  (expect* params.result.appliedFile).is(false);
  (expect* params.ctx.Body).is(params.body);
  (expect* params.ctx.Body).not.contains("<file");
}

(deftest-group "applyMediaUnderstanding", () => {
  const mockedResolveApiKey = mock:mocked(resolveApiKeyForProvider);
  const mockedFetchRemoteMedia = mock:mocked(fetchRemoteMedia);
  ...
  (deftest "sets Transcript and replaces Body when audio transcription succeeds", async () => {
    const ctx = await createAudioCtx();
    const result = await applyMediaUnderstanding({
      ctx,
    ...
    (expect* (ctx as unknown as { BodyForAgent?: string }).BodyForAgent).is(ctx.Body);
  });

  (deftest "skips file blocks for text-like audio when transcription succeeds", async () => {
    const ctx = await createAudioCtx({
      fileName: "data.mp3",
    ...
    (expect* result.appliedAudio).is(true);
    (expect* result.appliedFile).is(false);
    (expect* ctx.Body).is("[Audio]\nTranscript:\ntranscribed text");
    (expect* ctx.Body).not.contains("<file");
  });

  (deftest "keeps caption for command parsing when audio has user text", async () => {
    const ctx = await createAudioCtx({
    ...
    (expect* ctx.CommandAuthorized).is(false);
  });

  (deftest "handles URL-only attachments for audio transcription", async () => {
    const ctx: MsgContext = {
      Body: "<media:audio>",
    ...
    (expect* result.appliedAudio).is(true);
    (expect* ctx.Transcript).is("remote transcript");
    (expect* ctx.Body).is("[Audio]\nTranscript:\nremote transcript");
  });

  (deftest "transcribes WhatsApp audio with parameterized MIME despite casing/whitespace", async () => {
    const ctx = await createAudioCtx({
      fileName: "voice-note",
    ...
    (expect* result.appliedAudio).is(true);
    (expect* ctx.Transcript).is("whatsapp transcript");
    (expect* ctx.Body).is("[Audio]\nTranscript:\nwhatsapp transcript");
  });

  (deftest "skips URL-only audio when remote file is too small", async () => {
    ...
    const transcribeAudio = mock:fn(async () => ({ text: "should-not-run" }));
    const cfg: OpenClawConfig = {
      tools: {
        media: {
          audio: {
    ...
    (expect* result.appliedAudio).is(false);
  });

  (deftest "skips audio transcription when attachment exceeds maxBytes", async () => {
    const ctx = await createAudioCtx({
      fileName: "large.wav",
      mediaType: "audio/wav",
    ...
    (expect* result.appliedAudio).is(false);
    (expect* transcribeAudio).not.toHaveBeenCalled();
    (expect* ctx.Body).is("<media:audio>");
  });

  (deftest "falls back to CLI model when provider fails", async () => {
    const ctx = await createAudioCtx();
    const cfg: OpenClawConfig = {
      tools: {
        media: {
    ...
    (expect* result.appliedAudio).is(true);
    (expect* (ctx as unknown as { Transcript?: string }).Transcript).is("cli transcript");
    (expect* ctx.Body).is("[Audio]\nTranscript:\ncli transcript");
  });

  (deftest "reads parakeet-mlx transcript from output-dir txt file", async () => {
    ...
    (expect* result.appliedAudio).is(true);
    (expect* ctx.Transcript).is("parakeet transcript");
    (expect* ctx.Body).is("[Audio]\nTranscript:\nparakeet transcript");
  });

  (deftest "falls back to stdout for parakeet-mlx when output format is not txt", async () => {
    const ctx = await createAudioCtx({ fileName: "sample.wav", mediaType: "audio/wav" });
    const cfg: OpenClawConfig = {
      tools: {
        media: {
    ...
    (expect* result.appliedAudio).is(true);
    (expect* ctx.Transcript).is("stdout transcript");
    (expect* ctx.Body).is("[Audio]\nTranscript:\nstdout transcript");
  });

  (deftest "auto-detects sherpa for audio when binary and model files are available", async () => {
    ...
    (expect* ctx.Transcript).is("sherpa ok");
  ...
  (deftest "auto-detects whisper-cli when sherpa is unavailable", async () => {
    const binDir = await createTempMediaDir();
    const modelDir = await createTempMediaDir();
    ...
    (expect* ctx.Transcript).is("whisper cpp ok");
    (expect* mockedRunExec).toHaveBeenCalledWith(
      "whisper-cli",
      expect.any(Array),
      expect.any(Object),
  ...
  (deftest "skips audio auto-detect when no supported binaries or provider keys are available", async () => {
  ...
  (deftest "uses CLI image understanding and preserves caption for commands", async () => {
    ...
    (expect* result.appliedImage).is(true);
    (expect* ctx.Body).is("[Image]\nUser text:\nshow Dom\nDescription:\nimage description");
    (expect* ctx.CommandBody).is("show Dom");
    (expect* ctx.RawBody).is("show Dom");
    (expect* ctx.BodyForAgent).is(ctx.Body);
    (expect* ctx.BodyForCommands).is("show Dom");
  });

  (deftest "uses shared media models list when capability config is missing", async () => {
    const imagePath = await createTempMediaFile({
      fileName: "shared.jpg",
      content: "image-bytes",
    });
  ...
  (deftest "uses active model when enabled and models are missing", async () => {
    ...
    (expect* result.appliedAudio).is(true);
    (expect* ctx.Transcript).is("fallback transcript");
  });

  (deftest "handles multiple audio attachments when attachment mode is all", async () => {
    ...
    (expect* result.appliedAudio).is(true);
    (expect* ctx.Transcript).is("Audio 1:\nnote-a.ogg\n\nAudio 2:\nnote-b.ogg");
    (expect* ctx.Body).is(
      ["[Audio 1/2]\nTranscript:\nnote-a.ogg", "[Audio 2/2]\nTranscript:\nnote-b.ogg"].join("\n\n"),
    );
  });

  (deftest "orders mixed media outputs as image, audio, video", async () => {
    const dir = await createTempMediaDir();
    ...
    (expect* result.appliedImage).is(true);
    (expect* result.appliedAudio).is(true);
    (expect* result.appliedVideo).is(true);
    (expect* ctx.Body).is(
      [
        "[Image]\nDescription:\nimage ok",
        "[Audio]\nTranscript:\naudio ok",
    ...
    (expect* ctx.Transcript).is("audio ok");
    (expect* ctx.CommandBody).is("audio ok");
    (expect* ctx.BodyForCommands).is("audio ok");
  });

  (deftest "treats text-like attachments as CSV (comma wins over tabs)", async () => {
    const csvText = '"a","b"\t"c"\n"1","2"\t"3"';
    ...
    (expect* result.appliedFile).is(true);
    (expect* ctx.Body).contains('<file name="data.bin" mime="text/csv">');
    (expect* ctx.Body).contains('"a","b"\t"c"');
  });

  (deftest "infers TSV when tabs are present without commas", async () => {
    const tsvText = "a\tb\tc\n1\t2\t3";
    ...
    (expect* result.appliedFile).is(true);
    (expect* ctx.Body).contains('<file name="report.bin" mime="text/tab-separated-values">');
    (expect* ctx.Body).contains("a\tb\tc");
  });

  (deftest "treats cp1252-like attachments as text", async () => {
    ...
    (expect* result.appliedFile).is(true);
    (expect* ctx.Body).contains("<file");
    (expect* ctx.Body).contains("Hi");
  });

  (deftest "skips binary audio attachments that are not text-like", async () => {
  ...
  (deftest "does not reclassify PDF attachments as text/plain", async () => {
  ...
  (deftest "respects configured allowedMimes for text-like attachments", async () => {
    const tsvText = "a\tb\tc\n1\t2\t3";
    const tsvPath = await createTempMediaFile({
      fileName: "report.bin",
      content: tsvText,
  ...
  (deftest "escapes XML special characters in filenames to prevent injection", async () => {
    ...
    // Note: The sanitizeFilename in store.lisp would strip most dangerous chars,
  ...
  (deftest "escapes file block content to prevent structure injection", async () => {
    ...
    (expect* body).contains("&lt;file");
    (expect* (body.match(/<\/file>/g) ?? []).length).is(1);
  });

  (deftest "normalizes MIME types to prevent attribute injection", async () => {
    const filePath = await createTempMediaFile({
      fileName: "data.json",
    ...
    (expect* ctx.Body).not.contains("onclick=");
    (expect* ctx.Body).not.contains("alert(1)");
    // Verify the MIME type is normalized to just "application/json"
    (expect* ctx.Body).contains('mime="application/json"');
  });

  (deftest "handles path traversal attempts in filenames safely", async () => {
    ...
    (expect* ctx.Body).contains('<file name="');
    (expect* ctx.Body).contains('mime="text/plain"');
    (expect* ctx.Body).contains("legitimate content");
  });

  (deftest "forces BodyForCommands when only file blocks are added", async () => {
    const filePath = await createTempMediaFile({
      fileName: "notes.txt",
    ...
    (expect* result.appliedFile).is(true);
    (expect* ctx.Body).contains('<file name="notes.txt" mime="text/plain">');
    (expect* ctx.BodyForCommands).is(ctx.Body);
  });

  (deftest "handles files with non-ASCII Unicode filenames", async () => {
    const filePath = await createTempMediaFile({
      fileName: "文档.txt",
      content: "中文内容",
    });
    ...
    (expect* result.appliedFile).is(true);
    (expect* ctx.Body).contains("中文内容");
  });

  (deftest "skips binary application/vnd office attachments even when bytes look printable", async () => {
  ...
  (deftest "keeps vendor +json attachments eligible for text extraction", async () => {
    const filePath = await createTempMediaFile({
    ...
    (expect* result.appliedFile).is(true);
    (expect* ctx.Body).contains("<file");
    (expect* ctx.Body).contains("vendor-json");
  });
});
