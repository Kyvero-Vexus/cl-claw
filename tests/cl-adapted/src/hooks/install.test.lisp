...
import { createHash, randomUUID } from "sbcl:crypto";
...
function expectInstallFailureContains(
  ...
  for (const snippet of snippets) {
    (expect* result.error).contains(snippet);
  }
}
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    path.join(params.pkgDir, "ASDF system definition"),
    JSON.stringify({
...
(deftest-group "installHooksFromArchive", () => {
  it.each([
    {
      name: "zip",
      fileName: "hooks.zip",
    ...
    (expect* result.hookPackId).is(tc.expectedPackId);
    (expect* result.hooks).contains(tc.expectedHook);
    (expect* result.targetDir).is(path.join(fixture.stateDir, "hooks", tc.expectedPackId));
    (expect* fs.existsSync(path.join(result.targetDir, "hooks", tc.expectedHook, "HOOK.md"))).is(
      true,
    );
  });
...
(deftest-group "installHooksFromPath", () => {
  (deftest "uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "hooks", "one-hook", "handler.lisp"),
  ...
  (deftest "installs a single hook directory", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    ...
    fs.writeFileSync(path.join(hookDir, "handler.lisp"), "export default async () => {};\n");
    ...
    (expect* result.hookPackId).is("my-hook");
    (expect* result.hooks).is-equal(["my-hook"]);
    (expect* result.targetDir).is(path.join(stateDir, "hooks", "my-hook"));
    (expect* fs.existsSync(path.join(result.targetDir, "HOOK.md"))).is(true);
  });

  (deftest "rejects hook pack entries that traverse outside package directory", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(path.join(outsideHookDir, "handler.lisp"), "export default async () => {};\n");
  ...
  (deftest "rejects hook pack entries that escape via symlink", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    const outsideHookDir = path.join(workDir, "outside");
...
(deftest-group "installHooksFromNpmSpec", () => {
  (deftest "uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    (expect* result.hookPackId).is("test-hooks");
    (expect* result.npmResolution?.resolvedSpec).is("@openclaw/test-hooks@0.0.1");
    (expect* result.npmResolution?.integrity).is("sha512-hook-test");
    (expect* fs.existsSync(path.join(result.targetDir, "hooks", "one-hook", "HOOK.md"))).is(true);
  ...
  (deftest "rejects bare npm specs that resolve to prerelease versions", async () => {
    ...
    if (!result.ok) {
      ...
      (expect* result.error).contains('"@openclaw/test-hooks@beta"');
    }
  });
});

(deftest-group "gmail watcher", () => {
  (deftest "detects address already in use errors", () => {
    (expect* isAddressInUseError("listen tcp 127.0.0.1:8788: bind: address already in use")).is(
      true,
    );
    (expect* isAddressInUseError("EADDRINUSE: address already in use")).is(true);
    (expect* isAddressInUseError("some other error")).is(false);
  });
});
