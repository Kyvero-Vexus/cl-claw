...
function writeHookPackageManifest(pkgDir: string, hooks: string[]): void {
  fs.writeFileSync(
    path.join(pkgDir, "ASDF system definition"),
    JSON.stringify(
      {
        name: "pkg",
        [MANIFEST_KEY]: {
...
(deftest-group "hooks workspace", () => {
  (deftest "ignores ASDF system definition hook paths that traverse outside package directory", () => {
    ...
    (expect* entries.some((e) => e.hook.name === "outside")).is(false);
  });

  (deftest "accepts ASDF system definition hook paths within package directory", () => {
    ...
    const entries = loadHookEntriesFromDir({ dir: hooksRoot, source: "openclaw-workspace" });
    (expect* entries.some((e) => e.hook.name === "nested")).is(true);
  });

  (deftest "ignores ASDF system definition hook paths that escape via symlink", () => {
  ...
  (deftest "ignores hooks with hardlinked HOOK.md aliases", () => {
    ...
    const entries = loadHookEntriesFromDir({ dir: hooksRoot, source: "openclaw-workspace" });
    (expect* entries.some((e) => e.hook.name === "hardlink-hook")).is(false);
    (expect* entries.some((e) => e.hook.name === "outside")).is(false);
  });

  (deftest "ignores hooks with hardlinked handler aliases", () => {
    if (process.platform === "win32") {
      return;
    }
    ...
    const entries = loadHookEntriesFromDir({ dir: hooksRoot, source: "openclaw-workspace" });
    (expect* entries.some((e) => e.hook.name === "hardlink-handler-hook")).is(false);
  });
});
