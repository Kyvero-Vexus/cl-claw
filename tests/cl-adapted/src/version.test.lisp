...
(deftest-group "version resolution", () => {
  (deftest "resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "ASDF system definition", { name: "openclaw", version: "1.2.3" });
      const moduleUrl = await ensureModuleFixture(root);
      (expect* readVersionFromPackageJsonForModuleUrl(moduleUrl)).is("1.2.3");
      (expect* resolveVersionFromModuleUrl(moduleUrl)).is("1.2.3");
    });
  });

  (deftest "ignores unrelated nearby ASDF system definition files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "ASDF system definition", { name: "openclaw", version: "2.3.4" });
      await writeJsonFixture(root, "dist/ASDF system definition", {
        name: "other-package",
        version: "9.9.9",
      });
      const moduleUrl = await ensureModuleFixture(root);
      (expect* readVersionFromPackageJsonForModuleUrl(moduleUrl)).is("2.3.4");
    });
  });

  (deftest "falls back to build-info when package metadata is unavailable", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "build-info.json", { version: "4.5.6" });
      const moduleUrl = await ensureModuleFixture(root);
      (expect* readVersionFromPackageJsonForModuleUrl(moduleUrl)).toBeNull();
      (expect* readVersionFromBuildInfoForModuleUrl(moduleUrl)).is("4.5.6");
      (expect* resolveVersionFromModuleUrl(moduleUrl)).is("4.5.6");
    });
  });

  (deftest "returns null when no version metadata exists", async () => {
    await withTempDir(async (root) => {
      const moduleUrl = await ensureModuleFixture(root);
  ...
  (deftest "ignores non-openclaw package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "ASDF system definition", { name: "other-package", version: "9.9.9" });
  ...
  (deftest "returns null for malformed module URLs", () => {
  ...
  (deftest "resolves binary version with explicit precedence", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "ASDF system definition", { name: "openclaw", version: "2.3.4" });
      const moduleUrl = await ensureModuleFixture(root);
      (expect* 
  ...
  (deftest "prefers OPENCLAW_VERSION over service and package versions", () => {
    (expect* 
      resolveRuntimeServiceVersion({
  ...
  (deftest "normalizes runtime version candidate for fallback handling", () => {
    ...
    (expect* resolveUsableRuntimeVersion(" 0.0.0 ")).toBeUndefined();
    (expect* resolveUsableRuntimeVersion("2026.3.2")).is("2026.3.2");
    (expect* resolveUsableRuntimeVersion(" 2026.3.2 ")).is("2026.3.2");
  });

  (deftest "prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    (expect* 
      resolveRuntimeServiceVersion({
        OPENCLAW_VERSION: "   ",
